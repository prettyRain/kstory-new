...
public class OrderCouponServiceImpl implements OrderCouponService {
    ...
    public int modifyCouponByOrderId(Long orderId, Long customerId) {
        ...
        BigDecimal sumP = order.getOrderPrice();
        // 查询订单商品信息
        ...
        if(null != pointSet && "1".equals(pointSet.getIsOpen()) && order.getBusinessId()==0 && sumP.compareTo(new BigDecimal(20)) > -1){
            //只有当后台积分设置为开启时才赠送积分
                // 订单完成赠送积分
                customerPointServiceMapper.addIntegralByType(customerId, "6", sumP.doubleValue(),orderId+"",order.getCreateTime());
        }
    ...
    public void modifyCouponByOrderIdAndCustomerId(Long orderId, Long customerId) {
        couponService.modifyCouponByOrderIdAndCustomerId(orderId,customerId);
    }
    ...
    public void cancelOrderCoupon(Long orderId) {
        couponService.cancelOrderCoupon(orderId);
    }
}
