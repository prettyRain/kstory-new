...
<head lang="zh">
  <meta charset="UTF-8">
  <title>礼品卡注册页-${topmap.systembase.bsetName}</title>
...
<div class="container">
    ...
    <#--<a href="${topmap.systembase.bsetAddress}">-->
        <#--<img id="reg_pic" alt="" src="" style="height:70px;width:165px;"/>-->
    <#--</a>-->
      <#--<span>欢迎222注册</span>-->
    <#--</h1>-->
  <#--</div>-->
  ...
  <div class="user_box">
    ...
    <div class="reg_form">
    <form action="${basePath}/giftcardregistersecond.html" method="post" id="registerFirstForm">
      <input type="hidden" name="custId" id="custId"/>
      <div class="reg_form_item <#if giftcardPwdError??>error</#if>">
        <label class="for_text">礼品卡密码</label>
        <div class="item">
          <input type="text" class="text long" placeholder="请输入礼品卡密码" id="giftcardPwdInput" name="giftcardPwd">
          <i class="correct" style="display: none"></i>
          <p class="tips giftcardPwd_info error" <#if giftcardPwdError??><#else>style="display: none"</#if>><#if giftcardPwdError??>${msg!''}<#else>请输入正确的礼品卡密码</#if></p>
        </div>
      </div>
      <div class="reg_form_item <#if mobileError??>error</#if>">
        ...
        <div class="item">
          ...
          <i class="correct" style="display: none"></i>
          <p class="tips mobile_info error" <#if mobileError??><#else>style="display: none"</#if>><#if mobileError??>${msg!''}<#else>请输入正确的手机号码</#if></p>
        </div>
      </div>
      <div class="reg_form_item <#if codeError??>error</#if>">
        ...
        <div class="item">
          ...
          <p class="tips code_info error" <#if codeError??><#else>style="display: none"</#if>><#if codeError??>${msg!''}<#else>请输入验证码</#if></p>
        </div>
      </div>
      ...
      <div class="reg_form_item">
        <label class="for_text">&nbsp;</label>
        <div class="item">
          <button type="submit" class="sub_btn nextGrey" onclick="submitFirstGiftcard()">同意协议并注册</button>
            <a href="/registerfirst.html" style="position: absolute;right: -120px;top: 2px;font-weight: bold;font-size: 16px;">返回普通注册</a>
        </div>
      </div>
    </div>
...
<script>
  $(function(){
      ...
      $('.text').on('input change',function(){
          ...
          if(($.trim(giftcardPwd)!=="")&&($.trim(mobile)!=="")&&($.trim(checkCode)!=="")&&($("#readme").prop("checked"))){
              $('.sub_btn').removeClass("nextGrey");
          }else {
              $('.sub_btn').addClass("nextGrey");
          }
      });
      $("#readme").on("ifChanged",function () {
          var giftcardPwd = $("#giftcardPwdInput").val();
          var mobile = $("#mobileInput").val();
          var checkCode = $("#checkCode").val();
          if(($.trim(giftcardPwd)!=="")&&($.trim(mobile)!=="")&&($.trim(checkCode)!=="")&&($("#readme").prop("checked"))){
