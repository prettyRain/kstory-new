...
public class ExpressTemplateController {
    ...
    @ResponseBody
    public Map<String, Object> getnewexpressprice(Long addressId, HttpServletRequest request,Long isbuynow,String presentScopeIds,Long dianziCard) {

        try {
            String[] shopCartIds = request.getParameterValues("shopIds[]");

            // 如果没有地址 或者购物车id 则直接返回
            if(isbuynow == null){
                if (null == addressId || null == shopCartIds) {
                    return null;
                }
            }
            ...
            if(dianziCard != cartIds.length){
                ca = customerServiceInterface.queryCustAddress(addressId);
                if (ca == null) {
                    return null;
                }
            }else{
            ...
            if (shopCartIds.length != 0) {
                // 计算运费-不同地区的货品价格以及库存
                freightMap = shoppingCartService.getNewExpressPrice(wareUtil,request, ca.getDistrict().getDistrictId(), ca.getCity().getCityId(), cartIds, isbuynow);
            }
