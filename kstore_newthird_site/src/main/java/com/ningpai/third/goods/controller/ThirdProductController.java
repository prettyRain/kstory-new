...
public class ThirdProductController {
    ...
    public ModelAndView updateThirdProduct(GoodsProduct goodsProduct, HttpServletRequest request, String oldPrice,Long goodsAddedSta) {
        ...
        if (thirdId.equals(goodsBelo)&&thirdId.equals(goodsInfoThirdId)) {
            ...
            //只修改类目页推荐和手机端推荐不需要审核
            Boolean flag = goodsProductService.compare(product, goodsProduct, supportIds, specDetailId);
            if(flag == true){
                goodsProduct.setAuditStatus("3");
            }else{
                if (Integer.parseInt(isThirdAuditUsed) == 1) {

                    // 审核开关开启 如果 货品 是下架 则不需要审核
            ...
            try {
                ...
                // 设置跳转的页面 返回要在页面显示的数据
                return new ModelAndView(new RedirectView(ThirdPathUtil.THIRDPRODUCTMAPAGER + ThirdValueBean.GOODSID
                        + goodsProduct.getGoodsId() + "&goodsAddedSta=" + goodsAddedSta));
            } finally {
                specIds = null;
                specDetailId = null;
            }
    ...
    public ModelAndView updateThirdProductImage(Long goodsImgId, Long productId, Long goodsId, HttpServletRequest request, HttpServletResponse response,
            MultipartHttpServletRequest request2,Long goodsAddedSta) {
        ...
        try {
            ...
            return new ModelAndView(new RedirectView(ThirdPathUtil.THIRDPRODUCTMAPAGER + ThirdValueBean.GOODSID + goodsId+"&goodsAddedSta="+goodsAddedSta));
        } finally {
            defaultImage = null;
            delIamges = null;
        }
    ...
    public ModelAndView batchDownThirdProduct(Long goodsId, Long[] productId, HttpServletRequest request,
            Long goodsAddedSta) {
        /*this.goodsProductService.batchUploadProduct(
                ((Customer) request.getSession().getAttribute(ThirdValueBean.CUST)).getCustomerUsername(), productId,
                0);*/
        // 修改索引
        searchAsyncService.updateOneGoodsIndexToEs(goodsId);
        return new ModelAndView(new RedirectView(ThirdPathUtil.THIRDPRODUCTMAPAGER + ThirdValueBean.GOODSID + goodsId
    ...
    public ModelAndView newUploadProduct(@Valid GoodsProduct product, HttpServletRequest request, String addedTime,Long goodsAddedSta) {
        ...
        try {
            ...
            return new ModelAndView(new RedirectView(ThirdPathUtil.THIRDPRODUCTMAPAGER + ThirdValueBean.GOODSID
                    + product.getGoodsId() + "&goodsAddedSta=" + goodsAddedSta));
        } finally {
            supportId = null;
            images = null;
            specIds = null;
    ...
    public PageBean goodsManger(PageBean pb, GoodsSearchBean searchBean, HttpServletRequest request, String goodsInfoAdded,BigDecimal offValue) {
        ...
        if (searchBean.getGoodsInfoName() != null && !"".equals(searchBean.getGoodsInfoName())) {
            try {
                // 商品名称
                goodsInfoName = java.net.URLDecoder.decode(searchBean.getGoodsInfoName(), "UTF-8");
                // 设置商品名称
