...
public class InsideLetterController {
    ...
    @ResponseBody
    public Object insertNotices(InsideLetter insideLetter, Long[] customerIds,String linkType) {
        // 非空验证 站内信息标题
        if (null != insideLetter.getLetterTitle()) {
            // 日志记录
            LOGGER.info("发送标题为:" + insideLetter.getLetterTitle() + "的站内信息！");
        ...
        if(("all").equals(linkType)){
            insideLetter.setHrefUrl("");
        }else{
            insideLetter.setHrefUrl(linkType + insideLetter.getHrefUrl());
        }
