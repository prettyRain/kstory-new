...
function minus(obj){
    var $this = $(obj);
    var nownum = $(".product_buy_num").val()-1;
    nownum = Math.max(1,nownum);
    $(".product_buy_num").val(nownum);
}
function plus(obj){
    var $this = $(obj);
    var nownum = Number($(".product_buy_num").val())+1;
    //nownum = Math.min(nownum,$(".productStock").val());
    nownum = Math.min(99,nownum);
    $(".product_buy_num").val(nownum);
}
function product_buy_num(obj){
    var $this = $(obj);
    var nownum = Math.min(Number($this.val()),99);
    $(".product_buy_num").val(nownum);
}
$(function () {
    /*加载面包屑信息*/
    ...
    $(".go_buy").click(function () {
        ...
        /*var productBuyNum = $(".product_buy_num").val();
        var sellType =parseInt($(this).attr("sellType"));
        if(productBuyNum<10 && sellType == 0) {
            dia(666);
            return 0;
        }*/
        ...
        $.post("../goods/addProductToShopCarNew.html?" + params, function (data) {
            if (data >= 1) {
                //加入购物车成功后
                window.location.href = "../order/buynoworder.html";
            } else if (data == -2) {
                /*初始化弹框样式*/
               /* $(".info_tip_title").html("立即结算");*/
                $(".info_tip_content").html("该商品购买数量超出上限!!");
              ...
              /*  $(".info_tip_title").html("加入购物车");*/
               ...
               /* $(".info_tip_title").html("加入购物车");*/
                ...
                return 0;
                    ...
                    /*初始化弹框样式*/
                    $(".info_tip_content2").html("该商品购买数量超出上限!!");
                    $(".info_tip_img2").attr("src", "../images/outstock.png");
                    $(".info_tip_cancel2").attr("href", "javascript:;").hide();
                    $(".info_tip_submit2").attr("href", "javascript:;").show().html("确定").unbind("click");
                    $(".info_tip_submit2").click(function () {
...
function clickSpecDetail(obj, bool) {
    ...
    /*删除保存的以选中记录,并添加新的*/
    var spectitle=$(obj).attr("title");
    if($(obj).attr("title").length>13){
        spectitle=$(obj).attr("title").substring(0,13)+"...";
    }
    $(".sel_spec_" + $(obj).attr("data-parent")).html("“" + spectitle + "”");
...
function controlSpecBtn(tempProductList, obj, bool, checkRunAgain) {
    ...
    for (var i = 0; i < specList.length; i++) {
        ...
        if (count == 0) {
            ...
            //默认隐藏，符合条件的规格显示
            $(specList[i]).attr('style', 'display:block;');
        ...
        for (var i = 0; i < params.length; i++) {
            for (var k = 0; k < specList.length; k++) {
                if ($(specList[k]).attr("data-value") == $(params[i]).val()) {
                    ...
                    var spectitle=$(specList[k]).attr("title");
                    if($(specList[k]).attr("title").length>13){
                        spectitle=$(specList[k]).attr("title").substring(0,13)+"...";
                    }
                    spec += "<span class=sel_spec_" + $(specList[k]).attr("data-parent") + ">“" + spectitle + "”</span> , ";
    ...
    $.get("../gmarket/" + $("#productId").val() + "-" + $("#brandId").val() + "-" + $("#catId").val() + ".html", function (data) {
        ...
        if (null == data || data.length <= 0) {
            ...
            if ($("#isCustomerDiscount").val() == '1') {
                $(".moreinfo").append("<em class='sales_label' id='cut'>会员折扣</em>");
            } else {
                //$(".moreinfo").append("<em class='sales_label'>无</em>");
                // $(".saleswarp").hide();
               //$(".cxyj").hide();
            }
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (marketing.codexType == 1) {
                    ...
                    $(".moreinfo").append('<div style="position: relative;"><img class="img_img"src="/images/cuxiao/zhijiangshangping1.png" ><p class="div_title">' + marketing.marketingName + '</p></div>');

                    //$(".moreinfo").append("<span class='promotion_info'>直降" + marketing.priceOffMarketing.offValue + "元</span><br/>");
                    $(".cxyj").show();
                    $(".main_price").html("<span>¥ </span>" +formatPrice(accStur($('#mprice').val(),marketing.priceOffMarketing.offValue).toString()));
                    ...
                    $(".moreinfo").append('<div style="position: relative;"><img class="img_img"src="/images/cuxiao/zhekoushangping1.png" ><p class="div_title">'+marketing.marketingName+'</p></div>');
                    //$(".moreinfo").append("<em class='sales_label'>" + marketing.marketingName + "</em>");

                    //$(".moreinfo").append("<span class='promotion_info'>"+zkprice+"折优惠</span><br/>");
                    //满减促销
                    ...
                    if (marketing.fullbuyReduceMarketings != null && marketing.fullbuyReduceMarketings.length != 0) {
                        ...
                        str = '<div style="position: relative;"><img class="img_img"src="/images/cuxiao/manjiancuxiao1.png" ><p class="div_title">' + marketing.marketingName + '</p></div>';
                        $(".moreinfo").append(str);
                        $(".cxyj").hide();
                    }
                    //满金额赠促销
                    ...
                    if (marketing.fullbuyPresentMarketings != null && marketing.fullbuyPresentMarketings.length != 0) {
                        ...
                        //}
                        //$(".moreinfo").addClass("w400");
                        all = '<div style="position: relative;"><img class="img_img"src="/images/cuxiao/manzengcuxiao1.png" ><p class="div_title">' + marketing.marketingName + '</p></div>'
                        $(".moreinfo").append(all);
                        $(".cxyj").hide();
                    }
                    ...
                    if (marketing.fullbuyDiscountMarketings != null && marketing.fullbuyDiscountMarketings.length != 0) {
                        ...
                        //if (zhe.length >= 2) {
                            ...
                            zhe = '<div style="position: relative;"><img class="img_img"src="/images/cuxiao/manjiancuxiao1.png" ><p class="div_title">' + marketing.marketingName + '</p></div>';
                       // }
                        $(".moreinfo").append(zhe);
                    }
                }
                //团购促销
                    ...
                    //$(".moreinfo").append("<em class='sales_label'>" + marketing.marketingName + "</em>");
                    $(".moreinfo").append('<div style="position: relative;"><img class="img_img"src="/images/cuxiao/tuangoucuxiao1.png" ><p class="div_title">' + marketing.marketingName + '折团购</p></div>');
                    // $(".moreinfo").append("<span class='promotion_info'>" + Number(marketing.groupon.grouponDiscount).newmul(10) + "折团购</span><br/>");
                    $(".cxyj").show();
                    $("#priceflr").html("团&nbsp;&nbsp;购&nbsp;&nbsp;价：");
                    $(".main_price").html("<span>¥ </span>" + formatPrice(accMul(marketing.groupon.grouponDiscount, $("#mprice").val()).toString()));
                    count++;
                    isgroup = 1;
                }
                //抢购促销
                    ...
                    //$(".moreinfo").append("<em class='sales_label'>" + marketing.marketingName + "</em>");
                    $(".moreinfo").append('<div style="position: relative;"><img class="img_img"src="/images/cuxiao/xianshiqianggou1.png" ><p class="div_title">' + marketing.marketingName + '</p></div>');
                    //$(".moreinfo").append("<span class='promotion_info'>" + accMul(marketing.rushs[0].rushDiscount, 10) + "折抢购</span><br/>");
                    $(".cxyj").show();
                    $("#priceflr").html("抢&nbsp;&nbsp;购&nbsp;&nbsp;价");
                    $(".main_price").html("<span>¥ </span>" + formatPrice(accMul(marketing.rushs[0].rushDiscount, $("#mprice").val()).toString()));
                }
                    ...
                    $(".moreinfo").append('<div style="position: relative;">包邮促销<p class="div_title">支持' + marketing.shippingMoney + '免运费</p></div>');

                    //$(".moreinfo").append("<em class='sales_label'>" + marketing.marketingName + "</em>");

                    //$(".moreinfo").append("<span class='promotion_info'>支持" + marketing.shippingMoney + "免运费</span><br/>");
                }
...
function loadCoupons(){
    $.ajax({
        url:"/querySiteCouponList.htm?productId="+$("#productId").val()+"&cateId="+$("#catId").val()+"&brandId="+$("#brandId").val(),
        type:"post",
        success:function(data){
            ...
            if(null == coupons || coupons.length <= 0){
               /* $("#coupon").append("<span class=\"manjian\">无</span>");
                ...
                <div style="position: relative;">
                    <img class="img_img"src="/images/cuxiao/manzengcuxiao1.png" >
                    <p class="div_title">' + marketing.marketingName + '</p>
                </div>
                */
            }else{
                $(".saleswarp").show();
                ...
                for(var i = 0;i<coupons.length;i++){
                    var str = '<div style="position: relative;"><img class="img_img"src="/images/cuxiao/coupon.png" ><p class="div_title">';
                    if(coupons[i].couponRulesType=="1"){
                        str += '满0元减'+coupons[i].couponStraightDown.downPrice+'元优惠券';
                    }else{
                        str += '满'+coupons[i].couponFullReduction.fullPrice+'元减'+coupons[i].couponFullReduction.reductionPrice+'元优惠券';
                    }
                    str += '</p>';
                    for(var j=0 ; j<cList.length ; j++){
                        if(cList[j] == coupons[i].couponId){
                            str += '<b class="nowget"  style="color:#9e9e9e;padding-left:20px;font-weight:normal;">已领取</b>';
                            flag = false;
                        }
                    }
                    /*<b class="nowget" onclick="nowget(event)">立即领取</b>*/
                    if(flag){
                        str += '<b class="nowget" onclick="getOffCoupon('+coupons[i].couponId+',event)" style="padding-left:20px;color:#35a400;cursor:pointer;font-weight:normal;">立即领取</b>';
...
function getOffCoupon(couponId,event){
    $.ajax({
        url:"/getOffCouponSite.htm?couponId="+couponId,
        type:"post",
        success:function(data){
            if(data == 1){

                $(".info_tip_content").html("领取优惠券成功");
                $(".info_tip_img").attr("src", "../images/success.png");
                $(".info_tip_cancel").attr("href", "javascript:;").hide();
                $(".info_tip_submit").attr("href", "javascript:;").html("确定").show().unbind("click");
                $(".info_tip_submit").click(function () {
                    cls();
                ...
                $(".info_tip_content").html("领取优惠券失败，已领取完");
                $(".info_tip_img").attr("src", "../images/error.png");
                $(".info_tip_cancel").attr("href", "javascript:;").hide();
                $(".info_tip_submit").attr("href", "javascript:;").html("确定").show().unbind("click");
                $(".info_tip_submit").click(function () {
                ...
                location.href="/login.html?url=item/"+url;
            }else if(data == 4){

                $(".info_tip_content").html("领取优惠券失败，已领取过");
                $(".info_tip_img").attr("src", "../images/error.png");
                $(".info_tip_cancel").attr("href", "javascript:;").hide();
                $(".info_tip_submit").attr("href", "javascript:;").html("确定").show().unbind("click");
                $(".info_tip_submit").click(function () {
...
function formatPrice(price) {
    if (price.indexOf(".") >= 0) {
        if (price.length - price.indexOf(".") - 1 == 2) {
            price = price;
        } else if (price.length - price.indexOf(".") - 1 == 1) {
            price = price + "0";
        } else if (price.length - price.indexOf(".") - 1 == 0) {
            price = price + "00";
        } else if (price.length - price.indexOf(".") - 1 >= 3) {
            price = price.substring(0, price.indexOf(".") + 3);
        }
    } else {
        price = price + ".00";
