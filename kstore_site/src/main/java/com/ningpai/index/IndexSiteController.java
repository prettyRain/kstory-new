...
public class IndexSiteController {
    ...
    @RequestMapping("/getstatisticsCode")
    public ModelAndView getstatisticsCode(){
    ...
    public ModelAndView indexView(HttpServletRequest request) {
        ...
        if ("index/index".equals(temp.getTempUrl())) {
            ...
            // 获取当前统计代码
            mav.addObject("sCodeList", statisticsCodeBizImpl.getCurrStatisticsCode());
            ShopKuaiShang kuaishangtong=shopKuaiShangService.selectInitone();
            if(kuaishangtong != null && kuaishangtong.getKstEffectiveTerminal() !=null && kuaishangtong.getKstEffectiveTerminal().contains("1")){
                mav.addObject("shopKuaiShang", kuaishangtong);
            }
    ...
    private ModelAndView initOldTemp(ModelAndView mav, SysTemp temp) {
        ...
        // 获取页面轮播大广告
        mav.addObject("avc", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID1, ADVERTTYPE, null, "0", null, "0",null));
        // 获取页面轮播小广告
        mav.addObject("avs", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID2, ADVERTTYPE, null, "0", null, "0",null));
        // 获取页面广告
        mav.addObject("pageAdvs", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID3, ADVERTTYPE, null, "0", null, "0",null));
        // 热销推荐
     ...
     * @return
     */
    @RequestMapping("refreshStaticHtml")
    public void refreshStaticHtml(HttpServletRequest request,HttpServletResponse response){
        String jsoncallback = request.getParameter("jsoncallback");
        //删除newheader系列的html页面
        String realPath = request.getSession().getServletContext().getRealPath("/view/index");
        //删除并生成首页轮播图静态页面
        File indexHtml = new File(realPath+"/index960.html");
        if(indexHtml.exists()){
            indexHtml.delete();
        }
        topAndBottomService.staticizeIndex960(request);
        // 检查底部静态页面是否存在，不存在执行底部静态化方法
        File bottomHtml = new File(realPath + "/newbottom.html");
        if(bottomHtml.exists()){
            bottomHtml.delete();
        }
        // 静态化底部
        topAndBottomService.staticizeBottom960(request);
        //删除并生成首页静态页面
        File indexFile = new File(realPath+"/newheader960.html");
        if(indexFile.exists()){
            indexFile.delete();
        }
        ...
        SysTemp temp = getIndexDefalutTemp();
        //删除并生成频道静态页面
        List<Channel> channels = channelService.selectAllByTempId(temp.getTempId());
        for (Channel channel : channels) {
            File channelFile = new File(realPath + "/newheader960_"+channel.getChannelId()+".html");
        ...
        try {
            response.getWriter().write(jsoncallback+"(1)");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
