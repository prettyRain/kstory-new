    ...
    <div class="container">
        ...
        <div class="member_box mb20">
						...
						<ul class="item_list">
							<#if (infoType.childs??)>
								<#list infoType.childs as infoT2 >
									<#if (infoT2.infos??)>
										<#list infoT2.infos as info2 >
											<li <#if (cInfoType??
											&& info2.infoTypeId == cInfoType.infoTypeId && info2.infoId == info.infoId)>class="cur"</#if>><a href="${basePath}/information/${info2.infoId}"
												<#if (cInfoType?? && info2.infoTypeId == cInfoType.infoTypeId && info2.infoId == info.infoId)> style="color:#35a400;"</#if>>
											${info2.title?default("")}</a></li>
										</#list>
									</#if>
								</#list>
							</#if>
							<#if (infoType.infos??)>
								<#list infoType.infos as info1 >
									<li <#if (cInfoType??
									&& info1.infoTypeId == cInfoType.infoTypeId && info1.infoId == info.infoId)>class="cur"</#if>><a href="${basePath}/information/${info1.infoId}"
									 <#if (cInfoType?? && info1.infoTypeId == cInfoType.infoTypeId && info1.infoId == info.infoId)> style="color:#35a400;"</#if>>
									${info1.title?default("")}</a></li>
								</#list>
							</#if>
						</ul>
					</div>
            ...
            <div class="member_right fl ml10">
                <#--<div class="memebr_title mb20" style="border-bottom:1px solid #E6E6E6;">
                    <h2 class="f14 fb" style="line-height: inherit;">资讯中心</h2>
                </div>-->
                <div class="order_list switch_border">
                	<div class="tagMenu" style="height:0px;border-bottom:none;">
                    </div>
