...
public class ShoppingCartServiceImpl implements ShoppingCartService {
     ...
     */
    @Override
    public int delShoppingCartById(String[] cartIds, String[] goodsIds, HttpServletRequest request, HttpServletResponse response) {
        Long customerId = (Long) request.getSession().getAttribute(CUSTOMERID);
        if (customerId != null) {
            for(String cartId:cartIds){
                shoppingCartMapper.delShoppingCartById(Long.valueOf(cartId));
            }
            return 1;
        } else {
            // 调用cookie删除接
            try {
                delCookShopCar(goodsIds, request, response);
    ...
    public int changeShoppingCartById(Long shoppingCartId, Long num,HttpServletResponse response,Long goodsInfoId) {
        ...
        if (customerId != null) {
            ...
            try {
                ...
                sc.setDistinctId(dId);
                addCart2Cookie(sc, request, response,"1");
            }catch(Exception e){
                return 0;
            }
            return 1;
    ...
    public int addShoppingCart(ShoppingCart shoppingCart, HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        if (null != custId) {
            ...
            return addCart2Cookie(shoppingCart, request, response,"0");
        }
    }
     ...
     * @return
     */
    public Long searchShopcartNum(HttpServletRequest request,Long custId){
        if(null != custId){
            return searchNewShopcartNum(custId);
        }else{
            return searchNewShopcartNumByCookie(request,custId);
        }
    }
    public Long searchNewShopcartNumByCookie(HttpServletRequest request,Long custId) {
        List<Cookie> cookies = KCollection.defaultList(request.getCookies());
        Long i=0l;
        for (Cookie cookie : cookies) {
            if (NPSTORE_SHOPCAR.equals(cookie.getName())) {
                String cookieStr = URLDecoder.decode(cookie.getValue());
                if(StringUtils.isNotBlank(cookieStr)){
                    String[] splits = cookieStr.split("e");
                    for(String split:splits){
                        if(split.length()>3){
                          i += Long.valueOf(split.substring((split.indexOf("-")+1),(split.indexOf("&"))));
                        }
                    }
                }
    ...
    public Long searchNewShopcartNum(Long custId){
        int i = this.shoppingCartMapper.countByCustomerId(custId);
     ...
     * @param request
     * @param response
     * @param isCartAddNum  "1" 代表是购物车中改变商品的数量
     * @return
     * @throws UnsupportedEncodingException
     */
    private int addCart2Cookie(ShoppingCart shoppingCart, HttpServletRequest request, HttpServletResponse response,String isCartAddNum) throws UnsupportedEncodingException {
        String numStr = "";
        List<Cookie> cookies = KCollection.defaultList(request.getCookies());
        String cartStr = "";
        String mId = "";
        ...
        if(org.apache.commons.lang.StringUtils.isNotBlank(mId)){
            String[] goods = mId.split("-");
            for(String good:goods){
                if(org.apache.commons.lang.StringUtils.isNotBlank(good)){
                    if(!good.substring(0,4).equals(shoppingCart.getGoodsInfoId().toString())){
                        newmId += "-"+good;
                    }
                }
        ...
        //如果是在购物车页面改变商品数量
        if("1".equals(isCartAddNum)){
            numStr = String.valueOf(shoppingCart.getGoodsNum());
        }else{
            numStr = String.valueOf(num + shoppingCart.getGoodsNum());
        }

        if(Integer.valueOf(numStr)>99){
            numStr = "99";
        }
        //现在不考虑已下架 和 库存不足  2018.2.27
       /* if (shoppingCart.getFitId() == null) {
            ...
            if (goodsDetailBean != null) {
                ...
                if(goodsDetailBean.getProductVo().getGoodsInfoAdded()==null || Integer.valueOf(goodsDetailBean.getProductVo().getGoodsInfoAdded())!=1){
                    //已下架
                    return -3;
                }
                if(goodsDetailBean.getProductVo().getGoodsInfoStock()!=null && goodsDetailBean.getProductVo().getGoodsInfoStock()>0l){
                    if(Long.valueOf(numStr).longValue()>goodsDetailBean.getProductVo().getGoodsInfoStock().longValue()){
                        //库存不足
                        return -5;
                    }
                }else{
        ...
        //在购物车改变商品数量时不需要 改变促销活动
        if("0".equals(isCartAddNum)){
            if (shoppingCart.getFitId() == null) {
                ...
                /*activeMarketId = marketService.queryByCreatimeMarketings(goodsInfoId, 2L, cateId,
                        brandId);*/
                    //跟用户已登录时的逻辑改成一样的（每一个商品只能有一个活动 ）
                    //activeMarketId = marketService.queryByCreatimeMarketingsWithPanicBuying(goodsInfoId,cateId,brandId);
                    List<Marketing> markList = marketService.selectMarketingByGoodsInfoId(goodsInfoId, brandId, cateId);
                    if(markList!=null && markList.size()>0){
                        activeMarketId = markList.get(0).getMarketingId();
                    }
                }

                newmId += "-" + goodsInfoId + "e" + marketId + "e" + activeMarketId +"e"+"0" +"e" + "0";
            }
        }
        ...
        Cookie cookie = new Cookie(NPSTORE_MID, newmId);
        cookie.setMaxAge(15 * 24 * 3600);
        cookie.setPath("/");
        response.addCookie(cookie);
        return 1;
    ...
    public int selectSumByCustomerId(Long custId){
       return shoppingCartMapper.selectSumByCustomerId(custId);
    }
...
//                GoodsDetailBean goodsDetailBean = goodsProductService.queryDetailByProductId(shoppingCart.getGoodsInfoId(), Long.parseLong("0"));
                Long goodsInfoId = shoppingCart.getGoodsInfoId();
                GoodsDetailBean goodsDetailBean = goodsProductService.queryDetailByProductId(goodsInfoId, shoppingCart.getDistinctId() != null ? shoppingCart.getDistinctId() : Long.parseLong("0"));
                if (goodsDetailBean != null && "1".equals(goodsDetailBean.getProductVo().getGoodsInfoAdded())) {
                    // 活动优惠id
                    Long brandId = goodsDetailBean.getBrand().getBrandId();
                    Long catId = goodsDetailBean.getProductVo().getGoods().getCatId();
...
//                        }
                  ...
                  /*if(goodsDetailBean.getProductVo().getGoodsInfoStock()!=null && goodsDetailBean.getProductVo().getGoodsInfoStock()>0l){
                        if(shoppingCart.getGoodsNum().longValue()>goodsDetailBean.getProductVo().getGoodsInfoStock().longValue()){
                            //库存不足
                            return -5;
                        }
                  }else{
           ...
           /* if(shoppingCart.getFitId() == null){
                ...
                if(goodsDetailBean!=null){
                    if(goodsDetailBean.getProductVo().getGoodsInfoStock()!=null && goodsDetailBean.getProductVo().getGoodsInfoStock()>0l){
                        if((sc.getGoodsNum().longValue() + shoppingCart.getGoodsNum().longValue())>goodsDetailBean.getProductVo().getGoodsInfoStock().longValue()){
                            //库存不足
                            return -5;
                        }
                    }else{
            ...
            if ((sc.getGoodsNum() + shoppingCart.getGoodsNum())>99l) {
                shoppingCart.setGoodsNum(99l-sc.getGoodsNum());
            }
        ...
        if (productVo == null) {
            ...
            if (mk != null) {
                ...
                if (num != null && stock - num - shoppingCart.getGoodsNum() <= 0) {
                   // return 0;  抢购时可以超过限购数量  超过的部分价格变成正常价格
                }
            }
            return 1;
        }
     ...
     * @throws UnsupportedEncodingException
     */
    public int delCookShopCar(String[] goodsIds, HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        Integer count = 0;
        Cookie[] cookies = request.getCookies();
        String oldCar = "";
        String[] cars = null;
            ...
            for (Cookie cookie : cookies) {
                if (null != cookie && NPSTORE_SHOPCAR.equals(cookie.getName())) {
                    ...
                    for (String goodsId : goodsIds) {
                        Long productId = Long.valueOf(goodsId);
                ...
                if (cookie != null && NPSTORE_MID.equals(cookie.getName()) && cookie.getValue() != null && !"".equals(cookie.getValue())) {
                     newMid = URLDecoder.decode(cookie.getValue(),"utf-8");
                    // 取得所有cookie
                    for (String goodsId : goodsIds) {
                        Long productId = Long.valueOf(goodsId);
                        String[] mIds = newMid.split("-");
                        newMid = "";
                    for (int j = 0; j < mIds.length; j++) {
                            String[] mid = mIds[j].split("e");
                            // 判断是否该商品
                ...
                if (count == 0) {
                    ...
                    for (ShoppingCart sc : scList) {
                        if (shoppingCart.getGoodsInfoId().toString().equals(sc.getGoodsInfoId().toString())) {
                            sc.setGoodsNum(shoppingCart.getGoodsNum());
    ...
    public List<Coupon> getUsedCouponlist(HttpServletRequest request, Long[] box) {
        ...
        for (ShoppingCart cart : noFitList) {
            ...
            // 查询商品参加的促销信息
            List<Marketing> marketingList = marketService.selectMarketingByGoodsInfoId(cart.getGoodsInfoId(), detailBean.getBrand()
                    .getBrandId(), detailBean.getProductVo().getGoods().getCatId());
            //直降 团购 折扣（后的价格）
            for(Marketing mark:marketingList){
                if(mark.getGroupon()!=null &&cart.getMarketingActivityId()!=null&& cart.getMarketingActivityId().longValue()==mark.getMarketingId().longValue()) {
                    // isgroup = 1;
                    productVo.setGoodsInfoPreferPrice(goodsprice.multiply(mark.getGroupon().getGrouponDiscount()));
                }else if(mark.getPriceOffMarketing()!=null&&cart.getMarketingActivityId()!=null&& cart.getMarketingActivityId().longValue()==mark.getMarketingId().longValue()){
                     productVo.setGoodsInfoPreferPrice(goodsprice.subtract(mark.getPriceOffMarketing().getOffValue()));
                }else if (cart.getMarketingActivityId() != null && mark.getMarketingId().longValue()== cart.getMarketingActivityId().longValue()&&mark.getPreDiscountMarketings()!=null ) {
                    // 从购物车里得到促销id重新从数据库查询,防止当前(折扣促销)已经过期;
                    Marketing marke = marketService.marketingDetail(cart.getMarketingActivityId(), cart.getGoodsInfoId());
                    if (mark != null) {
     ...
     */
    @Override
    public Map<String, Object> getPayorderThirdPriceMap(Long businessId, List<ShoppingCart> shopdata, Long distinctId,Long customerId) {
        Map<String, Object> paramMap = new HashMap<>();
        ...
        //满减满折优惠金额
        BigDecimal myhPrice = BigDecimal.ZERO;
        ...
        if (CollectionUtils.isNotEmpty(shoplist)) {
            ...
            for (ShoppingCart cart : shoplist) {
                ...
                if (cart.getFitId() == null) {
                   ...
                   /* // 是否抹掉分角
                    ...
                    // 处理货品的价格()
                    productVo.setGoodsInfoPreferPrice(goodsprice);*/
                    // 货品价格  这个价格已经是折扣完或者团购折扣完后的价格
                    goodsprice = cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    // 货品购买件数
                ...
                if (mark != null) {
                    ...
                    for (Long markId : priceMap.keySet()) {
                        if (markId.equals(ob)) {
                            ...
                            //满减满折的优惠金额
                            myhPrice = myhPrice.add(prePrice);
                            //boss 优惠金额
                            if (businessId == 0) {
            ...
            for (int k = 0; k < cartList.size(); k++) {
                if (cartList.get(k).getMarketingActivityId() != null) {
                    // 直降
                    if (cartList.get(k).getMarketing() != null && cartList.get(k).getMarketing().getPriceOffMarketing() != null) {
                        ...
                        if (desPrice.compareTo(srcPrice) > 0) {
                            ...
                            if(cartList.get(k).getThirdId() == 0){
                                bossSumPrice = bossSumPrice.subtract(srcPrice.subtract(BigDecimal.valueOf(0.01)));
                            }
                        } else {
                            flag = flag.subtract(desPrice);
                            if(cartList.get(k).getThirdId() == 0){
                                bossSumPrice = bossSumPrice.subtract(desPrice);
                            }
                        }
...
//                }
                    ...
                    if (cartList.get(k).getMarketing() != null && cartList.get(k).getMarketing().getRushs() != null && !cartList.get(k).getMarketing().getRushs().isEmpty()) {
                        Map rushMap = new HashMap();
                        rushMap.put("rushId", cartList.get(k).getMarketing().getRushs().get(0).getRushId());
                        rushMap.put("customerId", customerId);
                        rushMap.put("goodsInfoId", cartList.get(k).getGoodsInfoId());
                        //当存在抢购时 得到每个会员可以购买的货品数量
                        Integer countRush = rushCustomerMapper.selectByParamMap(rushMap);
                        countRush = countRush == null ? 0 : countRush;
                        countRush = cartList.get(k).getMarketing().getRushs().get(0).getRushLimitation() - countRush;
                        ...
                        if (countRush > cartList.get(k).getGoodsNum()) {
                            multiply = (BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cartList.get(k).getGoodsNum()));
                            ...
                            multiply = (BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush));
                            flag = flag.subtract((BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                        }
                        //计算BOSS商品抢购促销优惠金额
                        if (businessId == 0) {
                            //bossPrePrice = bossPrePrice.add(multiply);
                           // rushPrePrice = rushPrePrice.add(multiply);
                            ...
                            if (cartList.get(k).getThirdId() == 0) {
                                //抢购商品原价总和
                                rushSumPrice = rushSumPrice.add(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.valueOf(cartList.get(k).getGoodsNum())));
                                //可以抢购的 商品数量 的优惠
                                //抢购后的boss商品总金额
                                if(cartList.get(k).getGoodsNum()>=countRush){
                                    bossSumPrice = bossSumPrice.subtract((BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                                    rushPrePrice = rushPrePrice.add((BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                                }else{
                                    bossSumPrice = bossSumPrice.subtract((BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cartList.get(k).getGoodsNum())));
                                    rushPrePrice = rushPrePrice.add((BigDecimal.ONE.subtract(cartList.get(k).getMarketing().getRushs().get(0).getRushDiscount())).multiply(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cartList.get(k).getGoodsNum())));
                                }
                               // rushSumPrice = rushSumPrice.add(cartList.get(k).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.valueOf(cartList.get(k).getGoodsNum())));
                            }
                        }
        ...
        paramMap.put(SUMPRICE,flag);//直降 折扣 团购 抢购 后的价格总和
        paramMap.put("myhPrice",myhPrice);//满减 满折 的优惠总和
        ...
        //
        paramMap.put("curZheKouPrice",(bossSumPrice.subtract(bossPrePrice).subtract(rushSumPrice.subtract(rushPrePrice))));
     ...
     */
    @Override
    public Map<String, Object> getEveryThirdPriceMap(Long customerId,Long businessId, List<ShoppingCart> shopdata, Long distinctId) {
        Map<String, Object> paramMap = new HashMap<>();
        ...
        paramMap.put(STOCK, "1");
        //表示商品在该地区的仓库无货
        paramMap.put("repertory","1");
        //表示商品在该地区无仓库
        paramMap.put("isware","1");
        List<ShoppingCart> shoplist = new ArrayList<>();
        ...
        BigDecimal myhPrice = BigDecimal.valueOf(0);
        // 原始总金额
...
//                        productWare = this.productWareService.queryProductWareByProductIdAndDistinctId(cart.getGoodsInfoId(), distinctId);
                        ...
                        if (null != productWare) {
                            ...
                            if (productWare.getWareStock() - goodsNum < 0) {
                                paramMap.put("repertory", "0");//表示库存不足
                                stockBean = new GoodsStockBean();
                                stockBean.setGoodsId(cart.getGoodsInfoId());
                                stockBean.setStockNum(String.valueOf(productWare.getWareStock()));
                            ...
                            paramMap.put("isware", "0"); //表示商品在此地区无对应的仓库
                            stockBean = new GoodsStockBean();
                            stockBean.setGoodsId(cart.getGoodsInfoId());
                            stockBean.setStockNum("0");
                            stockList.add(stockBean);
                ...
                if (null != goodsGroupVo) {
                    ...
                    for (int j = 0; j < goodsGroupReleProductVos.size(); j++) {
                        ...
                        goodsGroupReleProductVos.get(j).getProductDetail().setSellType(productVo.getGoods().getSellType());
                        goodsGroupReleProductVos.get(j).getProductDetail().setUnitPrice(productVo.getGoods().getUnitPrice());
                ...
                for (Long markId : priceMap.keySet()) {
                    if (markId.equals(ob)) {
                        ...
                        if (CollectionUtils.isNotEmpty(mark.getFullbuyReduceMarketings())) {
                            ...
                            //prePrice = prePrice.add(marketflag);
                            marketflag = BigDecimal.ZERO;
                            myhPrice = myhPrice.add(prePrice);
                        }
                        // 满折促销
        ...
        for (ShoppingCart cart : cartList) {
            ...
            if (cart.getGoodsDetailBean().getProductVo() != null) {
                marketings = marketService.selectMarketingByGoodsInfoId(cart.getGoodsInfoId(), cart.getGoodsDetailBean().getBrand()
                        .getBrandId(), cart.getGoodsDetailBean().getProductVo().getGoods().getCatId());
            }
            BigDecimal goodspriceGo = cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
            if(cart.getMarketingActivityId() !=null){
            for (Marketing marketing : marketings) {
                //如果促销与商品的活动id一样  说明是同一个活动
                if (marketing.getMarketingId().longValue() == cart.getMarketingActivityId().longValue()) {
                    if (marketing.getPriceOffMarketing() != null) {
                        //直降
                        if (marketing.getPriceOffMarketing().getOffValue() != null && goodspriceGo.compareTo(marketing.getPriceOffMarketing().getOffValue()) == 1) {
                            flag = flag.subtract(marketing.getPriceOffMarketing().getOffValue().multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                            if (cart.getThirdId() == 0) {
                                bossSumPrice = bossSumPrice.subtract(marketing.getPriceOffMarketing().getOffValue().multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                                //bossPrePrice = bossPrePrice.add(marketing.getPriceOffMarketing().getOffValue().multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                            }
                        }

                        //抢购
                        } else if (marketing.getRushs() != null && !marketing.getRushs().isEmpty()) {
                        Map<String, Object> rushMap = new HashMap<>();
                        rushMap.put("rushId", marketing.getRushs().get(0).getRushId());
                        rushMap.put("customerId", customerId);
                        rushMap.put("goodsInfoId", cart.getGoodsInfoId());
                        //当存在抢购时 得到每个会员还可以购买的货品数量
                        Integer countRush = rushCustomerMapper.selectByParamMap(rushMap);
                        countRush = countRush == null ? 0 : countRush;
                        //得到还可以购买的数量
                        countRush = marketing.getRushs().get(0).getRushLimitation() - countRush;
                        if(countRush<=0){
                            countRush = 0;
                        }
                        //可以抢购的 商品数量 的优惠
                        //BigDecimal prePrice1;
                        BigDecimal prePrice1 = (BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount()))
                                .multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice())
                                .multiply(BigDecimal.valueOf(cart.getGoodsNum()));
                        if(cart.getGoodsNum()>countRush){
                            flag = flag.subtract((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                        }else{
                            flag = flag.subtract((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                        }
                        //计算BOSS商品抢购促销优惠金额
                        if(cart.getThirdId()==0){
                            //抢购商品原价总和
                            rushSumPrice = rushSumPrice.add(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                            //可以抢购的 商品数量 的优惠
                            //抢购后的boss商品总金额
                            if(cart.getGoodsNum()>=countRush){
                                bossSumPrice = bossSumPrice.subtract((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                                rushPrePrice = rushPrePrice.add((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(countRush)));
                            }else{
                                bossSumPrice = bossSumPrice.subtract((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                                rushPrePrice = rushPrePrice.add((BigDecimal.ONE.subtract(marketing.getRushs().get(0).getRushDiscount())).multiply(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice()).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                            }
                        }
                        /*if (businessId == 0) {
                        ...
                        //团购促销
                    } else if (marketing.getGroupon() != null) {
                        flag = flag.subtract(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.ONE.subtract(marketing.getGroupon().getGrouponDiscount())).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                         //减掉团购优惠后的boss商品总金额
                        if(cart.getThirdId() == 0){
                         bossSumPrice = bossSumPrice.subtract(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.ONE.subtract(marketing.getGroupon().getGrouponDiscount())).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                        }
                        //折扣促销
                    } else if (marketing.getPreDiscountMarketings() != null) {
                        ...
                        for (PreDiscountMarketing premark : marketing.getPreDiscountMarketings()) {
                            ...
                            if (premark.getGoodsId().equals(cart.getGoodsInfoId())) {
                                ...
                                myformat.setRoundingMode(RoundingMode.FLOOR);
                                BigDecimal num = cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.ONE.subtract(premark.getDiscountInfo()));
                                goodspriceGo = BigDecimal.valueOf(Double.valueOf(myformat.format(num)));
                                flag = flag.subtract(goodspriceGo.multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                                //减掉折扣优惠后的boss商品总金额
                                if(cart.getThirdId() == 0){
                                    bossSumPrice = bossSumPrice.subtract(goodspriceGo.multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                                }
                            }
       ...
       /* for (ShoppingCart cart : cartList) {
        ...
        BigDecimal sumPrice = flag.subtract(taozhuan);
        //满减满折优惠价格
        paramMap.put("prePrice",myhPrice);
        paramMap.put("sumPrice", sumPrice.subtract(myhPrice));
        paramMap.put("sumOldPrice", sumPrice);
        //没有抢购的优惠金额
        paramMap.put("rushPrePrice", rushPrePrice);
        //没有抢购的总boss金额计算会员折扣
        paramMap.put("rushSumPrice", rushSumPrice);
        paramMap.put(BOSSSUMPRICE, bossSumPrice);
        paramMap.put(BOSSPREPRICE, bossPrePrice);
        paramMap.put("curZheKouPrice",(bossSumPrice.subtract(bossPrePrice).subtract(rushSumPrice.subtract(rushPrePrice))));
        return paramMap;
    ...
    public Map<String, Object> newsubOrder(HttpServletRequest request, Long[] box, CustomerAddress customerAddress, String[] boxgift) {
        //获取用户信息
        Long customer = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
        //购物车信息
        ...
        //根据session中城市 查新商品（只是为了查询商品是零售的还是批发的）
        Long distinct_Id = (Long)request.getSession().getAttribute("distinctId");
        //如果session中没有地址 那么获取默认地址
        if(distinct_Id==null || distinct_Id==0l){
            // 默认地区ID
            distinct_Id = addressService.getDefaultIdService();
            request.getSession().setAttribute("distinctId",distinct_Id);
        }
        wareUtil.setDistrictId(distinct_Id);
        shoppingCartDetail(customer,shoplist, wareUtil);
        Integer lingshou = 0;
        Integer num = 0;
        //判断零售批发
        for(ShoppingCart cart:shoplist){
             num++;
            if(cart.getGoodsDetailBean().getProductVo().getProductName().indexOf("零售")!=-1){
                lingshou++;
            };

            if(cart.getPresentGoodsProducts()!=null && cart.getPresentGoodsProducts().size()>0 && cart.getPresentGoodsProducts()!=null){
                for(GoodsProduct goodsProduct:cart.getPresentGoodsProducts()){
                    if(goodsProduct.getGoodsInfoName().indexOf("零售")!=-1){
                        lingshou++;
                    }
                    num++;
                }
        ...
        if(lingshou <1 || lingshou < num){
            //普通地址 如果用户的默认地址为社区店地址 那么设为null

            if(customerAddress != null && customerAddress.getAddressType()!=null && "1".equals(customerAddress.getAddressType())){
                customerAddress=null;
            }
        }else{
           //社区店地址  如果用户的默认地址是普通地址 那么设为null
            if(customerAddress != null && customerAddress.getAddressType()!=null && "0".equals(customerAddress.getAddressType())){
                customerAddress=null;
            }
        }

        Map<String, Object> cartMap = new HashMap<>();
         //如果收货地址为空 那么选择session中的区县id
        if (customerAddress == null || customerAddress.getInfoCounty() == null) {
            customerAddress = new CustomerAddress();
            Long distinctId = Long.valueOf(request.getSession().getAttribute("distinctId").toString());
            customerAddress.setInfoCounty(distinctId.toString());
            District distinct = districtService.findDistrictByPrimaryKey(distinctId);
            customerAddress.setInfoCity(distinct.getCityId().toString());
        }
        ...
        shoppingCartDetail(customer,shoplist, wareUtil);
        for(ShoppingCart cart:shoplist){
            cart.setDistinctId(wareUtil.getDistrictId());
        }

        // 判断购物车中的商品 是否有达到限购的 达到了限购 则直接返回
        //现在的业务逻辑是 超过抢购数量部分按正常价格算
...
//        }
        ...
        if (null != customerPoint && null != pointSet) {
            // 保存当前登录会员总积分
            cartMap.put("customerPoint", customerPointServiceMapper.getCustomerAllPointSimple(customerId+"") - customerPointServiceMapper.getCustomerReducePoint(customerId + ""));
            // 保存积分兑换金额的比例 （消费规则）
        ...
        for (StoreTemp st : storeList) {
            Map<String, Object> price = getEveryThirdPriceMap(customer,st.getThirdId(), shoplist, Long.parseLong(customerAddress.getInfoCounty()));
            //存在库存下架的商品
            ...
            //商品是否有货 0无货  1有货
            cartMap.put("repertory",price.get("repertory"));
            //商品在该地区是否有仓库
            cartMap.put("isware",price.get("isware"));
        ...
        //查询会员折扣
        BigDecimal pointDiscount = customerPointServiceMapper.getCustomerDiscountByPoint(customerPointServiceMapper.getCustomerAllPointSimple(customerId+"")-customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+""));
        //查询boss 商品的总价 和 所有商品的总价格
        Map<String, Object> bossPrice = getEveryThirdPriceMap(0l,0L, shoplist, Long.parseLong(customerAddress.getInfoCounty()));
        //boss商品总价减去促销优惠后金额
        ...
        //
        BigDecimal zheKPrice = ((BigDecimal)bossPrice.get("curZheKouPrice")).multiply(BigDecimal.ONE.subtract(pointDiscount));


        BigDecimal bossyouhui = BigDecimal.valueOf(Double.valueOf(bossPrice.get(BOSSPREPRICE).toString()));
        //优惠后(含满减满折 折扣 团购 抢购 直降) 含会员折扣后
        cartMap.put("bossPrice", ((BigDecimal) ((BigDecimal) bossPrice.get("bossSumPrice")).subtract((BigDecimal) bossPrice.get("bossPrePrice"))).subtract(zheKPrice).subtract((BigDecimal)((BigDecimal) bossPrice.get("rushSumPrice")).subtract((BigDecimal)bossPrice.get("rushPrePrice"))));
        cartMap.put("couponlist", couponList);
        //boss商品会员折扣优惠
        cartMap.put("zheKPrice", zheKPrice);
        //促销优惠价格
        cartMap.put(THIRDS, storeList);
        cartMap.put(MARKETINGLIST, marketinglist);
        cartMap.put("bossyouhui", bossyouhui);
        //计算运费
        Map<String, Object> fPrice = getNewExpressPrice(shoplist,Long.parseLong(customerAddress.getInfoCity()), Arrays.asList(box), Long.parseLong(customerAddress.getInfoCounty()));
    ...
    public BigDecimal computeFreightAll(String motheds, FreightExpressAll frall, Integer num, BigDecimal weight) {
        ...
        if ("0".equals(motheds)) {
            ...
            //计算运费时，订单总重量按90%计算规则
            weight = weight.multiply(BigDecimal.valueOf(0.9));
            // 如果购买重量大于或者等与会
    ...
    public BigDecimal calExpressPriceByThirdId(Long thirdId, Long cityId, List<ShoppingCart> cartList) {
        ...
        for (ShoppingCart sc : cartList) {
            if (sc.getThirdId().equals(thirdId)) {
                // 判断是否是套装
                if (sc.getFitId() == null) {
                    Long distinctId = (Long)request.getSession().getAttribute("distinctId");
                    // 如果是普通商品，执行普通商品的方法
                    GoodsProductVo goodsProduct = null;
                    if(distinctId!=null){
                        goodsProduct = goodsProductService.queryDetailByProductId(sc.getGoodsInfoId(), distinctId).getProductVo();
                    }else{
                        goodsProduct = goodsProductService.queryProductByProductId(sc.getGoodsInfoId());
                    }

                    BigDecimal finalPrice = goodsProduct.getGoodsInfoPreferPrice();
                    //查询商品是否有促销，有就显示促销价
                    List<GoodsMarketingCodeUtil> codeUtils = goodsElasticMapper.selectProductMarket(goodsProduct.getGoodsInfoId());
                    /*for (GoodsMarketingCodeUtil gms : codeUtils) {
                        if (gms.getCodexId() == 1) {
                            ...
                            finalPrice = goodsProduct.getGoodsInfoPreferPrice().subtract(discount);
                            totalPrice = totalPrice.add(finalPrice*s);
                        } else if (gms.getCodexId() == 10) {
                            //团购
                            BigDecimal discount = goodsElasticMapper.selectGrouponByMarketId(gms.getMarketingId());
                            finalPrice = goodsProduct.getGoodsInfoPreferPrice().multiply(discount);
                        } else if (gms.getCodexId() == 11) {
                            Map<String,Object> rushMap=new HashMap<>();
                            rushMap.put("rushId",sc.getMarketingList().get(0).getRushs().get(0).getRushId());
                            rushMap.put("customerId",request.getSession().getAttribute("customerId"));
                            rushMap.put("goodsInfoId",sc.getGoodsInfoId());
                            //当存在抢购时 得到每个会员还可以购买的货品数量
                            Integer countRush=rushCustomerMapper.selectByParamMap(rushMap);
                            countRush=countRush==null?0:countRush;
                            countRush=sc.getMarketingList().get(0).getRushs().get(0).getRushLimitation()-countRush;
                            countRush = countRush<=0?0:countRush;
                            //抢购
                            ...
                            param.put("goodsId", goodsProduct.getGoodsInfoId());
                            param.put("marketingId", gms.getMarketingId());
                            BigDecimal discount = goodsElasticMapper.selectDiscountByMarketId(param);
                            finalPrice = goodsProduct.getGoodsInfoPreferPrice().multiply(discount);
                        }
                    }*/
                    if(sc.getMarketingList()!=null && sc.getMarketingList().size()>0 && "1".equals(sc.getMarketingList().get(0).getCodexType())){
                        //直降
                        BigDecimal discount = goodsElasticMapper.selectPriceOffMarketingByMarketingId(sc.getMarketingActivityId());
                        totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().subtract(discount).multiply(BigDecimal.valueOf(sc.getGoodsNum())));
                        goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                    } else if (sc.getMarketingList()!=null && sc.getMarketingList().size()>0 && "10".equals(sc.getMarketingList().get(0).getCodexType())) {
                        //团购
                        BigDecimal discount = goodsElasticMapper.selectGrouponByMarketId(sc.getMarketingActivityId());
                        totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(discount).multiply(BigDecimal.valueOf(sc.getGoodsNum())));
                        goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                    }else if (sc.getMarketingList()!=null && sc.getMarketingList().size()>0 && "15".equals(sc.getMarketingList().get(0).getCodexType())) {
                        //折扣
                        Map<String, Object> param = new HashMap<String, Object>();
                        param.put("goodsId", goodsProduct.getGoodsInfoId());
                        param.put("marketingId", sc.getMarketingActivityId());
                        BigDecimal discount = goodsElasticMapper.selectDiscountByMarketId(param);
                        totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(discount).multiply(BigDecimal.valueOf(sc.getGoodsNum())));
                        goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                    }else if (sc.getMarketingList()!=null && sc.getMarketingList().size()>0 && "11".equals(sc.getMarketingList().get(0).getCodexType())) {
                        Map<String,Object> rushMap=new HashMap<>();
                        rushMap.put("rushId",sc.getMarketingList().get(0).getRushs().get(0).getRushId());
                        rushMap.put("customerId",request.getSession().getAttribute("customerId"));
                        rushMap.put("goodsInfoId",sc.getGoodsInfoId());
                        ...
                        countRush = countRush<=0?0:countRush;
                        //抢购
                        BigDecimal discount = goodsElasticMapper.selectRushByMarketId(sc.getMarketingList().get(0).getRushs().get(0).getMarketId());
                        if(countRush.longValue() >= sc.getGoodsNum().longValue()){
                            totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(discount).multiply(BigDecimal.valueOf(sc.getGoodsNum())));
                            BigDecimal rushDiscount = sc.getMarketingList().get(0).getRushs().get(0).getRushDiscount();
                            if(rushDiscount.compareTo(BigDecimal.ZERO)==0 && goodsProduct.getProductName().indexOf("零售")!=-1){
                                //
                            }else {
                                goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                            }
                        }else if(countRush.longValue() < sc.getGoodsNum().longValue()){
                            totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(BigDecimal.valueOf(sc.getGoodsNum()).subtract(BigDecimal.valueOf(countRush))));
                            totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(discount).multiply(BigDecimal.valueOf(countRush)));
                            BigDecimal rushDiscount = sc.getMarketingList().get(0).getRushs().get(0).getRushDiscount();
                            if(rushDiscount.compareTo(BigDecimal.ZERO)==0 && goodsProduct.getProductName().indexOf("零售")!=-1){
                                goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(BigDecimal.valueOf(sc.getGoodsNum()).subtract(BigDecimal.valueOf(countRush))));
                            }else {
                                goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                            }
                        }
                    }else{
                        totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(new BigDecimal(sc.getGoodsNum())));
                        goodsweight = goodsweight.add(goodsProduct.getGoodsInfoWeight().multiply(BigDecimal.valueOf(sc.getGoodsNum())));
                    }

                    goodsnum += Integer.parseInt(sc.getGoodsNum().toString());
                    if(goodsProduct.getProductName().indexOf("零售")<0) {
                        pifa = true;
                    }
                    /*totalPrice = totalPrice.add(goodsProduct.getGoodsInfoPreferPrice().multiply(new BigDecimal(sc.getGoodsNum())));*/
                } else {
                    ...
                    for (GoodsProductVo goodsProduct : goodsProducts) {
                        BigDecimal finalPrice = goodsProduct.getGoodsInfoPreferPrice();
                        //查询商品是否有促销，有就显示促销价
                        List<GoodsMarketingCodeUtil> codeUtils = goodsElasticMapper.selectProductMarket(goodsProduct.getGoodsInfoId());
                        for (GoodsMarketingCodeUtil gms : codeUtils) {
                        ...
                        goodsProduct.setGoodsInfoPreferPrice(finalPrice);
                    }
                    // 遍历套装下的商品
                    for (GoodsProductVo productVO : goodsProducts) {
                        if(productVO.getProductName().indexOf("零售")<0) {
                            pifa = true;
                        }
                        goodsweight = goodsweight.add(productVO.getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                        goodsnum += Integer.parseInt(sc.getGoodsNum().toString());
                        totalPrice = totalPrice.add(productVO.getGoodsInfoPreferPrice().multiply(new BigDecimal(sc.getGoodsNum())));
                    }
                ...
                for (int i = 0; i < fe.size(); i++) {
                    ...
                    if (fall != null && !fall.isEmpty()) {
                        for (int j = 0; j < fall.size(); j++) {
                            //判断订单的总金额所对应运费模板的价格区间
                            BigDecimal minPrice = fall.get(j).getMinPrice();
                            BigDecimal maxPrice = fall.get(j).getMaxPrice();
                            boolean qujian = false;
                            //最小值>0，最大值=0，表明该区间没有设置上限
                            qujian = maxPrice.compareTo(BigDecimal.ZERO) == 0 && minPrice.compareTo(BigDecimal.ZERO) == 1 && totalPrice.compareTo(minPrice) > 0;
                            //最大值>最小值，总金额>最小值，总金额<=最大值，符合区间范围
                            qujian = qujian || (maxPrice.compareTo(minPrice) == 1 && totalPrice.compareTo(minPrice) > 0 && totalPrice.compareTo(maxPrice) < 1);
                            if (qujian) {
                                // 获取其他地区设置
    ...
    @Override
    public Map<String, Object> getNewExpressPrice(List<ShoppingCart> shoppingCarts,Long cityId, List<Long> cartIds, Long cityCountryId) {
        ...
        List<ShoppingCart> cartList = getNobaoyouShoppingcarts(shoppingCarts, cityCountryId);
        //获取总重量
        BigDecimal goodsweight = BigDecimal.valueOf(0);
...
//                        }
        ...
        for(ShoppingCart cart:shoppingCarts){
            //shoppingCarts.get(0).setGoodsDetailBean(goodsProductService.queryDetailByProductId(shoppingCarts.get(0).getGoodsInfoId(), cityCountryId));
            if(cart.getMarketingList()!=null &&cart.getMarketingList().size()>0 && cart.getMarketingList().get(0).getRushs()!=null&&cart.getMarketingList().get(0).getRushs().size()>0){
            if(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(cart.getMarketingList().get(0).getRushs().get(0).getRushDiscount()).compareTo(BigDecimal.ZERO)==0){
                Map<String,Object> rushMap=new HashMap<>();
                rushMap.put("rushId",cart.getMarketingList().get(0).getRushs().get(0).getRushId());
                rushMap.put("customerId",request.getSession().getAttribute("customerId"));
                rushMap.put("goodsInfoId",cart.getGoodsInfoId());
                //当存在抢购时 得到每个会员还可以购买的货品数量
                Integer countRush=rushCustomerMapper.selectByParamMap(rushMap);
                countRush=countRush==null?0:countRush;
                countRush=cart.getMarketingList().get(0).getRushs().get(0).getRushLimitation()-countRush;
                countRush = countRush<=0?0:countRush;
                if(countRush>0&& countRush!=null){
                    if(countRush.longValue()>=cart.getGoodsNum().longValue()){
                        freightmoney=freightmoney.add(BigDecimal.valueOf(9.9).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                    }else if(countRush.longValue()<cart.getGoodsNum().longValue()){
                        freightmoney=freightmoney.add(BigDecimal.valueOf(9.9).multiply(BigDecimal.valueOf(countRush)));
                    }
                }
        ...
        //订单商品总重量
        freightMap.put("goodsweight",goodsweight);
        return freightMap;
    ...
    public Map<String, Object> shopCartMap(Long customerId, Long districtId, HttpServletRequest request) {
        ...
        // 查询购物车商品详细
        shoplist = selectShoppingCartDetail(customerId,shoplist, districtId);
        // 查询所有参与的促销
     ...
     * @param districtId
     */
    public List<ShoppingCart> selectShoppingCartDetail(Long customerId,List<ShoppingCart> shoppingCartList, Long districtId) {
        for (ShoppingCart cart : shoppingCartList) {
            ...
            if (cart.getFitId() == null) {
                ...
                for (Marketing mark : marketings) {
                    ...
                    BigDecimal goodsPriceRush = cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    //直降促销的价格中间变量
                    BigDecimal goodsPriceOff = cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    //团购促销价格的中间变量
                    BigDecimal goodsGroupPrice=cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    List<PreDiscountMarketing> discountMarketings = mark.getPreDiscountMarketings();

                    //当存在团购促销时(即使存在折扣促销)优先级为团购促销() && mark.getMarketingId().equals(shoppingCartList.get(i).getGoodsGroupId())
                    if(mark.getGroupon()!=null ) {
                       // isgroup = 1;
                        cart.getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsGroupPrice.multiply(mark.getGroupon().getGrouponDiscount()));
                    }else if (discountMarketings != null) {
                        ...
                        //抢购
                    }else if(mark.getRushs()!=null && !mark.getRushs().isEmpty()){
                        Map<String,Object> rushMap=new HashMap<>();
                        ...
                        //商品为抢购是设置商品的原价
                        cart.setGoodsPrice(cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice());
                        //如果可以抢购的数量大于零
                        if(countRush>0){
                            cart.getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsPriceRush.multiply(mark.getRushs().get(0).getRushDiscount()));
                        }
                    }else if(mark.getPriceOffMarketing()!=null){
                        //直降
                        if(mark.getPriceOffMarketing().getOffValue()!=null && goodsPriceOff.compareTo(mark.getPriceOffMarketing().getOffValue())==1){}
                        cart.getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsPriceOff.subtract(mark.getPriceOffMarketing().getOffValue()));
                    }
                ...
                //只能有一个促销 活动取消时取消  活动开启时开启
                if(marketings.size()>0 && marketings!=null){
                    cart.setMarketingActivityId(marketings.get(0).getMarketingId());
                }else {
                    cart.setMarketingActivityId(null);
                }
                //设置activityId
                this.shoppingCartMapper.setShoppingCartActivityIdMapper(cart);
                /*// 折扣促销过期或删除
                if (marketingNum == 0) {
                ...
                for (int j = 0; j < goodsGroupReleProductVos.size(); j++) {
                    ...
                    if (relProductDetail != null) {
                        ...
                        groupVo.getProductList().get(j).getProductDetail().setSellType(productVo.getGoods().getSellType());
                        groupVo.getProductList().get(j).getProductDetail().setUnitPrice(productVo.getGoods().getUnitPrice());
...
//        shoppingCartList
     ...
     * @param cartWareUtil
     */
    public void shoppingCartDetail(Long customerId,List<ShoppingCart> shoppingCartList, ShoppingCartWareUtil cartWareUtil) {
        ...
        for (ShoppingCart cart : shoppingCartList) {
            ...
            if (cart.getFitId() == null) {
                ...
                for (Marketing marketing : marketings) {
                    ...
                    if(marketing.getRushs()!=null && !marketing.getRushs().isEmpty()){
                        Map<String,Object> rushMap=new HashMap<>();
                        rushMap.put("rushId",marketing.getRushs().get(0).getRushId());
                        rushMap.put("customerId",customerId);
                        rushMap.put("goodsInfoId",cart.getGoodsInfoId());
                        ...
                        marketing.setCustomerbuynum(countRush);
                    }
                    // 判断折扣促销是否过期或删除
                    if (cart.getMarketingId() != null && cart.getMarketingId().equals(marketing.getMarketingId())) {
                        marketingNum += 1;
...
//                        String[] scopeIds = presentScopeIds.split("a");
                        ...
                        for (int k = 0; k < fullbuyPresentScopes.size(); k++) {
                            //scopeType  0:商品  1:优惠券
                            if("0".equals(fullbuyPresentScopes.get(k).getScopeType())){
                                //根据赠品id查询赠品信息(货品)
                                ...
                                goodsProducts.add(goodsProduct);
                            }else if("1".equals(fullbuyPresentScopes.get(k).getScopeType())){
                                //根据优惠券id查询优惠券
                                Coupon coupon = couponService.queryMZCoupon(customerId,fullbuyPresentScopes.get(k).getScopeId());
                                if(coupon !=null){
                                    coupon.setPresentScopeId(fullbuyPresentScopes.get(k).getPresentScopeId());
                                    coupons.add(coupon);
                                }
                            }
                        }
                        //赠送优惠券
                        shoppingCart.setPresentCoupon(coupons);
                        //赠送商品
