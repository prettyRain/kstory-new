...
public class ManagerService implements ManagerServiceInterface {
     ...
     */
    @Override
    public String ifManager(HttpServletRequest request, String name, String password) {
        ...
        if (!"0".equals(existsFlag.toString())) {
            ...
            if (manager != null && "0".equals(manager.getFlag())) {
                ...
                request.getSession().setAttribute("loginUserMob",manager.getMobile());
                ...
                try {
                    // 发送短信
                    if(MSMSendUtil.newSendSms(manager.getMobile(),num)){
                        // 如果短信发送成功 修改数据库
                        request.getSession().setAttribute("mcCode",num);
                        managerMapper.updateSmsCaptcha(manager);
                        return manager.getMobile().substring(0,3) + "****" + manager.getMobile().substring(7, manager.getMobile().length());
                    }else{
                        if (AliSmsUtil.send(num,manager.getMobile(),AliSmsUtil.smsTemplate.COMMON_TEMP.getContent(),null)) {
                            // 如果短信发送成功 修改数据库
                            request.getSession().setAttribute("mcCode",num);
                            managerMapper.updateSmsCaptcha(manager);
                            return manager.getMobile().substring(0,3) + "****" + manager.getMobile().substring(7, manager.getMobile().length());
    ...
    @Override
    public int sendlogincode(HttpServletRequest request, String mobile) {
        Map<String, Object> paramMap = new HashMap<String, Object>();
        String userName = (String)request.getSession().getAttribute("loginUsername");
        paramMap.put(USERNAME, userName);
        paramMap.put(MOBILE, mobile);
        manager = managerMapper.selectCustomerByMobile(paramMap);
        if (manager != null && "0".equals(manager.getFlag())) {
            // 将登陆名存在session
