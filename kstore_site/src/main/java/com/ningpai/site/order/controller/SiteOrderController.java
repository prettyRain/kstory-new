...
public class SiteOrderController {
     ...
     * @return
     */
    @RequestMapping("/beforeSuborder")
    @ResponseBody
    public String beforeSuborder(){
        return "1";
    }
    ...
    public ModelAndView buyNowOrder(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mav = null;
        request.getSession().setAttribute("tok", UUID.randomUUID().toString());
        // 获取用户ID
        Long customerId = (Long) request.getSession().getAttribute(CUSTOMERID);
        ...
        //判断商品数据是否存在下架或者删除异常信息
        boolean isNormal = siteOrderService.queryProductByGoodsInfoId(shoppingCart.getGoodsInfoId()) == 0?false:true;
        //存在异常信息则跳转异常页面
        if (!isNormal) {
            mav = new ModelAndView(GOODSNOEXIT);
            return topAndBottomService.getSimpleTopAndBottom(mav);
        ...
        // 积分兑换规则
        PointSet pointSet = this.couponService.selectPointSet();
        Map<String, Object> shopMap = shoppingCartService.subBuyNowMap(request, shoppingCart,wareUtil, pointSet);
        mav = new ModelAndView(ShoppingCartConstants.NEWSUBORDER)
                .addObject("cartMap", shopMap)
                .addObject("pro", marketingService.selectAll()).addObject("wareUtil", wareUtil)
                .addObject("sx", request.getSession().getAttribute("tok").toString())
    ...
    public ModelAndView newsubmitOrder(HttpServletRequest request, Long[] shoppingCartId, String customerRemark, String chInvoiceTitle, String chInvoiceContent,String presentScopeIds,Long isbuynow) throws IOException {
        Long customerId = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
...
//            }
        ...
        if (!customerRemark.matches(reg) || customerRemark.length() < 40) {
            try{
                orders = siteOrderService.newsubmitOrder(request, shoppingCartId, customerRemark, chInvoiceTitle, chInvoiceContent,isbuynow);
            }catch (Exception e){
                e.printStackTrace();
            }
...
//            throw new RuntimeException("客户留言格式不正确！");
        ...
        for (Order order : orders) {
            int count=0;
            List<OrderGoods> orderGoodsList = this.orderGoodsMapper.selectOrderGoodsList(order.getOrderId());
            record.setCustomerId(customerId);
            record.setOrderId(order.getOrderId());
            record.setNoticeTitle("您的订单已提交成功");
            if(orderGoodsList!=null &&orderGoodsList.size()>1){
                ...
                if(orderGoodsList.get(0).getGoodsInfoName().length()>10){
                    record.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  等"+count+"件商品订单已提交成功，请尽快付款！");
                }else{
                    record.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName()+"...  订单已提交成功，请尽快付款！");
                }
            }else{
                if(orderGoodsList.get(0).getGoodsInfoName().length()>10){
                    record.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  订单已提交成功，请尽快付款！");
                }else{
                    record.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName()+"...  订单已提交成功，请尽快付款！");
                }
            }
