        ...
        success:function(msg){
            ...
            for (var i = 0; i < msg.pb.list.length; i++) {
                ...
                '</button>' +
                    ...
                    '<li><a onclick="shownavAdver(' + msg.pb.list[i].barId + ')" href="javascript:;">设置广告</a></li>' +
                    '<li><a onclick="showNavGoodsList(' + msg.pb.list[i].barId + ')" href="javascript:;">设置商品</a></li>' +
                    '<li><a onclick="delBarById(\'deleteTempBarAjxs.htm?barId=' + msg.pb.list[i].barId + '\')" href="javascript:;">删除</a></li>' +
                    '</ul></div></td>';
                str += "</tr>";
                $(barHtml).append(str);
...
function updateBar(barId){
    $.post("showtempbarbyid.htm", { barId:barId },
        function(data){
            ...
            $(up_seoTitle).val(data.expFleid1);
            $(up_seoKeyword).val(data.expFleid2);
            $(up_seoDesc).val(data.des);
...
function showAdverPage(atId,pageNo){
    ...
    $.ajax({
        ...
        success:function(msg){
            ...
            for(var i=0;i<msg.pb.list.length;i++){
                var advLocation = "频道页广告";
                switch (msg.pb.list[i].temp5) {
                    case "0": advLocation = "首页广告";break;
                    case "1": advLocation = "商品列表页广告";break;
                    case "2": advLocation = "商品搜索页广告";break;
                    case "3": advLocation = "资讯页广告";break;
                    case "5": advLocation = "团购页广告";break;
                    case "6": advLocation = "抢购页广告";break;
                    case "4": advLocation = "其他。。。";break;
                    default : break;
                }
                var str="<tr>";
                str+=" <td><input  name='adverId'  value="+msg.pb.list[i].channelAdverId+" type='checkbox'></td>";
                str+=" <td>"+msg.pb.list[i].adverName+"</td>";
                str+=" <td>"+advLocation+"</td>";
...
function saveChoooseImage(url) {
    ...
    }else if($("#imgPath").val()==5){
        $("#newadverPath_y").attr("src",url);
        $("#newadverPath").val(url);
    }

}

function submitAdver(index){
    var subForm ;
    if(index==1){
        subForm = $("#newsubForm_Adver")
    }else{
        subForm = $("#subForm_Adver")
    }
    if(!subForm.valid()) return;
        ...
        async: false,
            ...
            if(data==1){
                ...
                if($("#isAdver").val()==3){
                    ...
                    showTagAdverPage();
                }else if($("#isAdver").val()==6){
                    showNavAdverPage();
                }
                $(newcls_adver).trigger('click');
            }else{
                $(newcls_adver).trigger('click');
                showTipAlert("添加失败！！");
            }
...
/**查询导航模板广告详情**/
function navigationShowAdver(channelAdverId){
    $.post("showtempadverajax.htm", { channelAdverId: channelAdverId,CSRFToken:$(CSRFToken).val()},
        function(data) {

            if($("#newstoreyAdverId").val()!=""){
                $("#newshowAdverTitle").html("修改楼层广告");
            }else if($("#newupAtId").val()!=""){
                $("#newshowAdverTitle").html("修改模板广告");

            }
            if($("#isAdver").val()==5){
                $("#newshowAdverTitle").html("修改标签广告");
            }
            if(data.userStatus==1){
                $(newup_adverStatus1).prop("checked","checked");
            }else{
                $(newup_adverStatus0).prop("checked","checked");
            }
            $("#newadverPath_y").attr("src",data.adverPath);
            $("#newadverPath").val(data.adverPath);
            $(newup_adverSort).val(data.adverSort);
            $(newup_adverHerf).val(data.adverHref);
            $(newup_adver_temp2).val(data.temp2);
            $(newup_adverName).val(data.adverName);
            $(newupChannelAdverId).val(data.channelAdverId);
            $(newup_adverDes).val(data.des);
            $(newsubAdverButton).attr("onclick","updateAdver(1)");
            $('#newupAtId').val(data.atId);
            $('#newAddSlideAd').modal('show');
        });

}
/**修改模板广告**/
function updateAdver(index){
    var subForm ;
    if(index==1){
        subForm = $("#newsubForm_Adver")
    }else{
...
function showStoreyGoods(pageNo){
    ...
    $.post("querychannelgoodsbypagebeanajax.htm", { tempId: $(tempId).val(),channelId: $(channelId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo},
...
function subStorey(){
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: $(upStoreyForm).val()+"?tempId="+$(tempId).val()+"&channelId="+$(channelId).val(),
...
function updateChannelStorey(channelStoreyGoodsproductId){
    $.post("showchannelgoodsajax.htm?CSRFToken="+$(CSRFToken).val(), { channelStoreyGoodsproductId:channelStoreyGoodsproductId },
        function(data){
            ...
            $(up_goodsproductSubTitle1).val(data.temp4);
...
function showfloorAdverPage(pageNo){
    ...
    $.post("querychannelstoreyadverbypagebeanajax.htm", { channelId:$("#channelId").val(),tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo,storeyId:$("#storeyAdverId").val()},
        function(data){
            $(floorAdverHtml).html("");
            if(data.pb.list.length==0){
...
//导航广告设置
function shownavAdver(obj){
    $("#newstoreyAdverId").val('');
    $('.nav_ad_set').show();
    $('.nav_set').hide();
    $(".nav_good_set").hide();
    $("#newshowNavAdver").hide();
    $("#newshowAdverTitle").html("添加导航广告");
    $("#newnavId").val(obj);
    $("#newup_storeyAdverTempId").val("");
    $("#newup_temp3").val(0);
    $("#newup_temp3").val(0);
    $("#newup_temp1").val("");
    $("#newstoreyTagAdverId").val("");
    $("#isAdver").val(6);
    showNavAdverPage();
}
function showNavAdverPage(pageNo){
    var pageSize=10;
    if($("#showNavAdverOpetion").val()!=null){
        pageSize=$(showNavAdverOpetion).val();
    }
    $.post("querychannelslideadverbypagebeanajax.htm", { navId:$("#newnavId").val(),channelId:$("#channelId").val(),tempId: $(tempId).val(),CSRFToken:$(CSRFToken).val(),pageSize:pageSize,pageNo:pageNo,storeyId:$("#newstoreyAdverId").val()},
        function(data){
            $(navAdverHtml).html("");
            if(data.pb.list.length==0){
                $(navAdverHtml).append('<tr><td colspan="9"><p class="text-center">暂无可用数据</p> </td></tr>');
            }
            for(var i=0;i<data.pb.list.length;i++){
                var str="<tr>";
                str+=" <td><input  name='adverId'  value="+data.pb.list[i].channelAdverId+" type='checkbox'></td>";
                ...
                if(data.pb.list[i].atId == '159'){
                    str+='<td>小轮播图</td>';
                }else if(data.pb.list[i].atId == '162'){
                    str+='<td>页面广告</td>';
                }else if(data.pb.list[i].atId == '157'){
                    str+='<td>大轮播图</td>';
                }

                str+= '<td><div class="btn-group"><button type="button" class="btn btn-default" onclick="navigationShowAdver('+data.pb.list[i].channelAdverId+')">编辑</button>'+
                    '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'+
                    '<span class="caret"></span>'+
                    '<span class="sr-only">Toggle Dropdown</span>'+
                    '</button><ul class="dropdown-menu" role="menu"><li><a onclick="delNavAdverById(\'deletetempadverajax.htm?CSRFToken='+$(CSRFToken).val()+'&adverId='+data.pb.list[i].channelAdverId+'\')" href="javascript:;">删除</a></li></ul></div></td>';
                str+="</tr>";
                $(navAdverHtml).append(str);

            }
            navAdverPageBean(data);
        });

}

function delNavAdverById(adverId){
    showAjaxDeleteConfirmAlertByTemp(adverId,17);
}

<!--页面导航广告分页-->
function navAdverPageBean(data){
    var footStr='<div class="table_pagenav pull-right">';
    footStr+='<div class="meneame">';
    if(data.pb.pageNo==1){
        footStr+='<span class="disabled"> 上一页 </span>';
    }else{
        footStr+='<a  href="javascript:;" onclick="showNavAdverPage('+(data.pb.pageNo-1)+')"> 上一页 </a>';
    }
    $(navAdverFootHtml).html("");
    for(var i = data.pb.startNo;i<data.pb.endNo+1;i++){
        if(i==data.pb.pageNo){
            footStr+=' <span class="current"> '+i+'</span>';
        }else{
            footStr+='<a href="javascript:;"  onclick="showNavAdverPage('+i+')">'+i+'</a>';
        }
    }
    if(data.pb.pageNo==data.pb.totalPages){
        footStr+='<span class="disabled"> 下一页 </span>';
    }else{
        footStr+='   <a href="javascript:;" onclick="showNavAdverPage('+(data.pb.pageNo+1)+')"> 下一页 </a>';
    }
    ...
    footStr+='<form role="form" class="form-inline">';
    footStr+=' <label class="control-label">每页显示：</label>';
    footStr+=' <select class="form-control" id="showNavAdverOpetion" onchange="showNavAdverPage('+data.pb.pageNo+')">';
    if(data.pb.pageSize==10){
        footStr+='<option value="10" selected="selected">10</option>';
    }else{
        footStr+='<option value="10">10</option>';
    ...
    $(navAdverFootHtml).append(footStr);
    cls();
}
//导航广告设置 END