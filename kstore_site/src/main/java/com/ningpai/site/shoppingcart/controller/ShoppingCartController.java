...
public class ShoppingCartController {
    ...
    public boolean addProductToShopCar(Long goodsNum, Long productId, HttpServletRequest request, Long distinctId, HttpServletResponse response, Long fitId)
        ...
        try {
           ...
           /* 如果session中的地区ID为空,就设置为默认江苏南京 */
            ...
            return this.shoppingCartService.addShoppingCart(shoppingCart, request, response,null) >= 1 ? true : false;
        } finally {
            shoppingCart = null;
        }
    }
    ...
    @ResponseBody
    public int addProductToShopCarNew(Long goodsNum, Long productId, Long productStock, HttpServletRequest request, Long distinctId, HttpServletResponse response, Long fitId, Long mark)
            throws UnsupportedEncodingException {
        ShoppingCart shoppingCart = new ShoppingCart();
        //抢购商品还可以购买的数量
        Integer num = 0;
        ...
        if(request.getSession().getAttribute("customerId") != null) {
            ...
            if(marketingList != null && marketingList.size() == 1 && "11".equals(marketingList.get(0).getCodexType())){
                ...
                if(allNum > marketingList.get(0).getRushs().get(0).getRushLimitation()){
                   // return -2;  抢购时 不限数量  超出部分按正常价格算2017.1.31
                }
            }
        }
        ...
        try {
            if(null == distinctId){
                ...
                shoppingCart.setDistinctId(distinctId);
                //同时将session的地区信息更新为当前选择的配送地区
                AddressUtil addressUtil = districtService.queryAddressNameByDistrictId(distinctId);
                request.getSession().setAttribute("chAddress", addressUtil.getProvinceName() + addressUtil.getCityName() + addressUtil.getDistrictName());
                request.getSession().setAttribute("chProvince", addressUtil.getProvinceName());
                request.getSession().setAttribute("chCity", addressUtil.getCityName());
                request.getSession().setAttribute("chDistinct", addressUtil.getDistrictName());
            ...
            if(shoppingcartNum+goodsNum.intValue()>99){
                return -2;
            }
            //没有库存
            if(productStock <= 0 || (goodsNum.intValue()+shoppingcartNum) > productStock){
                ...
                return this.shoppingCartService.addShoppingCart(shoppingCart, request, response,mark);
            }
        } finally {
            shoppingCart = null;
        }
        ...
        for (OrderGoods orderGoods : orderGoodsList) {
            ...
            if((null==orderGoods.getIsPresent()) || (null!=orderGoods.getIsPresent() && orderGoods.getIsPresent().equals("0"))){
                ...
                if(null!=productinfo && productinfo.getGoodsInfoAdded().equals("1")){
                    ...
                    try {
                        shoppingCartService.addShoppingCart(shoppingCart, request, response,null);
        ...
        if (null != custId) {
            ...
            if (null != productIds && productIds.length > 0) {
                for (int i = 0; i < productIds.length; i++) {
                    ...
                    shop.setGoodsNum(1L);
                    this.shoppingCartService.addShoppingCart(shop, request, response,null);
                }
            }
            return 1;
        } else {
    ...
    */
    ...
    public int buyaPrePackage(Long fitId, Long distinctId, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (null != fitId && fitId > 0L) {
            ...
            try {
                return shoppingCartService.addShoppingCart(shoppingCart, request, response,null);
            } catch (UnsupportedEncodingException e) {
    ...
    public ModelAndView addProductToShoppingCar(Long goodsNum, Long productId, HttpServletRequest request, Long distinctId, HttpServletResponse response, Long fitId)
        ...
        try {
            ...
            shoppingCart.setGoodsNum(goodsNum);
            // 添加到购物车
            this.shoppingCartService.addShoppingCart(shoppingCart, request, response,null);
