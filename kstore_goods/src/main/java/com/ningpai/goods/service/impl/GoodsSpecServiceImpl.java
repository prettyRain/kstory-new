...
public class GoodsSpecServiceImpl implements GoodsSpecService {
            ...
            for (int i = 0; i < specDetails.size(); i++) {
                ...
                if (goodsSpecDetail.getSpecDetailId() != -1) {
                    ...
                    if (goodsSpecDetail.getSpecDetailDelflag().equals(
                        ...
                        //标记商品开启规格值表中对应数据为删除
                        this.goodsSpecDetailService.delGoodsOpenspecvalue(goodsSpecDetail);
                    } else {
                        //更新规格值
                        this.goodsSpecDetailService.updateSpecDetail(
                                goodsSpecDetail, username);
                        //同时更新‘货品信息与规格值关联表’与‘商品开启规格值表’中对应id的规格值信息，实现统一
                        this.goodsSpecDetailService.updateOtherSpec(goodsSpecDetail);
                    }
                } else {
                    // 给规格值设置规格ID
     ...
     *
     * @param specIds 规格id
     * @return 删除的规格中只要有个被商品关联了 则不能删除 返回false  否则返回true
     */
    @Override
    public boolean isSpecCanDelete(Long[] specIds) {
        ...
        for (Long specId : specIds) {
            ...
            if (goodsSpecMapper.querySpecGoodsCount(specId) > 0) {
                return false;
            }
        }
     ...
     * 判断规格值是否可以删除
     *
     * @param sprcdetailIds 规格值id
     * @return 删除的规则值中只要有一个被商品关联了 则不能被删除 返回false  否则返回true
     */
    @Override
    public boolean isSpecValueCanDelete(List<Long> sprcdetailIds) {
        ...
        for (Long sprcdetailId : sprcdetailIds) {
            if (goodsSpecMapper.querySpecValueCoodsCount(sprcdetailId) > 0) {
                return false;
            }
        }
        return true;
