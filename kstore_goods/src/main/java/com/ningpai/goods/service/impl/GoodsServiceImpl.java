...
public class GoodsServiceImpl implements GoodsService {
    ...
    public int delGoods(Long goodsId, String username) {
        ...
        try {
            ...
            //删除货品对应的商品图片
            this.goodsProductMapper.deleteProductImageByGoodsId(map);
            // 执行删除方法
    ...
    public Long saveGoods(Goods goods, String username, Map<String, String[]> map) {
        ...
        if (this.goodsMapper.insertSelective(goods) > 0) {
            ...
            //保存商品关联商品认证标识  xcy
            saveGoodsSaveAuthRelation(map,newId);
    ...
    //保存商品关联商品认证标识
    private void saveGoodsSaveAuthRelation(Map<String, String[]> map, Long newId) {
        if(map.get("auths")!=null&&map.get("auths").length>0){
            String[] auths=map.get("auths");
            for(String str:auths){
                Map authMap=new HashMap();
                authMap.put("authId",Integer.parseInt(str));
                authMap.put("goodsId",newId);
                goodsBrandService.saveGoodsSaveAuthRelation(authMap);
            }
        }
    }
    //xcy 更新商品关联的商品认证标识
    private void updateGoodsAuthRelation(Long goodsId,Map<String,String[]> map){
        if(map.get("auths")!=null && map.get("auths").length>0){
            //先删除跟商品关联的认证标识
            Map deleteMap=new HashMap();
            deleteMap.put("goodsId",goodsId);
            goodsBrandService.updateGoodsAuthRelation(deleteMap);
            //保存跟商品关联的认证标识
            String[] auths=map.get("auths");
            for(String str:auths){
                Map authMap=new HashMap();
                authMap.put("authId",Integer.parseInt(str));
                authMap.put("goodsId",goodsId);
                goodsBrandService.saveGoodsSaveAuthRelation(authMap);
            }
        }
    ...
    public int updateGoodsNew(Goods goods, Map<String, String[]> map, String isThirdAuditUsed, String username) {
        ...
        updateGodsReleTag(goods, map, username);
        //xcy 更新关联的商品认证
        updateGoodsAuthRelation(goods.getGoodsId(),map);
        String[] aboutGoodsId = map.get(ABOUTGOODSID);
        // 更新商品关联商品
        updateRelaGoodsWhenUpdateGoods(goods, username, aboutGoodsId);
        // 给商品设置修改人名称
        ...
        if (null != goods.getGoodsAdded() && "0".equals(goods.getGoodsAdded())
                && null !=goods.getAuditStatus() && "2".equals(goods.getAuditStatus()) ) {
            // 定义一个HashMap集合
            Map<String, String> upProMap = new HashMap<String, String>();
            // 把参数放进Map集合中
            upProMap.put(GOODSID, goods.getGoodsId().toString());
        ...
        //待审核状态
        if (null != goods.getGoodsAdded() && "0".equals(goods.getGoodsAdded())
                && null !=goods.getAuditStatus() && "1".equals(goods.getAuditStatus()) ) {
            // 定义一个HashMap集合
            Map<String, String> upProMap = new HashMap<String, String>();
            // 把参数放进Map集合中
            upProMap.put(GOODSID, goods.getGoodsId().toString());
            upProMap.put(MODIFIEDNAME, goods.getGoodsModifiedName());
            // 修改商品为待审核时 把货品修改为待审核
            this.goodsProductMapper.updateProductAddedWithGoodsIdUp(upProMap);
        }
        /*if ("0".equals(goods.getGoodsAdded())) {
            // 定义一个HashMap集合
            Map<String, String> upProMap = new HashMap<String, String>();
    ...
    public void resetGoodsInfoSpec(String[] productIdSpecIdSpecdetailIds, Long goodsId, String ursename) {
        ...
        List<String> openSpecSet = new ArrayList<>();
    ...
    public int updateGoodsDescProduct(Goods goods) {
        // 货品修改为待审核时 更新商品信息
        return this.goodsMapper.updateByPrimaryKeySelective(goods);
    }
     //根据商品ID查询拒绝审核的理由
    @Override
    public String queryGoodsReasonById(Long goodsId) {
        return this.goodsMapper.queryGoodsReasonById(goodsId);
    }
    ...
    private void updateParamWhenUpdateGoods(Goods goods, String username, String[] paramId, String[] paramValue) {
        // 根据商品id查询所有的商品关联详细参数信息
        List<GoodsReleParam> allRele = goodsReleParamService.queryAllReleParamByGoodId(goods.getGoodsId());
        ...
        if (null != paramId) {
            ...
            // 最后allRele中存在的都是更新后没有设置的参数信息,全部删除
            for (GoodsReleParam param : allRele){
                goodsReleParamService.deleReleParamByReleParamId(param.getReleParamId());
            }
        }
    }
    ...
    public void loopSaveProductWhenClick(List<GoodsOpenSpecValueVo> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            // 循环获取规格值
            openSpecDeta = (GoodsOpenSpecValueVo) list.get(i);
            // 如果不是最后一个就循环取值
            if (index < count) {
     ...
     */
    @Override
    public void copyGoods(HttpServletRequest request,Long goodsId, Long thirdId, String thirdName) {
        // 所需的所有信息
    ...
    @Transactional
    @Override
    public int batchUpdateStock(HttpServletRequest request,Long[] goodsId, String stock) {
        // 定义一个Map集合
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("stock", stock);
        try {
            ...
            if (goodsId != null && goodsId.length > 0) {
                ...
                for (int i = 0; i < goodsId.length; i++) {
                    ...
                    // 如果商品下有货品就执行修改库存操作
                    if (productNum > 0) {
                        int updatrStock = this.goodsMapper.batchUpdateStock(map);
                        //到货通知
                        if(updatrStock >0){
                            List<GoodsProduct> gplist = goodsProductMapper.queryGoodsInfoIdByGoodsId(goodsId[i]);
                            if(gplist.size() > 0){
                                for (int j = 0; j < gplist.size(); j++){
                                    updateBatchArrivalNotice(request ,gplist.get(j).getGoodsInfoId());
                                }
                            }
                        }
                    }
     ...
     * 到货通知 用于后台修改库存时，提醒买家
     *
     * @auhor   wangtp 2016/5/28
     * @param goodsInfoId
     * */
    @Override
    public int updateArrivalNotice(HttpServletRequest request,Long goodsInfoId ,Long wareId) {
        ...
        if(anlist.size() >0){
            ...
            for(int i = 0; i < anlist.size(); i++){
                ...
                if(!anlist.get(i).getInfoMobile().equals(null) && !anlist.get(i).getInfoMobile().equals("")){
                    ...
                    //发送阿里大于短信通知 到货通知
                    AliSmsUtil.send(0, anlist.get(i).getInfoMobile(), AliSmsUtil.smsTemplate.TO_GOODS.getContent(), goodsProductVo.getGoodsInfoName());
                }
                if(!anlist.get(i).getInfoEmail().equals(null) && !anlist.get(i).getInfoEmail().equals("")){
                    emailUtils.sendANEmails(request, goodsProductVo, anlist.get(i).getInfoEmail());
                }
                //修改状态为已发送
                arrivalNoticeMapper.updateByPrimaryKeySelective(an);
            }
            //int size=mlist.size();
            //String[] mobile = (String[])mlist.toArray(new String[size]);
            //emailUtils.sendANPosts(request, goodsProductVo, mobile);
        }
        return 1;
    }

    @Override
    public int updateBatchArrivalNotice(HttpServletRequest request, Long goodsInfoId) {
        Map<String, Object> paraMap = new HashMap<String, Object>();
        paraMap.put(NOTICESTURTS, "0");
        paraMap.put(GOODSINFOID, goodsInfoId);
        List<ArrivalNotice> anlist = this.arrivalNoticeMapper.slelctGoodsInfoIdArrivalNotice(paraMap);
