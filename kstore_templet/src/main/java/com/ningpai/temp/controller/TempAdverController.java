...
public class TempAdverController {
    ...
    public ModelAndView queryTempAdverByPageBean(PageBean pb, Long tempId, Long atId, Long adverType) {
        ...
        map.put("temp", temp);
        // 设置分页参数
        map.put("pb", channelAdverService.selectchannelAdverByParam(pb, null, tempId, null, null, atId, adverType, null, null,null));
        // 设置参数
        map.put("atId", atId);
        // 设置类型
    ...
    public Map<String, Object> queryTempAdverByPageBeanAjax(PageBean pb, Long tempId, Long atId, Long adverType, Long storeyTagId) {
        ...
        map.put("pb", channelAdverService.selectchannelAdverByParam(pb, null, tempId, null, storeyTagId, atId, adverType, null, null,null));
        // 设置类型
        map.put("atId", atId);
        return map;
    }
    ...
    @ResponseBody
    public int createTempAdverAjax(HttpServletRequest request, HttpServletResponse response, ChannelAdver channelAdver, BindingResult bindingResult, Integer width,
            Integer height,String linkType) {
        // 判断是否符合规范
        if (bindingResult.hasErrors()) {
            // 返回结果
            return 0;
        ...
        if(("10").equals(channelAdver.getTemp5())){
            if(("all").equals(linkType)){
                channelAdver.setAdverHref("");
            }else{
                channelAdver.setAdverHref(linkType + channelAdver.getAdverHref());
            }
        }
    ...
    @ResponseBody
    public int updateTempAdverAjax(HttpServletRequest request, HttpServletResponse response, ChannelAdver channelAdver, BindingResult bindingResult, Integer width,
            Integer height,String linkType) {
        // 判断参数是否符合规范
        if (bindingResult.hasErrors()) {
            // 返回结果
