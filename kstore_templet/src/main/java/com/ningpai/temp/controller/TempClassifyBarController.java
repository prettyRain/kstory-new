...
public class TempClassifyBarController {
    ...
    public PageBean queryTempClassifyBar(PageBean pb, Long tempId, Long channelId, String definedCate) {
        return classifyBarService.selectClassifyBarByParam(pb, tempId, channelId, definedCate, null);
    }
    ...
    public Map<String, Object> showTempClassifyBarAndCateAndQuickAjax(Long classifyBarId, Long channelId, Long parentId) {
        ...
        if (null != classifyBarId) {
            ...
            if (null != parentId) {
                ClassifyBar parent = classifyBarService.getClassifyBarById(parentId);
                map.put("parentClassifyBar",parent);
                if (parent.getGoodsCatId() == -1) {// 自定义分类导航
                    map.put(ISDEFIND, 1);
                } else {
                    // 父分类不是一级分类导航，或者 是频道模板
    ...
    public ClassifyBar updateTempClassifyBarAjax(HttpServletRequest request, @Valid ClassifyBar classifyBar, BindingResult bindingResult) {
        ...
        // }
        int a= classifyBarService.updateClassifyBar(classifyBar);
        if(a==1){
            return classifyBar;
        }else{
            return null;
