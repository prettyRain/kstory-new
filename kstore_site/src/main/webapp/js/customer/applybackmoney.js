...
$(function(){
    ...
    $("#submitButton").click(function() {
        if($(".flag_saved").val()=="1"){
            ...
            if (backPrice == '' || parseFloat(backPrice) < 0) {
            ...
            if(backRemark.length>200){
                $(".backRemarkContent").text("问题描述字数过多!");
                return;
            }
            ...
            var maxBackPrice = $("#maxBackPrice").val();
            if (parseFloat(backPrice) > parseFloat(maxBackPrice)) {
                $(".backPriceContent").text("最高退款金额为￥"+maxBackPrice);
                return;
            }
            /*if ($("#backPrice").val() == '') {
...
function getprice(goodsId,orderGoodsId,price,goodsCouponPrice,goodsNum,obj){
    ...
    //退单的金额
    var backPrice = $("#maxBackPrice").val();
    ...
    if(isChecked=="checked" ){
        if(backPrice != ''){
            ...
            backGoodsId_sum = goodsId+","+goodsNum+","+orderGoodsId+"-";
        }
    }else{
        if(backPrice != ''){
    ...
    if((back_price+"").indexOf(".")==-1){
        $("#maxBackPrice").val(back_price+".00");
    }else if((back_price+"").length-1-(back_price+"").indexOf(".")==1){
        $("#maxBackPrice").val(back_price+"0");
    }else{
        $("#maxBackPrice").val(back_price);
    }
