//购物车
$(function(){
    $(".cartfd-mem").click(function(){
        $(this).find(".miniCart-mem").show();
        minicartonloadmem();
    });
    $(".cartfd-mem").mouseleave(function(){
            ...
            for(var i = 0 ; i < cartgoods.length ; i++) {
                ...
                if (cartgoods[i].fitId == null) {
                    ...
                    for (var j = 0; j < goodsList.length; j++) {
                        ...
                        '</div>' +
                        '<div class="mcCost-mem">' +
                        '<span class="mcPrice-mem">&yen;' +goodsList[j].productPrice  +'<em style="color:#666">×' + goodsList[j].buNum + '</em></span>' +
                        ' </div>' +
                        '</div>';
                        suitPrice=accAdd(suitPrice,accMul(goodsList[j].productPrice,goodsList[j].buNum));
                        $(".mcBoxList-mem").append(cartgood);
                    }
                }
