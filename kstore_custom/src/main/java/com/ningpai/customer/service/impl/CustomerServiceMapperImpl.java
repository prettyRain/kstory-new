...
//            allinfo.setCustomerNickname(allinfo.getCustomerUsername());
    ...
    public List<String> queryAllCustomerIp() {
        return customerMapper.queryAllCustomerIp();
    }
    ...
    public int auditImg(Map<String, Object> paramMap) {
        return customerMapper.auditImg(paramMap);
    }

    @Override
    public PageBean selectAllCustmerImg(PageBean pageBean) {
        Map<String, Integer> paramMap = null;
        int no = 0;
        try {
            pageBean.setRows(customerMapper.selectAuditImgCount());
            no = pageBean.getRows() % pageBean.getPageSize() == 0 ? pageBean.getRows() / pageBean.getPageSize() : (pageBean.getRows() / pageBean.getPageSize() + 1);
            no = no == 0 ? 1 : no;
            if (pageBean.getPageNo() >= no) {
                pageBean.setPageNo(no);
            ...
            paramMap.put(STARTROWNUM, pageBean.getStartRowNum());
            paramMap.put(ENDROWNUM, pageBean.getEndRowNum());
            pageBean.setList(customerMapper.queryAuditImg(paramMap));
        } finally {
            paramMap = null;
        }
        return pageBean;
    ...
    public void addAvatarNatice(AvatarNotice avatarNotice) {
        this.customerMapper.addAvatarNatice(avatarNotice);
    }
    ...
    public Map getDistinctByIP(HttpServletRequest request) {
        Map map = new HashMap();
        if(request.getSession().getAttribute("chProvince")!=null && request.getSession().getAttribute("chProvince").toString().length()>0){
            return map;
        }
        String ip = IPAddress.getIpAddr(request);
        //String ip = "58.18.96.140";
        String json_result = null;
        String province = (String) request.getSession().getAttribute("chProvince");
        String city = (String) request.getSession().getAttribute("chCity");
        String distinct = (String) request.getSession().getAttribute("chDistinct");
        Long distinctId = (Long) request.getSession().getAttribute("distinctId");
        try {
            json_result = IPAddressUtil.getAddresses("callback=testJson&level=3&ip=" + ip, "GBK");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return null;
        }
        if(json_result!=null && json_result.length()>0) {
            json_result = json_result.replace("if(window.testJson) {testJson(","").replace(");}","");
            province = JSONObject.fromObject(json_result).get("pro").toString().replace("省", "").replace("市", "").replace("自治区", "");
            city = JSONObject.fromObject(json_result).get("city").toString();
            distinct = JSONObject.fromObject(json_result).get("region").toString();
            if(distinct==null || "".equals(distinct)){
                if(city!=null && !"".equals(city)){
                    List<Object> objects = cityService.queryCityByProvinceIdAndCityName(null, city);
                    if (objects != null && objects.size() > 0) {
                        City city1 = (City) objects.get(0);
                        Long cityId = city1.getCityId();
                        List<DistrictVo> districtVos = districtService.queryDistrictByCityId(cityId);
                        if (districtVos != null && districtVos.size() > 0) {
                            distinctId = districtVos.get(0).getDistrictId();
                            distinct = districtVos.get(0).getDistrictName();
                        }
                    }
                }else{
                    //定位失败，则显示后台配置的默认地址
                    distinctId = addressService.getDefaultIdService();
                    AddressUtil addressUtil = districtService.queryAddressNameByDistrictId(distinctId);
                    province = addressUtil.getProvinceName();
                    city = addressUtil.getCityName();
                    distinct = addressUtil.getDistrictName();
                }
            }else {
                // 定位成功，通过区县名称获取区县id
                List<Object> objects = districtService.queryDistrictByCityIdAndDistName(null, distinct);
                if (objects != null && objects.size() > 0) {
                    District district = (District) objects.get(0);
                    distinctId = district.getDistrictId();
                }
            }
            //区县id
            request.getSession().setAttribute("distinctId", distinctId);
            // 省
            request.getSession().setAttribute("chProvince", province);
            // 市
            request.getSession().setAttribute("chCity", city);
            // 地区
            request.getSession().setAttribute("chDistinct", distinct);
            // 详细地址
            request.getSession().setAttribute("chAddress", province + city + distinct);
        }
        map.put("ipProvince",province);
        map.put("ipCity", city);
        map.put("ipDistinct", distinct);
        map.put("ipDistinctId", distinctId);
        return map;
    }
     ...
     */
    @Override
    public String getProvinceByIP(HttpServletRequest request, String ip) {
        try {
            String addresses = IPAddressUtil.getAddresses("callback=testJson&level=3&ip="+ip, "GBK");
            addresses = addresses.replace("if(window.testJson) {testJson(","").replace(");}","");
            String province = JSONObject.fromObject(addresses).get("pro").toString().replace("省", "").replace("市", "").replace("自治区", "");
            return province;
        }catch(Exception e){
            e.printStackTrace();
            return "";
    ...
    public List<CustomerAllInfo> selectAllCustmerinfo(Map map) {
        return customerMapper.selectAllCustmerinfo(map);
    }
    ...
    public Long getProvinceIdByName(String name) {
        return customerMapper.getProvinceIdByName(name);
    }
