...
public class GoodsBrandMapperImpl extends BasicSqlSupport implements
    ...
    public void saveAuth(GoodsAuth goodsAuth) {
        this.insert(
                "com.ningpai.goods.dao.GoodsBrandMapper.saveAuth",goodsAuth
        );
    }
    // 查询认证标识总条数
    @Override
    public Integer findAuthCount() {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findAuthCount").size();
    }
    // 查询当前页认证标识集合
    @Override
    public List<Object> findAuthList(Map map) {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findAuthList",map);
    }
    //删除商品认证标识
    @Override
    public void deleteAuth(Map map) {
        this.delete("com.ningpai.goods.dao.GoodsBrandMapper.deleteAuth",map);
    }
    //查询所有商品标识认证
    @Override
    public List<Auth> findAllAuth() {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findAllAuth");
    }
    //商品认证标识关联商品
    @Override
    public void saveGoodsSaveAuthRelation(Map authMap) {
        this.insert("com.ningpai.goods.dao.GoodsBrandMapper.saveGoodsSaveAuthRelation",authMap);
    }
    //查询与商品关联的认证标识
    @Override
    public List<GoodsAuth> findAuthByGoodsId(Long goodsId) {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findAuthByGoodsId",goodsId);
    }
    //xcy 删除商品与认证标识的关联
    @Override
    public void updateGoodsAuthRelation(Map deleteMap) {
        this.delete("com.ningpai.goods.dao.GoodsBrandMapper.updateGoodsAuthRelation",deleteMap);
    }
    //xcy 根据goodsinfoID 查询商品认证标识
    @Override
    public List<Object> findAuthByGoodsInfoId(Long productId) {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findAuthByGoodsInfoId",productId);
    }
    //xcy 根据商品认证id 查询认证
    @Override
    public GoodsAuth queryAuthById(Long authId) {
        return this.selectOne("com.ningpai.goods.dao.GoodsBrandMapper.queryAuthById",authId);
    }
    //xcy 根据id修改商品认证标识
    @Override
    public void updateCoodsAuth(GoodsAuth auth) {
        this.update("com.ningpai.goods.dao.GoodsBrandMapper.updateCoodsAuth",auth);
    }
    //xcy 查询品牌
    @Override
    public List<GoodsBrand> findBrands() {
        return this.selectList("com.ningpai.goods.dao.GoodsBrandMapper.findBrands");
    }
}
