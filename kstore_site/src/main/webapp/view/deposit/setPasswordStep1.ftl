                        ...
                        <form action="/deposit/validatecode.htm" method="post" onsubmit="return validateForm();">
                            <div class="n_password">
                                ...
                                <div class="n_item clearfix mb20">
                                    ...
                                    <button class="nw-submit sub_btn free" type="submit" disabled="true" style="background-color:#999999;">提交</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
    ...
    $("#code").on("input channgeproperty",function(){
          if($(this).val()==""){
    ...
    function validateForm(){
        var code = $("#code").val();
        if(!code){
//            $("#mcode_tip").parent().parent().addClass("error");
//            $(".resend.ne_tips").text("请输入验证码").show();
            $("#code").css("border-color","#c33");
            $('.mcode_tip').html('<img src="/images/ero.png">&nbsp;&nbsp;请输入验证码').show().css("color","#c33");
            return false;
        }else{
            $('.mcode_tip').hide();
            $("#m_tip").parent().parent().removeClass("error");
        ...
        $.ajax({
            ...
            success: function(data){
                if(data == 1){
                    ...
                    $("#code").css("border-color","#c33");
                    $('.mcode_tip').html('<img src="/images/ero.png">&nbsp;&nbsp;验证码已失效').show().css("color","#c33");
                    flag=false;
                }else{
                    $("#code").css("border-color","#c33");
                    $('.mcode_tip').html('<img src="/images/ero.png">&nbsp;&nbsp;验证码错误').show().css("color","#c33");
                    flag=false;
                }
            }
        });
