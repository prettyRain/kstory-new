...
public class OrderServiceImpl implements OrderService {
    ...
    public Map<String, Object> newsearchOrderList(String status, Order order, PageBean pageBean) {
        ...
        if ("2".equals(status)) {
            ...
            paramMap.put(ORDERLINEPAY, "");
            paramMap.put(ORDERSTATUS, "");
            paramMap.put(ORDERMTYPE, "1,2");
        } else if ("9".equals(status)) { // 社区店订单
            paramMap.put("communityStoreIdType", "1");
        } else {
            paramMap.put(ORDERLINEPAY, "");
            paramMap.put(ORDERSTATUS, "");
...
//                    }
    ...
    public void modifyOrderPrice(BigDecimal price, Long orderId) {
        ...
        if (order.getOrderMType().equals("2")){
            ...
            String timestamp = UtilDate.mathString(new Date());
            String s = timestamp + randomNum;
            map.put("orderCode",s  // 子订单号
                    );
            map.put("orderOldCode",timestamp);
             /* 根据订单号去查询是否有记录 因为此处改了订单orderCode 那么如果有收款单的话收款单的orderCode也得改 保持一致*/
            Receivables receivables = this.receivablesService.queryByOrderCode(order.getOrderCode());
            if(receivables!=null){
                receivables.setOrderCode(s);
                // 执行为收款单信息添加信息操作
    ...
    public int updateSetCargoStatusByOrderId(Long orderId, String status) {
        ...
        //订单通知消息   如果是出库的时候才通知
        if("3".equals(status)){
        Order newOrder=this.orderMapper.querySimpleOrder(orderId);
            List<OrderGoods> orderGoodsList = orderGoodsMapper.selectOrderGoodsList(orderId);

            OrderNotice orderNotice=new OrderNotice();
        orderNotice.setCustomerId(newOrder.getCustomerId());
        orderNotice.setOrderId(orderId);
        orderNotice.setCreateTime(new Date());
        orderNotice.setNoticeTitle("您的订单已发货");
        if(orderGoodsList!=null &&orderGoodsList.size()>1){
            for(OrderGoods orderGoods:orderGoodsList){
                count+=orderGoods.getGoodsInfoNum();
            }
            orderNotice.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  等"+count+"件商品已发货，请留意查收！");
        }else{
            orderNotice.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  已发货，请留意查收！");
        }
        //orderNotice.setNoticeContent("您的订单 "+newOrder.getOrderCode()+" 已经发货成功，请您注意查收");
        orderNotice.setIsRead("0");
        this.orderMapper.addNotive(orderNotice);
        }
        return 0;
    }
    ...
    public void cancelOrderByTime() {
        ...
        map.put("orderCancelRemark", "订单一定时间未付款系统自动会取消订单!");
        List<Order> orderList = orderMapper.queryOrderCountByDay(map);
        if(orderList.size() > 0 && orderList != null){
            for(int i = 0; i < orderList.size() ; i++){
                // 取消订单
                cancelOrder(orderList.get(i).getOrderId(),"订单一定时间内未付款系统会自动取消订单!");
                //取消与订单绑定的要赠送的优惠券
                orderCouponService.cancelOrderCoupon(orderList.get(i).getOrderId());
                int count=0;
                List<OrderGoods> orderGoodsList = this.orderGoodsMapper.selectOrderGoodsList(orderList.get(i).getOrderId());
                Order newOrder=this.orderMapper.querySimpleOrder(orderList.get(i).getOrderId());
                OrderNotice orderNotice=new OrderNotice();
                orderNotice.setCustomerId(newOrder.getCustomerId());
                orderNotice.setOrderId(orderList.get(i).getOrderId());
                orderNotice.setCreateTime(new Date());
                orderNotice.setNoticeTitle("您的订单已取消成功");
                if(orderGoodsList!=null &&orderGoodsList.size()>1){
                    for(OrderGoods orderGoods:orderGoodsList){
                        count+=orderGoods.getGoodsInfoNum();
                    }
                    orderNotice.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  等"+count+"件商品订单已取消成功，订单一定时间内未付款系统会自动取消订单!");
                }else{
                    orderNotice.setNoticeContent("您所购买的  "+orderGoodsList.get(0).getGoodsInfoName().substring(0,9)+"...  已取消成功，订单一定时间内未付款系统会自动取消订单!");
                }
                //orderNotice.setNoticeContent("您的订单 "+newOrder.getOrderCode()+" 已经取消成功，订单一定时间内未付款系统会自动取消订单!");
                orderNotice.setIsRead("0");
                this.orderMapper.addNotive(orderNotice);
            }
        }
            ...
            if (result == 1) {
                ...
                if(order.getBusinessId()==0){
                    if(order.getOrderIntegral() != null && order.getOrderIntegral().intValue() != 0 ){
                        ...
                        cusPoint.setPoint(order.getOrderIntegral().intValue());
                        cusPoint.setPointType("1");
                        cusPoint.setCustomerGradeType("1");
                        cusPoint.setDelFlag("0");
                        cusPoint.setCreateTime(new Date());
                        cusPoint.setCustomerId(order.getCustomerId());
                        // 扣除后的积分保存到数据库
                ...
                cc = new CustomerConsume();
                // 从订单中取出付款方式1.在线支付2.货到付款
                if ("1".equals(order.getOrderLinePay())) {
                    cc.setPayType("1");
                }
                if ("2".equals(order.getOrderLinePay())) {
                    cc.setPayType("2");
                }
                cc.setCustomerId(customerId);
                cc.setBalanceNum(BigDecimal.valueOf(orderPrice));
            ...
            for (Order order : orderList) {
                if (order.getSendExpressTime()!=null&&new Date().getTime() - order.getSendExpressTime().getTime() > (day * 60 * 60 * 24 * 1000)) {
                    ...
                    if(null != pointSet && "1".equals(pointSet.getIsOpen()) && order.getBusinessId()==0){
                        //只有当后台积分设置为开启时才赠送积分
                        // 订单完成赠送积分
                        customerPointServiceMapper.addIntegralByType(order.getCustomerId(), "6", sumP.doubleValue(), order.getOrderId() + "",order.getCreateTime());
                    }
    ...
    public int saveAddOrder(BigDecimal goodsAllPrice, BigDecimal freightPrice, String companyInfo, String customerRemark, Long[] goodsIdP, Long[] goodsNum, Long distinctId,
        ...
        if (CollectionUtils.isNotEmpty(goodsProductVos)) {
            ...
            // order.setOrderExpressType()设置配送方式 0 送货上门  ,1自提
            if("0".equals(ca.getAddressType())){
                //普通地址 选送货上门
                order.setOrderExpressType("0");
            }else if("1".equals(ca.getAddressType())){
                //社区店地址
                 if(StringUtils.isNotBlank(deliveryName)){
                    if("0".equals(deliveryName)){
                        //送货上门
                        order.setOrderExpressType("0");
                        //社区店id
                        if(ca.getCommunityStoreId()!=0l){
                            order.setCommunityStoreId(ca.getCommunityStoreId());
                        }
                    }else if("1".equals(deliveryName)){
                        //自提
    ...
    @Resource(name = "OrderRemarkMapper")
    ...
    public List<OrderRemark> selectOrderRemarkByOrderId(Map<String, Object> paramMap){
        List<OrderRemark> orderRemarks=orderRemarkMapper.selectOrderRemarkByOrderId(paramMap);
        return orderRemarks;

    }

    public Integer getTotalCount(Long orderId){
        Integer totalCount=orderRemarkMapper.getTotalCount(orderId);
        return totalCount;
    }
     ...
     * 拆分订单业务
     * @return
     * @param newnumList 子订单商品数量的数组集合
     * @param goodsInfoId 原订单商品ID数组
     * @param goodsprice 原订单商品单价数组
     * @param isPresent
     * @param orderId 原订单ID
     */
    @Override
    @Transactional
    public int splitOrder(List<Integer[]> newnumList, Long[] goodsInfoId, BigDecimal[] goodsprice, String[] isPresent, Long orderId){
        //需要根据权重计算的有原价格、优惠价格、订单价格、积分、订单积分、获得积分、运费、订单促销优惠、积分价格、促销金额、会员折扣金额、优惠券金额、礼品卡余额
        BigDecimal orderOldPrice = BigDecimal.ZERO;
        BigDecimal orderPrePrice = BigDecimal.ZERO;
        BigDecimal orderPrice = BigDecimal.ZERO;
        Long orderIntegral = 0L;
        BigDecimal jfPrice = BigDecimal.ZERO;
        Long orderGetPoint = 0L;
        BigDecimal expressPrice = BigDecimal.ZERO;
        BigDecimal backPrice = BigDecimal.ZERO;
        BigDecimal orderPrePriceOrder = BigDecimal.ZERO;
        BigDecimal modifyPrice = BigDecimal.ZERO;
        BigDecimal couponPrice = BigDecimal.ZERO;
        BigDecimal discountPrice = BigDecimal.ZERO;
        BigDecimal promotionsPrice = BigDecimal.ZERO;
        BigDecimal giftcardPrice = BigDecimal.ZERO;
        //循环子订单，依次添加到数据库
        //设置一个值，存放拆单过程中出现的误差值
        BigDecimal errorValue = BigDecimal.ZERO;
        List<Order> splitOrderList = new ArrayList<Order>();
        for (int i = 0; i < newnumList.size(); i++) {
            //获取原订单详情
            Order order = orderDetail(Long.valueOf(orderId));
            order = changeEmptyToZero(order);
            BigDecimal weightUp = BigDecimal.ZERO;
            for (int j = 0; j < goodsInfoId.length; j++) {
                weightUp = weightUp.add(goodsprice[j].multiply(BigDecimal.valueOf(newnumList.get(i)[j])));
            }
            BigDecimal weightDown = order.getOrderOldPrice();
            ...
            //根据权重计算子订单的价格、优惠价格、积分、运费等
            if(i==newnumList.size()-1){
                //原价
                order.setOrderOldPrice(order.getOrderOldPrice().abs().subtract(orderOldPrice));
                //优惠价格
                order.setOrderPrePrice(order.getOrderPrePrice().abs().subtract(orderPrePrice));
                //订单价格
                order.setOrderPrice(order.getOrderPrice().abs().subtract(orderPrice));
                //消费积分
                order.setOrderIntegral(order.getOrderIntegral() - orderIntegral);
                //积分价格
                order.setJfPrice(order.getJfPrice().abs().subtract(jfPrice));
                //获得积分
                order.setOrderGetPoint(order.getOrderGetPoint() - orderGetPoint);
                //运费
                order.setExpressPrice(order.getExpressPrice().abs().subtract(expressPrice));
                //订单优惠价格
                order.setOrderPrePriceOrder(order.getOrderPrePriceOrder().abs().subtract(orderPrePriceOrder));
                //修改价格
                order.setModifyPrice(order.getModifyPrice().abs().subtract(modifyPrice));
                //优惠券金额
                order.setCouponPrice(order.getCouponPrice().abs().subtract(couponPrice));
                //会员折扣金额
                order.setDiscountPrice(order.getDiscountPrice().abs().subtract(discountPrice));
                //促销金额
                order.setPromotionsPrice(order.getPromotionsPrice().abs().subtract(promotionsPrice));
                //礼品卡消费余额
                order.setGiftcardPrice(order.getGiftcardPrice().abs().subtract(giftcardPrice));
                //退单金额
                order.setBackPrice(order.getBackPrice().abs().subtract(backPrice));
            }else{
                //原价
                order.setOrderOldPrice(order.getOrderOldPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                orderOldPrice = orderOldPrice.add(order.getOrderOldPrice());
                //优惠价格
                order.setOrderPrePrice(order.getOrderPrePrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                orderPrePrice = orderPrePrice.add(order.getOrderPrePrice());
                //订单价格
                order.setOrderPrice(order.getOrderPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                orderPrice = orderPrice.add(order.getOrderPrice());
                //消费积分
                order.setOrderIntegral(BigDecimal.valueOf(order.getOrderIntegral()/10).multiply(weightUp).divide(weightDown,0,BigDecimal.ROUND_HALF_EVEN).longValue()*10);
                orderIntegral += order.getOrderIntegral();
                //积分价格
                order.setJfPrice(order.getJfPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                jfPrice = jfPrice.add(order.getJfPrice());
                //获得积分
                order.setOrderGetPoint(BigDecimal.valueOf(order.getOrderGetPoint()).multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN).longValue());
                orderGetPoint += order.getOrderGetPoint();
                //运费
                order.setExpressPrice(order.getExpressPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                expressPrice = expressPrice.add(order.getExpressPrice());
                //订单优惠价格
                order.setOrderPrePriceOrder(order.getOrderPrePriceOrder().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                orderPrePriceOrder = orderPrePriceOrder.add(order.getOrderPrePriceOrder());
                //修改价格
                order.setModifyPrice(order.getModifyPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                modifyPrice = modifyPrice.add(order.getModifyPrice());
                //优惠券金额
                order.setCouponPrice(order.getCouponPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                couponPrice = couponPrice.add(order.getCouponPrice());
                //会员折扣金额
                order.setDiscountPrice(order.getDiscountPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                discountPrice = discountPrice.add(order.getDiscountPrice());
                //促销金额
                order.setPromotionsPrice(order.getPromotionsPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                promotionsPrice = promotionsPrice.add(order.getPromotionsPrice());
                //礼品卡消费余额
                order.setGiftcardPrice(order.getGiftcardPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                giftcardPrice = giftcardPrice.add(order.getGiftcardPrice());
                //退单金额
                order.setBackPrice(order.getBackPrice().abs().multiply(weightUp).divide(weightDown,2,BigDecimal.ROUND_HALF_EVEN));
                backPrice = backPrice.add(order.getBackPrice());
            }
            order.setDelFlag("0");

            for (OrderGoods orderGoods : order.getOrderGoodsList()) {
                for (int j = 0; j < goodsInfoId.length; j++) {
                    if(orderGoods.getGoodsInfoId().compareTo(goodsInfoId[j])==0 && isPresent[j].equals(orderGoods.getIsPresent())){
                        //订单货品也要按照在原订单占的比例改变优惠金额
                        BigDecimal goodsWeight = BigDecimal.valueOf(newnumList.get(i)[j]).divide(BigDecimal.valueOf(orderGoods.getGoodsInfoNum()),2,BigDecimal.ROUND_HALF_EVEN);
                        orderGoods.setGoodsInfoNum(Long.valueOf(newnumList.get(i)[j]));
                        orderGoods.setGoodsCouponPrice(orderGoods.getGoodsCouponPrice().multiply(goodsWeight));
                        orderGoods.setGoodsInfoSumPrice(goodsprice[j].multiply(BigDecimal.valueOf(newnumList.get(i)[j])));
                        orderGoods.setGoodsBackPrice(orderGoods.getGoodsInfoSumPrice().subtract(orderGoods.getGoodsCouponPrice()));
                        //子订单中商品数量为0的，不添加
                        if (orderGoods.getGoodsInfoNum()!=0){
            ...
            //判断被拆分订单的编号末位是否为数字，是则订单属于第一次拆分，orderOldCode为原始订单编号
            if(Character.isDigit(order.getOrderCode().charAt(order.getOrderCode().length()-1))){
                order.setOrderOldCode(order.getOrderCode());
            }
            order.setOrderCode(order.getOrderCode()+suffix);
            splitOrderList.add(order);
            //计算订单价格误差（原始总金额+运费价格-优惠价格-积分价格-优惠券金额-会员折扣金额-礼品卡消费金额-订单实际金额）
            BigDecimal orderErrorValue = order.getOrderOldPrice().add(order.getExpressPrice())
                    .subtract(order.getOrderPrePrice()).subtract(order.getJfPrice()).subtract(order.getCouponPrice())
                    .subtract(order.getDiscountPrice())/*.subtract(order.getGiftcardPrice())*/
                    .subtract(order.getOrderPrice());
            if(orderErrorValue.compareTo(BigDecimal.ZERO)!=0){
                errorValue = errorValue.add(orderErrorValue);
                //将误差的金额放入非0的部分（优惠价格》运费价格》积分价格》优惠券金额》礼品卡消费金额》会员折扣金额）
                if(order.getOrderPrePrice().compareTo(BigDecimal.ZERO)!=0) {
                    order.setOrderPrePrice(order.getOrderPrePrice().add(orderErrorValue));
                }else if(order.getExpressPrice().compareTo(BigDecimal.ZERO)!=0){
                    order.setExpressPrice(order.getExpressPrice().subtract(orderErrorValue));
                }else if(order.getJfPrice().compareTo(BigDecimal.ZERO)!=0){
                    order.setJfPrice(order.getJfPrice().add(orderErrorValue));
                    order.setOrderIntegral(order.getOrderIntegral()+orderErrorValue.longValue());
                }else if(order.getCouponPrice().compareTo(BigDecimal.ZERO)!=0){
                    order.setCouponPrice(order.getCouponPrice().add(orderErrorValue));
                }else if(order.getGiftcardPrice().compareTo(BigDecimal.ZERO)!=0){
                    order.setGiftcardPrice(order.getGiftcardPrice().add(orderErrorValue));
                }else if(order.getDiscountPrice().compareTo(BigDecimal.ZERO)!=0){
                    order.setDiscountPrice(order.getDiscountPrice().add(orderErrorValue));
                }else{
                    order.setOrderPrice(order.getOrderPrice().add(orderErrorValue));
                }
            }
        }
        if(errorValue.compareTo(BigDecimal.ZERO)!=0){
            return 0;
        }
        return insertSplitOrder(Long.valueOf(orderId),splitOrderList);
    }
     ...
     * @return
     */
    private Order changeEmptyToZero(Order order) {
        if(order.getOrderPrice()==null){
            order.setOrderPrice(BigDecimal.ZERO);
        }
        if(order.getOrderOldPrice()==null){
            order.setOrderOldPrice(BigDecimal.ZERO);
        }
        if(order.getOrderPrePrice()==null){
            order.setOrderPrePrice(BigDecimal.ZERO);
        }
        if(order.getOrderIntegral()==null){
            order.setOrderIntegral(0L);
        }
        if(order.getJfPrice()==null){
            order.setJfPrice(BigDecimal.ZERO);
        }
        if(order.getOrderGetPoint()==null){
            order.setOrderGetPoint(0L);
        }
        if(order.getExpressPrice()==null){
            order.setExpressPrice(BigDecimal.ZERO);
        }
        if(order.getBackPrice()==null){
            order.setBackPrice(BigDecimal.ZERO);
        }
        if(order.getOrderPrePriceOrder()==null){
            order.setOrderPrePriceOrder(BigDecimal.ZERO);
        }
        if(order.getModifyPrice()==null){
            order.setModifyPrice(BigDecimal.ZERO);
        }
        ...
        if(order.getPromotionsPrice()==null){
            order.setPromotionsPrice(BigDecimal.ZERO);
        }
    ...
    @Override
    public void updategiftcardPrice(Map<String, Object> map) {
        orderMapper.updategiftcardPrice(map);
    }
     ...
     */
    @Override
    public List queryOrderIdByStatus(String status) {
        return orderMapper.queryOrderIdByStatus(status);
    }
     ...
     */
    @Transactional
    public int insertSplitOrder(Long oldOrderId, List<Order> splitOrderList) {
        boolean flag = false;
        for (Order order : splitOrderList) {
            //添加新拆单到订单表
            orderMapper.insertOrder(order);
            //添加新拆单到订单货品信息表
            for (OrderGoods orderGoods : order.getOrderGoodsList()) {
                Long orderGoodsId = orderGoods.getOrderGoodsId();
                orderGoods.setOrderGoodsId(null);
                orderGoods.setOrderId(order.getOrderId());
                orderGoods.setDelFlag("0");
                orderGoodsMapper.insertOrderGoodsInfo(orderGoods);
                orderGoodsMapper.deleteOrderGoodsById(orderGoodsId);
            }
            //添加新拆单到订单物流
            OrderExpress orderExpress = orderExpressMapper.selectOrderExpress(oldOrderId);
            orderExpress.setOrderId(order.getOrderId());
            orderExpress.setOrderExpressId(null);
            orderExpress.setDelFlag("0");
            orderExpressMapper.insertOrderExpress(orderExpress);
            OrderExpress orderExpress1 = new OrderExpress();
            orderExpress1.setOrderId(oldOrderId);
            orderExpress1.setDelFlag("1");
            orderExpressMapper.updateExpress(orderExpress1);
        }
        //orderService.deleteOrderById(Long.valueOf(orderId));
        return modifyOrderByKey(oldOrderId,"4","该订单已拆分");
    }
    ...
    public Long queryWareByDistrict(Long district) {
        return this.productWareMapper.queryWareByDistrict(district);
    }

    @Override
    public List<Map<String,Long>> queryNumByOrderId(Long orderId, Long wareId) {
        if(orderId!=null && wareId!=null){
            Map map=new HashMap();
            map.put("orderId",orderId);
            map.put("wareId",wareId);
            return this.productWareMapper.queryNumByOrderId(map);
        }
        return null;
    }
    ...
    public Long queryOrderWareByOrderId(Long orderId) {
        return this.productWareMapper.queryOrderWareByOrderId(orderId);
    }
    ...
    public List<Map<String, Long>> queryGoodinfoNumByOrderId(Long orderId) {
        return this.productWareMapper.queryGoodinfoNumByOrderId(orderId);
    }
     ...
     * @param distinctId 区县id
     * @param orderId  订单id
     */
    @Override
    public void saveAddressAfterUpdate(CustomerAddress customerAddress, String addressType, String distinctId, String orderId) {
        //查看此区县是否支持配送
        Long wareId=queryWareByDistrict(Long.parseLong(distinctId));
        //查看是不是在同一个仓库
        Long orderWareId=queryOrderWareByOrderId(Long.parseLong(orderId));
        if(!wareId.equals(orderWareId)&&wareId!=null&&orderWareId!=null){
            //不是同一个仓库
            //查询订单里面商品的数量
            List<Map<String,Long>> mapList=queryGoodinfoNumByOrderId(Long.parseLong(orderId));

            for(Map<String,Long> map1:mapList){
                Long productId = map1.get("goods_info_id");
                Long prodductNum = map1.get("goods_info_num");
                //根据旧的仓库跟商品 增加商品库存
                Map oldMap=new HashMap();
                oldMap.put("wareId",orderWareId);
                oldMap.put("productId",productId);
                oldMap.put("productNum",prodductNum);
                this.productWareMapper.updateOldWareStockByWareAndpro(oldMap);
                //根据新的仓库 跟商品 减少商品库存
                Map newMap=new HashMap();
                newMap.put("wareId",wareId);
                newMap.put("productId",productId);
                newMap.put("productNum",prodductNum);
                this.productWareMapper.updateNewWareStockByWareAndpro(newMap);
            }

        }

        Order orderOld=this.orderMapper.querySimpleOrder(Long.parseLong(orderId));
        //设置旧地址
        OrderOldAddress orderOldAddress=new OrderOldAddress();
        orderOldAddress.setOrderId(Long.parseLong(orderId));
        orderOldAddress.setOrderCode(orderOld.getOrderCode());
        orderOldAddress.setProvince(orderOld.getShippingProvince());
        orderOldAddress.setCity(orderOld.getShippingCity());
        orderOldAddress.setCounty(orderOld.getShippingCounty());
        orderOldAddress.setPerson(orderOld.getShippingPerson());
        orderOldAddress.setPhone(orderOld.getShippingPhone());
        orderOldAddress.setMobile(orderOld.getShippingMobile());
        orderOldAddress.setAddress(orderOld.getShippingAddress());
        orderOldAddress.setPostCode(orderOld.getShippingPostcode());
        orderOldAddress.setExpressType(orderOld.getOrderExpressType());
        orderOldAddress.setCommunityId(orderOld.getCommunityStoreId());
        //保存旧地址
        this.orderMapper.saveOrderOldAddress(orderOldAddress);
        Order order=new Order();
        order.setOrderId(Long.parseLong(orderId));
        customerAddress.setInfoCounty(distinctId.toString());
        // 保存收货地址信息
        customerAddress.setCustomerId(orderOld.getCustomerId());
        customerAddressMapper.insertSelective(customerAddress);
        // 重新查询收货地址信息
        CustomerAddress ca = customerAddressMapper.selectByaddressId(customerAddress.getAddressId());
        // order.setOrderExpressType()设置配送方式 0 送货上门  ,1自提
        ...
        WareHouse ware = productWareMapper.findWare(Long.parseLong(distinctId));
        if (ware != null) {
            // 收获地址信息
            order.setWareName(ware.getWareName());
            // 库存ID
        ...
        // 修改订单表
        int f = orderMapper.updateByPrimaryKeySelective(order);
        // 后台不需要添加到收货地址
        customerAddressMapper.deleteByPrimaryKey(customerAddress.getAddressId());
     ...
     */
    @Override
    public OrderOldAddress queryOldOrder(Long orderId) {
        return this.orderMapper.queryOldOrder(orderId);
    }
     ...
     */
    @Override
    public void updateOrderAddressState(Long orderId, String updateState) {
        Map map=new HashMap();
        map.put("orderId",orderId);
        map.put("updateState",updateState);
        this.orderMapper.updateOrderAddressState(map);
    }
     ...
     */
    @Override
    public String isOrderGargo(long l) {
        return this.orderMapper.isOrderGargo(l);
    }
     ...
     */
    @Override
    public String queryUpdateStateByOrderId(Long orderId) {
        return this.orderMapper.queryUpdateStateByOrderId(orderId);
    }
     ...
     */
    @Override
    public int synOrderSuccess(Long orderId){
        return this.orderMapper.synOrderSuccess(orderId);
    }
     ...
     */
    @Override
    public List querySynOrder() {
        return this.orderMapper.querySynOrder();
    }

    @Override
    public Long isOrderOldAddress(String orderId) {
        return this.orderMapper.isOrderOldAddress(Long.parseLong(orderId));
    }
    // 根据订单ID查询配送信息
    @Override
    public OrderDistribution queryOrderDistributionByOrderId(Long orderId) {
        return this.orderMapper.queryOrderDistributionByOrderId(orderId);
    }
    //配送信息 修改
    @Override
    public void updateOrderDistribution(OrderDistribution orderDistribution) {
        this.orderMapper.updateOrderDistribution(orderDistribution);
    }
    //配送信息 保存
    @Override
    public void saveOrderDistribution(OrderDistribution orderDistribution) {
        this.orderMapper.saveOrderDistribution(orderDistribution);
    }
     ...
     */
    @Override
    public String addorderremarksajax(OrderRemark orderRemark){
        try {
            int i=orderRemarkMapper.addorderremarksajax(orderRemark);
            return "ok";
        }catch (Exception e){
    ...
    @Override
    public String findOrderStatus(Map<String,Long> map){
        return this.orderMapper.findOrderStatus(map);
    }
    //添加消息提醒
    @Override
    public void addNotive(OrderNotice notive) {
        this.orderMapper.addNotive(notive);
    }

    @Override
    public void noticeByTime() {
        int i=0;
        //查询确定收货的和订单取消的  并且系统消息中没有的
        //List<Order> orders = this.orderMapper.queryNoticeOrder();
//        for(Order order:orders){
         //   List<OrderNotice> orderNotices = this.orderMapper.queryNoticeOrders(order.getOrderId());
//        }
    }
}
