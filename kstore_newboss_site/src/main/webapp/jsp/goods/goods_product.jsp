...
<head>
  ...
  <![endif]-->
  <%--shf增加图片拖拽功能--%>
  <link href="<%=basePath %>js/imgDrag/jquery.dad.css" rel="stylesheet">
...
//        $("#update_goodsInfoStock").val(data.goodsInfoStock);
              ...
              $("#update_goodsInfounitPriceStr").val(data.unitPriceStr);
      ...
      function updateProduct(){
          if(checkUpdateSpec()&&num==0){
              ...
              //同步商品信息到ERP
              /*$.ajax({
                  url:"/synUpdateGoodsToERP.htm?CSRFToken=" + $(".token_val").val(),
                  ...
                  success:function (data) {
                      if(data==1){
                          ...
                          showTipAlert("同步到ERP失败，请检查ERP商品信息！",function(){
                              $("#updateProduct").submit();
                          });
                      }
                  }
      ...
      function modifiedImage(productId)
      {
          ...
          $.get("queryImageListByProductId.htm?CSRFToken="+$(".token_val").val()+"&productId=" + productId, function (data)
          {
                  ...
                  if(!isdel) {
                      ...
                      if (data[i].imageInName != null) {
                          if (data[i].goodsImgSort == 1) {
                              ...
                              imageListHtml = imageListHtml + "<div class='col-sm-6 imgdiv' id='imagediv" + i + "'><div class='pic_item'><div class='img'><input name='customerImg' id='customerImg' type='hidden' value=''><input type='hidden' name='imgContent' value='"+data[i].goodsImgId+"' class='imgContentClaz'><input   type='hidden' class='setDefault' value='"+ data[i].imageInName.replace('\\', "\\\\") +""+'-'+""+data[i].goodsImgId+"'><img id='img' alt='' src='" + data[i].imageInName + "' /></div>" +
                                  "<p><button type='button' id='" + data[i].goodsImgId +  "_de' class='btn btn-default btn-sm pull-right' onmousedown=" + "delImage(" + "'" + data[i].goodsImgId + "',this,1);" + ">删除</button>" +
                                  "<button id=" + data[i].goodsImgId + " type='button' class='btn btn-default btn-sm default_btns' style='opacity:0; visibility: hidden;' onclick=" + "setDefaultImage(" + "'" + data[i].imageInName.replace('\\', "\\\\") + "'," + data[i].goodsImgId + ");" + ">" +
                                  "" + btnvalue + "</button></p></div></div>";
                          }else{
                              imageListHtml = imageListHtml + "<div class='col-sm-6 imgdiv' id='imagediv" + i + "'><div class='pic_item'><div class='img'><input name='customerImg' id='customerImg' type='hidden' value=''><input type='hidden' name='imgContent' value='"+data[i].goodsImgId+"' class='imgContentClaz'><input   type='hidden' class='setDefault' value='"+ data[i].imageInName.replace('\\', "\\\\") +""+'-'+""+data[i].goodsImgId+"'><img id='img' alt='' src='" + data[i].imageInName + "' /></div>" +
                                  "<p><button type='button' id='" + data[i].goodsImgId + "_de' class='btn btn-default btn-sm pull-right' onmousedown=" + "delImage(" + "'" + data[i].goodsImgId + "',this,0);" + ">删除</button>" +
                                  "<button id=" + data[i].goodsImgId + " type='button' class='btn btn-default btn-sm default_btns' style='opacity:0; visibility: hidden;' onclick=" + "setDefaultImage(" + "'" + data[i].imageInName.replace('\\', "\\\\") + "'," + data[i].goodsImgId + ");" + ">" +
                                  "" + btnvalue + "</button></p></div></div>";
                          }
                      }
              ...
              <%--shf增加图片拖拽功能开始--%>
              var imgLength = $("#imageList").find(".imgdiv").length;
              for(var i=1;i<=imgLength;i++){
                  items = "item"+i;
                  $("#imageList").find(".imgdiv").eq(i-1).addClass("item");
                  $("#imageList").find(".imgdiv").eq(i-1).addClass(items);
              }
              $(function(){
                  $('.dowebok').dad();
              });
      ...
      function setDefaultImage(imageName,imgId)
      {
          ...
          }else
          {
              ...
              $("#"+defaultPicId+"_de").attr("onmousedown","delImage('"+defaultPicId+"',this,0)");
              defaultPicId = imgId;
          }

          $("#"+imgId+"").text('默认图片');
          $("#"+defaultPicId+"_de").attr("onmousedown","delImage('"+defaultPicId+"',this,1)");
      ...
      function saveImage(){
          ...
          //设置每一个图片的id 跟 排序
          $(".imgContentClaz").each(function(index,item){
              var _this=$(item);
              _this.val(_this.val()+'-'+_this.parent().parent().parent('.imgdiv').attr('data-dad-position'));
          });
          //把第一个设置为默认
          var arr=$('.setDefault').eq(0).val().split("-");
          var imageName=arr[0];
          var imgId=arr[1];
          setDefaultImage(imageName,imgId);
       ...
       * 是否上架
       * */
      function updateGoodsInfoAdded(num,auditStatus,goodsId,goodInfoId){
          window.location="updateProSta.htm?CSRFToken=${token }&goodsId="+goodsId+"&goodsInfoId="+goodInfoId+"&goodsInfoAdded="+num+"&auditStatus="+auditStatus;
          ...
          <form action="batchDelProduct.htm?CSRFToken=${token }" method="post" id="queryProduct">
			...
			<td class="tc">
				 ${product.goodsInfoMarketPrice}
                  ...
                  <td>
                    <c:if test="${product.goodsInfoAdded==1&&product.auditStatus==3}"><a class="label label-success" onclick="updateGoodsInfoAdded(0,2,${product.goodsId },${product.goodsInfoId })">是</a></c:if>
                    <c:if test="${product.goodsInfoAdded==0&&product.auditStatus==2}"><a class="label label-default" onclick="updateGoodsInfoAdded(0,1,${product.goodsId },${product.goodsInfoId })">否</a></c:if>
                    <c:if test="${product.goodsInfoAdded==0&&product.auditStatus==1}"><a href="javascript:void(0);"  style="background-color:royalblue;text-align: center;color:lightgoldenrodyellow;border-radius:4px 4px 4px 4px;" title="请等待审核">待审核</a></c:if>
                  </td>
                  <td>
        ...
        <div class="modal-body">
          ...
          <div class="form-horizontal">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-16">
                <div class="input-group w200">
                  <input type="text" class="form-control  clear" name="unitPriceStr" id="update_goodsInfounitPriceStr">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-16">
                <label class="radio-inline">
                  <input type="radio" id="update_added" value="1" name="goodsInfoAdded"> 审核上架
                </label>
                <label class="radio-inline">
                  <input type="radio" id="update_unadded" value="0" name="goodsInfoAdded" checked="checked"> 下架
                </label>
              </div>
            </div>
            <%-- <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="container-fluid">
            <div class="row dowebok" id="imageList">
...
<%--shf增加图片拖拽功能--%>
<script src="<%=basePath %>js/imgDrag/jquery.dad.min.js"></script>
<script>
    $(function(){
        $('.dowebok').dad();
    });
