...
<xsl:stylesheet version="1.0"
	...
	<!--banner图-->
	<xsl:template match="roll_adv">
		<div class="pingBanner" style="display:none">
			<xsl:attribute name="id">roll_adv<xsl:value-of select="@id" /></xsl:attribute>
			<xsl:apply-templates select="roll_a"/>
		 </div>
	</xsl:template>
	...
	<!--<xsl:template match="imagenav">
		<div class="app_item app_cube" style="display: block;">
			<div class="img_adv item">
				<div class="image-nav-bar">
					<xsl:apply-templates select="img_nav_a"/>
				</div>
			</div>
		</div>
	</xsl:template>-->
	<!--专题-->
	<xsl:template match="imagenav">
		<div class="zhuanti" style="display:none">
			<xsl:apply-templates select="img_nav_a"/>
		</div>
	</xsl:template>

	<xsl:template match="img_nav_a">
		<a class="img-nav-item">
			<xsl:attribute name="href">
				<xsl:choose>
					<xsl:when test="@href != ''"><xsl:value-of select="@href" /></xsl:when>
					<xsl:otherwise>javascript:void(0);</xsl:otherwise>
