...
var currentPage=1;
var pageSize=2;
function backOrderRemark(bid,code){
    backOrderRemarkFun(bid,currentPage,pageSize,code);
}
function backOrderRemarkFun(bid,curr,size,code){
    curr=parseInt(curr);
     $.ajax({
         type:"post",
         data:{
         ...
         url:"backOrderRemarkAjax.htm?CSRFToken="+$("#CSRFToken").val(),
         success:function(result){
             $("#remarktr").html("");
             $('.backOrderId').val(bid);
             $('.backOrderCode').val(code);
             if(!!(result.totalPage>0)){
                 var backOrderRemarks=result.backOrderRemarks;
                 var totalPage=parseInt(result.totalPage);
                 //订单备注能容
                 var str="";
                 $.each(backOrderRemarks,function(index,item){
                     str+='<tr>' +
                          '<td>'+item.backOrderCode+'</td>' +
                          '<td>'+item.remarkContent+'</td>' +
                          '<td>'+item.backcustomer+'</td>'+
                          '<td>'+new Date(item.creatTime).toLocaleString()+'</td>' +
                          '</tr>';
                 ...
                 var sb='';
                 if(curr>1){
                     sb+="<a  href='javascript:backOrderRemarkFun("+bid+','+(curr-1)+','+pageSize+");'> 上一页 </a>";
                 }
                 for(var i=(((curr-3)>0)?(curr-3):1);i<=(((curr+4)>totalPage)?totalPage:(curr+4));i++){
                     if(i==curr){
                         sb+='<span class="current">'+i+'</span>';
                     }else{
                         sb+="<a href='javascript:backOrderRemarkFun("+bid+','+i+','+pageSize+");'>"+i+"</a>";
                     }
                 }
                 if(curr<totalPage){
                     sb+="<a  href='javascript:backOrderRemarkFun("+bid+','+(curr+1)+','+pageSize+");'>下一页 </a>";
                 }
                 $("#pagefenye").html(sb);
                 var backremarks=document.getElementById("backorderremarks");
                 $(backremarks).modal("show");
                 //赋值


             }else{
                 $("#remarktr").html('<tr><td colspan="4"><p class="text-center">暂无可用数据！</p></td></tr>');
                 var backremarks=document.getElementById("backorderremarks");
                 $(backremarks).modal("show");
             }
...
function addBackremark(){
    var backremarks=document.getElementById("backorderremarks");
    $(backremarks).modal("hide");
    $('.allfixed').show();
    $('.backremark').show();
}
...
function btnclick(){
    ...
    $.ajax({
        ...
        data:{
            ...
            backOrderCode:$('.backOrderCode').val(),
            remarkContent:$('#remarkContentId').val(),
        },
        url:"addBackOrderRemarkAjax.htm?CSRFToken="+$("#CSRFToken").val(),
        success:function(result){
            console.log(result);
            closespan(".backremark");
        }
    })
}
...
function saveorder() {
    if($(".flag_saved").val()=="1"){
        ...
        if(backLogStatus.val() == "5") {
            $("input[name='backLogStatus']").val("8");
            if (checkBackPrice == "") {
...
function submitOrder(){
   // $("#backOrderCodestr").val($("#backOrderCode").val());
    ...
    $("#conditionstr").val($("#conditionId").val());
