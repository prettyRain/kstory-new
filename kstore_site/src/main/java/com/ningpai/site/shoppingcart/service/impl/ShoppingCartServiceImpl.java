...
public class ShoppingCartServiceImpl implements ShoppingCartService {
                    ...
                    if (shoplist.get(i).getGoodsDetailBean() != null && shoplist.get(i).getGoodsDetailBean().getProductVo() != null) {
                        ...
                        if (shoplist.get(i).getMarketingList() != null) {
                            ...
                            for(Marketing marketing : shoplist.get(i).getMarketingList()){
                                    ...
                                    if(marketing.getMarketingId()!=null && marketing.getMarketingId()!=0 && shoplist.get(i).getMarketingActivityId()!=null && shoplist.get(i).getMarketingActivityId() !=0) {
                                        if (marketing.getMarketingId().longValue() == shoplist.get(i).getMarketingActivityId().longValue()) {
                                            if (marketing.getPriceOffMarketing() != null) {
                                                //直降
                                                if (marketing.getPriceOffMarketing().getOffValue() != null && goodsPrice.compareTo(marketing.getPriceOffMarketing().getOffValue()) == 1) {
                                                    goodsPrice = goodsPrice.subtract(marketing.getPriceOffMarketing().getOffValue());
                                                }
                                                //抢购
                                                ...
                                                rushMap.put("goodsInfoId", shoplist.get(i).getGoodsInfoId());
                                                ...
                                                if(countRush>0){
                                                    goodsPrice = goodsPrice.multiply(marketing.getRushs().get(0).getRushDiscount());
                                                }
                                                marketing.setCustomerbuynum(countRush);
                                                mg.setMarketing(marketing);
                                                if (shoplist.get(i).getGoodsNum() > countRush) {
                                                    //goodspriceSum = goodspriceGo.multiply(BigDecimal.valueOf(countRush));
                                                    // goodspriceSum = goodspriceSum.add(shoplist.get(i).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(BigDecimal.valueOf(cart.getGoodsNum()).subtract(BigDecimal.valueOf(countRush))));
                                                } else {
                                                    //goodspriceSum = goodspriceGo.multiply(BigDecimal.valueOf(shoplist.get(i).getGoodsNum()));
                                                }
                                                //团购促销
                                            } else if (marketing.getGroupon() != null) {
                                                goodsPrice = goodsPrice.multiply(marketing.getGroupon().getGrouponDiscount());
                                                //goodsPrice = goodsPrice.multiply(BigDecimal.valueOf(shoplist.get(i).getGoodsNum()));
                                                //折扣促销
                                                ...
                                                for (PreDiscountMarketing premark : marketing.getPreDiscountMarketings()) {
                                                    ...
                                                    if (premark.getGoodsId().equals(shoplist.get(i).getGoodsInfoId())) {
                                                        ...
                                                        //goodspriceSum = goodspriceGo.multiply(BigDecimal.valueOf(cart.getGoodsNum()));
                                                        // 不四舍五入
                                                    }
                                                }
                                            }
    ...
    public BigDecimal getEverythirdExpressPrice(Long thirdId, Long cityId, List<ShoppingCart> cartList) {
        ...
        for (ShoppingCart sc : cartList) {
            if (sc.getThirdId().equals(thirdId)) {
                ...
                if (sc.getFitId() == null) {
                    // 查询商品详细
                    sc.setGoodsDetailBean(
                            goodsProductService.queryDetailBeanByProductId(((ShoppingCart) sc).getGoodsInfoId(), ((ShoppingCart) sc).getDistinctId()));
                    // 查询商品参加的促销信息(查出未结束 未删除的促销)
                    sc.setMarketingList(
                            marketService.selectMarketingByGoodsInfoId(sc.getGoodsInfoId(), sc.getGoodsDetailBean().getBrand().getBrandId(), sc.getGoodsDetailBean().getProductVo().getGoods().getCatId()));

                    // 如果是普通商品，执行普通商品的方法
                    GoodsProductVo goodsProduct = null;
                    if(distinctId!=null){
                        goodsProduct = goodsProductService.queryDetailBeanByProductId(sc.getGoodsInfoId(), distinctId).getProductVo();
                    }else{
                    ...
                    if(sc.getMarketingList()!=null && sc.getMarketingList().size()>0 && "1".equals(sc.getMarketingList().get(0).getCodexType())){
                        ...
                        countRush = countRush<=0?0:countRush;
                        //抢购
                        BigDecimal discount = goodsElasticMapper.selectRushByMarketId(sc.getMarketingActivityId());
                        goodsProduct.getGoodsInfoPreferPrice().multiply(discount);
                    ...
                    for (int j = 0; j < goodsProducts.size(); j++) {
                        if(goodsProducts.get(j).getProductName().indexOf("零售")<0) {
                            pifa = true;
                        }
                        goodsweight = goodsweight.add(goodsProducts.get(j).getGoodsInfoWeight().multiply(new BigDecimal(sc.getGoodsNum())));
                        goodsnum += Integer.parseInt(sc.getGoodsNum().toString());
                        totalPrice = totalPrice.add(goodsProducts.get(j).getGoodsInfoPreferPrice().multiply(new BigDecimal(sc.getGoodsNum())));
                    }
     ...
     */
    @Override
    public Map<String, Object> getNewExpressPrice(ShoppingCartWareUtil wareUtil,HttpServletRequest request, Long addressId, Long cityId, Long[] cartIds,Long isbuynow) {

        List<ShoppingCart> shopdata = null;
        ShoppingCart cart = null;
        if(isbuynow == null){
            shopdata = shoppingCartMapper.shopCartListByIds(Arrays.asList(cartIds));
        }else{
            shopdata = new ArrayList<ShoppingCart>();
            cart = (ShoppingCart)request.getSession().getAttribute("buyNowGoods");
            shopdata.add(cart);
        }
            ...
            if(presents.length>0){
                for(int i=0;i<presents.length;i++){
                    if(presents[i].indexOf(":")!=-1){
                        ...
                        for(ShoppingCart shop:cartList){
                            if(shop.getShoppingCartId().equals(shopId)){
                                String[] _scopeIds = scopeIds.split(",");
                                for(String scopeId:_scopeIds){
                                    FullbuyPresentScope fullbuyPresentScope = this.fullbuyPresentScopeMapper.selectByPrimaryKey(Long.valueOf(scopeId));
                                    //赠品是商品
                                    if("0".equals(fullbuyPresentScope.getScopeType())){
                                        GoodsProductVo vo = this.goodsProductService.queryByProductIdForPresent(fullbuyPresentScope.getScopeId(),wareUtil.getDistrictId());
                                        goodsweight = goodsweight.add(vo.getGoodsInfoWeight().multiply(BigDecimal.valueOf(fullbuyPresentScope.getScopeNum())));
                                    }
...
//                        }
        ...
        Long customerIds = (Long) request.getSession().getAttribute(CUSTOMERID);
        Map<String, Object> map = siteOrderService.getEveryparamMap(customerIds,cartIds, addressId,cart);
        ...
        // boss价格
        freightMap.put(BOSSSUMPRICE, ((BigDecimal)map.get(BOSSSUMPRICE)).subtract(((BigDecimal)map.get("rushSumPrice")).subtract((BigDecimal)map.get("rushPrePrice"))));
        // boss优惠金额
        freightMap.put("bossPrePrice", map.get(BOSSPREPRICE));

        //满减 满折优惠价
        freightMap.put("myhPrice",map.get("myhPrice"));
        // 总价格
        ...
        // 会员等级折扣价
        BigDecimal discountPrice1 = customerPointServiceMapper.getCustomerDiscountByPoint(customerPointServiceMapper.getCustomerAllPointSimple(customerId+"")-customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+"")).multiply(BigDecimal.valueOf(Double.valueOf(map.get(CUSZHEKOUPRICE).toString())));
        // 减掉返现金额的总价格
        ...
        //bossSumPrice 优惠后  含会员折扣后
        freightMap.put(BOSSSUMPRICE, BigDecimal.valueOf(Double.valueOf(map.get("bossSumPrice").toString())).subtract(BigDecimal.valueOf(Double.valueOf(map.get("bossPrePrice").toString()))).subtract(discountPrice).subtract(((BigDecimal)map.get("rushSumPrice")).subtract((BigDecimal)map.get("rushPrePrice"))));
        // 总运费
        freightMap.put("freightmoney", ((BigDecimal)map.get("expressPrice")).add(freightmoney));
        // boss平台运费
    ...
    @Override
    @Transactional
    public int addShoppingCart(ShoppingCart shoppingCart, HttpServletRequest request, HttpServletResponse response,Long isMark) throws UnsupportedEncodingException {
        ...
        if (null != custId) {
            ...
            if (count == 0 || isMark != null) {
                ...
                if(isMark == null){
                    ...
                    for (Marketing mark : markList) {
                        //if (!"15".equals(mark.getCodexType()) && !"12".equals(mark.getCodexType())&& !"10".equals(mark.getCodexType())) {
                        shoppingCart.setMarketingActivityId( mark.getMarketingId());
                        shoppingCart.setMarketingList(markList);
                        break;
                    }
                    request.getSession().removeAttribute("buyNowGoods");
                    request.getSession().setAttribute("buyNowGoods",shoppingCart);
                    return 2;
                }
...
//                            .getBrandId());
                    //每个商品只能有一个促销活动
                    activeMarketId = marketService.queryByCreatimeMarketings(shoppingCart.getGoodsInfoId(), productVo.getGoods().getBrandId(), productVo
                            .getGoods().getCatId());
                }
            ...
            newmId += "-" + shoppingCart.getGoodsInfoId() + "e" + marketId + "e" + activeMarketId + "e"+ goodsGroupId+"e"+ "0";

            cook = new Cookie(NPSTORESHOPCAR, URLEncoder.encode(oldCar, ConstantUtil.UTF));
            cook.setMaxAge(15 * 24 * 3600);
            cook.setPath("/");
            response.addCookie(cook);
            Cookie cookie = new Cookie(NPSTOREMID, newmId);
            cookie.setMaxAge(15 * 24 * 3600);
            cookie.setPath("/");
            response.addCookie(cookie);
            return 1;
...
//                            }
    ...
    public int delGoodsGroupByS(Long shoppingCartId, Long goodsInfoId, Long fitId, HttpServletRequest request, HttpServletResponse response) {
        ...
        for (int i = 0; i < vos.size(); i++) {
            ...
            if (goodsInfoId.longValue() != id.longValue()) {
                ...
                try {
                    addShoppingCart(cart, request, response,null);
                } catch (UnsupportedEncodingException e) {
    ...
    public Long selectLastId(ShoppingCart shoppingCart, HttpServletResponse response, HttpServletRequest request) {
        try {
            addShoppingCart(shoppingCart, request, response,null);
        } catch (UnsupportedEncodingException e) {
            ...
            for (int i = 0; i < shoplist.size(); i++) {
                ...
                if (shoplist.get(i).getFitId() == null) {
                    ...
                    if(null!=marketingId){
                        ...
                        if(null!=mk){
                            if(mk.getCodexType().equals("11")){
                                if(mk.getRushs()!=null && !mk.getRushs().isEmpty() && customerId!=null) {
                                    Map rushMap = new HashMap();
                                    rushMap.put("rushId", mk.getRushs().get(0).getRushId());
                                    rushMap.put("customerId", customerId);
                                    rushMap.put("goodsInfoId", shoplist.get(i).getGoodsInfoId());
                                    //当存在抢购时 得到每个会员可以购买的货品数量
                                    Integer countRush = rushCustomerMapper.selectByParamMap(rushMap);
                                    countRush = countRush == null ? 0 : countRush;
                                    countRush = mk.getRushs().get(0).getRushLimitation() - countRush;
                                    if(countRush <=0 ){
                                        countRush = 0;
                                    }
                                    mk.setCustomerbuynum(countRush);
                                    shoplist.get(i).getMarketingList().get(0).setCustomerbuynum(countRush);
                                }
                            }
                            //如果促销是满赠促销
                    ...
                    if(shoplist.get(i).getMarketingList()!=null && shoplist.get(i).getMarketingList().size()>0){
                        shoplist.get(i).setMarketingActivityId(shoplist.get(i).getMarketingList().get(0).getMarketingId());
                    }else{
                        shoplist.get(i).setMarketingActivityId(null);
                    }
                } else {
                    Long stock = null;
                    // 如果商品是套装
                    ...
                    for (int j = 0; j < goodsGroupReleProductVos.size(); j++) {
                        ...
                        goodsGroupReleProductVos.get(j).getProductDetail().setSellType(goodsDetailBean.getSellType());
                        goodsGroupReleProductVos.get(j).getProductDetail().setUnitPrice(goodsDetailBean.getUnitPrice());
                        BigDecimal goodstock = BigDecimal.valueOf(goodsDetailBean.getProductVo().getGoodsInfoStock());
                    ...
                    for(String infoid:goodlist){
                        ...
                        if(null!=fpt){
                            ...
                            if("0".equals(fpt.getScopeType())){
                                ...
                                //赠品是优惠券
                            }else if("1".equals(fpt.getScopeType())){
                                HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
                                Long customerId = (Long)request.getSession().getAttribute("customerId");
                                Coupon coupon = couponService.queryMZCoupon(customerId,fpt.getScopeId());
                                if(coupon!=null){
                                    fpt.setCoupon(coupon);
                                    fullscope.add(fpt);
                                }
                            }
    ...
    public List<Coupon> getUsedCouponlist(HttpServletRequest request, List<ShoppingCart> shoplists) {
        ...
        //通过字符串赋值
        List<ShoppingCart> newshoplists = new ArrayList<ShoppingCart>();
        JSONArray jsonArray = JSONArray.fromObject(shoplists);
        for(int i =0 ; i<jsonArray.size() ;i++){
             JSONObject jsonObject = (JSONObject)jsonArray.get(i);
            ShoppingCart shoppingCart = (ShoppingCart) JSONObject.toBean(jsonObject,ShoppingCart.class);
            newshoplists.add(shoppingCart);
        }

        // 查询所有购物车商品
        List<ShoppingCart> shoplist = new ArrayList<>();

        if (CollectionUtils.isNotEmpty(newshoplists)) {
            List<ParamIds> infoIds = new LinkedList<>();
            // 去除套装,套装不允许使用优惠券
            for (ShoppingCart sp : newshoplists) {
                if (sp.getFitId() != null&&sp.getFitId()>0l ) {
                    // 如果商品是套装,套装不允许使用优惠券
                }
              else  if(sp!=null&&sp.getMarketingActivityId()!=null){
                    Marketing market = marketingMapper.marketingDetail(sp.getMarketingActivityId());
            ...
            if (CollectionUtils.isNotEmpty(shoplist)) {
                ...
                for (int i = 0; i < shoplist.size(); i++) {
                    ...
                    // 折扣促销  订单页面折扣促销价格在页面算
                        // 查询商品参加的促销信息
                        List<Marketing> marketingList = marketService.selectMarketingByGoodsInfoId(shoplist.get(i).getGoodsInfoId(), shoplist.get(i).getGoodsDetailBean().getBrand()
                                .getBrandId(), shoplist.get(i).getGoodsDetailBean().getProductVo().getGoods().getCatId());
                        //直降 团购 折扣（后的价格）
                        for(Marketing mark:marketingList){
                            //团购
                            if(mark.getGroupon()!=null &&shoplist.get(i).getMarketingActivityId()!=null&& shoplist.get(i).getMarketingActivityId().longValue()==mark.getMarketingId().longValue()) {
                                // isgroup = 1;
                                shoplist.get(i).getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsprice.multiply(mark.getGroupon().getGrouponDiscount()));
                                //直降
                            }else if(mark.getPriceOffMarketing()!=null&&shoplist.get(i).getMarketingActivityId()!=null&& shoplist.get(i).getMarketingActivityId().longValue()==mark.getMarketingId().longValue()){
                                shoplist.get(i).getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsprice.subtract(mark.getPriceOffMarketing().getOffValue()));
                                //折扣
                            }else if (shoplist.get(i).getMarketingActivityId() != null && mark.getMarketingId().longValue()== shoplist.get(i).getMarketingActivityId().longValue() && mark.getPreDiscountMarketings()!=null) {
                                // 从购物车里得到促销id重新从数据库查询,防止当前(折扣促销)已经过期;
                                Marketing marke = marketService.marketingDetail(shoplist.get(i).getMarketingActivityId(), shoplist.get(i).getGoodsInfoId());
                                if (mark != null) {
                                    para.put(MARKETINGID, mark.getMarketingId());
                                    para.put(GOODSID, shoplist.get(i).getGoodsInfoId());
                                    PreDiscountMarketing premark = preDiscountMarketingMapper.selectByMarketId(para);
                   ...
                   /*if (shoplist.get(i).getMarketingActivityId() != null && 0 != shoplist.get(i).getMarketingActivityId()) {
                        // 从购物车里得到促销id重新从数据库查询,防止当前(折扣促销)已经过期;
                        Marketing mark = marketService.querySimpleMarketingById(shoplist.get(i).getMarketingActivityId());
                        if (mark != null) {
                            para.put(MARKETINGID, mark.getMarketingId());
                            para.put(GOODSID, shoplist.get(i).getGoodsInfoId());
                            PreDiscountMarketing premark = preDiscountMarketingMapper.selectByMarketId(para);
                    ...
                    if("6".equals(marketinglist.get(i).getCodexType())){
                        ...
                        for (int j = 0; j < marketinglist.get(i).getFullbuyPresentMarketings().size(); j++) {
                            for (int k = 0; k < marketinglist.get(i).getFullbuyPresentMarketings().get(j).getFullbuyPresentScopes().size(); k++) {
                                FullbuyPresentScope fullbuyPresentScope = marketinglist.get(i).getFullbuyPresentMarketings().get(j).getFullbuyPresentScopes().get(k);
                                //赠送商品
                                if("0".equals(fullbuyPresentScope.getScopeType())){
                                    ...
                                    //赠送优惠券
                                }else if("1".equals(fullbuyPresentScope.getScopeType())){
                                    //得到优惠券id
                                    Long couponId = fullbuyPresentScope.getScopeId();
                                    Coupon coupon = couponService.queryMZCoupon(customerId,couponId);
                                    //有符合添加的优惠券
                                    if(coupon !=null){
                                        fullbuyPresentScope.setCoupon(coupon);
                                        //没有符合的优惠券就不显示该优惠
                                    }
                                }
        ...
        if (shoppingCartList != null && !shoppingCartList.isEmpty()) {
            ...
            for (int i = 0; i < shoppingCartList.size(); i++) {
                ...
                if (shoppingCartList.get(i).getFitId() == null) {
                    ...
                    BigDecimal goodsPriceRush = shoppingCartList.get(i).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    //直降促销的价格中间变量
                    BigDecimal goodsPriceOff = shoppingCartList.get(i).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice();
                    //折扣促销的价格中间变量
                   ...
                   //团购促销价格的中间变量
                    ...
                    if(mark.getRushs()!=null && !mark.getRushs().isEmpty() ){
                        ...
                        shoppingCartList.get(i).setGoodsPrice(shoppingCartList.get(i).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice());
                        if(countRush>0){
                            shoppingCartList.get(i).getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsPriceRush.multiply(mark.getRushs().get(0).getRushDiscount()));
                        }
                            ...
                            if (mark.getPreDiscountMarketings() != null && !mark.getPreDiscountMarketings().isEmpty() && isgroup==0) {
                                ...
                                shoppingCartList.get(i).getGoodsDetailBean().getProductVo().setGoodsInfoPreferPrice(goodsPriceOff.subtract(mark.getPriceOffMarketing().getOffValue()));
                            }
                        }
                        ...
                        //现在每个商品只有一个促销   当促销为空时置为null
                        shoppingCartList.get(i).setMarketingActivityId(marketings.get(0).getMarketingId());
...
//                    }
     ...
     */
    @Override
    public Map<String, Object> subBuyNowMap(HttpServletRequest request, ShoppingCart shoppingCart, ShoppingCartWareUtil wareUtil, PointSet pointSet) {
        ...
        shoppingCart.setShoppingCartId(999999L);
        shoplist.add(shoppingCart);
        //  初始化商家
        List<StoreTemp> storeList = new ArrayList<StoreTemp>();
        StoreTemp storeTemp = new StoreTemp();
        storeTemp.setThirdId(Long.valueOf(0));
        ...
        if (shoplist != null && !shoplist.isEmpty()) {
            for (int i = 0; i < shoplist.size(); i++) {
                ...
                if (shoplist.get(i).getFitId() == null) {
                    ...
                    //购物车商品所参加的促销id
                    Long marketingId = shoplist.get(i).getMarketingActivityId();
                    for(Marketing mark:shoplist.get(i).getMarketingList()){
                        if(mark.getRushs()!=null && !mark.getRushs().isEmpty() ){
                            Map rushMap = new HashMap();
                            rushMap.put("rushId",mark.getRushs().get(0).getRushId());
                            rushMap.put("customerId",customerId);
    ...
    public void selectBuyNowDetail(Long productId, Long goodsNum, ShoppingCartWareUtil cartWareUtil) {
        // 判断是否为null
        if (shoppingCartList != null && !shoppingCartList.isEmpty()) {
