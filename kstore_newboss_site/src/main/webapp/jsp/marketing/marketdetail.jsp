               ...
               <c:if test="${marketing.codexType==11}">
                    <div class="col-sm-6">
                        <p>
                            抢购类型：
                            <c:choose>
                                <c:when test="${marketing.rushs[0].temp1==2}">热销推荐</c:when>
                                <c:otherwise>
                                    普通商品
                                </c:otherwise>
                            </c:choose>
...
<c:if test="${marketing.codexType==11}">
               ...
               <div class="row">
                   <c:choose>
                       <c:when test="${marketing.codexType==11&&marketing.rushs[0].rushTimes.tId>0}">
                           <div class="col-sm-8">
                               <p>抢购时段：${marketing.rushs[0].rushTimes.tDate }点</p>
                           </div>
                       </c:when>
                       <c:otherwise>
                                   ...
                                   <tbody>
                                       <c:forEach items="${present.fullbuyPresentScopes}" var="scope" varStatus="i">
                                           <c:choose>
                                               ...
                                               <c:when test="${scope.scopeType=='1'}">
                                                  <tr>
                                                       <td>${scope.coupon.couponName }</td>
                                                     <c:choose>
                                                       <c:when test="${scope.coupon.timeType=='1'}">
                                                           <td>${scope.coupon.couponDays }天</td>
                                                       </c:when>
                                                       <c:otherwise>
                                                           <fmt:formatDate value="${scope.coupon.couponEndTime}" var="endTime" pattern="yyyy/MM/dd"></fmt:formatDate>
                                                           <fmt:formatDate value="${scope.coupon.couponStartTime}" var="startTime" pattern="yyyy/MM/dd"></fmt:formatDate>
