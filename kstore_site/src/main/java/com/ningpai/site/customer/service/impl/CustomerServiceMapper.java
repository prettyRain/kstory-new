...
public class CustomerServiceMapper implements CustomerServiceInterface {
    ...
    public Customer queryCustomerById(Long customerId) {
        ...
        if (null != customer) {
            ...
            com.ningpai.customer.bean.CustomerPointLevel customerPointLevel = customerPointServiceMapper.getCustomerPointLevelByPoint(customerAllInfo.getInfoPointSum()-customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+""));
    ...
    public CustomerAllInfo selectByPrimaryKey(Long customerId) {
        ...
        if (null != customerAllInfo) {
            com.ningpai.customer.bean.CustomerPointLevel customerPointLevel = customerPointServiceMapper.getCustomerPointLevelByPoint(customerPointServiceMapper.getCustomerAllPoint(customerId + "")-customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+""));
    ...
    public PageBean queryAllMyOrders(Map<String, Object> paramMap, PageBean pb) {
        ...
        // 查询所有订单
        List<Object> pbList = customerMapper.queryAllMyOrders(paramMap);
        ...
        for (Object o : pbList) {
            ...
            if (splitOrders.size()!=0){
                ...
                for (Order splitOrder : splitOrders) {
                    ...
                    if(!"4".equals(splitOrder.getOrderStatus())){
                        ...
                        OrderInfoBean oib = (OrderInfoBean) customerMapper.queryOrderByParamMap(paramMap);
                        //展示待处理订单时，过滤掉不满足条件的拆分子订单
                        if(null != type && "0".equals(type) && (!"0".equals(oib.getOrderStatus())) && (!"1".equals(oib.getOrderStatus())) && (!"2".equals(oib.getOrderStatus()))){
            ...
            //没有拆分订单，或者拆分订单下的子订单不符合查询条件
            if(!result){
                //排除拆分的订单下子订单都不符合条件造成的空订单展示
                if(splitOrders.size()!=0 && splitOrders2.size()==0){
...
//            sms.setMsgContext(((Integer) num).toString());
            ...
            try {
                // 短信发送
                if (AliSmsUtil.send(num,moblie,AliSmsUtil.smsTemplate.COMMON_TEMP.getContent(),null)) {
                    // 修改验证码
