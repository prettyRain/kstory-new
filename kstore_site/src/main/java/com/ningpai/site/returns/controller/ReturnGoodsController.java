...
public class ReturnGoodsController {
    ...
    public ModelAndView applybackmoneyprice(HttpServletRequest request, Long orderId) {
        ...
        // 必须信息完整 （goodsId , goodsNum , orderGoodsId）
        String str = "";
        for (int i = 0; i < backorder.getGoods().size(); i++) {
            ...
            Long orderGoodsId = backorder.getGoods().get(i).getOrderGoodsId();
            str += goodsids + "," + goodsNum + "," + orderGoodsId + "-";
        }
...
//        }
        ...
        for (int i = 0; i < backorder.getGoods().size(); i++) {
            if(backorder.getGoods().get(i).getGoodsCouponPrice() == null){
                total = total.add(backorder.getGoods().get(i).getGoodsInfoSumPrice());
            }else{
                total = (total.add(backorder.getGoods().get(i).getGoodsInfoSumPrice())).subtract(backorder.getGoods().get(i).getGoodsCouponPrice());
            }
        }
        for (int i = 0; i < backorder.getGoods().size(); i++) {
            // 查询货品详细信息
            ...
            OrderGoods orderGoods = orderService.queryOrderGoodsByIds(backorder.getOrderId(),goodsProductDetailViewVo.getGoodsInfoId(),backorder.getGoods().get(i).getGoodsInfoSumPrice());
//            if(backorder.getGoods().get(i).getGoodsCouponPrice() == null){
//                backorder.getGoods().get(i).setReturnMoney((backorder.getMoneyPaid().subtract(order.getExpressPrice())).multiply(backorder.getGoods().get(i).getGoodsInfoSumPrice()).divide(total, 2));
//
//            }else{
//                BigDecimal returnMoney = BigDecimal.ZERO;
//                returnMoney = backorder.getGoods().get(i).getGoodsInfoSumPrice().subtract(backorder.getGoods().get(i).getGoodsCouponPrice());
//                returnMoney = returnMoney.multiply(backorder.getMoneyPaid().subtract(order.getExpressPrice()));
//                returnMoney = returnMoney.divide(total,2);
//                backorder.getGoods().get(i).setReturnMoney(returnMoney);
//            }
            backorder.getGoods().get(i).setReturnMoney(orderGoods.getGoodsBackPrice());
            backorder.getGoods().get(i).setOrderGoodsId(orderGoods.getOrderGoodsId());
        }
        // 获取退货说明
        ...
        return topAndBottomService.getTopAndBottom(mav).addObject(ORDER, order).addObject(BACKORDER, backorder).addObject("backInfoRemark", backInfoRemark).addObject("price",price).addObject("staCheck",staCheck)
                .addObject("customerId", customerId).addObject("isUseCoupon", isUseCoupon);
    }
    ...
    public ModelAndView backDetailInfoprice(HttpServletRequest request, Long orderId) {
        ...
        Order order = siteOrderService.queryOrderDetailByIds(orderId, customerId);
        if (order == null) {
            return new ModelAndView(new RedirectView("/login.html"));
        }
        // 订单详细信息
    ...
    public ModelAndView backDetailInfo(HttpServletRequest request, Long orderId) {
        ...
        // 退货的时候 可能只退其中的一个 把没有退货的 从列表中去除
        removeOrderList(backorder.getOrderGoodsList(), bOrder.getBackGoodsIdAndSum());
