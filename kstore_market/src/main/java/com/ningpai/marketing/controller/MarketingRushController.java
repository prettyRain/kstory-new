...
public class MarketingRushController {
    ...
    public ModelAndView addMarketRushTime(MarketingRushTime rushTime,HttpServletRequest request) {
        rushTime.setTemp1(request.getParameter("rushTimename").toString());
        if(StringUtils.isNotBlank(request.getParameter("endrushTime"))){
            rushTime.setTemp2(request.getParameter("endrushTime"));
        }
    ...
    public ModelAndView updateMarketRushTime(MarketingRushTime rushTime,HttpServletRequest request) {
        rushTime.setTemp1(request.getParameter("rushTimename").toString());
        if(StringUtils.isNotBlank(request.getParameter("endrushTime"))){
            rushTime.setTemp2(request.getParameter("endrushTime"));
        }
    ...
    * 查询已被占用的时间段
    * */
    @RequestMapping("queryTDates")
    @ResponseBody
    public List<Object> queryTDates(){
        return marketingService.queryTDates();
    }
     ...
     * @return
     */
    @RequestMapping("queryrushtimeById")
    @ResponseBody
    public MarketingRushTime queryrushtimeById(Long tId){

        return marketingService.queryrushtimeById(tId);
    }
}
