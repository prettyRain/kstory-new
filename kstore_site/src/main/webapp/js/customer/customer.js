...
function wuliudanhao(){
	...
	if(no==''||no.length<0){
		...
		$(".yanzhengno").addClass("red");
		$(".yanzhengno").html("&nbsp;<img src='../images/red_cha.png'/>物流单号至少6位！");
...
function cancelOrder(url,orderId) {
    ...
    jQuery.ajax({
        type: 'post',
        url: 'ajaxCancelOrder.htm?orderId='+orderId,
        success:function(data) {
            if(data>0){
                cUrl = url;
                dia(2);
	...
	$.ajax({
		...
		success : function(data) {
			if (data >= 1) {
				...
				dia(2);
                //个人中心浏览记录点击爱心收藏成功后变红
                $(obj).find("img").prop("src","../images/loved.png");
			} else {
				$("#f_img").prop("src","../images/mod_war.png");
				$("#con_00").text("取消收藏成功！");
                $(obj).find("img").prop("src","../images/love.png");
				dia(2);
			}
		}
	});
...
function addcomplain() {
	...
	if (comCon == null || $.trim(comCon).length < 10 || $.trim(comCon).length > 250) {
		...
		$(".c_tip").removeClass("m_tip").html("不能包含特殊字符请您如实填写原因及商品情况，字数在10-250字内且不要包含特殊字符。");
