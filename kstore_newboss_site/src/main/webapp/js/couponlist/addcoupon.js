...
function selectChange(obj){
    ...
    if($(obj).val()==2){
        ...
        str+='<input name="fullPrice" class="form-control required number" maxlength="7">'
        str+='</div></div>';
        str+='<div class="form-group">';
        str+=' <label class="control-label col-sm-4"><span style="color: red;">*</span>减：</label>';
        str+='<div class="col-sm-1"></div>'
        str+='<div class="col-sm-5">'
        str+='<input name="reductionPrice" class="form-control required number" maxlength="7">'
        str+='</div></div>';
    }else{
        ...
        str+='<div class="col-sm-1"></div>'
        str+='<div class="col-sm-5">'
        str+='<input name="downPrice" class="form-control required number" maxlength="7">'
        str+='</div></div>';
    }
...
function addpro(obj){
    ...
    if(obj.checked==true){
        ...
        htm+='<td width="70"><input type="button" value="移除" onclick="removeTr(this);"></input></td>';
        htm+="</tr>";
...
function subFormOne(){
    var f = true;
    if($('#optionsRadios1').get(0).checked){
        $('#startTime').addClass("required");
        $('#endTime').addClass("required");
        $('#coupon_days').removeClass("required").val("");
    }
    if($('#optionsRadios2').get(0).checked){
        $('#coupon_days').addClass("required");
        $('#startTime').removeClass("required").val("");
        $('#endTime').removeClass("required").val("");
    }
...
function trueCate(){
    ...
    for(var i=0;i<nodes.length;i++){
        if(!nodes[i].isParent){
            ...
            htm+='<td width="108"><input type="button" value="移除" onclick="removeTr(this);"></input></td>';
            htm+="</tr>";
        }
    }
    $("#list tbody").html(htm);
