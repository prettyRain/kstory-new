			...
			if(data!=null){
				...
				if(data.goodsBrands != null && data.goodsBrands.length > 0){
					...
					for(var i=0;i<data.goodsBrands.length;i++){
						...
						brandHtml+="<li style='padding-left:31px;'>"
                                    + "<a class='aboutbrand' style='display:inline-block;' href='"+$("#basePathVal").val()+"/goods/searchProduct2.html?title=&brands="+brandvo.brand.brandName+"'>"
							         /*     brandvo.brand.brandName+"" +*/
							        +"<img src='"+brandvo.brand.brandLogo+"' style='width:136px;height:47px;'>"
							        +"</a></li>";
					}
					$("#relativeBrandGoods").html(brandHtml);
				}
                if($("#relativeCatGoods li").length>8){
                    for(var a=0;a<8;a++){
                        $("#relativeCatGoods li").eq(a).show();
                    }
                }else{
                    $("#relativeCatGoods li").show();
                }
			}
		});
	}
	...
	if(data.list!=null && data.list.length>0){
		for(var l=0;l<data.list.length;l++){
			...
			commentHtml+="<div class='comment_box clearfix'>" +
                ...
                var newstr = "";
                //规则：名字长度小于3位，显示第一个字后面加****；3位以上，显示首位字，中间****
                if(str.length<3){
                ...
                "<p class='uname'><a href='javascript:;'>"+newstr+"</a></p></div><!--/cmt_user--><div style='margin-left: 10px; margin-top:30px;' class='star_wp fl'>" +
        ...
        //    "<!-- <span class='cmt_date fr'>"+timeStamp2String(comment.publishTime)+"</span></div> --> <!--/cmt_tit-->"
            ...
            commentHtml+="<div class='clearfix'> <span class='buy_date'>"+timeStamp2String(comment.publishTime)+"</span> </div> <!--<div class='cmt_content mt10' style='display: none;'>-->";
