...
$(".J_AddRule").click(function(){
    ...
    logComId=$(this).attr('data-logComId');
                       ...
                       $("input[name='"+logCode+"_areas']").each(function(){
                           ...
                           for(var p=0;p<$arr.length;p++){
                               if($arr[p]==node.cityId){
                                   //treeObj.setChkDisabled(node, true, false, true);设置城市复选框可以重复选择
     ...
     if($('input:radio[name="freightPackageMail"]:checked').val()==0){
         $("#viewminprice").val(0);
         $("#viewminprice").attr('readonly',false);
         $("#viewmaxprice").val(0);
         $("#viewmaxprice").attr('readonly',false);
         $("#viewstart").val(1);
         ...
         $("#viewminprice").val(0);
         $("#viewminprice").attr('readonly',true);
         $("#viewmaxprice").val(0);
         $("#viewmaxprice").attr('readonly',true);
          $("#viewstart").val(1);
...
function toEditCity(obj,logCodes,logComIds){
    ...
    var minprice=trline.find('input[name="'+logCodes+'_min"]').val();
    var maxprice=trline.find('input[name="'+logCodes+'_max"]').val();
    ...
    var postageplus=trline.find('input[name="'+logCodes+'_postageplus"]').val();
                ...
                $.fn.zTree.init($("#treeDemo"), setting, zNodes);
                    ...
                    for(var k=0;k<treeObj.transformToArray(nodes).length;k++){
                        ...
                        if(areas!=null&&areas!=''){
                            ...
                            for(var p=0;p<$arr.length;p++){
                                if($arr[p]==node.cityId){
                                    //treeObj.setChkDisabled(node, false, false, true);设置城市复选框可以重复选择
                                    treeObj.setChkDisabled(node, false, true, true);
     ...
     if($('input:radio[name="freightPackageMail"]:checked').val()==0){
         $("#viewminprice").val(minprice);
         $("#viewminprice").attr('readonly',false);
         $("#viewmaxprice").val(maxprice);
         $("#viewmaxprice").attr('readonly',false);
         $("#viewstart").val(start);
         $("#viewstart").attr('readonly',false);
...
function doaddarea(){
    ...
    if($("#cityForm").valid()){
        ...
        htm += '  </div>';
        htm += ' <td>  <input type="hidden" value="'+$("#viewminprice").val()+'" name="'+logCode+'_min" id="'+logCode+'_min_n1" ><span>'+$("#viewminprice").val()+'</span></td>';
        htm += ' <td>  <input type="hidden" value="'+$("#viewmaxprice").val()+'" name="'+logCode+'_max" id="'+logCode+'_max_n1" ><span>'+$("#viewmaxprice").val()+'</span></td>';
        ...
        $('#editExpress').modal('hide');
            ...
            trline.find('input[name="'+logCode+'_min"]').val($("#viewminprice").val());
            trline.find('input[name="'+logCode+'_min"]').next().text($("#viewminprice").val());
            trline.find('input[name="'+logCode+'_max"]').val($("#viewmaxprice").val());
            trline.find('input[name="'+logCode+'_max"]').next().text($("#viewmaxprice").val());
        ...
        if(obj.checked==true){
            ...
            if($(obj).val()==0){
                ...
                $(".methodslist").each(function(){
                    $(this).html($(this).html().replace('重(g)','件'));
                });
            }
            //按重计价
            if($(obj).val()==1){
                ...
                $(".methodslist").each(function(){
                    $(this).html($(this).html().replace('件','重(g)'));
                });
            }
        }
    }
