...
public final class GoodsSiteController {
    ...
    /**
     * 根据分类查询商品列表  停用通过数据库按分类查询列表 2017/6/15 dll
     *
     * @param cid 分类ID
     */
    /*@RequestMapping("/list")//加入了三个属性 Long brandid,BigDecimal pricemin,BigDecimal pricemax
    public ModelAndView goodsList(Long cid, PageBean pb, GoodsSiteSearchBean searchBean, String[] params, HttpServletRequest request,String showStatus,Long brandid,BigDecimal pricemin,BigDecimal pricemax) {
        pb.setPageSize(20);
        ...
        try {
            ...
            {
                goodsCateVo = this.goodsCateService.queryCateByCatId(cid);
                //start xcy
                if (goodsCateVo != null)
                {
                    map.put("cid", cid);
                    map.put("cname", goodsCateVo.getCatName());
                }
                List<Object> goodsBrands = this.goodsCateService.queryBranndByCateId(cid);
                if (goodsBrands.size() > 0) {
                    map.put("brands", goodsBrands);
                }
                //end
                if (goodsCateVo == null) {
                    cid = Long.valueOf(-1L);
          ...
          Map paramMap=new HashMap();
            ...
            if(pricemax!=null&&pricemin!=null&&pricemax.compareTo(pricemin)==1){
            ...
            map.put("pb", this.goodsService.searchGoods(paramMap,pb, searchBean, cid, params, dId.toString()));
    ...
    public Map<String,Object> esearchProductNew(HttpServletRequest request, SearchPageBean<EsGoodsInfo> pageBean, String[] params, String keyWords, String sort, Long storeId,
                                       String showStock, String isThird,String cate,String brand,String priceMin,String priceMax,String specification) {
        ...
        if(StringUtils.isNotBlank(specification)){
            newKeyWords = keyWords==null?specification:keyWords+specification;
        }
        // 商品搜索结果
        Map<String, Object> resultMap = goodsElasticSearchServivice.searchGoods(pageBean, wares, indices, types, newKeyWords, brands, cats, params, sort, priceMinn, priceMaxx,
                (storeId != null && 0 == storeId) ? null : storeId, null, showStock, "1", isThird);
        if(resultMap.get("pb")!=null) {
            List<EsGoodsInfo> datas = ((SearchPageBean) resultMap.get("pb")).getData();
            if (datas != null && datas.size() > 0) {
                for (EsGoodsInfo esinfo : datas) {
                    /**
                     * 列表页显示两份价格
                     * 1-直降 15-折扣 10-团购 11-抢购 四个促销类型时需要对显示的价格进行计算
                     * 则需要关联上促销信息
                     */
                    esinfo.setTypeId(0L);
                    for (GoodsMarketingCodeUtil gms:esinfo.getCodeUtils()){
                        //放入优惠价格 或者 折扣
                        if(gms.getCodexId() == 1 || gms.getCodexId() == 10 || gms.getCodexId() == 11 || gms.getCodexId() == 15){
                            esinfo.setTypeId(gms.getCodexId());
                            esinfo.setGoodsInfoDiscount(productService.getGoodsInfoMarketing(esinfo.getGoodsInfoId(),gms.getCodexId(),gms.getMarketingId()));
                        }
                    }
    ...
    @RequestMapping("/list")
    public ModelAndView list(HttpServletRequest request, SearchPageBean<EsGoodsInfo> pageBean, String[] params, String keyWords, String sort, Long storeId,String showStatus
            ,String showStock, String isThird,String cate,String brand,String priceMin,String priceMax,Long cid,String specification) throws IOException {
        pageBean.setPageSize(20);
        // 索引
        String[] indices = new String[] { GoodsIndexConstant.PRODUCT_INDEX_NAME() };
        // 类型
        ...
        // 拼装返回结构
        ModelAndView model = new ModelAndView(GOODS_LIST);
        // 搜索条件
        ...
        model.addObject("disID", distinctId);
        //xcy start
        if(StringUtils.isNotBlank(specification)){
            //零售或者批发
            model.addObject("specification",specification);
        }
        if(StringUtils.isNotBlank(cid+"")){
            model.addObject("cate",cid);
        }
        if(StringUtils.isNotBlank(brand)){
        ...
        //用来查询分类和品牌的
        Map<String, Object> resultMapTwo = goodsElasticSearchServivice.searchGoods(pageBean, wares, indices, types, keyWords, null, cats, params, sort, null, null,
                (storeId != null && 0 == storeId) ? null : storeId, null, showStock, "1", isThird);
        List<Long> cateIds=new ArrayList<>();
        SearchPageBean spb=null;
        //品牌
        model.addObject("brands",resultMapTwo.get("brands"));
        if(resultMapTwo.get("pb")!=null){
            spb=(SearchPageBean)resultMapTwo.get("pb");

            List<EsGoodsInfo> datas=spb.getData();
            if(datas!=null&&datas.size()>0){
                for(EsGoodsInfo esinfo:datas){
                    cateIds.add(esinfo.getCatId());
                }
            }
        }
        if(cateIds!=null&&cateIds.size()>0){
            HashSet setCateIds=new HashSet(cateIds);
            Map cateIdsMap=new HashMap();
            cateIdsMap.put("list",setCateIds);
            List<GoodsCateVo> goodsCateVos=this.goodsCateService.queryCatesByCateIds(cateIdsMap);
            model.addObject("goodsCateVos",goodsCateVos);
        }
        //end
        // 页面数据
    ...
    public ModelAndView esearchProduct(HttpServletRequest request, SearchPageBean<EsGoodsInfo> pageBean, String[] params, String keyWords, String sort, Long storeId,String showStatus
                                       ,String showStock, String isThird,String cate,String brand,String priceMin,String priceMax,String specification) throws IOException {
        pageBean.setPageSize(20);
        // 索引
        String[] indices = new String[] { GoodsIndexConstant.PRODUCT_INDEX_NAME() };
        // 类型
        ...
        model.addObject("map", map);
        //用来查询分类和品牌的
        Map<String, Object> resultMapTwo = goodsElasticSearchServivice.searchGoods(pageBean, wares, indices, types, keyWords, null, null, params, sort, null, null,
                (storeId != null && 0 == storeId) ? null : storeId, null, showStock, "1", isThird);
        List<Long> cateIds=new ArrayList<>();
        SearchPageBean spb=null;
        //品牌
    ...
    @ResponseBody
    public Map<String,Object> allGoodsListAjax(PageBean pb, GoodsSiteSearchBean searchBean, String[] params, HttpServletRequest request,Long cid,Long brandid,BigDecimal pricemin,BigDecimal pricemax) {
        pb.setPageSize(20);
            Map<String, Object> map = new HashMap<String, Object>();
        GoodsTypeVo typeVo = null;
        List<GoodsListScreenVo> screenVo = null;
    ...
    public ModelAndView goodsDetail(Long productId,Long distinctId, String chAddress, HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        map.put("sessionOpedId", request.getSession().getAttribute("openid"));
        ...
        try {
            ...
            //零售批发规格
            Set set = new HashSet();
            List<GoodsProduct> goodsInfoList = detailBean.getProductVo().getGoodsInfoList();
            if(goodsInfoList!=null&&goodsInfoList.size()>0){
                for(GoodsProduct goodsProduct:goodsInfoList){
                    if(goodsProduct.getGoodsInfoName().indexOf("零售")!=-1){
                        set.add("零售");
                    }else if(goodsProduct.getGoodsInfoName().indexOf("批发")!=-1){
                        set.add("批发");
                    }
                }
            }
            map.put("proseps",set);
            // 判断商品是否下架
            if (null != detailBean&& "1".equals(detailBean.getProductVo().getGoodsInfoAdded())) {
                // 限购
                ...
                //根据商品查询是否是抢购商品
                Long longTime = 0l;
                MarketingRushUtil marketingRushUtil = panicBuyingService.queryProductByPidOrCidOrBid(detailBean.getProductVo().getGoodsInfoId(),detailBean.getCateVo().getCatId(),detailBean.getBrand().getBrandId(),request);
                if(marketingRushUtil!=null){
                    if(StringUtils.isNotBlank(marketingRushUtil.getRushTime()) && "1".equals(marketingRushUtil.getRushTime())){
                        longTime = System.currentTimeMillis()-marketingRushUtil.getMarketing().getMarketingBegin().getTime();
                        longTime = Math.abs(longTime);
                    }else if(StringUtils.isNotBlank(marketingRushUtil.getRushTime()) && "2".equals(marketingRushUtil.getRushTime())){
                        longTime = System.currentTimeMillis()-marketingRushUtil.getMarketing().getMarketingEnd().getTime();
                        longTime = Math.abs(longTime);
                    }
                }
                if(marketingRushUtil!=null){
                    map.put("beginToEnd",Math.abs(marketingRushUtil.getMarketing().getMarketingEnd().getTime()-marketingRushUtil.getMarketing().getMarketingBegin().getTime()));
                    map.put("rushDiscount",marketingRushUtil.getMarketing().getRushs().get(0).getRushDiscount());
                    map.put("rushTime",marketingRushUtil.getRushTime());
...
//        }
    ...
    public ModelAndView toActivity(){
        return new ModelAndView("/activity/ZQactive");
    }
    ...
    public ModelAndView toNewYearActivity(){
        return new ModelAndView("/activity/SKactive");
    }
    ...
    public ModelAndView toRegisterActivity(){
        return new ModelAndView("/activity/SCactive");
    }
...
//           }
    ...
    @ResponseBody
    public List<GoodsProduct> hotsales(){

        try{
            List<GoodsProduct> products = new ArrayList<GoodsProduct>();
            MobHomePage homePage = this.mobHomePageService.getCurrHomePageByMerchantId(-1L);
            Document document= DocumentHelper.parseText(homePage.getDoc());
            ...
            for(Element element:elementList){
                ...
                if("goodsmod".equals(element.getName())){
                    ...
                    for(Element element1:elementList1){
                        ...
                        goodsProduct.setGoodsInfoId(Long.valueOf(href_Id));
                        //货品名称
                        goodsProduct.setGoodsInfoName(element1.element("goods_a").attributeValue("name").toString());
                        //货品图片
                        String images = element1.element("goods_a").element("img").attributeValue("src");
                        goodsProduct.setGoodsInfoImgId(replaceImgUrl(images));
                        //货品价格
                        String price = element1.element("goods_a").attributeValue("price").toString();
                        System.out.println(price.substring(1,price.length()));
                        goodsProduct.setGoodsInfoPreferPrice(new BigDecimal(price.substring(1,price.length())));
