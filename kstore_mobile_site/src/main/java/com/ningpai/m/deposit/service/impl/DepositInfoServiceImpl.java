...
//        CustomerAllInfo customerInfo = customerMapper.selectByPrimaryKey(customerId);
        ...
        String securityPassword = SecurityUtil.getStoreLogpwd(customerId.toString(), password, salt);
        DepositInfo depositInfo = new DepositInfo();
        depositInfo.setPayPassword(securityPassword);
        depositInfo.setCustomerId(customerId);
        depositInfo.setUserSaltVal(salt);
        //更新支付密码
...
//            LOGGER.info(e1);
    ...
    public JSONObject checkPayPassword(String payPassword, Long customerId, String type,BigDecimal price) {
        ...
        DepositInfo depositInfo =  depositInfoMapper.selectDepositByCustId(customerId);
        //先判断是否绑定礼品卡
        if(null == depositInfo.getGiftcardNo() || "".equals(depositInfo.getGiftcardNo())){
            resultJson.put(DepositInfoCons.RETURN_CODE, "nocard");
        }else {
...
//                CustomerAllInfo customerAllInfo = customerMapper.selectByPrimaryKey(customerId);
                String encrypPassword = SecurityUtil.getStoreLogpwd(customerId.toString(), payPassword, depositInfo.getUserSaltVal());
                if (!StringUtils.equals(encrypPassword, password)){
                    ...
                    if (errorCount>= DepositInfoCons.ERROR_THRESHOLD){
                        ...
                        if (leftCount > 0 ){
                            resultJson.put(DepositInfoCons.RETURN_MSG, DepositInfoCons.NOT_REACHE_ERROR_THRESHOLD_TIPS + leftCount +"次尝试机会");
                        }
                    }
                    resultJson.put(DepositInfoCons.FAIL_CODE, DepositInfoCons.FROZEN_FAIL_CODE);
                    //更新预存款错误信息
...
//        DepositInfo depositInfo =  depositInfoMapper.selectDepositByCustId(customerId);
        ...
        if(null == depositInfo.getGiftcardNo() || "".equals(depositInfo.getGiftcardNo())){
            ...
            String activeFlag = "0";
            try {
                activeFlag = giftcardService.getGiftcard(giftcardVo).getActiveFlag();
            }catch (Exception e){
                e.printStackTrace();
            }
            resultJson.put("activeFlag",activeFlag);
            resultJson.put("order_price", orderPrice);
            if (depositInfo.getPasswordErrorCount() != null && depositInfo.getPasswordErrorCount() >= DepositInfoCons.ERROR_THRESHOLD) {
                ...
                //预存款余额不足
                if ("0".equals(type)) {
                    resultJson.put(DepositInfoCons.RETURN_MSG, depositInfo.getPreDeposit());
                } else if ("1".equals(type)) {
     ...
     */
    @Override
    public void updateDepositInfo(DepositInfo depositInfo, Order order, Long customerId){
        ...
        if(order.getGiftcardPrice() != null && order.getGiftcardPrice().compareTo(new BigDecimal(0)) > 0){
            tradeInfo.setOrderPrice(order.getGiftcardPrice());
            depositInfo.setPreDeposit(depositInfo.getPreDeposit().subtract(order.getGiftcardPrice()));
        }else{
