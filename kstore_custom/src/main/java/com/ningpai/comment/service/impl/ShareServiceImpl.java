...
public class ShareServiceImpl implements ShareService {
    ...
    public List<Object> getTopShare(int size) {
    	...
    	lists =  shareMapper.queryTopShare(size);
     ...
     *
     * @param third 第三方id
     * @return 第三方 返回true  否则返回false
     */
    private boolean isDisplay(Long third) {
        if (null == third || third.equals(0L)) {
            return false;
        } else {
            return true;
        }
    ...
    public int saveShare(Long goodsInfoId,Share share, Long customerId, Long orderGoodsId, String imageNames) {
        ...
        share.setThirdId(shareMapper.queryThirdIdByGoodsInfoId(goodsInfoId));

        share.setIsDisplay("0");
        ...
        if(StringUtils.isNotBlank(imageNames)) {
            ...
            for (String imageName : imageArray) {
                ...
                try {
                    //通过阿里检测图片是否合格
                    String result = AliSyncImageFilter.checkImage(imageName);
                    ...
                    if (("pass").equals(result)) {
                        ...
                        shareImg.setShareId(share.getShareId());
                        shareMapper.saveShareImg(shareImg);
                    } else if (("review").equals(result)||result==null) {
                        //需要人工审核的图片 此处不做处理
                        flag = false;
                        ShareImg shareImg = new ShareImg();
                        shareImg.setImageName(imageName);
                        ...
                        shareMapper.saveShareImg(shareImg);
                                             ...
                                             //违规图片  删除图片
                        RemoveServerImage.execute(new String[]{imageName});
    ...
    public int saveShareReply(ShareReply reply) {
        ...
        if (isDisplay(shareMapper.queryThirdIdByShareId(reply.getShareId()))) {
    ...
    public int deleteShare(String[] parameterValues) {
        ...
        redisAdapter.delete(CacheKeyConstant.TOPSHARE_KEY);
        return count;
    }
