...
public class AfterLoginQQController extends HttpServlet {
   ...
   // @RequestMapping("afterloginqq")
    public ModelAndView fterLoginQQ(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    public ModelAndView afterLoginQQ(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (auth != null) {
            ...
            ThreePart tp = threePartService.selectThreePart(openid.toString().trim());
            if(tp!=null){
                com.ningpai.m.customer.bean.Customer cus = this.customerService.selectCustomermByPrimaryKey(tp.getThreePartMemberId());
                 if(cus==null){
                     this.threePartService.deleteThreepart(openid);
                     //如果没有与用户绑定就跳到登录页
                     return new ModelAndView("login/third_party").addObject("openId",openid).addObject("accessTokenn",accessToken).addObject("url",preurl).addObject("partType","1");
                 }
                //修改登录时间  登录ip 赠送积分
