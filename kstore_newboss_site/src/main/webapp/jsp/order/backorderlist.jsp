...
<jsp:include page="../page/header.jsp"></jsp:include>
                ...
                <form role="form" class="form-inline">
                  <div class="form-group">
                   <%-- <div class="input-group">--%>
                      ...
                      <input type="text" class="form-control" id="backOrderCode" value="${pageBean.objectBean.backOrderCode}">--%>
                        <select class="form-control" name="condition" id="conditionId">
                          <option value="1" <c:if test="${condition == '1'}">selected="selected" </c:if>>订单号</option>
                          <option value="2" <c:if test="${condition == '2'}">selected="selected" </c:if>>退单单号</option>
                        </select>
                  </div>
                  <div class="form-group">

                    <%--<input type="text" class="form-control" id="backOrderCode" value="${pageBean.objectBean.backOrderCode}">--%>
                    <input type="text" class="form-control" id="searchText" value="${searchText}">
                  </div>
                  <div class="form-group">
                    <div class="input-group date form_datetime w300">
                      <span class="input-group-addon">开始时间</span>
                      <input class="form-control" type="text" name="startTimeName" id="startTime" value="${startTime}" readonly>
                  ...
                  <div class="form-group">
                    <div class="input-group date form_datetime w300">
                      ...
                      <input class="form-control" type="text" name="endTimeName" value="${endTime}" id="endTime" readonly>
                      <span class="input-group-addon"><i class="glyphicon glyphicon-remove"></i></span>
                      <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                    </div>
                  </div>
		         ...
		         <div class="form-group">
                    <div class="input-group">
                      ...
                      <select class="form-control" id="up_backCheck" value="${pageBean.objectBean.backCheck}">
						...
						<option value="3" <c:if test="${pageBean.objectBean.backCheck eq '3'}">selected ="selected"</c:if>><%--待退款--%>待商家收货</option>
                    ...
                    <div class="btns">
                      ...
                      --%>
                          ...
                          <c:if test="${order.isBack==1}">
                              <a href="javascript:;" onclick="backdetail('${order.backOrderId}','${order.order.orderId}','${businessId}');">退货审核</a>
                              <b> - </b>
                          </c:if>
                          <c:if test="${order.isBack==2}">
                              <a href="javascript:;" onclick="backprice('${order.backOrderId}','${order.order.orderId}','${businessId}');">退款审核</a>
                              <b> - </b>
                          </c:if>
                        ...
                        <b>-</b>
                        <a href="javascript:;" onclick="backOrderRemark('${order.backOrderId }','${order.backOrderCode}')">查看备注</a>
                    </div>
                  </td>
                </tr>
                <tr>
                  <td>
                    <div class="mini_goods" style="text-align:left;">
                     ...
                     <c:forEach items="${order.orderGoodslistVo}" var="ordergoods" varStatus="i">
                         <a href="${bsetaddress }/item/${ordergoods.goodsInfoId}.html" target="_blank"><img alt="" title="${ordergoods.goodsInfoName}" src="${ordergoods.goodsInfoImgId}" height="50" width="50"></a>
                     </c:forEach>
                    </div>
                  </td>
                    <td align="center">
                  ...
                  <td align="center">
					...
					<c:if test="${order.backCheck eq 1}"><a href="javascript:;"><span class="label label-success">待商家收货</span></a></c:if>
					<c:if test="${order.backCheck eq 2}"><a href="javascript:;"><span class="label label-danger">拒绝退货</span></a></c:if>
					<c:if test="${order.backCheck eq 3}"><a href="javascript:;"><span class="label label-success"><%--待退款--%>待商家收货</span></a></c:if>
                  ...
                  <td align="center"><c:if test="${order.businessId!='0'}">${order.storeInfo.storeName}</c:if><c:if test="${order.businessId =='0'}">BOSS</c:if></td>
                </tr>
                </c:forEach>
                </tbody>
              </table>
   ...
   <div class="allfixed"></div>
   <div class="backremark">
     <input type="hidden" name="backOrderId" class="backOrderId" value=""/>
     <input type="hidden" name="backOrderCode" class="backOrderCode" value=""/>
     <div class="contentdiv">
       <div class="headdiv"><h3>添加备注内容：</h3><span onclick="closespan('.backremark')">×</span></div>
       <div class="text" style="text-align:center">
         <textarea name="remarkContent" id="remarkContentId" cols="30" rows="10" style="width:260px;height:100px;"></textarea>
       </div>
       <botton class="btnn" onclick="btnclick()">确定</botton>
     </div>

   </div>
   
   <%--退单备注列表--%>
   <div class="modal fade" id="backorderremarks"  role="dialog">
     ...
     <div class="modal-dialog modal-lg">
       <div class="modal-content">
         <div class="modal-header">
           ...
           <h4 class="modal-title">退单备注</h4> <input type="button" value="添加退单备注" id="addremark" onclick="addBackremark()"/>
         </div>
         <div class="modal-body">
           <div class="common_info common_tabs mt20">
             <ul class="nav nav-tabs" role="tablist">
               <li role="presentation" class="active"><a  href="#tab3" aria-controls="tab3" role="tab" data-toggle="tab">退单备注列表</a></li>
             </ul>
   ...
   <div class="modal fade" id="dialog-Back-price"  role="dialog">
    ...
    <form role="form" method="post" action=<c:if test="${ businessId ==0}">"backorderlist.htm"</c:if><c:if test="${ businessId !=0}">"backorderlististhird.htm"</c:if> id="ordercxform">
        <%-- <input type="hidden" id="backOrderCodestr" name="backOrderCode" value="${pageBean.objectBean.backOrderCode }" />--%>
         <input type="hidden" name="searchText" id="searchTextstr" value="${searchText}">
         ...
         <input type="hidden" name="condition" id="conditionstr" value="${condition}"/>
         <input type="hidden" name="startTime" id="startTimestr" value="${startTime}"/>
         <input type="hidden" name="endTime"   id="endTimestr" value="${endTime}"/>
