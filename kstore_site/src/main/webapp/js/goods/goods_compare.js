		...
		success:function(data) {
			...
			for(var i=0;i<data.length;i++) {
				...
				if(data[i].productVo.imageList.length>0) {
					var src11 = data[i].productVo.imageList[0].imageThumName;
                    src11 = src11.substring(0,src11.indexOf("!")) + "!160" + src11.substring(src11.lastIndexOf("."));
					html += '<dt><a href="../item/'+data[i].productVo.goodsInfoId+'.html" target="_blank"><img width="70" height="70" style="padding: 40px 0 40px 20px;" alt="'+data[i].productVo.productName+'" src="'+src11+'" /></a></dt>';
				} else {
					html += '<dt>'+(i+1)+'</dt>';
				}
				html += '<dd><p class="ct-item-name"><a href="../item/'+data[i].productVo.goodsInfoId+'.html" target="_blank">'+productName+'</a></p>'
						+'<span class="ct-item-price">￥'+accAdd(data[i].productVo.goodsInfoPreferPrice,0)+'</span>'
			...
			for(var i=data.length+1;i<=4;i++) {
				html += '<dl class="empty_item clearfix fl"><dt>'+i+'</dt>'
						+'<dd style="margin-top: 68px;">你还可以继续添加</dd></dl>';
			}
			html += '<div class="ct_operation fl tc">';
			if(goodsIds.length<2) {
				html += '<a class="contrast_btn dis_ct" href="javascript:;" class="compare_btn">商品对比</a>';
			} else {
				html += '<a class="contrast_btn" href="javascript:;" onclick="compareGoods();" class="compare_btn">商品对比</a>';
			}
