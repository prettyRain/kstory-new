...
import com.ningpai.goods.service.SearchAsyncService;
...
public class GoodsImageController {
    ...
    public ModelAndView updateProductImage(Long productId, Long goodsId,
            ...
            Long goodsImgId, PageBean pb,String[] imgContent) {

        //xcy start 排序  默认的图片排在了第一位  所以只排默认以外的图片
        for(int i=1;i<imgContent.length;i++){
            Map map=new HashMap();
            String[] arr=imgContent[i].split("-");
            map.put("imgId",Integer.parseInt(arr[0]));
            map.put("imgSort",Integer.parseInt(arr[1]));
            this.goodsImageService.setImageSort(map);
        }
        //xcy end
        // 设置默认图片
        ...
        goods.setGoodsImg(defaultImage);
