...
function call_update_goods() {
    ...
    $(".update_goodsSpecsification_desc").val(editor4.html());
    $(".update_goodsWarp_desc").val(editor5.html());
    $(".update_goodsBrand_desc").val(editor6.html());
...
function queryGoodsByParam(pageNo) {
    ...
    /*根据选中分类加载相关商品*/
    //goodsCateId=" + $('#parentId3').val() + "   xcy 可以添加所有商品进行关联
...
function allunchecked(obj,name){
    if(obj.checked){
        $(obj).parents(".table").find($("input[name='"+name+"']")).each(function(){
            this.checked=true;
        });
    }else{
        $(obj).parents(".table").find($("input[name='"+name+"']")).each(function(){
