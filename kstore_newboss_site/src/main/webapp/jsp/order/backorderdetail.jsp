...
<head>
    ...
    <style>
        ...
        .sf-item input[type="radio"] {margin-right:5px;margin-left:3px;}
...
<div class="mb20 container-fluid">
        ...
        <tbody>
            <tr>
                <td>
                    ...
                    <c:if test="${backorder.backReason eq '2'}">收货信息有误</c:if>
                    <c:if test="${backorder.backReason eq '3'}">商品未按时发货</c:if>
                    <c:if test="${backorder.backReason eq '4'}">其他</c:if>
                    <c:if test="${backorder.backReason eq '5'}">商品质量问题</c:if>
                    <c:if test="${backorder.backReason eq '6'}">收到商品与描述不符</c:if>
                </td>
                <td>
                ...
                <td>${backorder.backPrice}<input type="hidden" id="backPrice" value="${backorder.backPrice}">
                <c:if test="order.giftcardPrice != null && order.giftcardPrice > 0">
                    （礼品卡账户支付 &yen;${order.giftcardPrice}）
                </c:if>
                </td>
                <c:if test="${fn:length(backorder.backRemark) gt '3'}">
                    <td  title="${backorder.backRemark}">${fn:substring(backorder.backRemark,0 ,3)}...</td>
                </c:if>
                <c:if test="${fn:length(backorder.backRemark) le '3'}">
                ...
                <td>
                    <c:if test="${backorder.backWay eq '0'}"><%--快递--%>上门取件</c:if>
    ...
    <table class="table table-striped table-hover table-bordered">
        ...
        <tbody>
        <c:forEach items="${backorder.backOrderGoodsList }" var="backGoods">
            <tr>
                <td>
                    <li><img src="${backGoods.goodsImg}" alt="${backGoods.goodsInfoName}" style="width:30px;height:30px;"/></li>
                </td>
                <td><c:if test="${backGoods.isPresent!= null && backGoods.isPresent=='1'}" ><span style="color: white;background-color: lightgrey;">&nbsp;赠品&nbsp;</span>&nbsp;</c:if>${backGoods.goodsInfoName}</td>
                <td>${backGoods.goodsInfoItemNo}</td>
                <td>${backGoods.goodsInfoPrice}</td>
                <%--随意找的货品字段装的数据 只用于前台显示--%>
                <td>${backGoods.goodsInfoNum}</td>
                <%--随意找的货品字段装的数据 只用于前台显示--%>
                <td>
                    <c:if test="${backGoods.goodsBackPrice != null}">
                        &yen;${backGoods.goodsBackPrice}
                    </c:if>
                    <c:if test="${backGoods.goodsBackPrice == null}">
                        &yen;${backGoods.goodsInfoSumPrice}
                    </c:if>
                </td>
            </tr>
        </c:forEach>
                ...
                <c:forEach items="${backOrderLogs}" var="backOrderLog">
                    <c:if test="${backOrderLog.backLogStatus eq '7'}">
                        <p>
                            ...
                            <i></i>
                            <span class="track-cont">退款${backorder.actualBackPrice}成功 (操作：平台)<c:if test="${backOrderLog.backRemark != null}">留言：${backOrderLog.backRemark}</c:if></span>
                        </p>
                    </c:if>
        ...
        <div class="sf-item">
            <c:if test="${backorder.isBack eq '1'}">
                <c:if test="${backorder.backCheck eq '0'}">
                    <label><input type="radio" name="backLogStatus" class="backLogStatus" value="2">同意退货</label>
                    <label><input type="radio" name="backLogStatus" class="backLogStatus" value="3">拒绝退货</label>
                    <span class="remark_content" style="color:red;"></span>
                    <div class="sf-item">
                        <span>给客户留言：</span>
                        <textarea class="customerRemark" name="backRemark" maxlength="255"></textarea>
                    </div>
                    <input type="hidden" id="backOrderId" name="backOrderId" value="${backorder.backOrderId}">
                ...
                <%--<c:if test="${backorder.backCheck eq '1'}"> xcy--%>
                <c:if test="${backorder.backCheck eq '3'}">
                    ...
                    <span class="remark_content" style="color:red;"></span>
                    <span id="pointtip"   style="display: none;color:red">注意：用户使用了此订单的赠送积分，该用户返还积分不足，请注意退款金额</span>
                    <div class="sf-tit"><b><c:if test="${order.payId eq '5'}">注：该订单使用了预存款进行支付，退费将直接返回用户账户，无需财务人员额外操作</c:if></b></div>
                    <div class="sf-item" id="orderProceStatus">
                    <label><input type="radio" name="backLogStatus" checked="checked" class="backLogStatus" value="8">退款</label>
                    <span>退款金额：<input type="text" class="checkBackPrice isNoInteger" name="backPrice" style="width:40px;"
                                      onkeyup="if(isNaN(value))execCommand('undo')"
                                      onafterpaste="if(isNaN(value))execCommand('undo')"
                                      onkeydown="if(event.keyCode==13)return false;"
                            ><label id="backMoneytip" class="error" style="display: none">输入金额必须小于等于退款金额</label></span>
                    ...
                    <input type="hidden" id="backOrderId" name="backOrderId" value="${backorder.backOrderId}">
                    <input id="myorderId" type="hidden" name="orderId" value="${orderId}">
                    <div class="sf-btns">
                        <input type="button" value="保存" onclick="saveorder();">
                        <input type="button" value="取消" onclick="cancle();">
                    </div>
                ...
                <c:if test="${backorder.backCheck eq '6'}">
                    <label><input type="radio" name="backLogStatus" onchange="showPrice(0)" class="backLogStatus" value="8">同意退款</label>
                    <label><input type="radio" name="backLogStatus" onchange="showPrice(1)" class="backLogStatus" value="9">拒绝退款</label>
                    <span class="remark_content" style="color:red;"></span>
                    <span id="pointtip"   style="display: none;color:red">注意：用户使用了此订单的赠送积分，该用户返还积分不足，请注意退款金额</span>
                    <div class="sf-tit"><b><c:if test="${order.payId eq '5'}">注：该订单使用了预存款进行支付，退费将直接返回用户账户，无需财务人员额外操作</c:if></b></div>
                    <div class="sf-item" id="orderProceStatus">
                ...
                <%--<c:if test="${backorder.backCheck eq '3'}">---xcy
...
<script>
    $(function(){
         ...
         * 退单确认收货 退款金额验证
         */
        $(".checkBackPrice").blur(function(){
            var backorderid = $("#backOrderId").val();
            var money = $(this).val();
            var backPrice = $("#backPrice").val();
            if(money !=""){
                if(parseFloat(money) > parseFloat(backPrice)){
