...
public class CouponController {
    ...
    public ModelAndView doAddCouponNew(HttpServletRequest request, @Valid Coupon coupon, String couponSTime, String couponETime, Long[] lelvelId, String status, String csrFToken,
        ...
        if(StringUtils.isBlank(coupon.getCouponGetType())){
            coupon.setCouponGetType("1");
        }
        //如果是起止时间
     ...
     * @param response
     */
    @RequestMapping("exportCouponCodeNo")
    @ResponseBody
    public void exportCouponCodeNo(HttpServletResponse response, Long couponId) {
        LOGGER.info("导出优惠券券码");
        couponNoService.exportCouponCodeNo(response,couponId);
    }
