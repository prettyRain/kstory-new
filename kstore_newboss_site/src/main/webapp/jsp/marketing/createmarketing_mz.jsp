                                        ...
                                        <div class="col-sm-19">
                                            <table class="table table-striped table-hover table-bordered" style="margin-bottom:0px;">
                                                ...
                                                <tr>
                                                    ...
                                                    <th width="200">货品规格</th>
                                                    <th width="100">货品编号</th>
                                                    <th width="250">货品名称</th>
...
<!-- Modal -->
<%--modal--%>
<div class="modal fade" id="addCoupont"  role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <div class="modal-body clearfix">
                <div class="container-fluid">
                    <select class="col-sm-9 col-md-offset-1 beforeCoupont" style="height:200px;" multiple>
                        <c:forEach var="item" items="${couponlist}">
                            <option style="cursor:pointer;" data-couponId="${item.couponId}" data-couponName="${item.couponName}" data-couponRemark="${item.couponRemark}" data-couponStartTime="${item.couponStartTime}" data-couponEndTime="${item.couponEndTime}" > ${item.couponName}</option>
                        </c:forEach>
                    </select>
                    <div class="col-sm-4" id="bacBtn">

                            <botton class="btn btn-info btn-block btn-sm" style="margin:0 auto;width:47px;margin-bottom:3px;">&gt;</botton>
                            <botton class="btn btn-info btn-block btn-sm" style="margin:0 auto;width:47px;margin-bottom:3px;">&lt;</botton>
                            <botton class="btn btn-info btn-block btn-sm" style="margin:0 auto;width:47px;margin-bottom:3px;">&gt;&gt;</botton>
                            <botton class="btn btn-info btn-block btn-sm" style="margin:0 auto;width:47px;margin-bottom:3px;">&lt;&lt;</botton>

                    </div>
                    <select class="col-sm-9 afterCoupont"  style="height:200px;" multiple ></select>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-coupon" data-dismiss="modal">确定</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function chooseProductForCoupont(){
         $("#addCoupont").modal('show');
    }
...
//                }
    ...
    function addcouG(){
        ...
        if(!!$coupon){
            ...
            var htm = "<tr id='coupon_tr_"+couponId+"'>";
            ...
            htm+='<td width="200">备注:'+couponRemark+'</td>';
            if(timeType=='0'){
                htm+='<td width="100">起止时间</td>';
                htm+='<td  width="250">'+new Date(couponStartTime).toLocaleDateString()+'-'+new Date(couponEndTime).toLocaleDateString()+'</td>';
            }else if(timeType == '1'){
                htm+='<td width="100">固定时段</td>';
                htm+='<td  width="250">'+couponDays+'天</td>';
            }
    ...
    function addproG(obj,count,number){
        ...
        if(obj.checked==true){
            ...
            htm+='<td width="200">'+goodsTag+'</td>';
            htm+='<td width="100">'+goodsNo+'</td>';
            htm+='<td  width="250">'+goodsName+'</td>';
    ...
    function subFormOne(obj){
        ...
        if($("#addFormOne").valid()&&f){
            ...
            if(codexType=='6'){
                $(".fullcount").find("select").attr("name","presentMode");
...
//        }
    ...
    (function(){
        $('#bacBtn').on("click","botton",function(){
            var _this = this;
            var $this = $(this);
             var index = $this.index();
             if(index == 0){
                 $('.afterCoupont').append($('.beforeCoupont>option:selected'));
             }else if(index == 1){
                 $('.beforeCoupont').append($('.afterCoupont>option:selected'))
             }else if(index == 2){
                 $('.afterCoupont').append($('.beforeCoupont>option'));
             }else if(index == 3){
                 $('.beforeCoupont').append($('.afterCoupont>option'));
             }
        })
        /*
         <tr>
         <th width="100">优惠券ID</th>
         <th width="100">优惠券名称</th>
         <th width="150">开始时间</th>
         <th width="300">结束时间</th>
         <th width="100">优惠券备注</th>
         </tr>
        * */
        $('.btn-coupon').on('click',function(){
              $('.couponOne table tbody').html("");
              $('.afterCoupont>option').each(function(index,item){

                   var couponEndTime = $(item).attr("data-couponEndTime");
                   var couponId = $(item).attr("data-couponId");
                   var couponName = $(item).attr("data-couponName");
                   var couponRemark = $(item).attr("data-couponRemark");
                   var couponStartTime = $(item).attr("data-couponStartTime");
                   var str = '<tr>'
                               +'<td width="100">'+couponId+'</td>'
                               +'<td width="100">'+couponName+'</td>'
                               +'<td width="200">'+couponStartTime+'</td>'
                               +'<td width="200">'+couponEndTime+'</td>'
                               +'<td width="150">'+couponRemark+'</td>'
                               +'</tr>';
              $('.couponOne table tbody').append(str);
              })

        })
        $('option:odd').addClass('bg-danger');
        $('option:even').addClass('bg-info');
    })();
</script>
</body>
</html>
