...
public class GoodsProductServiceImpl implements GoodsProductService {
    ...
    public GoodsProductVo findProductById(Long productId, Long districtId) {
        ...
        // 如果不是第三方商品就去查分仓库存
        if ("0".equals(productVo.getIsThird()) && null != districtId && districtId > 0) {
                /* 根据选择的地区查询库存及价格信息 */
    ...
    public GoodsDetailBean queryDetailBeanByProductId(Long productId, Long districtId) {
        ...
        GoodsProductVo productVo = findProductById(productId,districtId);
        ...
        detailBean.setCateVo(this.cateService.queryCateAndParCateByCatId(productVo.getGoods().getCatId()));
        // 查询商品关联的标签
        detailBean.setTags(goodsReleTagMapper.queryAllByGoodsId(productVo.getGoodsId()));
        // 查询品牌
        detailBean.setBrand(goodsBrandService.queryBrandById(productVo.getGoods().getBrandId()));
        // 查询商品关联的扩展属性
        detailBean.setExpandPrams(expandParamMapper.queryAllByGoodsId(productVo.getGoodsId()));
        // 查询商品关联的详细参数
        detailBean.setParam(goodsReleParamMapper.queryAllByGoodsId(productVo.getGoodsId()));
        // 查询商品关联的货品
        List<GoodsRelatedGoodsVo> relaProducts = this.goodsRelatedGoodsMapper.queryAllByGoodsId(productVo.getGoodsId());

        // 如果关联的不为空
        if (null != relaProducts && !relaProducts.isEmpty()) {
            List<GoodsProduct> relaProduct = new ArrayList<>();
        ...
        // 查询组合集合
        detailBean.setGroupVos(this.goodsPub.getGoodsGroupService().queryGroupVoListWithOutProductId(productVo.getGoodsInfoId()));
    ...
    public GoodsDetailBean queryDetailByProductId(Long productId, Long distinctId) {
        ...
        if (null != productVo) {
            ...
            detailBean.setSellType(goods.getSellType());
            detailBean.setUnitPrice(goods.getUnitPrice());
            ...
            if (null != relaProducts && !relaProducts.isEmpty()) {
                ...
                for (int i = 0; i < relaProducts.size(); i++) {
                    relaProduct.add(goodsProductMapper.queryFirstProductByGoodsId(relaProducts.get(i).getReleatedGoods().get(0).getGoodsId()));
                }
                detailBean.setReleProductList(relaProduct);
            }
            // 查询组合集合
            detailBean.setGroupVos(goodsPub.getGoodsGroupService().queryGroupVoListWithOutProductId(productVo.getGoodsInfoId()));
            // 如果不是第三方商品就去查分仓库存
            if (productVo.getIsThird() != null && productVo.getIsThird().equals("0") && null != distinctId && distinctId > 0) {
                /* 根据选择的地区查询库存及价格信息 */
     ...
     * 查询货品信息，不关联查询其他信息
     * (non-Javadoc)
     * @see com.ningpai.m.goods.service.GoodsProductService#queryByGoodsInfoDetail(java.lang.Long)
     */
    @Override
     ...
     * 查询货品，按销量降序
     * (non-Javadoc)
     * @see com.ningpai.m.goods.service.GoodsProductService#queryInfosOrderBySales(java.util.Map)
     */
    @Override
    ...
    @Override
    public List<GoodsProduct> queryHotSales(Long pageSize) {
        return this.goodsProductMapper.queryHotSales(pageSize);
    }

    @Override
    public String getGoodsInfoMarketing(Long goodsInfoId,Long codeType,Long marketId) {
        if(codeType == 1){
            //直降
            return priceOffMarketingMapper.selectPriceOffMarketingByMarketingId(marketId).getOffValue().toString();
        }else if(codeType == 10){
            //团购
            return grouponMapper.selectByMarketId(marketId).getGrouponDiscount().toString();
        }else if(codeType == 11){
            //抢购
            return rushMapper.selectByMarketId(marketId).get(0).getRushDiscount().toString();
        }else if(codeType == 15){
            //折扣
            Map<String, Object> param = new HashMap<String, Object>();
            param.put("goodsId", goodsInfoId);
            param.put("marketingId", marketId);
            return preDiscountMarketingMapper.selectByMarketId(param).getDiscountInfo().toString();
        }
        return "";
    }
}
