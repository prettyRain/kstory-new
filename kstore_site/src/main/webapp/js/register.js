...
$(function () {
    ...
    $("#giftcardPwdInput").focus(function () {
        $(".giftcardPwd_info").parents(".reg_form_item").removeClass("error");
        $(".giftcardPwd_info").hide();
    });
    ...
    $(".passwordconf2").focus(function () {
        $(".pass").parents(".reg_form_item").removeClass("error");
        $(".pass").hide();
    });
    $(".repasswordconf2").focus(function () {
        $(".repass").parents(".reg_form_item").removeClass("error");
        $(".repass").hide();
    });
    ...
    $(".passwordconf2").blur(function () {
        psdchk2($(this));
    });

    $(".repasswordconf2").blur(function () {
        psdconf2($(this));
    });
    ...
    /*$("#mobileInput").blur(function () {
     ...
     $("#giftcardPwdInput").blur(function () {
     var giftcardPwd = $("#giftcardPwdInput").val();
     if (giftcardPwd == '' || giftcardPwd == null) {
     $(".giftcardPwd_info").show();
     $(".giftcardPwd_info").html("请输入您的礼品卡密码");
     $(".giftcardPwd_info").parents(".reg_form_item").addClass("error");
     }
     });*/
...
function submitFirst() {
    if (!$(".sub_btn").hasClass("nextGrey")) {
        ...
        if (mobile == '' || mobile == null) {
            ...
            var flag = true;
            $.ajax({url:"checkMobileExist.htm?mobile="+mobile,async:false,success:function(data){
                if(data==1){
                    $(".mobile_info").show();
                    $(".mobile_info").html("手机号已被注册");
                    $(".mobile_info").parents(".reg_form_item").addClass("error");
                    flag = false;
                }
            }
            ...
            $.ajax({
                url:"checkpatchca.htm?enterValue="+checkCode,
                async:false,
                success:function(data){
                    if(data==0){
                        $(".code_info").html("验证码错误");
                        $(".code_info").parents(".reg_form_item").addClass("error");
                        $(".code_info").show();
                        /*如果失败则更新验证码*/
                        $("#checkCodeImg").click();
        ...
        $.ajax({
            url: "/checkpatchca.htm?enterValue=" + checkCode, async: false, success: function (data) {
                if (data == 0) {
                    ...
                    $.ajax({url:"checkMobileCode.htm?checkCode="+code,async:false,success:function(data){
                        if(data==1){
                            $(".code_send").hide();
                            $(".code_send").hide();
                            $(".code_info").parents(".reg_form_item").addClass("error");
                            $(".code_info").show().html("验证码错误");
                            return false;
                        }
                        else if(data==2){
                            $(".code_send").hide();
...
function register() {
    if ($(".sub_btn").hasClass("loading")) return;
    if (!$(".sub_btn").hasClass("nextGrey")) {
        var valid_flag = true;
        var id = rsgister();
        ...
        $(".sub_btn").addClass('loading').html("");
        valid_flag = psdconf($(".repasswordconf")) && valid_flag;
        if (valid_flag) {
            if (isNaN(id)) {
                //$("#regform").submit();
                $.ajax({
                    url:"/beforeSuborder.htm",
                    type:"post",
                    success:function(data){
                        $("#regform").submit();
                    }
            ...
            $(".sub_btn").removeClass('loading').html("下一步");
            return false;
        }
    }
...
function psdchk(obj) {
    ...
    if (regS.test(x)) {
        ...
        obj.parents(".reg_form_item").addClass("error");
        $(".pass").show().html('密码为6-16位字母加数字的组合，不得含特殊字符');
        return false;
    } else if (checkPass(x) == 0) {
        obj.parents(".reg_form_item").addClass("error");
        $(".pass").show().html('密码为6-16位字母加数字的组合，不得含特殊字符');
...
function psdconf(obj) {
    ...
    if (regS.test(x)) {
        ...
        obj.parents(".reg_form_item").addClass("error");
        $(".repass").show().html('密码为6-16位字母加数字的组合，不得含特殊字符');
        return false;
...
function psdchk2(obj) {
    ...
    if (regS.test(x)) {
        ...
        obj.parents(".reg_form_item").addClass("error");
        $(".pass").show().html('密码必须是6-16位数字或字母或符号组成');
        return false;
    } else if (checkPass2(x) == 0) {
        obj.parents(".reg_form_item").addClass("error");
        $(".pass").show().html('密码必须是6-16位数字或字母或符号组成');
        return false;
    } else {
...
function psdconf2(obj) {
    ...
    if (regS.test(x)) {
        ...
        obj.parents(".reg_form_item").addClass("error");
        $(".repass").show().html('密码必须是6-16位数字或字母或符号组成');
        return false;
    } else if (x != $('.passwordconf2').val()) {
        obj.parents(".reg_form_item").addClass("error");
        $(".repass").show().html('两次填写的密码必须一致');
        return false;
    } else {
...
function submitFirstGiftcard() {
    if (!$(".sub_btn").hasClass("nextGrey")) {
        ...
        $("#custId").val(id);
        $.post(
            "/giftcardregisterFirst.htm",
            "mobile=" + mobile + "&checkCode=" + checkCode + "&giftcardPwd=" + giftcardPwd,
            function (data) {
                if (data == 1) {
                    //验证码错误
                    errorMessage(".code_info", "验证码错误");
                } else if (data == 2) {
                    //该手机号已被注册
                    errorMessage(".mobile_info", "该手机号已被注册");
                } else if (data == 7) {
                    //手机号码格式错误
                    errorMessage(".mobile_info", "请输入正确的手机号");
                } else if (data == 3) {
                    //该礼品卡无效
                    errorMessage(".giftcardPwd_info", "该礼品卡无效");
                } else if (data == 4) {
                    //该礼品卡未激活
                    errorMessage(".giftcardPwd_info", "该礼品卡未激活");
                } else if (data == 5) {
                    //该礼品卡已经被绑定
                    errorMessage(".giftcardPwd_info", "该礼品卡已经被绑定");
                } else if (data == 6) {
                    //该礼品卡已经过期
                    errorMessage(".giftcardPwd_info", "该礼品卡已经过期");
                } else if (data == 0) {
                    $("#registerFirstForm").submit();
                }
            }
        ...
        $.post(
            ...
            function (data) {
                if (data == 1) {
                    //验证码错误
                    $(".code_send").hide();
                    errorMessage(".code_info", "请输入正确的验证码");
                } else if (data == 2) {
                    //错误次数过多，验证码已失效
                    $(".code_send").hide();
                    errorMessage(".code_info", "错误次数过多，验证码已失效，请重新获取");
                } else if (data == 3) {
                    //手机号或礼品卡密码在session中失效，回到首页
...
function registerGiftcard() {
    if (!$(".sub_btn").hasClass("nextGrey")) {
        var valid_flag = true;
        var id = rsgister();
        if ($(".passwordconf").length > 0 && $(".repasswordconf").length > 0) {
            valid_flag = psdchk($(".passwordconf")) && valid_flag;
            valid_flag = psdconf($(".repasswordconf")) && valid_flag;
        } else if ($(".passwordconf2").length > 0 && $(".repasswordconf2").length > 0) {
            valid_flag = psdchk2($(".passwordconf2")) && valid_flag;
            valid_flag = psdconf2($(".repasswordconf2")) && valid_flag;
        }
        if (valid_flag) {
            if (isNaN(id)) {
                $("#regform").submit();
