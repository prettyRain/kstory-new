...
function doAjaxForCate(pageNo, pageSize)
{
    ...
    var definedCate = $(".defined[checked]").val();
    /*AJAX查询分类信息*/
    $.get("queryTempClassifyBar.htm?pageNo=" + pageNo + "&pageSize=" + pageSize
        + "&channelId=" + channelId + "&definedCate=" + definedCate + "&CSRFToken="+$("#CSRFToken").val(),
        function (data)
        {
...
function addSetboxPaternity(obj,id){
    ...
    if(id!=-1){
        ...
        function(data) {
                ...
                if(data.parentClassifyBar.grade==2){
                    if(id==0) {
                        $("#provinceChoose").show();
                    }
                    $("#cateGrade").val(3);
    ...
    $("#paternity_subClasifybarBtn").attr("onclick","updateCatePater()");
        $.post("showtempclassifybarandcateandquickajax.htm?CSRFToken="+$("#CSRFToken").val(), { parentId:obj , classifyBarId:cId },
            function(data) {
                ...
                $("#up_provinceId").val(data.classifyBar.temp5);
...
function suboodsCatePater(){
    ...
    if($("#cateGrade").val()==3 && $("#definedCate").val()==1) {
        if ($("#up_provinceId").val() == null || $("#up_provinceId").val() == 0) {
            showTipAlert("地区必须选择！！");
            return;
        }
    }
        ...
        function(data){
            ...
            if(data.classifyBar.goodsCatId==-1){
                ...
                if(data.barCates[0]!=null){
                    $("#goodsCate1").val(data.classifyBar.goodsCatId);
                    $("#goodsCateName1").val(data.classifyBar.name);
                }
