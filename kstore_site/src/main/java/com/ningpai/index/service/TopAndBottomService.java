...
public class TopAndBottomService {
    ...
    public ModelAndView getTopAndBottom(ModelAndView mav) {
        SysTemp sysTemp = getIndexDefalutTemp();
        // 模板id
        ...
        //加载移动端站点信息
        topmap.put("mobSite",mobSiteBasicService.selectCurrMobSiteBasic(request));
        List<ChannelAdver> pageAdvList = channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(tempId), null, null, ATID3, ADVERTTYPE, null, "0", null,
                "0",null);
        topmap.put("advs",pageAdvList);
    ...
    public ModelAndView getBottom(ModelAndView mav) {
        ...
        topmap.put("advs",pageAdvList);
        //获取所有楼层信息及楼层商品
        List<ChannelStorey> channelStoreys = channelStoreyService.selectchannelStoreyByParamForSite(null, Long.valueOf(temp.getTempId()), null);
        for (ChannelStorey channelStorey : channelStoreys) {
            List<ChannelStoreyGoods> channelStoreyGoods = channelStoreyGoodsService.selectchannelStoreyGoodsByParamForSite(channelStorey.getChannelStoreyId(), null, null);
            for (ChannelStoreyGoods goods : channelStoreyGoods) {
            ...
            channelStorey.setChannelStoreyGoods(channelStoreyGoods);
        }
        bottom.put("channelStoreys",channelStoreys);
        //底部加载限时抢购
        PageBean pb = new PageBean();
        pb.setPageSize(5);
//        pb = goodsProductService.selectMarketingRushList(pb, request,null,null);
        bottom.put("marketingRush",pb);
        //加载申请第三方接口登录得到的随机码
        //qq登录
        topmap.put("qqCode", authService.findAuthByAuthId(17L));
        //新浪微博登录
        topmap.put("sinaCode", authService.findAuthByAuthId(19L));
        mav.addObject(BOTTOM, bottom);
        mav.addObject(TOPMAP, topmap);
        return mav;
    }
     ...
     */
    @SuppressWarnings("deprecation")
    private void checkChangeTempTwentyOne() {
        ...
        if (checkToken(request)) {
            ...
            if (!topHtml.exists()) {
                try {
                    // 静态化头部
                    this.staticizeTopTwentyOne(request);
                } catch (Exception e) {
                    LOGGER.info(e);
                }
            }
            // 检查首页静态页面是否存在，不存在执行首页静态化方法
            String indexHtmlPath = templatePath + "/index21.html";
            File indexHtml = new File(indexHtmlPath);
            if (!indexHtml.exists()) {
                try {
                    // 静态化首页
                    this.staticizeIndexTwentyOne(request);
                } catch (Exception e) {
                    LOGGER.info(e);
                }
            }
     ...
     */
    @SuppressWarnings("deprecation")
    private void checkChangeTemp960() {

        if (checkToken(request)) {
            // 静态化头部 注释掉后，就不用每次都重新加载，但如果后台模板有变化就必须点击生成模板
            //this.staticizeTop960(request);
            // 静态化首页 注释掉后，就不用每次都重新加载，但如果后台模板有变化就必须点击生成模板
            //this.staticizeIndex960(request);
            // 检查头部静态页面是否存在，不存在执行头部静态化方法
            String templatePath = request.getRealPath("/") + VIEW_INDEX;
            String topHtmlPath = templatePath + "/newheader960.html";
            File topHtml = new File(topHtmlPath);
            if (!topHtml.exists()) {
                try {
                    // 静态化头部
            ...
            // 检查底部静态页面是否存在，不存在执行底部静态化方法
            String bottomHtmlPath = templatePath + "/newbottom.html";
            File bottomHtml = new File(bottomHtmlPath);
            if (!bottomHtml.exists()) {
                try {
                    // 静态化底部
                    this.staticizeBottom960(request);
                } catch (Exception e) {
                    LOGGER.info(e);
                }
            }
            // 检查首页静态页面是否存在，不存在执行首页静态化方法
            String indexHtmlPath = templatePath + "/index960.html";
            File indexHtml = new File(indexHtmlPath);
            if (!indexHtml.exists()) {
                try {
                    // 静态化首页
        ...
        if (null == token || "".equals(token)) {
            ...
            if (null != tk) {
                ...
                try {
                    ...
                    // 删除缓存中的token
                    redisAdapter.delete(CacheKeyConstant.TEMPTOKEN_KEY);
     ...
     */
    @SuppressWarnings("deprecation")
    public void staticizeTopTwentyOne(HttpServletRequest request) {
        String templatePath = request.getRealPath("/") + VIEW_INDEX;
     ...
     */
    @SuppressWarnings("deprecation")
    public void staticizeTop960(HttpServletRequest request) {
        String templatePath = request.getRealPath("/") + VIEW_INDEX;
        String templateName = "/newheader960_ftl.ftl";
        String fileName = templatePath + "/newheader960.html";
        ...
        try {
            Map<String, Object> topmap = new HashMap<String, Object>();
            if(channel!=null) {
                topmap = this.getTop(channel);
                fileName = templatePath + "/newheader960_"+channel.getChannelId()+".html";
                topmap.put("backgroundColor",channel.getKeyword());
            }else{
                topmap = this.getTop();
                //公共头背景颜色使用内有大聚会的颜色
                Channel defaultChannel = channelService.findChannelByID(247L);
                topmap.put("backgroundColor",defaultChannel.getKeyword());
            }
            map.put(TOPMAP, topmap);
            // FreeMarker生成静态页面
            IndexStaticizeUtil.analysisTemplate(templatePath, templateName, fileName, map);
     ...
     */
    @SuppressWarnings("deprecation")
    public void staticizeBottom960(HttpServletRequest request) {
        String templatePath = request.getRealPath("/") + VIEW_INDEX;
        String templateName = "/newbottom.ftl";
        String fileName = templatePath + "/newbottom.html";
        Map<String, Object> map = new HashMap<String, Object>();
        map.put(REQUEST, request);
        try {
            Map<String, Object> top = new HashMap<String, Object>();
    ...
    private Map<String, Object> getTop(Channel channel) {
         /* 头部信息 */
        Map<String, Object> top = new HashMap<String, Object>();

        top.put("channel",channel);
        ...
        if (!pageAdvList.isEmpty()) {
            top.put("logoAdv1", pageAdvList.get(0));
            if(pageAdvList.size() > 1){
                top.put("logoAdv2", pageAdvList.get(1));
            }
            if(pageAdvList.size() > 2){
                top.put("topAdv", pageAdvList.get(2));
            }
        }
        top.put("advs",channelAdverService.selectchannelAdverByParamForSite(null, channel.getTempId(), null, null, ATID3, ADVERTTYPE, null, "0", null, "0",null));
        // 2.加载热门搜索
        top.put("hotsearch", hotSearchService.selectHotBySelectiveForSite(channel.getTempId(), channel.getChannelId()));
        // 3.加载分类导航
        // 加载分类导航
        top.put("classifyBar", channelSiteService.getClassifyBar(channel.getChannelId(), channel.getTempId()));
        // 4.加载站点信息
        top.put(SYSTEMBASE, basicSetService.findBasicSet());
        // 5.加载导航
        top.put("navList", channelBarService.selectAllChannelBarByParam(channel.getChannelId(), channel.getTempId(), null));
     ...
     */
    @SuppressWarnings("deprecation")
    public void staticizeIndexTwentyOne(HttpServletRequest request) {
        String templatePath = request.getRealPath("/") + VIEW_INDEX;
     ...
     */
    @SuppressWarnings("deprecation")
    public void staticizeIndex960(HttpServletRequest request) {
        String templatePath = request.getRealPath("/") + VIEW_INDEX;
        String templateName = "/index960_ftl.ftl";
        String fileName = templatePath + "/index960.html";
        try {
            // FreeMarker生成静态页面
            IndexStaticizeUtil.analysisTemplate(templatePath, templateName, fileName, this.getIndexInfo(request));
        } catch (Exception e) {
    ...
    public Map<String, Object> getIndexInfo(HttpServletRequest request) {
        ...
        // 根据栏目ID查询文章
        if(temp.getExpFleid1()!=null) {
            map.put("infoList", infoService.selectByInfoType(Long.valueOf(temp.getExpFleid1())));
        }

        // 获取页面轮播大广告
        map.put("avc", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID1, ADVERTTYPE, null, "0", null, "0",null));
        // 获取页面轮播小广告
        map.put("avs", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID2, ADVERTTYPE, null, "0", null, "0",null));
        // 获取页面广告
        map.put("pageAdvs", channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(temp.getTempId()), null, null, ATID3, ADVERTTYPE, null, "0", null, "0",null));
        // 热销推荐
        ...
        if (null != temp.getStandby4()) {
            // 根据模板ID和标签ID查询单页
            map.put("onepages", onePageService.selectInfoOPByTempAndTag(Long.valueOf(temp.getTempId()), Long.valueOf(temp.getStandby4())));
        }
        // 客服
    ...
    public Map<String, Object> getTop() {
        ...
        top.put("cust", request.getSession().getAttribute("cust"));
        // 根据栏目ID查询文章
        if(temp.getExpFleid1()!=null) {
    ...
    public SeoConf getSeo()
    {
        ...
        SeoConf seo = (SeoConf) redisAdapter.get(CacheKeyConstant.SEO_KEY);
        ...
        // 获取已启用的SEO设置对象
        seo = seoconfbiz.querySeoByUsedStatus();
        if (null == seo) {
            seo = new SeoConf();
            seo.setMeteKey("");
            seo.setMeteDes("");
        ...
        redisAdapter.put(CacheKeyConstant.SEO_KEY, seo);
        return seo;
    }
    ...
    private List<HelpCateBean> getHelpCateList() {
        ...
        List<HelpCateBean> helpCateBeans = (List<HelpCateBean>) redisAdapter.get(CacheKeyConstant.HELPCATELIST_KEY);

        if (!CollectionUtils.isEmpty(helpCateBeans))
        {
        ...
        redisAdapter.put(CacheKeyConstant.HELPCATELIST_KEY, new ArrayList<>(helpCateBeans));
        return helpCateBeans;
    }
