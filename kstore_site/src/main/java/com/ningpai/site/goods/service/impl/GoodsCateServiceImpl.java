...
public class GoodsCateServiceImpl implements GoodsCateService {
    ...
    public GoodsCate queryGoodsCateByCatId(Long catId){
        return this.goodsCateMapper.queryCateByPrimaryKey(catId);
    }
    /**
     ...
     */
    @Override
    public Channel queryChannelByCatId(Long catParentId) {
        //内有大聚会包含其他四个子商城分类
        List<Channel> channels = goodsCateMapper.queryChannelByCatId(catParentId);
        if(channels.size()>1){
