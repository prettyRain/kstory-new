...
public class LoginController {
    ...
    public ModelAndView login(HttpServletRequest request, String url, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        if(urlEmp!=null && (urlEmp.indexOf("customercenter") != -1 || urlEmp.indexOf("myorder") != -1 || urlEmp.indexOf("myshoppingmcart") != -1)){
            preUrl = null;
        }
        if (preUrl != null) {
        ...
        if (urlEmp.indexOf("valididentity") != -1 || urlEmp.indexOf("mysetting") != -1|| urlEmp.indexOf("tochangepassword") != -1) {
    ...
    public ModelAndView loginOut(HttpServletRequest request, HttpServletResponse response, String url) throws UnsupportedEncodingException {
        ...
        return seoService.getCurrSeo(new ModelAndView(new RedirectView(request.getContextPath() + "/main.html")));
    }
    ...
    @ResponseBody
    public Object checkLogin(HttpServletRequest request, HttpServletResponse response, String username, String password, String url, String code, String keeppwd)
            throws UnsupportedEncodingException {
        if (username == null || password == null) {
            // 非法登录
            return -10024;
        }
        int status = loginService.checkCustomerExists(request, response, username, password, code, keeppwd);
        if (status == 1) {
            return url;
        } else {
            return status;
