...
import com.ningpai.site.customer.service.*;
...
import com.ningpai.util.AliSmsUtil;
...
public class CustomerController {
    ...
    public ModelAndView customerIndex(HttpServletRequest request, HttpServletResponse response, PageBean pb) {
        ...
        if (request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID) != null) {
            ...
            customerServiceInterface.modifyCustomerInfo(info,null);
            //根据查询到的用户总积分来更新其用户等级
            Long levelId=0L;
            String levelName="";
            Integer gradePoint = pointSum-customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+"");
            List<CustomerPointLevel> pointList = this.customerServiceInterface.queryPointLevel();
            if(null != pointList && pointList.size()>0){
                for(int i = 0;i<pointList.size();i++){
                    ...
                    if(gradePoint >= Integer.parseInt(a[0]) && gradePoint <= Integer.parseInt(a[1])){
                        levelId = pointList.get(i).getPointLevelId();
            ...
            // 查询浏览记录
            List<Browserecord> browserecords = browserecordService.selectBrowserecord(customerId);
            ...
            resultMap.put("browses", browserecords);
            // 查询通知内容数量 如 :待处理订单数量...
            ...
            CustomerAllInfo customerAllInfo = (CustomerAllInfo) customerServiceInterface.queryCustomerById(customerId);
            ...
            // 根据会员编号查询会员信息
            resultMap.put(CustomerConstantStr.CUSTOMER, customerAllInfo);
            // 是否允许退单
    ...
    public ModelAndView securityCenter(HttpServletRequest request) {
        ...
        if (checkLoginStatus(request)) {
            ...
            Deposit deposit = depositService.getDeposit(queryParam);
            // 根据会员编号查询会员信息
...
//        }
    ...
    public ModelAndView modifyPwd(HttpServletRequest request, String type, String ut, String status, String mcode, String password) {
        ...
        if("null".equals(emailCheck)){
            int emailCheckPwd = (int)emailCheck;
            if(emailCheckPwd != 1){
                return new ModelAndView(new RedirectView(CustomerConstantStr.STARTINDEX));
            }
        }
        ...
        if (!isAccessValid(ut, request)) {
            ...
            return new ModelAndView(new RedirectView(request.getContextPath() + "/validate/validateidentity.htm?type=" + type + "&ut=" + status1 + "&checkedUsing=0"));
        }
        ...
        //验证短信是否正确
        if(mcode != null){
            int i =this.customerServiceInterface.getMCode(request, mcode);
            if(i != 1){
                LOGGER.info(ERRORINFO);
                return new ModelAndView(new RedirectView(CustomerConstantStr.STARTINDEX));
            }
    ...
    public ModelAndView newvalidbindemail(HttpServletRequest request, String d, String mail, String k, String checkCode) {
        ...
        if (err == 0 && customerId != null && Long.valueOf(outputStr1).equals(customerId)) {
            mav = new ModelAndView(new RedirectView("../resetpwd.htm?emailCode=" + checkCode));
    ...
    public Long checkMobileExist(HttpServletRequest request, String mobile) {
        ...
        if (checkLoginStatus(request)) {
            // 验证手机存在性
            Long result = customerServiceInterface.checkMobileExist(mobile);
            CustomerAllInfo customer = (CustomerAllInfo) request.getSession().getAttribute("cust");
            // 当手机号码存在 并且手机号码不为当前的登录账号 则返回存在
            if (result ==1 && !mobile.equals(customer.getCustomerUsername()))
            {
                return 1L;
            }
    ...
    public int sendcode(HttpServletRequest request, String moblie,String code) throws IOException {
        ...
        // 如果session中没有验证码 则返回2
        if (StringUtils.isEmpty((String) request.getSession().getAttribute("MPATCHCA"))) {
            return 2;
        }
     ...
     * @throws IOException
     */
    @RequestMapping("validate/getMCodeFindPwd")
    @ResponseBody
    public int getMCodeFindPwd(HttpServletRequest request, String code) throws IOException {
     ...
     * @return
     */
    @RequestMapping("validate/getNoLoingMCode")
    @ResponseBody
    ...
    public ModelAndView queryOrderByOrderId(HttpServletRequest request, Long orderId) {
        ...
        if (checkLoginStatus(request)) {
            ...
            if(orderOldAddress!=null){
                resultMap.put("oldOrderAddress",orderOldAddress);
                if(orderOldAddress.getCommunityId()!=null && orderOldAddress.getCommunityId()!=0l){                    CommunityStore oldCommunityStore = communityStoreService.selectVo(order.getCommunityStoreId());
                resultMap.put("oldCommunityStore",oldCommunityStore);
                }
            }
            CommunityStore communityStore = communityStoreService.selectVo(order.getCommunityStoreId());
            resultMap.put("communityStore",communityStore);
            String expressType = order.getOrderExpressType();

            if("1".equals(expressType)){//自提,查用户收货地址和自提地址，否则的话，orderInfoBean里面的信息就是收货地址
                DeliveryPoint deliveryPoint = deliveryPointService.getDeliveryPoint(order.getAddressId());
            ...
            if("1".equals(order.getDirectType())){
                //直营店订单
                OrderExpress oe=orderService.queryOrderExpress(orderId);
                ...
                String url=orderService.queryExpressInfoUrl(relation);
     ...
     * @return ModelAndView
     */
    @RequestMapping("customer/ajaxCancelOrder")
    @ResponseBody
    ...
    public ModelAndView comfirmofGoods(HttpServletRequest request, PageBean pb, Long orderId, String fromUrl) {
        ...
        if (checkLoginStatus(request)) {
            Order order = siteOrderService.getPayOrder(orderId);
            //选择货到付款，确认收货时 赋值payTime（后台首页统计，货到付款的也算）
            ...
            orderCouponService.modifyCouponByOrderIdAndCustomerId(orderId,(Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID));
            // 根据订单id赠送优惠劵信息
    ...
    public ModelAndView queryCustomerIntegral(HttpServletRequest request, PageBean pb, Long date) {
        ...
        if (checkLoginStatus(request)) {
            ...
            CustomerAllInfo customerAllInfo = (CustomerAllInfo) customerServiceInterface.queryCustomerByCustomerId(customerId);
            customerAllInfo.setInfoPointSum(customerAllInfo.getInfoPointSum() - customerPointServiceMapper.getCustomerReducePoint(customerId + ""));
    ...
    public ModelAndView showMyBrowserecord(HttpServletRequest request, PageBean pb) {
        ...
        if (checkLoginStatus(request)) {
            // 查询浏览记录
            List<Browserecord> browserecords = browserecordService.selectBrowserecord((Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID));
            for (Browserecord browserecord : browserecords) {
                GoodsBean goods = browserecord.getGoods();
                BigDecimal finalPrice = goods.getGoodsPrice();
                //查询商品是否有促销，有就显示促销价
            ...
            followMap.put("customerId",request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID));
            //查询当前用户收藏商品商品id集合（用于个人中心浏览记录判断商品收藏与否）
            resultMap.put("goodsIds", customerFollowService.selectCustomerFollowForList(followMap));
     ...
     * @return
     */
    @RequestMapping("customer/timeBackOrderById")
    @ResponseBody
     ...
     * @return
     */
    @RequestMapping("customer/selectBackOrderById")
    @ResponseBody
