...
function getCateList(data)
{
    ...
    for (var i = 0; i < data.length; i++)
    {
        ...
        html = html +"<span  class='name'>" + data[i].name+"</span>" ;
        html+= "</td>" + "<td>";
...
//	        "<td class='sec_td' colspan='6'>" + "<table class='sec_tb'><tr>" +
        ...
        if(data[i].isUsing == '1'){
            html+="<td><a href='javascript:;' class='isUsing'><span class='label label-success'>是</span></a></td>";
        }else{
            html+="<td><a href='javascript:;' class='isUsing'><span class='label label-default'>否</span></a></td>";
        }
...
function updateCatePater(){
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data!=null){
                $("#goodsCateHtml").find("#"+data.classifyBarId).find(".name").text(data.name);
                if(data.isUsing == '1'){
                    $("#goodsCateHtml").find("#"+data.classifyBarId).find(".isUsing").html("<span class='label label-success'>是</span>");
                }else{
                    $("#goodsCateHtml").find("#"+data.classifyBarId).find(".isUsing").html("<span class='label label-default'>否</span>");
                }
                $('#addSetbox_paternity').modal('hide');
                clsClassifyBar();
            }else{
               // $(cls_adver).click();
                showTipAlert("修改失败！！");
...
function refreshStaticHtml() {
    var host = window.location.host;
    var location = "";
    if(host.startsWith("testboss.ge960.com")){
        location = "testsite.ge960.com";
    }else if(host.startsWith("zboss.ge960.com")){
        location = "zsite.ge960.com";
    }else if(host.startsWith("boss.ge960.com")){
        location = "www.ge960.com";
    }else{
        location = "localhost:8080";
    }
    $.ajax({
        url:"http://"+location+"/refreshStaticHtml.htm",
        type:"post",
        dataType:"jsonp",
        jsonp:"jsoncallback",
        ...
        success:function (data) {
            if (data==1)
            {
                showTipAlert("导航静态页面更新成功!");
            }else {
                showTipAlert("导航静态页面更新失败!");
            }
        }
    })
}