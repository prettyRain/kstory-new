...
public class SynOrderServiceImpl implements SynOrderService {
    ...
    public int SynOrder(Long orderId) {
        ...
        // 添加请求参数——appscret
        apiparamsMap.put(APPSCRET, eStore.getSecret());

        // 添加请求参数——token
        apiparamsMap.put(TOKEN, eStore.getSessionkey());
        ...
        if (netorder != null) {
            ...
            // 仓库id
            orderInfo.setStorage_id(Integer.parseInt(eStore.getStorageId()));
            // 线上请使用注释内容
            ...
            orderInfo.setShop_id(eStore.getShopId());
            ...
            if (goodslist != null && !goodslist.isEmpty()) {
                for (int i = 0; i < goodslist.size(); i++) {
                    ...
                    // 铺编号
                    pi.setShop_id(eStore.getShopId());
                    // 网店品名
...
*/
    ...
    public int getOrder(Long[] orderId) {
        ...
        for (int i = 0; i < orderId.length; i++) {
            ...
            postData = param.toString().substring(1);
            String result;
            result = HttpMethodUtil.getResult(eStore.getUrl(), postData);
            // xml解析
            try {
