    ...
    public ModelAndView showInsideLetter(HttpServletRequest request, PageBean pb) {
        ...
        try {
            ...
            if (checkLoginStatus(request)) {
                resultMap.put(CustomerConstantStr.RECEIVE_CUSTOMERID, (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID));
                // 查询全部站内信
                PageBean pageBean = insideLetterService.queryInsideLetter(resultMap, pb);
                List<Object> list = pageBean.getList();
                List<Object>  list1= new ArrayList<Object>();
                //把object转换成InsideLetterVo类  修改分类 给分类加上父分类
                for(Object obj:list){
                    net.sf.json.JSONObject json = net.sf.json.JSONObject.fromObject(obj);
                    InsideLetterVo letter = (InsideLetterVo)net.sf.json.JSONObject.toBean(json, InsideLetterVo.class);
                    if(StringUtils.isNotBlank(letter.getHrefUrl())){
                       if(letter.getHrefUrl().indexOf("list-")!=-1){
                           String cateThree = letter.getHrefUrl().substring(letter.getHrefUrl().indexOf("-")+1);
                           GoodsCate goodsCate = goodsCateService.queryGoodsCateByCatId(Long.valueOf(cateThree));
                           if(goodsCate!=null&& goodsCateService.queryGoodsCateByCatId(goodsCate.getCatParentId())!=null){
                               Long parentId = goodsCateService.queryGoodsCateByCatId(goodsCate.getCatParentId()).getCatParentId();
                               if(parentId==null){
                                   parentId = 88888l;
                                   letter.setHrefUrl("list-"+goodsCate.getCatId()+"-"+parentId+"-88888");
                               }else {
                                   parentId = parentId==0?goodsCate.getCatParentId():parentId;
                                   letter.setHrefUrl("list-"+goodsCate.getCatId()+"-"+parentId);
                               }

                           }else{
                               letter.setHrefUrl("list-88888-88888-88888");
                           }
                       }
                   }
                   list1.add(letter);
                ...
                mav = new ModelAndView("customer/newinsideletter").addObject(CustomerConstantStr.PB, pageBean);
