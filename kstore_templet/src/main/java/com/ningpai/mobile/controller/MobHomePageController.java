...
public class MobHomePageController {
    ...
    public ModelAndView setMobMain(Long merchantId, Long homePageId, HttpServletRequest request) throws DocumentException {
        ...
        //验证商品是否可用
        org.dom4j.Document document= DocumentHelper.parseText(homePage.getDoc());
        ...
        for(org.dom4j.Element element:elementList) {
            if ("goodsmod".equals(element.getName())) {
                List<org.dom4j.Element> elementList1 = element.elements();
                for (org.dom4j.Element element1 : elementList1) {
                    //货品id
                    ...
                    GoodsProduct goodsProduct = goodsProductService.queryProductByGoodsId(Long.valueOf(href_Id));
                    //判断商品是否下架
                    if("0".equals(goodsProduct.getGoodsInfoAdded())){
                        homePage.setDoc(homePage.getDoc().replace(goodsProduct.getGoodsInfoName(),goodsProduct.getGoodsInfoName()+"（已下架）"));
                    }
                }
            }
        }
        ...
        String title = mobHomePageService.queryAllTitle();
    ...
    public ModelAndView saveAllMod(Long homePageId, HttpServletRequest request) {
        ...
        xmlElementUtil.removeElement(root, IMGNAV);
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (itemId.indexOf(APPCONT) != -1) {// 魔方
                ...
                xmlElementUtil.setElementAttr(newImgAdv, "tag", id);
                ...
                if(StringUtils.isNotEmpty(n)){
                    xmlElementUtil.setElementAttr(newGoodsmod, "tag", n);
                }
                ...
                for (int j = 0; j < as.size(); j++) {
                    ...
                    //发布时获取最新的商品信息
                    String goodsId = href.substring(href.indexOf("/")+1,href.indexOf("."));
                    GoodsProduct goodsProduct = goodsProductService.queryProductByGoodsId(Long.valueOf(goodsId));
                    String src = goodsProduct.getGoodsInfoImgId();
                    String name = goodsProduct.getGoodsInfoName();
                    String price = "￥" + goodsProduct.getGoodsInfoPreferPrice();
                    // 创建li标签元素
                ...
                for (int j = 0; j < navItem.size(); j++) {
                    ...
                    xmlElementUtil.setElementAttr(newA, "href", href);
                    xmlElementUtil.setElementAttr(newA, "data-img", data_img);
                    xmlElementUtil.setElementAttr(newA, "data-name", data_name);
                    xmlElementUtil.setElementAttr(newA, "data-chs", data_chs);
                    xmlElementUtil.setElementAttr(newA, "data-chs2", data_chs2);
                    xmlElementUtil.setElementAttr(newA, "data-tag", data_tag);
                    // 创建img元素
                    Element newImg = newA.getOwnerDocument().createElement("img");
                    xmlElementUtil.setElementAttr(newImg, "src", src);
                    newA.appendChild(newImg);
        ...
        //0:草稿  1:发布中
        homePage.setTemp4("0");//改为可用
        homePage.setPageState(0);
        if("1".equals(request.getParameter("pageState"))){
            homePage.setPageState(1);
        }
    ...
    public ModelAndView queryMobHomeList(MobHomePageVo vo,PageBean pageBean,HttpServletRequest request) {
        ...
        try {
            ...
            mav.addObject("pageCateList", this.mobPageCateServcie.selectAll(params));
            mav.addObject("siteAddress",this.getBsetAddress(request));

            mav.addObject("search",vo);
            vo.setIsThird("0");
            vo.setTemp4("0");
            vo.setTemp3("0");//页面类型
            mav.addObject("pb", this.mobHomePageService.selectMobPageByPb(vo,pageBean));
        }catch(Exception e){
     ...
     * @return ModelAndView
     */
    @RequestMapping("/ajaxQueryMobPageList")
    @ResponseBody
    public Map<String,Object> ajaxQueryMobPageList(MobHomePageVo vo,PageBean pageBean,HttpServletRequest request) {
        ...
        try {
            ...
            mav.put("siteAddress",this.getBsetAddress(request));
    ...
    public ModelAndView addMobPage(MobHomePageVo vo, HttpServletRequest request){
        ModelAndView mav = new ModelAndView(new RedirectView("chooseMobPage.htm"));
        try {
            vo.setMerchantId(-1l);
            vo.setIsThird("0");
            vo.setStoreId(0l);
            long res = this.mobHomePageService.addMobPage(vo);
            if( res > 0l){
                mav.setView(new RedirectView("setMobHomePage.htm".concat(ValueUtil.TOKENPARAM).concat(CSRFTokenManager.getTokenFromRequest(request)).concat("&homePageId=").concat(String.valueOf(res))));
                return mav;
            }
        }catch (Exception e){
            LOGGER.error("模板选择列表错误",e);
        }
        mav.addObject("msg","选中失败");
        return mav;
    }
     ...
     * @return
     */
    private String getBsetAddress(HttpServletRequest request){
        // 拼接展示地址
        ...
        return siteAddress + mAddress;
    }

    @RequestMapping("/qrcodeMob")
    public ModelAndView qrcodeMob() {
        ModelAndView mav = new ModelAndView();
        mav.setViewName("jsp/mobile/mob_qrcode");
        return mav;
    }
    ...
    public String getshortlink(String code) {
        try {
            String redirect_uri = URLEncoder.encode(code,"UTF-8");
            String url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc3d05e27929ba2a5&redirect_uri=" + redirect_uri + "&response_type=code&scope=snsapi_base&state=123#wechat_redirect";
            ...
            if(("0").equals(jsonData.get("status").toString())){
                return jsonData.get("tinyurl").toString();
            }
        }catch (Exception e){
            e.printStackTrace();
        }
