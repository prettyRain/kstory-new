...
public class DepositInfoController {
    ...
    public ModelAndView selectDepositInfoById(HttpServletRequest request, DepositInfoVo depositInfoVo, PageBean pageBean) {
        ...
        try {
            ...
            if(StringUtils.isNotEmpty(depositInfo.getGiftcardNo())){
                //查看会员绑定的礼品卡状态
                Giftcard giftcard = new Giftcard();
                giftcard.setGiftcardNo(depositInfo.getGiftcardNo());
                activeFlag = Integer.valueOf(giftcardService.getGiftcard(giftcard).getActiveFlag());
            }
        }catch (Exception e){
            LOGGER.error("查看会员资金明细：",e);
        }
        return new ModelAndView(CustomerConstantStr.DEPOSITINFOCUSTOMER,PAGEBEAN,pageBean).addObject("depositInfo",depositInfo).addObject("pageNos",pageNos).addObject("startTime", startTime).addObject("endTime", endTime).addObject("activeFlag",activeFlag);
    }
