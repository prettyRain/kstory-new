...
public class GoodsAuditController {
    ...
    //商品批量审核通过或拒绝审核
    @RequestMapping("batchAuditByGoodsIds")
    public ModelAndView batchAuditByGoodsIds(HttpServletRequest request, String batchGoodsIds,String autidPassAndTurn,String refuseReason){
        String[] arr=new String[]{};
       if(StringUtils.isNotBlank(autidPassAndTurn)){
           if(StringUtils.isNotBlank(batchGoodsIds)){
               arr=batchGoodsIds.split(",");
           }
           //审核通过
           if("1".equals(autidPassAndTurn)){
               for(String goodsId:arr){
                   // 更改审核状态
                   int count = goodsAuditService.auditByGoodsId(Long.parseLong(goodsId));
                   //清空拒绝理由
                   goodsAuditService.clearReasonById(Long.valueOf(goodsId));
                   // 根据商品id查询所有货品信息
                   List<GoodsProductVo> goodsProducts = goodsProductService.queryProductListByGoodsId(Long.parseLong(goodsId));
                   // 审核通过商品后，将货品也都改为已审核
                   if (goodsProducts != null) {
                       for (int i = 0; i < goodsProducts.size(); i++) {
                           //如果商品下的货品是为审核状态 那么审核通过
                           if("0".equals(goodsProducts.get(i).getGoodsInfoAdded())&&"1".equals(goodsProducts.get(i).getAuditStatus())){
                               goodsProductService.auditProductAction(goodsProducts.get(i).getGoodsInfoId());
                               //如果库存大于0就发送点击了到货通知的会员
                               List<ProductWare> productWares = goodsProducts.get(i).getProductWares();
                               if(productWares!=null && productWares.size()>0){
                                   for (ProductWare productWare : productWares) {
                                       if(productWare.getWareStock()>0){
                                           goodsService.updateArrivalNotice(request,goodsProducts.get(i).getGoodsInfoId(),productWare.getWareId());
                                       }
                                   }
                               }
                           }
                   ...
                   // 修改索引
                   searchAsyncService.insertOneGoodsIndexToEs(Long.parseLong(goodsId));
               }
           //审核没通过
           }else if("2".equals(autidPassAndTurn)){
               for(String goodsId:arr){

               // 更改审核状态 同时改为下架
               int count = goodsAuditService.refuseAuditByGoodsId(Long.parseLong(goodsId), getReason(request,refuseReason,Long.valueOf(goodsId)));
               // 根据商品id查询所有货品信息
               List<GoodsProduct> goodsProducts = goodsProductService.queryProductsByGoodsId(Long.parseLong(goodsId));
               // 审核不通过后，将货品也都改为审核不通过 同时改为下架状态
               if (goodsProducts != null) {
                   for (int i = 0; i < goodsProducts.size(); i++) {
                       goodsProductService.refuseAuditProductAction(goodsProducts.get(i).getGoodsInfoId(),"");
                   }
               }
               // 修改索引
               searchAsyncService.updateOneGoodsIndexToEs(Long.parseLong(goodsId));
               }
           }
       }
        return new ModelAndView(new RedirectView(GOODSAUDIT_HTM));
    }
    public String getReason(HttpServletRequest request,String refuseReason,Long goodsId){
        // 获取当前登录的用户名
        String name = (String) request.getSession().getAttribute(ValueUtil.NAME);
        String reason = this.goodsService.queryGoodsReasonById(goodsId);
        ...
        if(listreason.size()>=5){
            listreason.remove(0);
        }
        String refuseReasonNew = refuseReason;
        //拒绝理由为  操作人、理由、操作时间组成
        if(StringUtils.isNotBlank(refuseReason)){
            refuseReasonNew = request.getSession().getAttribute(ValueUtil.NAME)+"-"+refuseReason+"-"+new Date().getTime()+"&#";
        }
        StringBuffer sb = new StringBuffer();
        for(String strreson:listreason){
            sb.append(strreson+"&#");
        }
        sb.append(refuseReasonNew);
        return sb.toString();
    }
    /**
     * 商品审核通过
    ...
    public ModelAndView auditByGoodsId(HttpServletRequest request, Long goodsId) {
        ...
        //清空拒绝理由
        goodsAuditService.clearReasonById(goodsId);
        // 根据商品id查询所有货品信息
        List<GoodsProductVo> goodsProducts = goodsProductService.queryProductListByGoodsId(goodsId);
        // 审核通过商品后，将货品也都改为已审核
        if (goodsProducts != null) {
            for (int i = 0; i < goodsProducts.size(); i++) {
                //如果商品下的货品是为审核状态 那么审核通过
        ...
        // 修改索引
        searchAsyncService.insertOneGoodsIndexToEs(goodsId);
        if (count > 0) {
            // 非空验证 商品名称
    ...
    public ModelAndView refuseAuditByGoodsId(HttpServletRequest request, Long goodsId, String refuseReason) {
        ...
        Goods goods = goodsProductService.selectGoodsByGoodsId(goodsId);
        // 更改审核状态 同时改为下架
        int count = goodsAuditService.refuseAuditByGoodsId(goodsId, getReason(request,refuseReason,goodsId));
        ...
        // 审核不通过后，将货品也都改为审核不通过 同时改为下架状态
        if (goodsProducts != null) {
            //如果商品下的货品是为审核状态 那么审核不通过
            for (int i = 0; i < goodsProducts.size(); i++) {
                if("0".equals(goodsProducts.get(i).getGoodsInfoAdded())&&"1".equals(goodsProducts.get(i).getAuditStatus())) {
                    goodsProductService.refuseAuditProductAction(goodsProducts.get(i).getGoodsInfoId(), refuseReason);
                }
            }
        }
        // 修改索引
    ...
    public ModelAndView previewGoods(Long goodsId,Long goodsInfoId) {
        ...
        try {
            ...
            //商品关联类型详细参数
            List<GoodsReleParamVo> releParamVos = new ArrayList<>();
            //商品详细参数
            List<GoodsReleExpandParamVo> expandParamVos = new ArrayList<>();
            //货品
            ...
            if(goodsId == null && goodsInfoId != null){
                ...
                if(goodsProductVo != null && goodsProductVo.getGoodsId() != null){
                    ...
                    //商品详细
                    goods = goodsService.selectByPrimaryKey(goods.getGoodsId());
                ...
                goods = goodsService.selectByPrimaryKey(goods.getGoodsId());
                resultMap.put("goodsProductVos", goodsProductService.queryProductsForPreview(goods.getGoodsId()));
            }
            resultMap.put("goods", goods);
            resultMap.put("goodsCateVo", goodsCateService.queryGoodsCateById(goods.getCatId()));
            resultMap.put("goodsBrand", goodsBrandService.queryBrandById(goods.getBrandId()));
            resultMap.put("releParamVos", goodsReleParamService.queryAllByGoodsId(goods.getGoodsId()));
            resultMap.put("expandParamVos", goodsReleExpandParamService.queryExpandParamsByGoodsId(goods.getGoodsId()));
            return new ModelAndView("jsp/goods/goodsPreviewDetail").addAllObjects(resultMap);
        }finally {
            resultMap = null;
        }
    }
    ...
    public ModelAndView auditProductAction(HttpServletRequest request, Long goodsInfoId) {
        ...
        // 修改索引
        searchAsyncService.updateOneGoodsIndexToEs(goodsProductVo.getGoodsId());
        if (count > 0) {
            // 非空验证 商品名称
