                                          ...
                                          <div style="max-height:300px;overflow-y:auto;position:relative;">
                                              ...
                                              <table class="table table-striped table-hover table-bordered" id="fullcash${status.index+1}">
                                                  ...
                                                  <c:forEach items="${presents.fullbuyPresentScopes}" var="scopes" varStatus="scopeStatus">
                                                      ...
                                                      <c:choose>
                                                          <c:when test="${scopes.scopeType=='0'}">
                                                          ...
                                                          <c:otherwise>
                                                              <tr id="coupon_tr_${scopes.coupon.couponId}_1_${status.index+1}">
                                                                  <td width="100">${scopes.coupon.couponName}<input type="hidden" name="couponIdPG${status.index+1}" id="coupon_Id_${scopes.goodsProduct.goodsInfoId}" value="${scopes.coupon.couponId}"/></td>
                                                                  <td width="100">
                                                                     ${scopes.coupon.couponRemark}
                                                                  </td>
                                                                  <c:choose>
                                                                      <c:when test="${scopes.coupon.timeType=='0'}">
                                                                          <td width="150">起止时间</td>
                                                                          <td  width="300">
                                                                              <fmt:formatDate value="${scopes.coupon.couponStartTime}" var="couponStartTimeFm" pattern="yyyy/MM/dd"></fmt:formatDate>
                                                                              <fmt:formatDate value="${scopes.coupon.couponEndTime}" var="couponEndTimeFm" pattern="yyyy/MM/dd"></fmt:formatDate>
                                                                              ${couponStartTimeFm}-${couponEndTimeFm}
                                                                          </td>
                                                                      </c:when>
                                                                      <c:otherwise>
                                                                          <td width="150">固定时段</td>
                                                                          <td width="150">${scopes.coupon.couponDays}天</td>
                                                                      </c:otherwise>
                                                                  </c:choose>

                                                                  <td  width="100"><input type="hidden" readonly value="scopeNum${status.index+1}"/><input class="form-control valid required integer present" style="text-align: center" type="text" value="${scopes.scopeNum}" name="couponScopeNum${status.index+1}" readonly/></td>
                                                                  <td width="100"><button onclick="removeTr(this);">移除</button></td>
                                                              </tr>
                                                          </c:otherwise>
                                                      </c:choose>
                                              ...
                                              <table class="table table-striped table-hover table-bordered" id="fullcount${status.index+1}">
                                                  ...
                                                  <c:forEach items="${presents.fullbuyPresentScopes}" var="scopes" varStatus="scopeStatus">
                                                      <c:choose>
                                                          ...
                                                          <c:otherwise>
                                                              <tr id="coupon_tr_${scopes.coupon.couponId}_2_${status.index+1}">
                                                                  <td width="100">${scopes.coupon.couponName}<input type="hidden" name="couponIdPG${status.index+1}" id="coupon_Id_${scopes.goodsProduct.goodsInfoId}" value="${scopes.coupon.couponId}"/></td>
                                                                  <td width="100">
                                                                          ${scopes.coupon.couponRemark}
                                                                  </td>
                                                                  ...
                                                                  <td  width="100"><input type="hidden" readonly value="scopeNum${status.index+1}"/><input class="form-control valid required integer present" style="text-align: center" type="text" value="${scopes.scopeNum}" name="couponScopeNum${status.index+1}"/></td>
                                                                  <td width="100"><button onclick="removeTr(this);">移除</button></td>
                                                              </tr>
                                                          </c:otherwise>
                                                      </c:choose>
				...
				<div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <c:if test="${marketingFlag == 'PCG'}">
                        ...
                        <div class="form-group">
                            <label class="control-label col-sm-4"><span class="text-danger">*</span>抢购商品类型：</label>
                            <div class="col-sm-1"></div>
                            <div class="col-sm-6">
                                <select name="temp1" class="form-control" value="${marketing.rushs[0].temp1}">
                                    <c:if test="${marketing.rushs[0].temp1==1}">
                                        <option value="1" selected>普通商品</option>
                                        <option value="2">热销推荐</option>
                                    </c:if>
                                    <c:if test="${marketing.rushs[0].temp1==2}">
                                        <option value="1" >普通商品</option>
                                        <option value="2" selected>热销推荐</option>
                                    </c:if>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-sm-4"><span class="text-danger">*</span>抢购限时：</label>
                            <div class="col-sm-1"></div>
                            <div class="col-sm-6">
                                <select name="tId" class="form-control" value="${marketing.rushs[0].rushTimes.tId}">
                                    <option value="">请选择时间</option>
                                    <c:forEach var="rushTime" items="${rushTimes}">
                                        <c:choose>
                                            <c:when test="${marketing.rushs[0].rushTimes.tId==rushTime.tId}">
                                                <option selected value="${rushTime.tId}">时间段：${rushTime.tDate}点</option>
                                            </c:when>
                                            <c:otherwise>

                                                <option value="${rushTime.tId}">时间段：${rushTime.tDate}点</option>
                                            </c:otherwise>
                                        </c:choose>
                  ...
                  <div class="modal-body">
                      <div class="mb10">
                          <form class="form-inline" id="searchGoodsInfoG" action="" method="post" >
                              ...
                              <div class="form-group">
                                  <select name="" id="select_coupon" class="form-control" style="width:200px;">
                                      <c:forEach var="item" items="${couponlist}">
                                          <option style="cursor:pointer;" data-couponId="${item.couponId}" data-couponName="${item.couponName}" data-couponRemark="${item.couponRemark}" data-couponStartTime="${item.couponStartTime}" data-couponEndTime="${item.couponEndTime}" data-timeType="${item.timeType}" data-couponDays="${item.couponDays}"> ${item.couponName}</option>
                                      </c:forEach>
                                  </select>
                              </div>
                          </form>
...
//                    html+='<div class="col-sm-4" onclick="dellevel(this)"><label class="radio-inline" style="color:blue">删除本级促销</label></div></div>';
	...
	function mofensingle(price,obj,fixed){ 
        ...
        function chooseProductForGift(num,number){
            ...
            $('#blankCheckbox').get(0).checked=false;

            //添加赠品优惠券
            $('#select_coupon').attr("data-number",number);
            $('#select_coupon').attr("data-num",num);
        }
        /*添加赠品使用*/
...
//                    }
        ...
        $('#select_coupon').on("change",function(){
            if($('#blankCheckbox').get(0).checked){
                addcouG();
            }
        })
        ...
        function addcouG(){
            //num1满金额赠  2满件数赠
            var num = $('#select_coupon').attr("data-num");
            var number = $('#select_coupon').attr("data-number");
            var $coupon = $('#select_coupon').find("option:selected");
            if(!!$coupon){
                if(num==1){
                    $("#fullcash"+number+" tbody tr[id*='coupon_tr']").remove();
                }else if(num==2){
                    $("#fullcount"+number+" tbody tr[id*='coupon_tr']").remove();
                }
                var couponEndTime = $coupon.attr("data-couponEndTime");
                var couponId = $coupon.attr("data-couponId");
                var couponName = $coupon.attr("data-couponName");
                var couponRemark = $coupon.attr("data-couponRemark");
                var couponStartTime = $coupon.attr("data-couponStartTime");
                var timeType = $coupon.attr("data-timeType");
                var couponDays = $coupon.attr("data-couponDays");
                var htm = "<tr id='coupon_tr_"+couponId+"_"+num+"_"+number+"'>";
                htm+='<td width="100">'+couponName+'<input type="hidden" class="form-control present" name="couponIdPG'+number+'" id="coupon_Id_'+couponId+'" value="'+couponId+'"/></td>';
                htm+='<td width="100">备注:'+couponRemark+'</td>';
                if(timeType=='0'){
                    htm+='<td width="150">起止时间</td>';
                    htm+='<td  width="300">'+new Date(couponStartTime).toLocaleDateString()+'-'+new Date(couponEndTime).toLocaleDateString()+'</td>';
                }else if(timeType == '1'){
                    htm+='<td width="150">固定时段</td>';
                    htm+='<td  width="300">'+couponDays+'天</td>';
                }

                htm+='<td  width="100"><input type="hidden" value="scopeNum'+number+'"/><input type="text" class="form-control valid required integer present" value="1" name="couponScopeNum'+number+'" readonly/></td>';
                htm+='<td width="100"><button onclick="removeTr(this);">移除</button></td>';
                htm+="</tr>";
                //num1满金额赠  2满件数赠
                if(num==1){
