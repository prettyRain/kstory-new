...
public class GoodsBrandController {
    ...
    @ResponseBody
    public List<GoodsBrand> findBrands(){
        return goodsBrandService.findBrands();
    }
    /**
     * 查询所有的商品认证标识
     * @param response
    ...
    public ModelAndView findAllAuth(HttpServletResponse response,HttpServletRequest request,PageBean pb){
        pb.setPageSize(3);
        ModelAndView mav=new ModelAndView();
        mav.setViewName("jsp/goods/goods_auth");
        mav.addObject("pb",goodsBrandService.searchAuthByPageBean(pb));
        return mav;
    }
    ...
    @ResponseBody
    public GoodsAuth queryAuthById(Long authId){
        GoodsAuth auth=null;
        if(authId!=null){
            auth=goodsBrandService.queryAuthById(authId);
        }
        if(auth!=null){
            return auth;
        }else{
     ...
     * @return
     */
    @RequestMapping("/updateProductAuth")
    public String updateProductAuth(GoodsAuth auth){
        if(auth!=null){
            goodsBrandService.updateGoodsAuth(auth);
        }
        return "redirect:/findAllAuth.htm";
    }
    /**
     * xcy
     * 商品认证标识添加
     * @param authName 商品认证说明
     * @param authsrc  认证图片路径
     * @return
     */
    @RequestMapping("/saveAuth")
    public String  saveAuth(HttpServletRequest request,HttpServletResponse response,String authName,String authsrc){
        GoodsAuth goodsAuth=new GoodsAuth();
        goodsAuth.setAuthName(authName);
        goodsAuth.setAuthsrc(authsrc);
        goodsBrandService.saveAuth(goodsAuth);
        return  "redirect:/findAllAuth.htm";
    }
    /**
     * xcy
    ...
    @ResponseBody
    public String deleteAuth(HttpServletRequest request,HttpServletResponse response,String id,String ids){
        ...
        if(StringUtils.isNotBlank(id)){
            ...
            list.add(id);
            map.put("ids",list);
            goodsBrandService.deleteGoodsAuth(map);
        ...
        if (goodsBrandService.selectByBrandName(goodsBrand.getBrandName()) == 0) {
            ...
            if (null != goodsBrand.getBrandName() && 1 == result) {
                ...
                OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), "保存商品品牌",
                        (String) request.getSession().getAttribute(ValueUtil.OPERAPATH) + LOGGERINFO1 + goodsBrand.getBrandName() + LOGGERINFO2
    ...
    public String delBrand(Long brandId, HttpServletRequest request,PageBean pb) {
        ...
        if (null != goodsBrand.getBrandName() && 1 == result) {
            // 操作日志
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), "删除商品品牌", (String) request.getSession().getAttribute(ValueUtil.OPERAPATH)
                    + LOGGERINFO1 + goodsBrand.getBrandName() + LOGGERINFO2 + (String) request.getSession().getAttribute(ValueUtil.NAME));
            // 记录日志
     ...
     * 判断 商品品牌是否关联了商品
     *
     * @param brandIds 商品品牌id
     * @return 如果关联了商品 则返回1  否则返回0
     */
    @RequestMapping("/isbrandlinked")
    @ResponseBody
    public int isBrandLinked(Long[] brandIds) {
        return goodsBrandService.isBrandLinked(brandIds);
    }
    ...
    public ModelAndView batchDelBrand(Long[] brandIds, HttpServletRequest request, PageBean pb) {
        ...
        // 操作日志
        OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), "批量删除商品品牌", (String) request.getSession().getAttribute(ValueUtil.OPERAPATH)
    ...
    public ModelAndView updateBrand(@Valid GoodsBrand goodsBrand, MultipartHttpServletRequest request, PageBean pb) {
        ...
        if (null != goodsBrand.getBrandNickname() && 1 == result) {
            // 操作日志
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), "更新商品品牌", (String) request.getSession().getAttribute(ValueUtil.OPERAPATH)
                    + LOGGERINFO1 + goodsBrand.getBrandName() + LOGGERINFO2 + (String) request.getSession().getAttribute(ValueUtil.NAME));
     ...
     * 验证品牌名称名称是否已经存在
     *
     * @param brandName 待验证的品牌名称 {@link String}
     * @return 可用返回true 不可用就返回false
     */
    ...
    public boolean checkThirdBrandName(HttpServletRequest request, String brandName) {
        ...
        return this.goodsBrandService.checkBrandName(brandNameNew) && goodsBrandService.checkThirdApplayBrandName((Long) request.getSession().getAttribute("thirdId"), brandNameNew);
    }
     ...
     */
    @RequestMapping("/ajaxFindAllBrand")
    @ResponseBody
    public Map<String, Object> ajaxFindAllBrand(HttpServletRequest request, HttpServletResponse response, PageBean pb, SelectBean selectBean) {
        ...
        result.put("pb", goodsBrandService.searchByPageBean(pb, selectBean));
        result.put("ufs", uploadFileSetService.getCurrUploadFileSet());
        // 参数设置到作用域中
        // 拼接展示地址
        ...
        result.put("address", siteAddress + mAddress);
        return result;
    }
