...
public class OrderMServiceImpl extends BasicSqlSupport implements OrderMService {
    ...
    public Order getThirdOrderPrice(Long thirdId, String codeNo, Long amount, Long customerId, String orderCode, Long cityId, Long distinctId, List<ShoppingCart> shoplist,
        ...
        Map<String, Object> map = shoppingCartService.getPayorderThirdPriceMap(thirdId, shoplist, distinctId,customerId);
        ...
        BigDecimal sumOldPrice = BigDecimal.valueOf(Double.valueOf(map.get("sumPrice").toString()));
        ...
        BigDecimal sumPrice = sumOldPrice.subtract(prePrice);
...
//            BigDecimal marketSumPrice = sumPrice;
            ...
            BigDecimal discountPrice = ((BigDecimal)map.get("curZheKouPrice")).multiply(BigDecimal.ONE.subtract(pointDiscount))
                    .setScale(2, BigDecimal.ROUND_HALF_DOWN);
                ...
                if (cart.getMarketingList() != null && cart.getMarketingList().size() > 0 && cart.getMarketingList().get(0).getRushs() != null && cart.getMarketingList().get(0).getRushs().size() > 0) {
                    if (cart.getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice().multiply(cart.getMarketingList().get(0).getRushs().get(0).getRushDiscount()).compareTo(BigDecimal.ZERO) == 0 ) {
                        ...
                        if (countRush > 0 && countRush != null) {
                            if(countRush.longValue() >= cart.getGoodsNum()){
                                expressPrice = expressPrice.add(BigDecimal.valueOf(9.9).multiply(BigDecimal.valueOf(cart.getGoodsNum())));
                            }else if(countRush.longValue() < cart.getGoodsNum()){
                                expressPrice = expressPrice.add(BigDecimal.valueOf(9.9).multiply(BigDecimal.valueOf(countRush)));
                            }
                        }
    ...
    @Override
    @Transactional
    public List<Order> newsubmitOrder(Long duiHuanJiFen, String invoiceType, String invoiceTitle,String taxpayerNumber, HttpServletRequest request, Long[] shoppingCartId, Long typeId,
                                      OrderAddress orderAddress, Long deliveryPointId, String[] presentScopeId,String[] coupon_presentScopeId) throws UnsupportedEncodingException {
        // 优惠劵码
        ...
        //获取电子礼品卡数量
        String dianziCard = request.getParameter("dianziCard");
        Long dianziCardNum = 0L;
        if(StringUtils.isNotEmpty(dianziCard)){
            dianziCardNum = Long.parseLong(dianziCard);
        }
        ...
        String custAddress = request.getParameter("addressId");
        if (StringUtils.isNotEmpty(custAddress)) {
            Long custAddresss = Long.parseLong(custAddress);
            //根据用户id和地址id查询用户地址
            ca = addressService.queryCustomerAddressById(custAddresss,customerId);
...
//                            og.setGoodsInfoPrice(og.getGoodsInfoOldPrice().subtract(og.getGoodsCouponPrice().divide(BigDecimal.valueOf(ogNum), 2, BigDecimal.ROUND_HALF_EVEN)));
                ...
                oglist.addAll(groupOrderGoodslist);
                    ...
                    if (thirdId.equals(cart.getGoodsDetailBean().getProductVo().getThirdId())) {
                        ...
                        List<Marketing> markList = marketService.selectMarketingByGoodsInfoId(cart.getGoodsInfoId(), cart.getGoodsDetailBean().getProductVo().getGoods().getBrandId(), cart.getGoodsDetailBean().getProductVo().getGoods().getCatId());
                        cart.setMarketingList(markList);
...
//                            og.setGoodsInfoOldPrice(cartlist.get(i).getGoodsDetailBean().getProductVo().getGoodsInfoPreferPrice());
                ...
                if (typeId != null && typeId == 1 && "0".equals(thirdId.toString()) && chPay != null && chPay == 1) {
                   ...
                   //跟送货上门的地址一样
                    if (orderAddress.getAddressName() == null || orderAddress.getAddressPhone() == null || orderAddress.getAddressDetail() == null
                        ...
                        order.setShoppingAddrId(Long.valueOf(custAddress));
            ...
            if (f == 1) {
                ...
                //满赠优惠券
                if(coupon_presentScopeId!=null && coupon_presentScopeId.length>0){
                    List<FullbuyPresentScope> fullbuyPresentScopes = fullbuyPresentScopeMapper.queryByScopeIds(Arrays.asList(coupon_presentScopeId));
                    for(FullbuyPresentScope fullbuyPresentScope:fullbuyPresentScopes){
                        //获取满赠优惠券id
                        ...
                        if(coupon!=null){
                            ...
                            couponNo.setCustomerId(customerId);
                ...
                if (oglist != null && !oglist.isEmpty()) {
                    ...
                    for (OrderGoods og : oglist) {
                        ...
                        if (og.getMarketing() != null && og.getMarketing().getRushs() != null && !og.getMarketing().getRushs().isEmpty()) {
                            if(og.getCountRush()!=null&&og.getCountRush()>0){
...
//                            setErrorPrice(oglist,allbackPrice,orderPrice,"5",null,null,null);
                ...
                for(OrderGoods og:oglist){
                    ...
                    if(og.getGoodsInfoName().indexOf("电子卡") != -1){
                        ...
                        for(int n = 0 ;n < og.getGoodsInfoNum() ; n++){
                            ...
                            giftcard.setOrderCode(order.getOrderCode());
    ...
    public int updatePoint(Long customerId, Long point) {
        ...
        if (null != customerPoint) {
            // 更新当前登录会员的总积分  总积分是计算赠送积分的和
            customerPoint.setPointSum(customerPoint.getPointSum());
            // 更新积分对象
    ...
    public Map<String, Object> getWXUrl(HttpServletRequest request, HttpServletResponse response, Order order, Pay pay, String goodsName) {
        ...
        String openId = (String)request.getSession().getAttribute("payopenid");
        ...
        // 商户订单号
        String outTradeNo = order.getOrderCode()+"_"+strRandom;
    ...
    private void presentOrderGood(String[] presentScopeId, Long distinctId, List<OrderGoods> orderGoodses, List<CalcStockUtil> stockUtils) {
        ...
        if (ArrayUtils.isNotEmpty(presentScopeId) && distinctId != null && CollectionUtils.isNotEmpty(orderGoodses) && CollectionUtils.isNotEmpty(stockUtils)) {
            ...
            for (int k = 0; k < fullbuyPresentScopes.size(); k++) {
                ...
                og.setGoodsInfoName(goodsProduct.getGoodsInfoName());
                // 货品价格
    ...
    private void setBackPrice(List<OrderGoods> oglist, Order order) {
        ...
        if (null != order.getJfPrice() && order.getJfPrice().compareTo(BigDecimal.ZERO) == 1 && order.getBusinessId().intValue() == 0) {
            BigDecimal errPirce = BigDecimal.ZERO;
            //不知道为什么 还要给运费算积分 现在的业务是积分不可以抵扣积分
