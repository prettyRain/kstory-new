...
public class InsideLettersController {
    ...
    public ModelAndView showInsideLetter(HttpServletRequest request, PageBean pb,String mark) {
        ...
        try {
            ...
            if (checkLoginStatus(request)) {
                ...
                // 查询全部站内信
                mav = new ModelAndView("member/notice1").addObject("pb", insideLetterService.queryInsideLetters(resultMap)).addObject("mark",mark);

            } else {
                mav = new ModelAndView("redirect:/loginm.html").addObject("url", "/adminnotice.html").addObject("mark",mark);
            }
            return mav;
        } finally {
            mav = null;
     ...
     * @return
     */
    @RequestMapping("/remove_ordernotice")
    @ResponseBody
    public int removeNotice1(HttpServletRequest request) {
        // 获得用户Id
        Long customerId = (Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID);
        // 判断是否登录
        if (customerId != null) {
            orderNoticeService.removeNoticeAll(customerId);
            return 1;
        } else {
            // 去登录
            return 0;
     ...
     * @return
     */
    @RequestMapping("/remove_adminnotice")
    @ResponseBody
    public int removeNotice2(HttpServletRequest request) {
        // 获得用户Id
        Long customerId = (Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID);
        // 判断是否登录
        if (customerId != null) {
             insideLetterService.removeNoticeAll(customerId);
            return 1;
        } else {
            // 去登录
            return 0;
    ...
    public ModelAndView ordernotice(HttpServletRequest request, PageBean pb,String mark) {
        ModelAndView mav = null;
        ...
        try {
            ...
            if (checkLoginStatus(request)) {
                // 查询全部订单通知
                mav = new ModelAndView("member/notice2").addObject("pb", orderNoticeService.selectOrderNotices((Long) request.getSession().getAttribute(CustomerConstants.CUSTOMERID))).addObject("mark",mark);

            } else {
                mav = new ModelAndView("redirect:/loginm.html").addObject("url", "/ordernotice.htm").addObject("mark",mark);
            }
            return mav;
        } finally {
            mav = null;
