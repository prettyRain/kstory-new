...
public class GoodsElasticSearchServiceImpl implements GoodsElasticSearchService {
    ...
    /**
     * 价格排序脚本（goodsInfoPreferPrice为原价格，finalPrice为促销活动的最终价格）
     **/
    ...
    private static final String sort_script = "def price=_source.finalPrice;return price;";
     ...
     * 批发》零售》其他排序脚本
     */
    private static final String type_script = "def name=_source.goodsInfoName;if(name.contains('批发')){return 1;}else if(name.contains('零售')){return 2;}else{return 3;};";
    /**
     * 有货》无货排序脚本
     */
    private static final String stork_script = "def wares = _source.wareList;def stock = 0;for (ware in wares){ stock = stock + ware.wareStock;};if(stock>0){return 1;}else{return 0;};";
        ...
        for (int i = 0; i < listCount; i = i + temp) {
            ...
            if (egoodslist != null && !egoodslist.isEmpty()) {
                ...
                for (EsGoodsInfo goodsInfo : egoodslist) {
                    ...
                    //设置营销商品的最终价格
                    BigDecimal finalPrice = goodsInfo.getGoodsInfoPreferPrice();
                    if("1".equals(goodsInfo.getInMarketing())) {
                        List<GoodsMarketingCodeUtil> codeUtils = goodsElasticMapper.selectProductMarket(goodsInfo.getGoodsInfoId());
                        for (GoodsMarketingCodeUtil gms : codeUtils) {
                            if (gms.getCodexId() == 1) {
                                ...
                                finalPrice = goodsInfo.getGoodsInfoPreferPrice().subtract(discount);
                            } else if (gms.getCodexId() == 10) {
                                //团购
                                BigDecimal discount = goodsElasticMapper.selectGrouponByMarketId(gms.getMarketingId());
                                finalPrice = goodsInfo.getGoodsInfoPreferPrice().multiply(discount);
                                ...
                                param.put("goodsId", goodsInfo.getGoodsInfoId());
                    ...
                    goodsInfo.setFinalPrice(finalPrice);
                    //设置拼写建议
    ...
    public Map<String, Object> searchGoods(SearchPageBean<EsGoodsInfo> pageBean, Long[] wareIds, String[] indices, String[] types, String keyWords, String[] brands, String[] cats,
        ...
        if (sort != null && !"".equals(sort)) {
            ...
            //默认排序方式
            //按照有货》无货排序
            searchRequest.addSort(new ScriptSortBuilder(stork_script, "number").order(SortOrder.DESC));
            // 按照批发》零售》其他排序
            searchRequest.addSort(new ScriptSortBuilder(type_script, "number").order(SortOrder.ASC));
            //按照上架时间降序排序
            searchRequest.addSort(GOODSINFOCREATETIME, SortOrder.DESC);
        }
    ...
    @Override
    public String searchGoodsByInfoId(String goodsInfoId, String[] indices, String[] types) {
        // 构造请求数据
        IndexSearchRequest searchRequest = new IndexSearchRequest(indices, types);
        TransportClient transportClient = clientManager.getClient();
        SearchRequestBuilder searchRequestBuilder = transportClient.prepareSearch(searchRequest.getIndexes()).setTypes(searchRequest.getTypes()).setQuery(QueryBuilders.termQuery("goodsInfoId", goodsInfoId));
        // 执行查询,获取查询结果
        ...
        if(searchResponse.getHits().getTotalHits() > 0){
            return searchResponse.getHits().getAt(0).getSource().get("finalPrice").toString();
        }
        return null;
    }
}
