...
public class GoodsProductServiceImpl implements GoodsProductService {
            ...
            if (null != productVo) {
                ...
                if (null != relaProducts && !relaProducts.isEmpty()) {
                    for (GoodsRelatedGoodsVo relaProduct1 : relaProducts) {
                        GoodsProduct goodsProduct = goodsProductMapper.queryFirstProductByGoodsId(relaProduct1.getReleatedGoods().get(0).getGoodsId());
                        //查找352的图片
                        if(goodsProduct!=null&&goodsProduct.getGoodsInfoId()!=null){
                            String imgBigName = goodsProductMapper.queryProductImageBigName(goodsProduct.getGoodsInfoId());
                            if(StringUtils.isNotBlank(imgBigName)){
                                goodsProduct.setGoodsInfoImgId(imgBigName);
                            }
                            relaProduct.add(goodsProduct);
                        }
                    }
     ...
     */
    @Override
    public PageBean selectMarketingRushList(PageBean pb, HttpServletRequest request,Long tId,Long show) {
    ...
    @Override
    public List<GoodsProduct> queryRelatedByPid(Long goodsId) {
        ...
        if (null != relaProducts && !relaProducts.isEmpty()) {
            for (GoodsRelatedGoodsVo relaProduct1 : relaProducts) {
                list.add(goodsProductMapper.queryFirstProductByGoodsId(relaProduct1.getReleatedGoods().get(0).getGoodsId()));
            }
