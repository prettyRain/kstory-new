...
<head>
    ...
    <style>
        #goodsTitle th{text-align: center}
    </style>
    <style type="text/css">
        ...
        .order_details .total ul li p{
            width: 92px;
            text-align: right;
            display: inline-block;
        }
        .order_details .total ul li{
            width: 270px;
        }
        .ftx04{
            display: block;
...
<#if order??>
    <div class="order_state p10">
        ...
        <#--</#list>-->
        <#--</#if>-->
        <#--&lt;#&ndash;<p>订单已经完成，感谢您在商城购物，欢迎您对本次交易及所购商品进行评价。<a href="#">发表评价</a></p>&ndash;&gt;-->
        <#--</div>-->
        </#if>
    </div><!-- /order_state -->
    <div class="order_track mt20">
        <div class="tagMenu">
            <ul class="menu">
                <li class="current" data-localize="common.text">物流跟踪</li>
                <li>付款信息</li>
            </ul>
        </div>
                                ...
                                <tbody id="tbody_track">
                                    ...
                                    <tr>
                                            ...
                                            <#if relation.containers??>
                                                ...
                                                <#--<#if container.containerStatus=='0'>-->
                                                    <a class="img-box" title="${container.goodsProductDetailViewVo.goodsInfoName!''}"  target="_blank" href="${basePath}/item/${container.goodsProductDetailViewVo.goodsInfoId}.html" style="width:260px;white-space:nowrap;overflow:hidden;height:50px;display:inline-block;text-overflow:ellipsis;" ><img height="50px" style="width:50px;max-width:70px;" src="${container.goodsProductDetailViewVo.goodsInfoImgId}"/>
                                                        <#if container.containerStatus?? && container.containerStatus=="1"><span style='color: white;background-color: lightgrey;font-family: "Helvetica Neue", "Helvetica", "tahoma", "arial", "\5FAE\8F6F\96C5\9ED1", "\5b8b\4f53"!important;'>&nbsp;赠品&nbsp;</span>&nbsp;</#if>
                                        ...
                                        <td style="text-align:left;">
                                            ...
                                            <!--<div>${relation.expressInfoUrl!''}</div> -->
                                        </td>
                                    </tr>
                                    </#list>
                                </tbody>
                            ...
                            <#if relation??>
                                <table border="1" cellpadding="0" cellspacing="0" width="100%">
                                    ...
                                    <tbody id="tbody_track">
                                        ...
                                        <tr>
                                            <td style="text-align:left;">
                                                送货人姓名：${order.sendPerson!''}&nbsp;&nbsp;&nbsp;&nbsp;送货人手机：${order.sendMobile!''} &nbsp;&nbsp;&nbsp;&nbsp;
                                            </td>
                                        </tr>
                                        </#if>
                                    </tbody>
            ...
            <div class="layout">
                <table cellpadding="0" cellspacing="0" width="100%">
                    ...
                    <tr>
                        ...
                        <td  style="font-family: arial;">
                            积分兑换：￥<#if order.jfPrice??><#assign  yhPrice ="${yhPrice?number+order.jfPrice?number}"> ${(order.jfPrice)?string('0.00')}<#else>0.00</#if>
                        </td>
                        <td  style="font-family: arial;">
                            会员折扣：￥<#if order.discountPrice??><#assign  yhPrice ="${yhPrice?number+order.discountPrice?number}">${(order.discountPrice)?string('0.00')}<#else>0.00</#if>
                        </td>
                    </tr>
                    <tr>
                        <td  style="font-family: arial;">
                            优惠金额：￥<#if order.prePrice??><#assign  yhPrice ="${yhPrice?number+order.prePrice?abs}"><#if order.prePrice?string("0.00")?index_of("-")!=-1>${order.prePrice?string('0.00')?substring(1)}<#else>${order.prePrice?string("0.00")}</#if><#else>0.00</#if>
                        </td>

                        <td  style="font-family: arial;">
                            修改金额：￥<#if order.modifyPrice??>${(order.modifyPrice)?string('0.00')}<#else>0.00</#if>
                        </td>

                        <td  style="font-family: arial;">
                            <#--实际运费：￥<#if order.shippingFee??> ${(order.shippingFee)?string('0.00')} <#else>0.00</#if>-->
                            优惠券：￥<#if order.couponPrice?? ><#assign  yhPrice ="${yhPrice?number+order.couponPrice?number}"> ${(order.couponPrice)?string('0.00')}<#else>0.00</#if>
                        </td>
                        <td  style="font-family: arial;">
                            应支付金额：<span style="color:#f00;">￥<#if order.moneyPaid??>${(order.moneyPaid)?string('0.00')}<#else>0.00</#if></span>
                            <#if (order.giftcardPrice?? && order.giftcardPrice > 0 && order.orderStatus!='0')>
                                <span style="color: #FF0000;font-size: 12px;">（使用礼品卡支付￥<span>${order.giftcardPrice}</span>）</span>
                            </#if>
                        </td>
                    </tr>
                    ...
                    <dl class="fore">
                        ...
                        <dd>
                            <ul>
                                <li>
                                    <span>收货人：</span>
                                    <#if oldOrderAddress.person??>
                                    ${oldOrderAddress.person}</#if>
                                </li>
                                <#if oldCommunityStore??>
                                    <li>
                                        <span>社区店：</span>
                                    ${(oldCommunityStore.communityStoreName)!''}(
                                    ${(oldCommunityStore.province.provinceName)!''}
                                    ${(oldCommunityStore.city.cityName)!''}
                                    ${(oldCommunityStore.district.districtName)!''}
                                    ${(oldCommunityStore.communityStoreAddress)!''})
                                    ${oldCommunityStore.contact!''}
                                    ${oldCommunityStore.contactMobile!''}
                                    </li>
                                </#if>
                                <li>
                                    <span>地址：</span>
                                    <#if oldOrderAddress.province??>${oldOrderAddress.province}</#if>
                                    <#if oldOrderAddress.city??>${oldOrderAddress.city}</#if>
                                    <#if oldOrderAddress.county??>${oldOrderAddress.county}</#if>
                                    <#if oldOrderAddress.address??>${oldOrderAddress.address}</#if>
                                </li>
                                <li>
                                    <span>邮编：</span>
                                    <#if oldOrderAddress.postCode??>${oldOrderAddress.postCode}</#if>
                                </li>
                                <li>
                                    <span>固定电话：</span>
                                    <#if oldOrderAddress.phone??>${oldOrderAddress.phone}</#if>
                                </li>
                                <li>
                                    <span>手机号码：</span>
                                    <#if oldOrderAddress.mobile??>${oldOrderAddress.mobile}</#if>
                                </li>
                                <li>
                                    <span>收货方式：</span>
                                    <#if oldOrderAddress.expressType=='0'>
                                        送货上门-新规
                                    <#elseif oldOrderAddress.expressType=='1'>
                                        社区店自提
                                    </#if>
                                </li>
                            </ul>
                            ...
                            <dd>
                                ...
                                <#-- <#if order.orderExpressType == '1'>
                                     ...
                                     <#if communityStore??>
                                         <li>
                                             <span>社区店：</span>
                                         ${(communityStore.communityStoreName)!''}(
                                 ...
                                 <#else>-->
                                    ...
                                    <li>
                                        ...
                                        <#if order.orderExpressType=='1'>
                                            社区店自提
                                        <#elseif order.orderExpressType=='0'>
                                            送货上门-新规
                                        <#elseif order.orderExpressType=='9'>
                                            电子邮箱
                                        </#if>
                                    <#-- <span>配送方式：</span>
            ...
            <dl>
                ...
                <dd>
                    <ul>
                        ...
                        <li>
                            <span>联系电话：</span>
                            ${(deliveryPoint.telephone)!''}
                        </li>
                        <li>
                            <span>联系人：</span>
                            ${(deliveryPoint.linkman)!''}
                        </li>
                    </ul>
                </dd>
            </dl>
                        ...
                        <#if order.invoiceType??>
                            ...
                            <#if order.invoiceType != '0'>
                                ...
                                <li>
                                    <span>纳税人号：</span>
                                    <#if order.taxpayerNumber??>
                                    ${order.taxpayerNumber?default('无')}
                                    <#else>
                                        无
                                    </#if>
                                </li>
                ...
                <dd class="p_list">
                    <table cellpadding="0" cellspacing="0" width="100%">
                        ...
                        <#--  <th width="12%">
                            ...
                            <th width="42%" style="text-align: left;text-indent: 140px;">
                                商品
                            </th>
                            <th width="10%">
                                单价
                            ...
                            <#-- <td>
                                 <#if good.specVo??>
                                     ...
                                     <#list good.specVo as specVo>
                                         ${specVo.spec.specName}:<#if specVo.specValueRemark??&&specVo.specValueRemark!='undefined'>${specVo.specValueRemark}<#else>${specVo.goodsSpecDetail.specDetailName!''} </#if><br/>
                                     </#list>
                                 </#if>
                             </td>-->
                                <td>
                                    <div class="al fl">
                                        <a class="flk13" target="_blank" href="${basePath}/item/${good.goodsId}.html" ><#if good.isPresent?? && good.isPresent=="1"><span style='color: white;background-color: lightgrey;font-family: "Helvetica Neue", "Helvetica", "tahoma", "arial", "\5FAE\8F6F\96C5\9ED1", "\5b8b\4f53"!important;'>&nbsp;赠品&nbsp;</span>&nbsp;</#if>
                                            <#if good.goodsName??>
                                            ${good.goodsName}
                                ...
                                <td>
                                    <#if good.countRush?? && good.countRush?number!=0>
                                        <#if good.goodsNum?number gt good.countRush?number>
                                            ...
                                            <span class="ftx04" style="font-family: '微软雅黑'">
                                                    ￥<#if good.oldGoodsPrice??>
                                            ${good.oldGoodsPrice?string('0.00')}
                                            </#if>
                                                    x <#if good.goodsNum??>
                                ...
                                <td>
                                    <span class="ftx04" style="font-family: '微软雅黑'">
                                        <#if good.goodsNum?? && good.goodsPrice??>
                                            <#if good.countRush?? && good.goodsNum?number gt good.countRush?number && good.countRush?number!=0>
                                                ￥${((good.countRush?number*good.goodsPrice?number)+(good.goodsNum?number-good.countRush?number)*good.oldGoodsPrice?number)?string("0.00")}
                                            <#else>
                                                ￥${(good.goodsNum * good.goodsPrice)?string('0.00')}
                                            </#if>
									 	</#if>
                                    </span>
                                </td>
        ...
        <div class="total">
            <ul>
                <li>
                    ...
                    <p> ￥<#if order.oldPrice??> ${(order.oldPrice)?string('0.00')} </#if></p>
                </li>
                <li>
                            <span>
                                - 减扣金额：
                            </span>
                    <p>  ￥<#--<#if order.prePrice??> ${order.prePrice?string('0.00')} </#if>-->${yhPrice?number?string('0.00')}</p>
                </li>
                <li>
                            <span>
                                - 修改金额：
                            </span>
                    <p> ￥<#if order.modifyPrice??>${order.modifyPrice?string('0.00')}</#if></p>
                ...
                <li>
                    ...
                    <p>￥${order.shippingFee?string('0.00')}</p>
                </li>
            </ul>
