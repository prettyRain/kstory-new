...
//            int allpoint = info.getInfoPointSum();
            ...
            allpoint = allpoint - customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+"") < 0 ? 0 : allpoint - customerPointServiceMapper.getCustomerOrderSubtractPoint(customerId+"");
            // 扣除后的积分重新判断会员级别
        ...
        //如果是预存款支付的订单那么现在就退款到预存款

        /*if("5".equals(order.getPayId())){
            //如果订单用户不为空并且退款金额大于0
            if(customerId!=null&&(1==backprice.compareTo(new BigDecimal(0)))){
                Map<String,Object>  paramsMap = new HashMap<>();
                paramsMap.put("customerId",customerId);
                Deposit deposit = depositMapper.getDeposit(paramsMap);
                String orderCode = order.getOrderCode();
                Trade tTrade = new Trade();
                tTrade.setCustomerId(customerId);
                tTrade.setOrderPrice(backprice);
                tTrade.setCreatePerson(customerId);
                tTrade.setCreateTime(new Date());
                tTrade.setOrderCode(orderCode);
                //类型为订单消费
                tTrade.setOrderType("1");
...
//        }
     ...
     */
    @Override
    public BackOrder selectBackOrderByOrderCode(String orderCode) {
        return this.backOrderMapper.selectBackOrderByOrderCode(orderCode);
    }
     ...
     */
    @Override
    public List querySynBackOrder() {
        return this.backOrderMapper.querySynBackOrder();
    }
     ...
     */
    @Override
    public int synBackOrderSuccess(String orderCode) {
        return this.backOrderMapper.synBackOrderSuccess(orderCode);
    }
