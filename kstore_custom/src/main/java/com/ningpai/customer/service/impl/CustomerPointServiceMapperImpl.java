...
public class CustomerPointServiceMapperImpl implements CustomerPointServiceMapper {
     ...
     */
    @Override
    public int addIntegralByType(Long customerId, String type, Double orderPrice,String orderId,Date createTime) {
        if (type != null) {
            CustomerPoint customerPoint = new CustomerPoint();
            IntegralSet inte = integralSetmapper.findPointSet();
            CustomerInfo info = customerInfoMapper.selectByCustomerId(customerId);
            ...
            double ex = 100;
            if ("6".equals(type)) {
                int getPointPrice = orderPrice.intValue();
                //如果订单金额大于 > 指定金额20
                if(getPointPrice >= 20){
                    SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
                    try {
                        //若用户在个人资料中填写了生日信息，则用户生日当天生成的订单支付金额记双倍积分
                        if(info.getInfoBirthday() != null){
                            String[] c_array = sdf.format(createTime).split("-");
                            String[] b_array = sdf.format(sdf.parse(info.getInfoBirthday())).split("-");
                            if((c_array[1]+c_array[2]).equals(b_array[1]+b_array[2])){
                                getPointPrice = getPointPrice * 2;
                            }
                        }
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    customerPoint.setPoint(getPointPrice);
                }else{
            ...
            for (CustomerPointLevel level : pointLevelServiceMapper.selectAllPointLevel()) {
                String[] points = level.getPointNeed().split("~");
                if (Integer.valueOf(points[0]) <= (info.getInfoPointSum()-getCustomerOrderSubtractPoint(customerId+"")) && (info.getInfoPointSum()-getCustomerOrderSubtractPoint(customerId+"")) <= Integer.valueOf(points[1])) {
                    info.setPointLevelName(level.getPointLevelName());
                    info.setPointLevelId(level.getPointLelvelId());
                }
            }
    ...
    /**
     * 获得订单取消或退货退款的返回的积分和退货扣除的订单消费系统赠送积分
     */
    public Integer getCustomerOrderSubtractPoint(String customerId){
        if(StringUtils.isEmpty(customerId)){
            return 0;
        }
        Integer result = customerPointMapper.queryCustomerOrderSubtractPoint(customerId);
        if(result == null){
            return 0;
        }
        return customerPointMapper.queryCustomerOrderSubtractPoint(customerId);
    }
    /**
     * 获得 用户的总积分 （获得的-扣除的）
     *
