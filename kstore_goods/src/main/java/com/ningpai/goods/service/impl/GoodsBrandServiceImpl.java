...
public class GoodsBrandServiceImpl implements GoodsBrandService {
    ...
    public GoodsBrand selectGoodsBrandByBrandName(String brandName){
        return goodsBrandMapper.selectGoodsBrandByBrandName(brandName);
    }
    ...
    public int isBrandLinked(Long[] brandIds) {
        ...
        for (Long barndId : brandIds) {
            if (null != barndId) {
                // 如果有一个商品品牌 关联了商品 则直接返回1
                if (goodsBrandMapper.queryBrandLinkedCount(barndId) > 0) {
                    return 1;
                }
            }
        }
    ...
    public boolean checkThirdApplayBrandName(Long thirdId, String brandName) {
        return goodsBrandMapper.selectApplyBrandByBrandName(thirdId, brandName) > 0 ? false : true;
    }
    //商品认证标识添加
    @Override
    public void saveAuth(GoodsAuth goodsAuth) {
        goodsBrandMapper.saveAuth(goodsAuth);
    }
    //查询商品认证标识
    @Override
    public Object searchAuthByPageBean(PageBean pb) {
        //查询总条数
        Integer count=goodsBrandMapper.findAuthCount();
        Integer pageTotal=count%pb.getPageSize()==0 ? count/pb.getPageSize() : (count/pb.getPageSize())+1;
        Map map=new HashMap();
        Integer startNum=(pb.getPageNo()-1)*pb.getPageSize();
        map.put("startNum",startNum);
        map.put("pageSize",pb.getPageSize());
        List<Object> list=goodsBrandMapper.findAuthList(map);
        //从哪页开始
        Integer startNo=pb.getPageNo()-2>0?pb.getPageNo()-2:1;
        //从哪页结束
        Integer endNo=pb.getPageNo()+2>pageTotal?pageTotal:pb.getPageNo()+2;
        pb.setStartNo(startNo);
        pb.setEndNo(endNo);
        pb.setTotalPages(pageTotal);
        pb.setList(list);
        pb.setRows(count);
        return pb;
    }
    //删除商品认证标识
    @Override
    public void deleteGoodsAuth(Map map) {
        goodsBrandMapper.deleteAuth(map);
    }
    //查询所有商品认证标识
    @Override
    public List<Auth> findAllAuth() {
        return goodsBrandMapper.findAllAuth();
    }
    //商品认证标识关联商品
    @Override
    public void saveGoodsSaveAuthRelation(Map authMap) {
        goodsBrandMapper.saveGoodsSaveAuthRelation(authMap);
    }
    //查询与商品关联的认证标识
    @Override
    public List<GoodsAuth> findAuthByGoodsId(Long goodsId) {
        return goodsBrandMapper.findAuthByGoodsId(goodsId);
    }
    //xcy 删除商品与认证标识的关联
    @Override
    public void updateGoodsAuthRelation(Map deleteMap) {
        goodsBrandMapper.updateGoodsAuthRelation(deleteMap);
    }
    //xcy 根据goodsinfoID 查询商品认证标识
    @Override
    public List<Object> findAuthByGoodsInfoId(Long productId) {
        return goodsBrandMapper.findAuthByGoodsInfoId(productId);
    }
    //xcy 根据商品认证id 查询认证
    @Override
    public GoodsAuth queryAuthById(Long authId) {
        return goodsBrandMapper.queryAuthById(authId);
    }
    //xcy 根据id修改商品认证标识
    @Override
    public void updateGoodsAuth(GoodsAuth auth) {
        this.goodsBrandMapper.updateCoodsAuth(auth);
    }
    //xcy 查询所有品牌
    @Override
    public List<GoodsBrand> findBrands() {
        return this.goodsBrandMapper.findBrands();
    }
}
