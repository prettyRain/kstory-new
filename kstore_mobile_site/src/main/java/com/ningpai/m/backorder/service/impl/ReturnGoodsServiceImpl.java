...
public class ReturnGoodsServiceImpl implements ReturnGoodsService {
                ...
                if (!"".equals(bo.getBackGoodsIdAndSum())) {
                    ...
                    if (strs.length > 0) {
                        ...
                        for (int j = 0; j < strs.length; j++) {
                            ...
                            param.put("orderId",bo.getOrder().getOrderId());
                            OrderGoods orderGoods = backOrderMapper.selectBackOrderGoods(param);
                            bo.getOrderGoodsList().add(orderGoods);
    ...
    @Override
    public GoodsProductVo selectGoodsById(Long goodsId) {
        return this.backOrderMapper.selectGoodsById(goodsId);
    }

    @Override
    public OrderGoods selectBackOrderGoods(Map map) {
        return this.backOrderMapper.selectBackOrderGoods(map);
    }
