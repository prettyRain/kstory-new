...
import com.ningpai.goods.bean.GoodsAuth;
...
public class GoodsController {
    ...
    public ModelAndView test(PageBean pb, HttpServletRequest request, GoodsSearchBean searchBean, Long oneCateId, Long twoCateId) {
        ...
        try{
            ...
            pageNoInt = Integer.valueOf(pageNos);
        }catch(Exception e){
            LOGGER.error("",e);
            pageNoInt = 0;
        }
    ...
    @ResponseBody
    public List<Map<String,Object>> findReason(Long goodsId){
     ...
     * @return 视图 {@link org.springframework.web.servlet.ModelAndView}
     */
    @RequestMapping("/findAllGoodsAudit")
    public ModelAndView audit(PageBean pb, HttpServletRequest request, GoodsSearchBean searchBean, Long oneCateId, Long twoCateId) {
        Integer pageNoInt = 1;
        try{
            String pageNos = request.getParameter("pageNos");
            pageNoInt = Integer.valueOf(pageNos);
        ...
        ModelAndView mav = new ModelAndView("jsp/goods/goods_listAudit", "pb", this.goodsService.searchBySearchBeanAndPageBean(pb, searchBean)).addObject(SEARCHBEAN, searchBean)
                .addObject(ValueUtil.BRANDLIST, this.goodsBrandService.queryAllBrand()).addObject(ValueUtil.TAGLIST, this.goodsTagService.queryAllTag());
    ...
    public ModelAndView delGoods(Long goodsId, HttpServletRequest request) {
        ...
        // 删除索引
        searchAsyncService.batchDeleteGoodsIndexToEs(sList);
        // 非空验证 商品名称 是否删除成功
    ...
    public ModelAndView batchDelGoods(Long[] goodsIds, HttpServletRequest request) {
        ...
        OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), ValueUtil.BATCHDELGOODSINFO,
                (String) request.getSession().getAttribute(ValueUtil.OPERAPATH) + VARNAME2 + (String) request.getSession().getAttribute(ValueUtil.NAME));
        // 返回结果
        return new ModelAndView(new RedirectView(PathUtil.ALLGOODSCONTROLLER + ValueUtil.TOKENPARAM + CSRFTokenManager.getTokenFromRequest(request)));
    }
    ...
    public ModelAndView updateGoods(@Valid Goods goods, HttpServletRequest request, MultipartHttpServletRequest request2, int pageNo, int pageSize) {
        try {
            ...
            // 更新索引
            searchAsyncService.updateOneGoodsIndexToEs(goods.getGoodsId());
            // 后台boss的商品 1是第三方商家
    ...
    public ModelAndView updateGoodsNew(Goods goods, HttpServletRequest request, MultipartHttpServletRequest request2, int pageNo, int pageSize) {
        try {
            ...
            String[] tags = request.getParameterValues("update_goods_tags");
            //xcy 获取页面 选中的商品认证标识
            String[] auths=request.getParameterValues("update_goods_auths");
           ...
           //审核中
            ...
            //未审核  审核失败
            if("1".equals(goods.getGoodsAdded())){
    ...
    public ModelAndView updateGoods(Goods upGoods, HttpServletRequest request) {
        ...
        try {
            ...
            //审核状态
            goods.setAuditStatus(upGoods.getAuditStatus());
            // 获取页面中的值并设置商品的商品信息
            ...
            // 修改索引
            searchAsyncService.updateOneGoodsIndexToEs(upGoods.getGoodsId());
            // 非空验证 商品名称 和是否更新成功
    ...
    public ModelAndView queryByParamisthird(PageBean pb, GoodsSearchBean searchBean, Long oneCateId, Long twoCateId) {
        ...
        if (StringUtils.isEmpty(searchBean.getStatus())) {
    ...
    public ModelAndView toModifyGoodsNew(Long goodsId, int pageNo, int pageSize, Long isThird) {
        ...
        //xcy   authList auths
        List<GoodsAuth> authList=this.goodsBrandService.findAuthByGoodsId(goodsId);
        return new ModelAndView("jsp/goods/goodsModify").addObject("goodsMoifiedVo", goodsMoifiedVo).addObject(BRANDLIST, this.goodsBrandService.queryAllBrand())
                ...
                .addObject(TAGLIST, this.goodsTagService.queryAllTag()).addObject("goodsId", goodsId).addObject(PAGENO, pageNo).addObject("pageSize", pageSize)
                .addObject("isThird", isThirdNew).addObject("products", products).addObject  ("auths",goodsBrandService.findAllAuth()).addObject("authList",authList);
    }
    ...
    public ModelAndView batchCreateProduct(Long goodsId, HttpServletRequest request) {
        try {
            ...
            // 记录操作日志
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), "批量生成货品", (String) request.getSession().getAttribute(ValueUtil.OPERAPATH)
                    + VARNAME2 + (String) request.getSession().getAttribute(ValueUtil.NAME));
            // 返回视图
    ...
    public ModelAndView newUploadGoods(HttpServletRequest request) {
        return new ModelAndView("jsp/goods/newupload").addObject(BRANDLIST, this.goodsBrandService.queryAllBrand()).addObject(TAGLIST, this.goodsTagService.queryAllTag())
                .addObject("wareHouse", this.wareHouseService.queryAllWareHouse()).addObject("support", serviceSupportMapperService.selectAll()).addObject("auths",goodsBrandService.findAllAuth());
    }
    ...
    public void newUploadGood(Goods goods, HttpServletRequest request, HttpServletResponse resp, Long importGoodsId, String importGoodsIdArray) throws IOException {
        ...
        if("1".equals(goods.getGoodsAdded())){
            goods.setAuditStatus("1");
            goods.setGoodsAdded("0");
        }else if("0".equals(goods.getGoodsAdded())){
            goods.setAuditStatus("2");
        }
        ...
        //xcy 商品认证标识
        String[] auths=request.getParameterValues("auths");
    ...
    public void newUploadSaveGoodsDesc(Long goodsId, String goodsDetailDesc, String goodsMobileDesc, HttpServletResponse resp, HttpServletRequest request,String goodsSpecsificationDesc,String goodsWarpDesc,String goodsBrandDesc) throws IOException {
        PrintWriter out = resp.getWriter();
        // 实例化一个商品对象
        Goods goods = new Goods();
        // 设置商品的id为传过来的值
        ...
        if(StringUtils.isNotBlank(goodsSpecsificationDesc)){
            goods.setGoodsSpecsificationDesc(goodsSpecsificationDesc);
        }else{
            goods.setGoodsSpecsificationDesc("");
        }
        if(StringUtils.isNotBlank(goodsWarpDesc)){
            goods.setGoodsWarpDesc(goodsWarpDesc);
        }else{
            goods.setGoodsWarpDesc("");
        }
        if(StringUtils.isNotBlank(goodsBrandDesc)){
            goods.setGoodsBrandDesc(goodsBrandDesc);
        }else{
            goods.setGoodsBrandDesc("");
        }
    ...
    public ModelAndView batchUpdateStock(HttpServletRequest request, Long[] goodsIds, String stock) {
        ...
        int num = 0;
        if (stock != null) {
            num = this.goodsService.batchUpdateStock(request ,goodsIds, stock);
        }
        return new ModelAndView(new RedirectView(PathUtil.ALLGOODSCONTROLLER + ValueUtil.TOKENPARAM + CSRFTokenManager.getTokenFromRequest(request)));
    }
