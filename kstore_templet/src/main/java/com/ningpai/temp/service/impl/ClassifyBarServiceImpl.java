...
public class ClassifyBarServiceImpl implements ClassifyBarService {
    ...
    public int saveClassifyBar(ClassifyBar record) {
        try {
            // 大Boss分类导航
            if (record.getTemp1() == null && record.getGoodsCatId()!=null && record.getGoodsCatId() !=0 && record.getGoodsCatId() > -1) {
                // 是否自定义分类导航
                    // 获取当前商品分类
    ...
    public int updateClassifyBar(ClassifyBar record) {
        try {
            ...
            int i = classifyBarMapper.updateByPrimaryKeySelective(record);
            if(i==1 && record.getGoodsCatId() !=0){
                //同步更新np_classify_bar该分类id对应的分类名称（频道页、子商城页、首页）
                Map<String ,Object> map=new HashMap<>();
                map.put("cateName",record.getName());
                map.put("cateId",record.getGoodsCatId());
                goodsCateMapper.updateClassifybarByCateId(map);
                goodsCateMapper.updateClassifybarCateByCateId(map);
                //同步更新np_goods_category的分类名称
                com.ningpai.goods.bean.GoodsCate goodsCate = new com.ningpai.goods.bean.GoodsCate();
                goodsCate.setCatId(record.getGoodsCatId());
                goodsCate.setCatName(record.getName());
     ...
     */
    @Override
    public PageBean selectClassifyBarByParam(PageBean pb, Long tempId, Long channelId, String definedCate, String thirdId) {
        try {
            Map<String, Object> map = new HashMap<String, Object>();
            map.put("channelId", channelId);
            map.put("definedCate", definedCate);
    ...
    public List<ClassifyBarVo> selectClassifyBarByParamThirdSite(Long tempId, Long channelId, String thirdId) {
        try {
            ...
            map.put("tempId", tempId);
            map.put(TEMP1, thirdId);
            return classifyBarMapper.selectClassifyBarByParamThirdSite(map);
        } catch (Exception e) {
            LOGGER.error("分页查询分类导航列表错误:", e);
            return Collections.emptyList();
        }
    ...
    public int updateClassifyBarAndCateAndQuick(ClassifyBar record, List<ClassifyBarCate> barCates, List<ClassifyBarQuick> barQuicks) {
        ...
        try {
            ...
            if(count==1 && "0".equals(record.getDefinedCate())){
                //同步更新np_classify_bar该分类id对应的分类名称（频道页、子商城页、首页）
                Map<String ,Object> map=new HashMap<>();
                map.put("cateName",record.getName());
                map.put("cateId",record.getGoodsCatId());
            ...
            if (null != barCates && !barCates.isEmpty()) {
                for (ClassifyBarCate barCate : barCates) {
                    ...
                    barCate.setCateName(record.getName());
                    barCate.setClassifybarId(record.getClassifyBarId());
                    barCateMapper.insertSelective(barCate);
                }
            }
            if (null != barQuicks && !barQuicks.isEmpty()) {
                for (ClassifyBarQuick barQuick : barQuicks) {
                    ...
                    barQuick.setCateName(record.getName());
                    barQuick.setClassifybarId(record.getClassifyBarId());
                    barQuickMapper.insertSelective(barQuick);
                }
            }
     ...
     */
    @Override
    public List<ClassifyBarVo> getClassifyBarByProvinceId(String id) {
        return classifyBarMapper.getClassifyBarByProvinceId(id);
    }
