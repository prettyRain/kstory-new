    ...
    if(ch_spec.length==1){
        ...
        if(spec_vals.length>0){
            for(var i = 0;i<spec_vals.length;i++){
                if($(spec_vals[i]).prop("checked")){
                    ...
                    $("#tab"+(i+1)).find(".batch_set_price").val($("#tab"+(i+1)).find(".batch_set_price").parent().parent().prev().find(".ware_price").val());
                    ...
                    $('.productsImg').popover({
                        content : '最低350*350px，建议800*800px',
                        trigger : 'hover'
                    });
        ...
        if (empArray2.length == 0) {
            for (var x = 0; x < empArray3.length; x++) {
                ...
                $("#tab"+(x+1)).find(".batch_set_price").val($("#tab"+(x+1)).find(".batch_set_price").parent().parent().prev().find(".ware_price").val());
...
KindEditor.ready(function (K)
{
    editor3 = K.create('textarea[name="specsification_desc"]',
        {
            width:"100%",
            height:"425px",
            cssPath : 'js/kindeditor/plugins/code/prettify.css', uploadJson : 'js/kindeditor/jsp/upload_json.jsp',
...
KindEditor.ready(function (K)
{
    editor4 = K.create('textarea[name="warp_desc"]',
        {
            width:"100%",
            height:"425px",
            cssPath : 'js/kindeditor/plugins/code/prettify.css', uploadJson : 'js/kindeditor/jsp/upload_json.jsp',
...
KindEditor.ready(function (K)
{
    editor5 = K.create('textarea[name="brand_desc"]',
        {
            width:"100%",
            height:"425px",
            cssPath : 'js/kindeditor/plugins/code/prettify.css', uploadJson : 'js/kindeditor/jsp/upload_json.jsp',
...
function saveProduct(){
    ...
    $(".save_goods_info").append("<input type='hidden' name='sellType' value="+$(".sellType:checked").val()+">");
    // $(".save_goods_info").append("<input type='hidden' name='unitPrice' value="+$(".unitPrice").val()+">");
    ...
    //xcy auth
    var auths = $(".goods_auth:checked");
    if(null != auths && auths.length>0){
        for(var i =0;i<auths.length;i++){
            $(".save_goods_info").append("<input type='hidden' name='auths' value="+$(auths[i]).val()+" >");
        }
    }
    ...
    $(".goods_specsification_desc").val(editor3.html());
    $(".goods_warp_desc").val(editor4.html());
    $(".goods_brand_desc").val(editor5.html());
}
    ...
    if(null != pro_box && pro_box.length>0){
        for(var j=0;j<pro_box.length;j++){
            ...
            if(_pro.find(".product_spec").length>0){
                ...
                param+="&goodsInfoName="+encodeURIComponent(_pro.find(".name_input").val());
                param+="&unitPriceStr="+encodeURIComponent(_pro.find(".unitPriceStr").val());
                param+="&goodsInfoSubtitle="+encodeURIComponent(_pro.find(".des_input").val());
...
function callSaveChoooseImage(url, width, height) {
    ...
    $(".dinfo_wp").find(".active").find(".choose_imgs").append('<span class="inline"><a class="del_ts" href="javascript:;" onclick="del_ts(this)"><i class="glyphicon glyphicon-remove-sign"></i></a><input type="hidden" class="choose_img" value=' + url + '><a href="javascript:;"><img alt="" class="choose_pro_img" src=' + url + ' width="100" height="100" /></a></span>');
    ...
    if (currentIndex == allSize - 1) {
        if (errorIds.length == 0) {
            ...
            for (var i = 0; i < paths.length; i++) {
                var path3 = paths[i];
                $(".dinfo_wp").find(".active").find(".choose_imgs").append('<span class="inline"><a class="del_ts" href="javascript:;" onclick="del_ts(this)"><i class="glyphicon glyphicon-remove-sign"></i></a><input type="hidden" class="choose_img" value=' + path3 + '><a href="javascript:;"><img alt="" class="choose_pro_img" src=' + path3 + ' width="100" height="100" /></a></span>');
            }
...
function queryGoodsByParam(pageNo) {
    ...
    /*根据选中分类加载相关商品*/
    //goodsCateId=" +$('#parentId3').val()+"  xcy可以添加任何商品进行关联
    $.get("queryByParamAjax.htm?&pageNo="+pageNo+"&goodsNo="+goodsNo+"&goodsName="+goodsName+"&goodsBrandId="+goodsBrandId+"&showFlag=1&CSRFToken="+$("#CSRFToken").val(), function (data)
    {
        var aboutGoodsHtml = "";
        var list = data.list;
        if (list.length > 0)
...
function checkProductForm(){
    ...
    for(var i=0;i<forms.length;i++){
        ...
        var cashs = $(forms[i]).find(".cash");
        var cashReg =/^([1-9][0-9]*(\.[0-9]{1,2})?|0\.(?!0+$)[0-9]{1,2})$/;
        for(var j=0;j<cashs.length;j++){
            if(cashs[j].value!=''&&(cashReg.test(cashs[j].value))){
                $(cashs[j]).removeClass('error');
                $(cashs[j]).next('.error').remove();
            }else{
                if(cashs[j])
                $(cashs[j]).addClass('error');
                $(cashs[j]).next('.error').remove();
                $(cashs[j]).after('<label class="error">请输入大于0的合法金额</label>');
                f=f&&false;
            }
        }
