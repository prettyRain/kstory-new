...
<div class="mb20 container-fluid">
    <table class="table table-striped table-hover table-bordered">
        ...
        <tr>
            ...
            <td>${backorder.backPrice}
                <c:if test="${order.giftcardPrice != '' || order.giftcardPrice > 0}">
                    <span style="color: red;">（礼品卡账户支付 &yen;${order.giftcardPrice}）</span>
                </c:if>
            </td>
...
<c:if test="${businessId eq '0'}">
    <form id="submitForm" action="newsavebackorderprice.htm" type="post" method="post">
        <div class="sf-tit"><b>操作（重要 <c:if test="${order.payId eq '5'}"></c:if>）</b></div>
        ...
        <div class="sf-item">
            <c:if test="${backorder.isBack eq '2'}">
                <c:if test="${backorder.backCheck eq '6'}">
                    <label><input type="radio" id="backLogStatus1" checked="checked" name="backLogStatus" class="backLogStatus" value="8">
                        同意退款
                    </label>
