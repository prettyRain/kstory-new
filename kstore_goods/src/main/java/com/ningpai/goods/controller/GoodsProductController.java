...
public class GoodsProductController {
    ...
    public ModelAndView queryAllByGoodsId(HttpServletRequest request,Long goodsId, PageBean pb, SelectBean selectBean) {
        PageBean pbNew = pb;
        /**
         * 解决货品列表返回商品列表页面的问题
         * */
        Integer pageNo;
        try{
            pageNo = Integer.valueOf(request.getParameter("pageNos"));
        }catch(Exception e){
            LOGGER.error("",e);
            pageNo = 1;
        }
        ...
        if ("0".equals(g.getIsThird())) {
            ...
            return new ModelAndView(PathUtil.GOODSPRODUCT, "pb", pbNew).addObject(SPECS, this.goodsOpenSpecService.queryOpenListByGoodsIdInBoss(goodsId)).addObject(GOODSID, goodsId)
                    .addObject(SELECTBEAN, selectBean).addObject(WAREHOUSE, this.wareHouseService.queryAllWareHouse()).addObject(SUPPORT, serviceSupportMapperService.selectAll()).addObject("pageNo",pageNo);
        }
        // 第三方
    ...
    public ModelAndView delProduct(Long productId, Long goodsId, Integer flag, HttpServletRequest request) {
        ...
        // 删除索引
        searchAsyncService.deleteGoodsIndexToEs(productId);
        ...
        if (null == flag && null != goodsId && null != goodsProductVo.getGoodsInfoName()) {
            ...
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), ValueUtil.DELPRODUCTINFO,
                    (String) request.getSession().getAttribute(ValueUtil.OPERAPATH) + "名称【" + goodsProductVo.getGoodsInfoName() + "】-->用户名："
    ...
    public ModelAndView batchDelProduct(Long[] productIds, Long goodsId, Integer flag, HttpServletRequest request) {
        ...
        // 删除索引
        searchAsyncService.batchDeleteGoodsIndexToEs(list);


        if (null == flag && null != goodsId) {
            // 记录当前登录者的操作日志
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), ValueUtil.BATCHDELPRODUCTINFO, (String) request.getSession()
                    .getAttribute(ValueUtil.OPERAPATH) + LOGGERINFO1 + (String) request.getSession().getAttribute(ValueUtil.NAME));
    ...
    public ModelAndView batchUploadedProduct(Long[] productIds, Long goodsId, Integer flag, HttpServletRequest request) {
        ...
        // 批量上架，设置状态为1 表示上架状态      上架状态为“0”审核为“1” 表示在审核中
        this.goodsProductService.batchUploadProduct((String) request.getSession().getAttribute(ValueUtil.NAME), productIds, 0,1);
        // 更新索引
        searchAsyncService.updateOneGoodsIndexToEs(goodsId);
        if (null == flag && null != goodsId) {
            // 记录当前登录者的操作日志
    ...
    public ModelAndView batchDown(Long[] productIds, Long goodsId, Integer flag, HttpServletRequest request) {
        ...
        // 批量上架，设置状态为0 表示下架状态   上架状态为“0”审核为“2” 表示在下架
        this.goodsProductService.batchUploadProduct((String) request.getSession().getAttribute(ValueUtil.NAME), productIds, 0,2);
        // 修改索引
        searchAsyncService.updateOneGoodsIndexToEs(goodsId);
        if (null == flag && null != goodsId) {
            // 记录当前登录者的操作日志
    ...
    public ModelAndView saveProduct(@Valid GoodsProduct product, HttpServletRequest request, Long[] wareId, Long[] productStocks, BigDecimal[] productPrices, PageBean pb) {
        ...
        //如果为上架1时 变为审核中
        if("1".equals(product.getGoodsInfoAdded())){
            product.setAuditStatus("1");
            product.setGoodsInfoAdded("0");
        }else if("0".equals(product.getGoodsInfoAdded())){
        ...
        /* 保存分仓信息 */
        this.productWareService.calcProductWare(request ,Long.parseLong(String.valueOf(newId)), productStocks, productPrices, wareId);
        // 批量保存关联信息
        this.goodsProductSuppService.batchInsert(suppIds, newId);
        // 插入索引
        searchAsyncService.insertOneGoodsIndexToEs(product.getGoodsId());
        ...
        OperaLogUtil.addOperaLog(
                ...
                (String) request.getSession().getAttribute(ValueUtil.OPERAPATH) + "名称为【" + product.getGoodsInfoName() + "】,用户名："
                        + (String) request.getSession().getAttribute(ValueUtil.NAME));
    ...
    public ModelAndView udpateProduct(@Valid GoodsProduct goodsProduct, Integer flag, HttpServletRequest request, Long[] wareId, Long[] productStocks, BigDecimal[] productPrices,
            String oldPrice, PageBean pb) {
        // 打印日志
        LOGGER.info(ValueUtil.UPDATEPRODUCTINFO);
        if("0".equals(goodsProduct.getGoodsInfoAdded())){
            goodsProduct.setAuditStatus("2");
        }
        ...
        //当货品改为待审核状态的时候 商品也为待审核状态
        if ("0".equals(goodsProduct.getGoodsInfoAdded())&&"1".equals(goodsProduct.getAuditStatus())) {
            // 实例化一个商品对象
            Goods goods = new Goods();
            // 获取页面中的值并设置商品的商品id
            goods.setGoodsId(goodsProduct.getGoodsId());
            // 获取页面中的值并设置商品的是否上架
            goods.setGoodsAdded("0");
            goods.setAuditStatus("1");
            // 商品上架
            int a=this.goodsService.updateGoodsDescProduct(goods);
        }
        // 判断加入的规格值是否开启，如果没开启则开启该规格值
        ...
        for (int i = 0; i < wareId.length; i++) {
            // 判断该仓下面降价的商品对象
            List<FollowAndCityVo> andCityVo = wareHouseService.selectFollow(wareId[i], productPrices[i], goodsProduct.getGoodsInfoId());
            // 该仓对应的商品有被收藏
            if (null != andCityVo && !andCityVo.isEmpty()) {
                for (int j = 0; j < andCityVo.size(); j++) {
                    ...
                    if (infoEmails!=null && infoEmails.getInfoEmail() != null) {
                       // emailUtils.sendEmails(request, infoEmails.getInfoEmail(), goodsProduct);
                    }
                    if (mobiles !=null && mobiles.getInfoMobile() != null) {
                       // emailUtils.sendPosts(request, goodsProduct, mobiles.getInfoMobile());
                        //发送阿里大于短信通知 降价通知
                        AliSmsUtil.send(0, mobiles.getInfoMobile(), AliSmsUtil.smsTemplate.DROP_PRICE.getContent(), goodsProduct.getGoodsInfoName());
                    }
        ...
        // 保存关联信息
        this.productWareService.calcProductWare(request,goodsProduct.getGoodsInfoId(), productStocks, productPrices, wareId);
        /* 保存服务支持信息 */
        ...
        // 更新索引
        searchAsyncService.updateOneGoodsIndexToEs(goodsProduct.getGoodsId());
        if (null == flag) {
            // 记录当前登录者的操作日志
    ...
    public ModelAndView udpateProductSta(GoodsProduct goodsProduct, Integer flag, HttpServletRequest request, PageBean pb) {
        ...
        try {
            ...
            if (null != goodsProduct.getGoodsInfoAdded() && !"".equals(goodsProduct.getGoodsInfoAdded())) {
                ...
                //审核 状态   1审核中  2审核失败或为审核  3审核完成
                pro.setAuditStatus(goodsProduct.getAuditStatus());
                // 判断是否是上架操作
                if ("0".equals(goodsProduct.getGoodsInfoAdded())&&"1".equals(goodsProduct.getAuditStatus())) {
                    // 实例化一个商品对象
                    Goods goods = new Goods();
            ...
            if (null == flag && null != goodsProductVo.getGoodsInfoName()) {
                // 操作日志
                OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), ValueUtil.UPDATEPRODUCTINFO, (String) request.getSession()
                        .getAttribute(ValueUtil.OPERAPATH) + "货品名称【" + goodsProductVo.getGoodsInfoName() + "】,用户名：" + (String) request.getSession().getAttribute(ValueUtil.NAME));
                // 重定向到queryAllByGoodsId.htm中
    ...
    /**
     * 满赠促销时，搜索参与促销的货品（同一件货品只能参加一次满赠促销）
     * @param request
     * @param pb
     * @param searchBean
     * @param marketType
     * @return
     */
    @RequestMapping("/queryProductForFullbuyPresent")
    @ResponseBody
    public PageBean queryProductForFullbuyPresent(HttpServletRequest request, PageBean pb, GoodsSearchBean searchBean, Long marketType) {
        ...
        // 返回结果
        return this.goodsProductService.queryProductListForPresent(pb, (Long) request.getSession().getAttribute(THIRDID), searchBean, marketType);
    }
    ...
    public void exportProductList(Long goodsId, SelectBean selectBean, HttpServletResponse response) {
        ...
        try {
            ExportGoodsList.exportProductList(this.goodsProductService.queryAllProductByGoodsIdForExport(goodsId, selectBean),productWareService, response);
        } catch (Exception e) {
            // 打印日志
    ...
    public void exportProductPage(Long goodsId, PageBean pb, SelectBean selectBean, HttpServletResponse response) {
        ...
        try {
            ExportGoodsList.exportProductList(this.goodsProductService.queryByGoodsId(goodsId, pb, selectBean).getList(),productWareService, response);
        } catch (Exception e) {
            // 打印日志
    ...
    public void exportProductByChecked(Long[] productIds, HttpServletResponse response) {
        ...
        try {
            ExportGoodsList.exportProductList(this.goodsProductService.queryAllProductByProductIdsForExport(productIds),productWareService, response);
        } catch (Exception e) {
            // 打印日志
    ...
    public int newUploadProduct(@Valid GoodsProduct product, HttpServletRequest request, Long[] ware, Long[] wareStock, BigDecimal[] warePrice, String addedTime) {
        ...
        try {
            ...
            /* 保存分仓信息 */
            this.productWareService.calcProductWare(request, Long.parseLong(String.valueOf(newId)), wareStock, warePrice, ware);
            /* 保存图片 */
            this.goodsPub.getGoodsImageService().batchSaveImageWithDefaultPic(images, newId, (String) request.getSession().getAttribute(ValueUtil.NAME));
            /* 保存服务支持 */
