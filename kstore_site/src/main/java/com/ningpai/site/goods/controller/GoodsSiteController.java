...
import com.ningpai.site.goods.vo.GoodsShowListVo;
...
public final class GoodsSiteController {
    ...
    public ModelAndView esGoodsList(Long cid, Long pid, HttpServletRequest request, HttpServletResponse response, SearchPageBean<EsGoodsInfo> pageBean, String[] brands,
        ...
        if (null != request.getSession().getAttribute(DISTINCTID)) {
            ...
            //session中没有ip定位的区域id时，就根据ip查询地区信息
            Map distinctMap = customerServiceMapper.getDistinctByIP(request);
            distinctId = (Long) distinctMap.get("ipDistinctId");
            /*distinctId = addressService.getDefaultIdService();*/
            if (distinctId == null) {
                distinctId = 1103L;
            }
        ...
        SearchPageBean searchPageBean = (SearchPageBean)resultMap.get("pb");
        List list = searchPageBean.getData();
        for(int i=0 ; i<list.size() ; i++){
            EsGoodsInfo esGoodsInfo = (EsGoodsInfo)list.get(i);
            List marketings = this.marketingService.selectMarketingByGoodsInfoId(esGoodsInfo.getGoodsInfoId(),esGoodsInfo.getBrandId(), esGoodsInfo.getCatId());
            if(marketings!=null && marketings.size()>0){
               esGoodsInfo.setMarketings(marketings);
            }
        }
        // 展示分类
        // 查询父亲分类
        ...
        Long catParentId = cateVo.getParentCat().getCatParentId();
        Channel channel = new Channel();
        if(catParentId != null){
            if(catParentId == 0){
                catParentId = cateVo.getCatParentId();
            }
            //根据货品父类型查询对应的频道
            channel = this.cateService.queryChannelByCatId(catParentId);
            resultMap.put("channel",channel);
        }else{
            channel = this.cateService.queryChannelByCatId(cid);
            resultMap.put("channel",channel);
        }
        String channelPath = request.getRealPath("/") + "/view/index" + "/newheader960_"+channel.getChannelId()+".html";
        if(!new File(channelPath).exists()){
            //商品列表的导航与商品所在频道对应
            request.setAttribute("channel",channel);
            topAndBottomService.staticizeTop960(request);
        }
        //将列表页的商品库存更换成实时库存，与详情页一致
        SearchPageBean<EsGoodsInfo> pb = (SearchPageBean<EsGoodsInfo>) resultMap.get("pb");
        List<EsGoodsInfo> data = pb.getData();
        for (EsGoodsInfo esGoodsInfo : data) {
            GoodsDetailBean goodsDetailBean = productService.querySimpleDetailBeanByProductId(esGoodsInfo.getGoodsInfoId());
            if(wareId == null || goodsDetailBean == null || goodsDetailBean.getProductVo() == null){
                esGoodsInfo.setGoodsInfoStock(0L);
            }else {
                List<ProductWare> productWares = goodsDetailBean.getProductVo().getProductWares();
                Long totalWare = 0L;
                for (ProductWare productWare : productWares) {
                    if(productWare.getWareId() == wareId) {
                        totalWare += productWare.getWareStock();
                    }
                }
                esGoodsInfo.setGoodsInfoStock(totalWare);
            }
            //追加部分评论带来的销量
            Integer plusNum = commentServiceMapper.queryPlusNumByGoodsId(esGoodsInfo.getGoodsInfoId());
            esGoodsInfo.setSaleCount(esGoodsInfo.getSaleCount() + plusNum);
        }
        ...
        //三级分类列表，热销排行展示对应的二级分类所有商品，并根据促销情况，显示现价
        if(cateVo.getCatGrade()==3){
            GoodsCateVo parentCat = new GoodsCateVo();
            parentCat.setCatId(cateVo.getParentCat().getCatId());
            parentCat.setCatGrade(cateVo.getParentCat().getCatGrade());
            model.addObject(HOTSALES, changeMarketingPrice(productService.queryHotSalesTopSixRandom(gcv,parentCat,6)));
        }else {
            model.addObject(HOTSALES, changeMarketingPrice(productService.queryHotSalesTopSixRandom(gcv, cateVo, 6)));
        }
    ...
    public ModelAndView goodsDetail(Long productId, Long distinctId, String chAddress, HttpServletRequest request, HttpServletResponse response)
        ...
        if (null == distinctId1) {
            if (null != request.getSession().getAttribute(ValueUtil.ADDRESS) && null == request.getSession().getAttribute(CHADDRESS)) {
                ...
                if (null == request.getSession().getAttribute(CHADDRESS)) {
                    //session中没有ip定位的区域id时，就根据ip查询地区信息
                    Map distinctMap = customerServiceMapper.getDistinctByIP(request);
                    Long dId = (Long) distinctMap.get("ipDistinctId");
                    /*Long dId = addressService.getDefaultIdService();*/
                    if (dId == null) {
                        dId = 1103L;
                        // 默认设置为南京建邺区仓库
        ...
        GoodsDetailBean detailBean = this.productService.queryDetailBeanByProductId(productId, distinctId1);
            if (null != detailBean.getProductVo()) {
                ...
                Long catParentId = detailBean.getCateVo().getParentCat().getCatParentId();
                if(catParentId != null){
                    //根据货品父类型查询对应的频道
                    Channel channel = this.cateService.queryChannelByCatId(catParentId);
                ...
                //相关产品显示现价
                detailBean.setReleProductList(changeMarketingPrice(detailBean.getReleProductList()));
                //套装显示现价
                List<GoodsGroupVo> groupVos = detailBean.getGroupVos();
                for (GoodsGroupVo groupVo : groupVos) {
                    ...
                    for (GoodsGroupReleProductVo goodsGroupReleProductVo : productList) {
                        GoodsProductDetailVo productDetail = goodsGroupReleProductVo.getProductDetail();
                        BigDecimal finalPrice = productDetail.getGoodsInfoPreferPrice();
                        //查询商品是否有促销，有就显示促销价
                        List<GoodsMarketingCodeUtil> codeUtils = goodsElasticMapper.selectProductMarket(productDetail.getGoodsInfoId());
                        for (GoodsMarketingCodeUtil gms : codeUtils) {
                            if (gms.getCodexId() == 1) {
                                ...
                                finalPrice = productDetail.getGoodsInfoPreferPrice().subtract(discount);
                            } else if (gms.getCodexId() == 10) {
                                //团购
                                BigDecimal discount = goodsElasticMapper.selectGrouponByMarketId(gms.getMarketingId());
                                finalPrice = productDetail.getGoodsInfoPreferPrice().multiply(discount);
...
//                map.put(FINALBUY, productService.browCatFinalBuyAndPrecent(detailBean.getProductVo().getGoods().getCatId(), 4L));
                ...
                //xcy 根据goodsinfo id 查询商品认证标识
                map.put("auths",this.goodsBrandService.findAuthByGoodsInfoId(productId));
...
//                map.put("goodsBrands", cateAndBrandmap.get("goodsBrands"));
                map.put("commentCount",this.goodsService.getPrductCommentCount(productId));
                // 保存浏览记录
    ...
    @RequestMapping("/queryHistoryPros")
    @ResponseBody
    public Map queryHistoryPros(HttpServletRequest request,Long cateIdOne,Long cateIdTwo,Long goodsId){
        //热销排行
        GoodsCateVo goodsCateVoOne = this.goodsCateService.queryCateById(cateIdOne);
        GoodsCateVo goodsCateVoTwo = this.goodsCateService.queryCateById(cateIdTwo);
        List<GoodsProduct> productList=this.productService.queryHotSalesTopSixRandom(goodsCateVoOne,goodsCateVoTwo,6);
        if(productList.size()<4){
            List<GoodsProduct> productListRela=this.productService.queryRelatedByPid(goodsId);
            if(productListRela.size()>0&&productListRela!=null){
                productList.addAll(productListRela);
            }
        }
        //如果里面的重复的  就消除重复 start
        Map relaMap=new HashMap();
        for(int i=0 ; i<productList.size() ; i++){
            relaMap.put(productList.get(i).getGoodsInfoId().toString(),productList.get(i));
        }
        List listrela=new ArrayList();
        Set<Map.Entry> entrySet = relaMap.entrySet();
        for(Map.Entry entry:entrySet){
            listrela.add(entry.getValue());
        }
        //消除重复结束
        Map<String,Object> map=this.browerService.loadBrowHist(request);
        map.put("listrela",changeMarketingPrice(listrela));
        return map;
    }
    /**
...
//        map.put("goodsBrands", cateAndBrandmap.get("goodsBrands"));
    ...
    public ModelAndView allComment(Long productId, HttpServletRequest request) {
        ...
        try {
            if (null != detailBean.getProductVo() && null != detailBean.getCateVo().getParentCat()) {
                map.put(DETAILBEAN, detailBean);
                Channel channel = this.cateService.queryChannelByCatId(detailBean.getCateVo().getParentCat().getCatParentId());
                map.put("channel",channel);
                String channelPath = request.getRealPath("/") + "/view/index" + "/newheader960_"+channel.getChannelId()+".html";
                if(!new File(channelPath).exists()){
                    //商品列表的导航与商品所在频道对应
                ...
                sb.append("<a style='font-size: 14px;color: #020202;letter-spacing: 1.17px;'  href='/index.html'>首页</a>");
                sb.append("&nbsp;&nbsp;&gt;&nbsp;&nbsp;");
                //隐藏内有大聚会面包屑
                if(channel.getChannelId()!=247L) {
                    sb.append("<a style='font-size: 14px;color: #020202;letter-spacing: 1.17px;' href='/channel/" + channel.getChannelId() + ".html'>" + channel.getChannelName() + "</a>");
                    sb.append("&nbsp;&nbsp;&gt;&nbsp;&nbsp;");
                }
                sb.append("<a style='font-size: 14px;color: #020202;letter-spacing: 1.17px;' href='/list/"+detailBean.getCateVo().getParentCat().getCatId()+"-"+detailBean.getCateVo().getParentCat().getCatParentId()+".html'>"+detailBean.getCateVo().getParentCat().getCatName()+"</a>");
                map.put("location",sb);
                /* 查询商品关联的标签 */
                map.put("tags", this.goodsPub.getGoodsReleTagService().queryreleListByProductId(detailBean.getProductVo().getGoodsInfoId()));
    ...
    public ModelAndView toActivityPage(){
        return topAndBottomService.getTopAndBottom(new ModelAndView("activity/autumn"));
    }
     ...
     * @return
     */
    @RequestMapping("/toNewYearActivity")
    public ModelAndView toNewYearActivity(){
        return topAndBottomService.getTopAndBottom(new ModelAndView("activity/activity1"));
    }
     ...
     * @return
     */
    @RequestMapping("/toRegisterActivity")
    public ModelAndView toRegisterActivity(){
        return topAndBottomService.getTopAndBottom(new ModelAndView("activity/activity2"));
    }
     ...
     * @return
     */
    @RequestMapping("/toActivity3")
    public ModelAndView toActivity3(){
        return topAndBottomService.getTopAndBottom(new ModelAndView("activity/activity3"));
    }
    ...
    private List<GoodsProduct> changeMarketingPrice(List<GoodsProduct> list){
        if(list==null || list.size()==0){
            return null;
        }
            for (GoodsProduct goods : list) {
                BigDecimal finalPrice = goods.getGoodsInfoPreferPrice();
                //查询商品是否有促销，有就显示促销价
                List<GoodsMarketingCodeUtil> codeUtils = goodsElasticMapper.selectProductMarket(goods.getGoodsInfoId());
                for (GoodsMarketingCodeUtil gms : codeUtils) {
                    if (gms.getCodexId() == 1) {
                        //直降
                        BigDecimal discount = goodsElasticMapper.selectPriceOffMarketingByMarketingId(gms.getMarketingId());
                        finalPrice = goods.getGoodsInfoPreferPrice().subtract(discount);
                    } else if (gms.getCodexId() == 10) {
                        //团购
                        BigDecimal discount = goodsElasticMapper.selectGrouponByMarketId(gms.getMarketingId());
                        finalPrice = goods.getGoodsInfoPreferPrice().multiply(discount);
                    } else if (gms.getCodexId() == 11) {
                        //抢购
                        BigDecimal discount = goodsElasticMapper.selectRushByMarketId(gms.getMarketingId());
                        finalPrice = goods.getGoodsInfoPreferPrice().multiply(discount);
                        ...
                        //折扣
                        Map<String, Object> param = new HashMap<String, Object>();
                        param.put("goodsId", goods.getGoodsInfoId());
                        param.put("marketingId", gms.getMarketingId());
                        BigDecimal discount = goodsElasticMapper.selectDiscountByMarketId(param);
                        finalPrice = goods.getGoodsInfoPreferPrice().multiply(discount);
                    }
                }
                goods.setGoodsInfoMarketPrice(finalPrice);
            }
            return list;
    }
}
