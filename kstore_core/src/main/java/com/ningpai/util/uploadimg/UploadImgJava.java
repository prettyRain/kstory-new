...
public class UploadImgJava {
    ...
    public Map<String, String> uploadForABCSize(MultipartFile muFile) {
        ...
        if (muFile != null && muFile.getSize() != 0) {
            ...
            try {
                ...
                // 获取原图地址
                LOGGER.info(LOGGERINFO1 + (url + fileNamess).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH));
                imgMap.put(OLDIMG, (url + fileNamess).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH));
                ...
                for (int i = 0; i < imgSet.size(); i++) {
                    ...
                    LOGGER.info(width + LOGGERINFO2 + (url + UploadImgCommon.picPath + UploadImgCommon.prefix + "!" + width + UploadImgCommon.suffix)
                            .replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"), IMGPATH));
    ...
    /**
     * 获得当前项目访问的地址 比如http://127.0.0.1:8080
     *
     * @param request
     * @return 返回当前项目访问的地址
     */
    ...
    public Map<String, String> uploadForOldAndSmall(MultipartFile muFile) {
        ...
        if (muFile != null && muFile.getSize() != 0) {
            ...
            try {
                muFile.transferTo(file);
                // 获取原图地址
                LOGGER.debug(LOGGERINFO4 + (url + fileNamess).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH));
                imgMap.put(OLDIMG, (url + fileNamess).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH));
                ...
                String resoultPath = getResoultPath(fileNamess, widthAndHidth, widthAndHidth);
                // 获取小图片地址
                LOGGER.debug(LOGGERINFO5 + SMALL + LOGGERINFO2 + (url + resoultPath).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH));
                imgMap.put(_SMALL, url + resoultPath.replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"), IMGPATH));
    ...
    public String uploadForRichEdit(FileItem item) {
        ...
        if (item != null && item.getSize() != 0) {
            ...
            return (url + fileNamess).replace(UploadImgCommon.readValue().getProperty("IMAGEPATH"),IMGPATH);
        } else {
            return null;
        }
    }
    ...
    private String getResoultPath(String baisc, String fileNamess, String width, String height) {
        ...
        ImageCompressUtil.zipImageFile(fileNamess, Integer.valueOf(width), Integer.valueOf(height), 1f, resoultPath);
//        try {
...
//        }
    ...
    public static String getPicNamePathSuffix() {
        ...
        // 根据真实路径创建目录文件
        File picSaveFile = new File(UploadImgCommon.picPath);
        if (!picSaveFile.exists()) {
            try {
                picSaveFile.mkdirs();
            } catch (Exception e) {
     ...
     * @return
     */
    private String getResoultPath(String fileNamess, String width, String height) {
        // 处理后保存的图片地址
        String resoultPath = UploadImgCommon.picPath + UploadImgCommon.prefix + "!" + width + UploadImgCommon.suffix;
        ImageCompressUtil.zipImageFile(fileNamess, Integer.valueOf(width), Integer.valueOf(height), 1f, resoultPath);
        /*try {
            Thumbnails.of(fileNamess).size(Integer.valueOf(width), Integer.valueOf(height)).keepAspectRatio(false).toFile(resoultPath);
        } catch (IOException e) {
