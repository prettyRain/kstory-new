...
public class GoodsServiceImpl implements GoodsService {
    ...
    public PageBean searchGoods(PageBean pb, GoodsSiteSearchBean searchBean, Long catId, String[] params, String distinctId) {
        Map<String, Object> map = new HashMap<String, Object>();
        List<Object> goodsList = new ArrayList<Object>();
        String brandId = ObjectUtils.toString(searchBean.getBrandId());
        ...
        try {
            ...
            if (catId != null && catId.longValue() > 0) {
                ...
                this.cateService.calcAllSonCatIds(cate, catIds);
                map.put("catIds", catIds);
            }else if(catId != null && catId == -1){//如分类导航没有关联商品分类，直接显示没有相关的商品
                catIds.add(-1l);
                map.put("catIds", catIds);
            }
    ...
    //start xcy  分类过来的商品列表
    public PageBean searchGoods(Map paramMap,PageBean pb, GoodsSiteSearchBean searchBean, Long catId, String[] params, String distinctId) {
        Map<String, Object> map = new HashMap<String, Object>();
        List<Object> goodsList = new ArrayList<Object>();
        String brandId = ObjectUtils.toString(searchBean.getBrandId());
        List<String> paramsList = new ArrayList<String>();
