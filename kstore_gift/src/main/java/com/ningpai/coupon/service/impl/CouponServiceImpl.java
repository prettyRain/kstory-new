...
public class CouponServiceImpl implements CouponService {
    ...
    public CustomerPoint selectCustomerPointByCustomerId(Long customerId) {
        ...
        if (null != customerPoint) {
            ...
            customerPoint.setPointSum(Long.valueOf(customerPointServiceMapper.getCustomerAllPoint(customerId + "")));
        }
        ...
        try {
            ...
            if (couponlist != null && !couponlist.isEmpty()) {
                for (int i = 0; i < couponlist.size(); i++) {
                    //如果是固定时间段 自己算出结束时间 用于页面展示结束时间
                    if("1".equals(((Coupon)(couponlist.get(i))).getTimeType())){
                        Long couponDays = ((Coupon) (couponlist.get(i))).getCouponDays();
                        long longTime = ((Coupon) (couponlist.get(i))).getCodeGetTime().getTime() + couponDays * 24 * 60 * 60 * 1000;
                        ((Coupon) (couponlist.get(i))).setCouponEndTime(new Date(longTime));
                    }
                    // 直降
    ...
    @Override
    public List<Coupon> selectThirdCouponListByIds(List<ParamIds> list, HttpServletRequest request, Long third) {
        Map<String, Object> paramMap = new HashMap<String, Object>();
        paramMap.put("list", list);
        paramMap.put(CUSTOMERID, (Long) request.getSession().getAttribute(CUSTOMERID));
     ...
     */
    @Override
    public List<Coupon> selectGoodsDetailCouponListByIdsNew(List<ParamIds> list,Long thirdId, HttpServletRequest request) {
        Map<String, Object> paramMap = new HashMap<String, Object>();
        paramMap.put("list", list);
        paramMap.put("thirdId",thirdId);
        List<Coupon> couponlist = couponMapper.selectGoodsDetailCouponListByIdsNew(paramMap);
        if (couponlist != null && !couponlist.isEmpty()) {
            for (int i = 0; i < couponlist.size(); i++) {
                // 直降
                if ("1".equals(couponlist.get(i).getCouponRulesType())) {
     ...
     */
    @Override
    public List<Coupon> selectCouponListByIdsNew(List<ParamIds> list,Long thirdId, HttpServletRequest request) {
        Map<String, Object> paramMap = new HashMap<String, Object>();
        paramMap.put("list", list);
        paramMap.put("thirdId",thirdId);
    ...
    public int modifyNoStatusNew(String couponNo, String codeStatus) {
        ...
        // 要修改的优惠劵
        no.setCodeStatus(codeStatus);
        return couponNoMapper.modifyNoStatusNew(no);
    }
    ...
    public List<Coupon> selectCouponListByCustomerId(Long customerId) {
        List<Coupon> couponlist=  couponMapper.selectCouponListByCustomerId(customerId);
        if (couponlist != null && !couponlist.isEmpty()) {
            for (int i = 0; i < couponlist.size(); i++) {
                // 直降
                if ("1".equals(((Coupon) couponlist.get(i)).getCouponRulesType())) {
    ...
    public List<Object> myCouponListM(Long customerId, String codeStatus) {
        ...
        try {
            // 查询列表页
            couponlist = couponMapper.myCouponList(paramMap);
            if (couponlist != null && !couponlist.isEmpty()) {
                for (int i = 0; i < couponlist.size(); i++) {
                    //如果是固定时间段 自己算出结束时间 用于页面展示结束时间
                    if("1".equals(((Coupon)(couponlist.get(i))).getTimeType())){
    ...
    public int myCouponListCount(Map<String, Object> paramMap) {
        return couponMapper.myCouponListCount(paramMap);
    }
     ...
     */
    @Override
    public Coupon queryMZCoupon(Long customerId, Long couponId) {
        //根据优惠券id查询优惠券
        Coupon coupon = this.searchCouponById(couponId);
        // 查询被领取的优惠券总数
        ...
        if ((countAll - couponNoService.queryUsedCountByCouponId(couponId).intValue()) > 0){
            if(customerId!=null){
                // 查询该用户此优惠券领取的张数
                int counts = couponNoService.selectReadyGet(couponId, customerId);
                // 如果等于0代表没有领取，可以进行领取
                if (counts < coupon.getCouponGetNo().intValue()) {
                    //起止时间
                       Date d = new Date();
                       if("0".equals(coupon.getTimeType())){
    ...
    public void updateCouponNo(CouponNo couponNo){
          couponMapper.updateCouponNo(couponNo);
   }
     ...
     */
    @Override
    public void modifyCouponByOrderIdAndCustomerId(Long orderId, Long customerId) {
        Map map = new HashMap();
        map.put("orderId",orderId);
        map.put("customerId",customerId);
        couponMapper.modifyCouponByOrderIdAndCustomerId(map);
    }
     ...
     */
    @Override
    public void cancelOrderCoupon(Long orderId) {
        couponMapper.cancelOrderCoupon(orderId);
    }
