...
<jsp:include page="../page/header.jsp"></jsp:include>
              ...
              <table class="table order_table">
                ...
                <tr>
                  ...
                  <td align="center">
					...
					<c:if test="${order.orderStatus eq 1}"><span class="label label-danger">已付款未发货</span></c:if>
					<c:if test="${order.orderStatus eq 2}"><span class="label label-info">已发货</span></c:if>
					<c:if test="${order.orderStatus eq 3}"><span class="label label-info">已完成</span></c:if>
					<c:if test="${order.orderStatus eq 4}"><span class="label label-default">已取消</span></c:if>
                      <c:if test="${order.orderStatus eq 13}"><span class="label label-default">拒绝退款</span></c:if>
                  </td>
                  <td align="center">
                  	<c:if test="${order.orderCargoStatus eq 0}"><span class="label label-danger">未拣货</span></c:if>
					<c:if test="${order.orderCargoStatus eq 1}"><span class="label label-danger">未装箱</span></c:if>
					<c:if test="${order.orderCargoStatus eq 2}"><span class="label label-danger">未出库</span></c:if>
					<c:if test="${order.orderCargoStatus eq 3}"><span class="label label-info">已出库</span></c:if>
                  </td>
          ...
          <div class="modal-body">
            <form class="form-horizontal" id="updateOrder">
              <div class="form-group">
              	...
              	<input  type="hidden" id="cargo" value="${order.orderCargoStatus}" />
                ...
                <div class="col-sm-10">
                	<textarea class="form-control required" rows="5" cols="60" id="orReason" name="reason" maxlength="250"></textarea>
                </div>
              </div>
            </form>
          </div>
  ...
  <div class="modal fade" id="outWarehouse"  role="dialog">
      <div class="modal-dialog" style="width:1000px">
          <div class="modal-content">
              ...
              <div class="modal-footer">
                  <button type="button" class="btn btn-primary" onclick="doPrint3()">打印物流单</button>
                  <button type="button" class="btn btn-primary" onclick="doPrint2()">打印出库单</button>
  ...
  <div class="modal fade" id="packing"  role="dialog">
      <div class="modal-dialog modal-lg" id="updatexcy">
          <div class="modal-content">
              <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal"   onclick="updateOrderUpdateStateOne($('#orderIdId').val(),$('#csrf_token').val()+'','2')"  ><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
              ...
              <div class="modal-body">
                  ...
                  <div class="modal-footer">
                      ...
                      <button type="button" class="btn btn-default" data-dismiss="modal" onclick="updateOrderUpdateStateOne($('#orderIdId').val(),$('#csrf_token').val()+'','2')">取消</button>
                  </div>
              </div>
          </div>
      </div>
  ...
  <div class="modal fade" id="changePackage"  role="dialog">
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body">
                  <form role="form" action="updatecontainer.htm" id ="updatecontainer" class="form-horizontal">
                      <div class="form-group">
                          ...
                          <div class="col-sm-11">
                            ...
                            <input  type="hidden" name="CSRFToken" id="CSRFToken" value="${sx}" />
                              <select class="form-control" name="relationId" id="relationIds" >
  ...
  <form action="printView.htm?CSRFToken=${sx}" id="printView" method="post" target="_blank" >
      ...
      <input name="personName" value="" type="hidden"/>
      <input name="cartName" value="" type="hidden"/>
      <input name="cartPersonName" value="" type="hidden"/>
      <input name="orderId" value="" type="hidden"/>
      <input name="logisticsName" value="" type="hidden"/>
      <input name="logisticsMobileName" value="" type="hidden"/>
  ...
  <input type="hidden" id="orderIdId">
  ...
  <script>
    $(function () {
        if($("#cargo").val() == "1"){
            $("input[name=sign]").val("2");
        }
      $(".modal").on("hidden.bs.modal", function () {
        $(".modal").each(function () {
          if (this.style.display == 'block') {
            $("#bodynode").addClass("modal-open");
          }
        })
      })
    })
