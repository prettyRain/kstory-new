...
function showResponse(responseData) {
		 ...
		 if(returnjson.pb.list!=null && returnjson.pb.list.length >0 ){
			 for(var i=0; i<returnjson.pb.list.length; i++) {
                 ...
                 // nextpagehtml +='<h3 class="item-head"><label for="">退款金额：&nbsp;&nbsp;￥</label><span class="pay-money">';
                 if(backorder.isBack != null && backorder.isBack !=''){
                     if(backorder.isBack == "2"){
                         nextpagehtml +='<h3 class="item-head"><label for="">退款金额：&nbsp;￥</label><span class="pay-money">';
                     }else{
                         nextpagehtml +='<h3 class="item-head"><label for="">退货金额：&nbsp;￥</label><span class="pay-money">';
                     }
                 }
                 ...
                 nextpagehtml +='<i>'+backorder.backPrice.toFixed(2).split(".")[0]+'</i><i>.'+backorder.backPrice.toFixed(2).split(".")[1]+'</i>';
                 ...
                 if(backorder.orderGoodsList !=null){

                     if(backorder.orderGoodsList.length > 2){
                         nextpagehtml +='<div class="list-item">';
                         nextpagehtml +='<div class="box-body">';
                         nextpagehtml +='<ul>';
                         for(var n=0;n<backorder.orderGoodsList.length;n++){
                             var backgoods = backorder.orderGoodsList[n];
                             nextpagehtml +='<li>';
                             ...
                             if(backorder.isBack != null && backorder.isBack !=''){
                                 if(backorder.isBack == "2"){
                                     nextpagehtml +='<a title="'+backgoods.goodsInfoName+'" href="'+basePath+'/customer/backorderpricedetail-'+backorder.order.orderId+'.html">';
                                 }else{
                                     nextpagehtml +='<a title="'+backgoods.goodsInfoName+'" href="'+basePath+'/customer/backordergoodsdetail-'+backorder.order.orderId+'.html">';
                                 }
                             }
							 nextpagehtml +='<img width="78" height="78" title="'+backgoods.goodsInfoName+'" alt="'+backgoods.goodsInfoName+'" src="'+backgoods.goodsImg+'" />';
                             nextpagehtml +='</a>';
                             nextpagehtml +='</li>';
                         }
                         nextpagehtml +='</ul>';
                         ...
                         for(var m=0;m<backorder.orderGoodsList.length;m++){
                             var backgoods = backorder.orderGoodsList[m];
                             ...
                             nextpagehtml +='</h3>';
                         if (!!backgoods.countRush&& backgoods.countRush < backgoods.goodsInfoNum) {
                             nextpagehtml += '<div>'
                                 + '<p class="price">¥&nbsp;'
                                 + '<span class="num" style="display: initial">' + backgoods.goodsInfoOldPrice.toFixed(2).substring(0, backgoods.goodsInfoOldPrice.toFixed(2).indexOf('.')) + '</span>'
								 +'<span  style="font-size: 12px; display: inline-block;vertical-align: bottom;">' + backgoods.goodsInfoOldPrice.toFixed(2).substring(backgoods.goodsInfoOldPrice.toFixed(2).indexOf('.')) + '</span>'
                                 + '</p>'
                                 + '<span class="pro-num">×' + (backgoods.goodsInfoNum - backgoods.countRush) + '</span>'
                                 + '</div>'
                                 + '<div>'
                                 + '<p class="price">¥&nbsp;'
                                 + '<span class="num" style="display: initial">' + backgoods.goodsInfoPrice.toFixed(2).substring(0, backgoods.goodsInfoPrice.toFixed(2).indexOf('.')) + '</span>'
								 +'<span style="font-size: 12px;display: inline-block;vertical-align: bottom;">' + backgoods.goodsInfoPrice.toFixed(2).substring(backgoods.goodsInfoPrice.toFixed(2).indexOf('.')) + '</span>'
                                 + '</p>'
                                 + '<span class="pro-num">×' + backgoods.countRush + '</span>'
                                 + '</div>';

                         }else{
                             nextpagehtml += '<div style="visibility:hidden;">'
                                           +'<p class="price" >¥&nbsp;'
                                           +'<span class="num" style="display: initial">'+backgoods.goodsInfoPrice.toFixed(2).substring(0,backgoods.goodsInfoPrice.toFixed(2).indexOf('.'))+'</span>'
								           +'<span style="font-size: 12px;display: inline-block;vertical-align: bottom;">'+backgoods.goodsInfoPrice.toFixed(2).substring(backgoods.goodsInfoPrice.toFixed(2).indexOf('.'))+'</span>'
                                           +'</p>'
                            ...
                            /* nextpagehtml +='<p class="price">¥&nbsp;<span class="num" style="display: initial">';
