...
public class SiteDepositService extends DepositService {
    ...
    public JSONObject pay(BigDecimal totalFee, Deposit deposit, String payPassword, Long orderId, String orderCode) {
        ...
        if ("-1".equals(retcode) || "-2".equals(retcode)) {
            return checkjson;
        }
    ...
    public void genTradeByOrder(Order order,Long customerId){
        ...
        if(order.getGiftcardPrice() != null && order.getGiftcardPrice().compareTo(new BigDecimal(0)) > 0){
            tTrade.setOrderPrice(order.getGiftcardPrice());
            //总账减去订单金额
            deposit.setPreDeposit(preDeposit.subtract(order.getGiftcardPrice()));
            updateParam.put("preDeposit", preDeposit.subtract(order.getGiftcardPrice()));
        }else{
            updateParam.put("preDeposit", preDeposit.subtract(totalFee));
            tTrade.setOrderPrice(totalFee);
            //总账减去订单金额
...
//            }
    ...
    public JSONObject preCheck(BigDecimal totalFee, Deposit deposit, String payPassword, Long orderId, String orderCode) {
        ...
        giftcardVo.setGiftcardNo(deposit.getGiftcardNo());
        String activeFlag = "0";
        try {
        ...
        retjson.put("activeFlag",activeFlag);
        //余额账户的支付密码未设置
...
//            Customer customer = customerServiceInterface.queryCustomerById(customerId);
            String encodePwd = SecurityUtil.getStoreLogpwd(customerId.toString(), payPassword, deposit.getUserSaltVal());
            //输入的密码和数据库不一样
