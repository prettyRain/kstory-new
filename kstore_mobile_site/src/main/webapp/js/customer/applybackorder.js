...
$(function(){
    ...
    $("#submitButtonprice").click(function() {
        ...
        if($(".flag_saved").val()=="1"){
            ...
            //$("#upload-form").submit();
            $.ajax({
                url:"/back/keepBackOrderInfoprice.htm",
                type:"post",
                ...
                success:function (orderCode) {
                    if(orderCode!=null){
                        //ERP添加退款单
                        $.ajax({
                            url:"/synAddBackOrderPrice.htm",
                            type:"post",
                            data:"orderCode="+orderCode,
                            dataType:"json",
                            async:false,
                            success:function (result) {
                                location.href = "/applyBackSuccess.htm?orderCode="+orderCode;
                            }
                        })
                    }else{
                        location.href = "/customer/myorder.html";
                    }
                }
            })
        }
    ...
    $("#submitButtongoods").click(function() {
    	...
    	if($(".flag_saved").val()=="1"){
            ...
            if (!new RegExp(regu).test($("#backPrice").val())) {
                alertStrNew("填写金额有误");
                return false;
            }
            if (Number($("#backPrice").val()) > Number($("#maxBackPrice").text())) {
                alertStrNew("填写金额大于最高金额");
                return false;
            }
            if($("#num").text()>200){
                alertStrNew("问题描述字数过多");
    		...
    		/*if ($("#backPrice").val() == '') {
                $('body').append('<div class="tip-box" style="z-index:99999"><div class="tip-body"><h3>请输入退款金额！</h3></div></div>');
                setTimeout(function(){
                    $('.tip-box').remove();
                },1000);
                return;
    		}*/
            /*if ($("#backPrice").val() > $("#maxBackPrice").val()) {
                $('body').append('<div class="tip-box" style="z-index:99999"><div class="tip-body"><h3>退款金额不能超过退额上限！</h3></div></div>');
                setTimeout(function(){
                    $('.tip-box').remove();
                },1000);
                return;
            ...
            //$("#upload-form").submit();
            $.ajax({
                url:"/back/keepBackOrderInfo.htm",
                type:"post",
                data:$("#upload-form").serialize(),
                dataType:"json",
                success:function (orderCode) {
