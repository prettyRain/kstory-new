    ...
    public ModelAndView queryMyConpons(HttpServletRequest request,
        ...
        try {
            ...
            if (LoginUtil.checkLoginStatus(request)) {
                ...
                mav.setViewName("customer/functiondeveloping");
        ...
        if (cId == null) {
            ...
            if (counts < coupon.getCouponGetNo().intValue()) {
                ...
                if ((countAll - couponNoService.queryUsedCountByCouponId(
                    ...
                    }else if("1".equals(coupon.getTimeType())){
                        // 返回劵码
                        CouponNo couponNo = couponNoService
                                .selectNoByCouponIdByStatus(couponId);
                        if (couponNo != null) {
    ...
    public ModelAndView addCoupons(HttpServletRequest request) {
        ...
        try {
            ...
            if (LoginUtil.checkLoginStatus(request)) {
                ...
                //纳税人识别号
                resultMap.put("taxpayerNumber",request.getParameter("taxpayerNumber"));
                resultMap.put("addressId",request.getParameter("addressId"));
                resultMap.put("orderComment",request.getParameter("orderComment"));
                resultMap.put("deliveryPointId",request.getParameter("deliveryPointId"));
                resultMap.put("typeId",request.getParameter("typeId"));
                resultMap.put("ch_pay",request.getParameter("ch_pay"));
                //使用优惠券编号
