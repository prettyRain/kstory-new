...
//            }
    ...
    public ModelAndView backOrderPriceDetail(HttpServletRequest request, Long orderId){
        ...
        if (LoginUtil.checkLoginStatus(request)) {
            ...
            if (!"".equals(bOrder.getBackGoodsIdAndSum())) {
                // 获取退单对象 下面的退单的 商品Id
                String[] strs = bOrder.getBackGoodsIdAndSum().split("-");
                // 处理数据
                if (strs.length > 0) {
                    // 遍历退单对象下面的商品Id
                    for (int j = 0; j < strs.length; j++) {
                        ...
                        // 根据ID获取单个商品的详细信息
                        GoodsProductVo orderProductVo = returnGoodsService.selectGoodsById(goodsId);
                        // 循环把查询获取到的商品信息放入到退单对象的商品集合中
                        bOrder.getOrderGoodslistVo().add(orderProductVo);
                        bOrder.setBackNum(backNum);
                        //根据订单id和货品id查退货商品信息
                        Map param = new HashMap();
                        param.put("orderGoodsId",orderGoodsId);
                        param.put("orderId",orderId);
                        param.put("goodsId",goodsId);
                        OrderGoods orderGoods = returnGoodsService.selectBackOrderGoods(param);
                        bOrder.getOrderGoodsList().add(orderGoods);
            ...
            /*Collections.sort(backOrderLogs, new Comparator<BackOrderLog>() {
                @Override
                public int compare(BackOrderLog o1, BackOrderLog o2) {
                    return (int)(o1.getBackLogTime().getTime()-o2.getBackLogTime().getTime());
                }
            });*/
