...
public class MarketingServiceImpl implements MarketingService {
    ...
    public Marketing marketingDetail(Long marketingId, Long goodsInfoId) {
        ...
        if (market != null) {
            ...
            if ("6".equals(market.getCodexType())) {
                List<FullbuyPresentMarketing> list = new ArrayList<>();
                // 查询所有此促销的赠品级联list
                ...
                if (pmList != null && !pmList.isEmpty()) {
                    for (int i = 0; i < pmList.size(); i++) {
                        List<FullbuyPresentScope> scopes = fullbuyPresentScopeMapper.selectPresentsByPresentMarketingId(pmList.get(i).getFullbuyPresentMarketingId());
                        pmList.get(i).setFullbuyPresentScopes(scopes);
                        list.add(pmList.get(i));
                    }
                    market.setFullbuyPresentMarketings(list);
     ...
     * @param goodsNum 数量
     * @param goodsInfoPrice 价格
     * @return Marketing对象
     */
    @Override
    public Marketing marketingDetailNew(Long marketingId, Long goodsInfoId, Long goodsNum, BigDecimal goodsInfoPrice) {
        Marketing market = marketingMapper.marketingDetail(marketingId);
        if (market != null) {
            // 直降
            if ("1".equals(market.getCodexType())) {

                PriceOffMarketing priceOffMarketing = priceOffMarketingMapper.selectPriceOffMarketingByMarketingId(marketingId);
                market.setPriceOffMarketing(priceOffMarketing);
                market.setProductReduceMoney((BigDecimal.valueOf(priceOffMarketing.getOffValue().intValue()).multiply(BigDecimal.valueOf(goodsNum))));
            }
            // 买送赠品
            if ("2".equals(market.getCodexType())) {
                // 查询所有此促销的赠品级联list
            ...
            // 满减优惠
            if ("5".equals(market.getCodexType())) {
                List<FullbuyReduceMarketing> list = fullbuyReduceMarketingMapper.selectFullbuyReduceMarketingsByMarketingId(marketingId);
                market.setFullbuyReduceMarketings(list);
                market.setProductReduceMoney(list.get(0).getReducePrice());
            }
            // 满赠赠品
            if ("6".equals(market.getCodexType())) {
                // 查询所有此促销的赠品级联list
            ...
            // 满折
            if ("8".equals(market.getCodexType())) {
                List<FullbuyDiscountMarketing> dislist = fullbuyDiscountMarketingMapper.selectFullbuyDiscountMarketingsByMarketingId(marketingId);
                market.setFullbuyDiscountMarketings(dislist);
                market.setProductReduceMoney((goodsInfoPrice.multiply(BigDecimal.ONE.subtract(dislist.get(0).getFullbuyDiscount()))).multiply(BigDecimal.valueOf(goodsNum)));
            }
            // 限购
            if ("9".equals(market.getCodexType())) {
                market.setLimitBuyMarketing(limitBuyMarketingMapper.selectLimitBuyMarketingByMarketingId(marketingId));
            ...
            // 抢购
            if ("11".equals(market.getCodexType())) {
                List<MarketingRush> rushList = rushMapper.selectByMarketId(marketingId);
                market.setRushs(rushList);
                market.setProductReduceMoney((goodsInfoPrice.multiply(BigDecimal.ONE.subtract(rushList.get(0).getRushDiscount()))).multiply(BigDecimal.valueOf(goodsNum)));
            }
            // 满x件打y折
            if ("13".equals(market.getCodexType())) {
                market.setFullbuyNoDiscountMarketing(fullbuyNoDiscountMarketingMapper.selectByMarketId(marketingId));
    ...
    public Marketing marketingDetail(Long marketingId) {
        ...
        if (market != null) {
            ...
            if ("6".equals(market.getCodexType())) {
                ...
                List<FullbuyPresentMarketing> pmList = fullbuyPresentMarketingMapper.selectFullbuyPresentMarketingListByMarketingId(marketingId);
                if (pmList != null && !pmList.isEmpty()) {
                    market.setFullbuyPresentMarketings(pmList);
                    market.setFullbuyPresentMarketing(pmList.get(0));
                    List<Long> list = new ArrayList<Long>();
                    for (int i = 0; i < pmList.size(); i++) {
                        list.add(pmList.get(i).getProductId());
        ...
        if (market != null) {
            ...
            if ("6".equals(market.getCodexType())) {
                ...
                if (pmList != null && !pmList.isEmpty()) {
                    for (int i = 0; i < pmList.size(); i++) {
                        List<FullbuyPresentScope> scopes = fullbuyPresentScopeMapper.selectPresentsByPresentMarketingId(pmList.get(i).getFullbuyPresentMarketingId());
                        for(FullbuyPresentScope scope :scopes){
                            //赠品范围类型0商品1优惠券
                            if("1".equals(scope.getScopeType())){
                                //得到优惠券id
                                Long couponId = scope.getScopeId();
            ...
            // 满数量赠赠品
            if ("16".equals(market.getCodexType())) {
                List<FullbuyPresentMarketing> list = new ArrayList<>();
                // 查询所有此促销的赠品级联list
                List<FullbuyPresentMarketing> pmList = fullbuyPresentMarketingMapper.selectFullbuyPresentMarketingListByMarketingId(marketingId);
                if (pmList != null && !pmList.isEmpty()) {
    ...
    @Override
    @Transactional
    public int doAddPresentMarketing(Marketing marketing, String codexType, HttpServletRequest request, String status) {
        // 初始化促销
        marketing.setCreateTime(new Date());
        marketing.setModTime(new Date());
        marketing.setFlag("0");
            ...
            if (goodsIds != null && goodsIds.length != 0) {
                for (int i = 0; i < goodsIds.length; i++) {
                    ...
                    if (goodsInfo != null) {
                        MarketingScope ms = new MarketingScope();
                        ms.setCreateTime(new Date());
                        ms.setModTime(ms.getCreateTime());
                        ms.setFlag(flage);
                        ms.setMarketingId(marketingId);
                        ms.setScopeId(goodsInfo.getGoodsInfoId());
                        ms.setScopeType("2");
    ...
    private void judgeCodexType(String codexType, HttpServletRequest request, String flage, Long marketingId, BigDecimal fullPrice) {
        ...
        if ("1".equals(codexType)) {
            ...
            if(StringUtils.isNotBlank(request.getParameter("tId").toString())){
                rush.settId(Long.valueOf(request.getParameter("tId")));
            }
            ...
            rush.setMobileRushImage(request.getParameter("mobileRushImage"));
            rush.setMarketId(marketingId);
            rush.setTemp1(request.getParameter("temp1"));
    ...
    private void newJudgeCodexType(Marketing marketing, String codexType, HttpServletRequest request, String flage, Long marketingId, BigDecimal fullPrice) {
        ...
        if ("1".equals(codexType)) {
            ...
            String[] modes = request.getParameterValues("presentMode");
            String[] gifts;
            String[] giftCoupons;
            String[] scopeNums;
            String[] couponScopNums;
            // 获取多级满多少元
            String[] prices = request.getParameterValues(MarketingValue.FULLPRICE);
            // 判断是否为空
            if (prices != null && prices.length > 0) {
                // 循环变量多级促销
                for (int i = 0; i < prices.length; i++) {
                    if(prices[i] != null && prices[i] != "" && !"".equals(prices[i])){
                        FullbuyPresentMarketing fpm = new FullbuyPresentMarketing();
                        for (int m = 0; m < modes.length; m++) {
                            if(i==m){
                                fpm.setPresentMode(modes[m]);
                            }
                        }
                        fpm.setDelFlag(flage);
                        fpm.setFullPrice(BigDecimal.valueOf(Double.valueOf(prices[i])));
                        fpm.setMarketingId(marketingId);
                        //赠品类型0商品1优惠券
                        fpm.setGiftType("0");
                        //赠送类型0金额1件数
                        fpm.setPresentType(request.getParameter("presentType"));
                        // 插入满赠
                        fullbuyPresentMarketingMapper.insertSelective(fpm);
                        giftCoupons = request.getParameterValues("couponIdPG"+(i+1));
                        couponScopNums = request.getParameterValues("couponScopeNum"+(i+1));
                        gifts = request.getParameterValues("goodsIdPG"+(i+1));
                        scopeNums = request.getParameterValues("scopeNum"+(i+1));
                        //赠商品
                        if (gifts != null && gifts.length != 0) {
                            for (int j = 0; j < gifts.length; j++) {
                                Long id = fpm.getFullbuyPresentMarketingId();
                                FullbuyPresentScope fps = new FullbuyPresentScope();
                                fps.setFullbuyPresentMarketingId(id);
                                fps.setCreateTime(new Date());
                                fps.setDelFlag("0");
                                fps.setScopeId(Long.valueOf(gifts[j]));
                                fps.setScopeType("0");//赠送商品
                                for (int k = 0; k < scopeNums.length; k++) {
                                    if(j==k){
                                        fps.setScopeNum(Long.valueOf(scopeNums[j]));
                                    }
                                }
                                fullbuyPresentScopeMapper.insertSelective(fps);
                            }
                        }
                        //赠优惠券
                        if(giftCoupons != null && giftCoupons.length != 0){
                            for(int m = 0; m < giftCoupons.length; m++){
                                Long id = fpm.getFullbuyPresentMarketingId();
                                FullbuyPresentScope fps = new FullbuyPresentScope();
                                fps.setFullbuyPresentMarketingId(id);
                                fps.setCreateTime(new Date());
                                fps.setDelFlag("0");
                                fps.setScopeId(Long.valueOf(giftCoupons[m]));
                                fps.setScopeType("1");//赠送优惠券
                                for (int n = 0; n < couponScopNums.length; n++) {
                                    if(m==n){
                                        fps.setScopeNum(Long.valueOf(couponScopNums[n]));
                                    }
                                }
                                fullbuyPresentScopeMapper.insertSelective(fps);
                            }
            ...
            if (goodsIds != null && goodsIds.length != 0 && discountInfo != null && discountInfo.length != 0) {
                for (int i = 0; i < goodsIds.length; i++) {
                    ...
                    if (discountFlag != null && discountFlag.length != 0 && "0".equals(marketing.getIsAll())) {
                        pdm.setDiscountFlag(discountFlag[i]);
                    }
                    // 全部商品
     ...
     * @param marketingId 促销id
     */
    private void delAllPresentShip(Long marketingId) {
        //满赠规则list
        List<FullbuyPresentMarketing> list = fullbuyPresentMarketingMapper.selectFullbuyPresentMarketingListByMarketingId(marketingId);
        if(list != null && list.size() > 0){
            for (int i = 0; i < list.size(); i++) {
                fullbuyPresentScopeMapper.deleteByPresentMarketingId(list.get(i).getFullbuyPresentMarketingId());
            }
        }
        // 删除满赠规则
        fullbuyPresentMarketingMapper.updateFullbuyPresentMarketing(marketingId);
    ...
    public List<Marketing> selectMarketingByGoodsInfoId(Long goodsInfoId, Long brandId, Long cateId) {
        ...
        //团购促销中间变量
        Marketing groupMarketing=null;
        Long group=0L;
        //折扣促销中间变量
        Marketing zhekouMarketing=null;
        Long zhekou=0L;
        //满金额赠促销中间变量
        Marketing fullbuyPresentMarketing=null;
        Long fullcash=0L;
     ...
     */
    @Override
    public int selectMarketingByGoodsInfoIdWithDay(Long goodsInfoId, Long brandId, Long cateId) {
        Map<String, Object> paramMap = new HashMap<String, Object>();
        paramMap.put(GOODSINFOID, goodsInfoId);
        paramMap.put(BRANDIDP, brandId);
        paramMap.put(CATEID, cateId);
        // 查询所在范围的促销
        return marketingScopeMapper.selectMarketScopeByMapMay2(paramMap);
    }
    ...
    * */
    @Override
    public List<Object> queryTDates() {
        return rushTimeMapper.queryTDates();
    }
     ...
     */
    @Override
    public MarketingRushTime queryrushtimeById(Long tId) {
        return rushTimeMapper.queryrushtimeById(tId);
    }
    ...
    @Override
    public long queryByCreatimeMarketings(Long goodsId,Long cateId, Long brandId) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put(GOODSINFOID, goodsId);
        map.put(CATEID, cateId);
        map.put(BRANDIDP, brandId);
     ...
     */
    @Override
    public long queryByCreatimeMarketingsWithPanicBuying(Long goodsId, Long cateId, Long brandId) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put(GOODSINFOID, goodsId);
        map.put(CATEID, cateId);
        map.put(BRANDIDP, brandId);
        List<Marketing> marketing = marketingMapper.queryByCreatimeMarketingsWithPanicBuying(map);
        // 判断该商品是否存在优惠
        if (marketing == null || marketing.isEmpty()) {
            return 0;
        }
    ...
    @Transactional
    @Override
    public int doUpdatePresentMarketing(Marketing marketing, String codexType, String status, HttpServletRequest request) {
        String flage = "0";
        // 查询促销
        Marketing mt = this.marketingMapper.marketingDetailNotTime(marketing.getMarketingId());
        if ((mt.getBusinessId()).equals(marketing.getBusinessId())) {
            // 删除促销关联信息
            this.delAllPresentShip(marketing.getMarketingId());
            // 修改时间
            marketing.setModTime(new Date());
            int modifyFlag = marketingMapper.modifyMarketing(marketing);
            if (modifyFlag == 1) {
