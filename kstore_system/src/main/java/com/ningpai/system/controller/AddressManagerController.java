...
public class AddressManagerController {
     ...
     * 地区管理里面 添加地址到地址列表
     */
    @RequestMapping("/addAddressToAddressList")
    @ResponseBody
    public int addAddressToAddressList(Long districtId){
        return this.addressService.insertAddressToAddressList(districtId);
    ...
    @ResponseBody
    public String queryAddressList(Integer currentPage,Integer pageSize){

        Map<String ,Object> map = this.addressService.queryAddressList(currentPage,pageSize);
        if(map == null){
            return "";
        }
        ...
        for(ProvinceDefault provinceDefault:provinceDefaultList){
            Long defaultDistrictId = provinceDefault.getDistrictId();
            // 默认地区信息
            District district = null;
            if (defaultDistrictId != null) {
                district = districtService.findDistrictByPrimaryKey(defaultDistrictId);
                provinceDefault.setDistrict(district);
            }
            // 默认城市
            City city = null;
            if (district != null) {
                city = cityService.findCityByPrimaryKey(district.getCityId());
                provinceDefault.setCity(city);
            }
            // 默认省份
            Province province = null;
            if (city != null) {
                province = provinceService.findProvinceByPrimaryKey(city.getProvinceId());
                provinceDefault.setProvince(province);
            }
    ...
    @RequestMapping(value="/saveUpdateAddress",method = RequestMethod.GET)
    @ResponseBody
    public String saveUpdateAddress(HttpServletRequest request){
        Long distictId = Long.valueOf(request.getParameter("distictId"));
        Long did = Long.valueOf(request.getParameter("did"));
       return this.addressService.saveUpdateAddress(distictId,did);
    ...
    @ResponseBody
    public String setTheDefault(Long districtId) {
        //设置默认地址
        addressService.insertAddressDefaultService(districtId);
        // 默认地区ID
        Long defaultDistrictId = addressService.getDefaultIdService();
        District district = null;
        if (defaultDistrictId != null) {
    ...
    @ResponseBody
    public String deleteAddress(String arrString){
        if(StringUtils.isNotBlank(arrString)){
         String[] arr = arrString.split(",");
         List list = new ArrayList();
         for(String str:arr){
             list.add(Long.valueOf(str));
         }
         this.addressService.deleteAddressNew(list);
        }
        return "";
    }
    /**
     ...
     * @param provinceName
     * @return int
     * @author houyichang 2016/02/18
     * */
    @RequestMapping("/ajaxqueryprovincename")
    @ResponseBody
    public int queryProvinceNameCount(String provinceName){
        //查询该省份名称是否已存在，如果已存在则不给添加
     ...
     * @author houyichang 2016/02/18
     * */
    @RequestMapping("/ajaxquerydistrictname")
    @ResponseBody
