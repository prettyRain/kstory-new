  ...
  <div class="user_box">
    ...
    <form action="${basePath}/registerFirst.htm" method="post" id="registerFirstForm">
      ...
      <div class="reg_form_item">
        <label class="for_text">&nbsp;</label>
        <div class="item">
          <button type="submit" class="sub_btn nextGrey" onclick="submitFirst()">同意协议并注册</button>
          <a href="/giftcardregister.html" style="position: absolute;right: -120px;top: 2px;font-weight: bold;font-size: 16px;">礼品卡注册</a>
        </div>
      </div>
    </div>
...
<script>
  $(function(){
      ...
      $('.text').on('input change',function(){
          ...
          if(($.trim(mobile)!=="")&&($.trim(checkCode)!=="")&&($("#readme").prop("checked"))){
