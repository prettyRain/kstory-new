...
&lt;#&ndash;一引入头部 <#include "/index/topnew.ftl" />  &ndash;&gt;
<#if (topmap.temp)??>
                ...
                <form action="/deposit/setpaypassword.htm" method="post" onsubmit="return validateForm()">
                    <div class="n_password">
                        ...
                        <div class="n_item clearfix mb20">
                            ...
                            <button class="nw-submit" type="submit" tabindex="4" disabled="true" style="background-color:#999999;">提交</button>
                        </div>
                    </div>
                </form>
...
<script>
    function isbtn(){
        var flag = true;
        $('.new_member-right input').each(function(){
            if($(this).val()==""){
                flag = false;
            }
        })
        if(flag){
           $(".nw-submit").attr("disabled",false).css("backgroundColor","#0d9901");
        }else{
            $(".nw-submit").attr("disabled",true).css("backgroundColor","#999999");
        }
    }
    $("#code").on("channgeproperty input",isbtn).on("focus",function(){
        $(this).css("border-color","#dddddd");
        $("#yzcode .tips").hide();
    });
    $("#password").on("channgeproperty input",isbtn).on("focus",function(){
        $(this).css("border-color","#dddddd");
        $("#typePwd .tips").hide();
    });
    $("#repassword").on("channgeproperty input",isbtn).on("focus",function(){
        $(this).css("border-color","#dddddd");
        $("#reTypePwd .tips").hide();
    });
    function countDown(){
    ...
    function sendValidateCode() {
        ...
        defer.done(function(data){
            if(data==1) {
                ...
                $("#yzcode .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;网络异常').css("color","#c33").show();
                $("#sendCode").removeAttr("disabled");
            }else if(data==-1){
    ...
    function validateForm(){
        ...
        if(!r){
            ...
            $("#typePwd .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;密码必须是6-20位数字或字母或符号组成').show().css("color","#c33");
        ...
        if(pwdVal!=repwdVal){
            ...
            $("#reTypePwd .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;两次输入的密码不一致').show().css("color","#c33");
        ...
        if(!code){
            ...
            $("#yzcode .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;请输入验证码').show().css("color","#c33");
            return false;
        }else{
            $("#yzcode .tips").hide();
        }
        ...
        var flag = true;
          $.ajax({
                url: "/validate/getMCode.htm?code="+code,
                context: document.body,
                async:false,
                success: function(data){
                    if(data == 1){
                        $("#yzcode .tips").hide();
                        $("#yzcode .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;请输入验证码').css("color","#c33");
                        return true;
                    }else if(data == -1){
                        $("#code").css("border-color","#c33");
                        $("#yzcode .tips").show();
                        $("#yzcode .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;短信验证码已失效').show().css("color","#c33");
                        flag=false;
                    }else{
                        $("#code").css("border-color","#c33");
                        $("#yzcode .tips").show();
                        $("#yzcode .tips").html('<img src="/images/ero.png">&nbsp;&nbsp;短信验证码错误').show().css("color","#c33");
                        flag = false;
                    }
                }
            });
