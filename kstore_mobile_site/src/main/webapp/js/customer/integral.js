...
function showResponseType(responseData) {
		 ...
		 for(var i=0; i<returnjson.list.length; i++) {
		    nextpagehtml +='  <div class="income_info">';
		    nextpagehtml += '<p>';
		    // nextpagehtml += '<dt><span>';
		    var detail='';
		    var integral =  returnjson.list[i];
		    if(integral.pointDetail!=''&&integral.pointDetail!=null){
		    	if(integral.pointDetail =="订单取消返还订单使用积分"){
                    detail = "返还使用积分";
				}else if(integral.pointDetail =="扣除订单消费系统赠送积分"){
                    detail = "扣除消费赠送积分";
                }else{
	 ...
	 /* $newElems =$("#items .income_item");
         ...
         alert(returnjson.nextPageNo)
         alert(pageNo)
	     if(returnjson.nextPageNo==pageNo+1){
	    	 clearInterval(iIntervalId);
	    	 $('#showmore').hide();
	     }else{
	    	 $('#showmore').html(' <img src='+basePath+'/images/loading.gif /><span>加载中……</span>');
		 ...
		 $("#no").remove();
		 $('#items').html('<div class="no_tips" id="no"> <p>木有积分哦</p></div>' ); 
		 showFlag = 1;
