...
function callback(data) {
    if(data != "error"){
        ...
        $("#logoFile").addClass("error");
        $("#logoFile_update").addClass("error");
        flag=1;
    }
...
function submitSaveAuthForm(){
    ...
    $("#authsrc").val($("#preview_image").attr("src"));
    var save="saveAuth.htm?CSRFToken="+$("#CSRFToken").val();
    $("#saveBrandForm").attr("action",save);
    $("#saveBrandForm").attr("target","_self");
    $("#saveBrandForm").submit();
}
/**
 ...
 * @param brandId
 */
function showEditAuthForm(authId) {
    $.ajax({
        url:'queryAuthById.htm?authId='+authId+"&CSRFToken="+$("#CSRFToken").val(),
        success:function(data) {
            $("#authName").val(data.authName);
            $("#preview_image_update").attr("src",data.authsrc);
            $("#updateauthid").val(data.id);
            $("#editBrand").modal("show");
        }
    });
}
 ...
 * 提交编辑商品认证标识form
 */
function submitUpdateAuthForm() {
    $("#editBrandForm input").each(function () {
        $(this).removeClass("ignore");
    });
    $("#editBrandForm").attr("action","updateProductAuth.htm?CSRFToken="+$('#CSRFToken').val());
    $("#editBrandForm").attr("target","_self");
    $("#updateauthsrc").val($("#preview_image_update").attr("src"));
    if(flag == 0) {
        $("#editBrandForm").submit();
    }else{
        $("#logoFile_update").addClass("error");
