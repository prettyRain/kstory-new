        ...
        if(browereList != null && !browereList.isEmpty()){
            for(int i = 0;i < browereList.size();i++){
              ...
              //根据货品id查询货品
                ...
                /*if(null == browserecord.getModifiedTime()){*/
                    browserecord.setIsMobile(dateFormat.format(browserecord.getCreateTime()));
                /*}else{
                    browserecord.setIsMobile(dateFormat.format(browserecord.getModifiedTime()));
                }*/
                if(goodsProductVo!=null){
                   //判断货品是否是第三方货品
                    if(goodsProductVo.getIsThird() !=null && !"".equals(goodsProductVo.getIsThird()) && "1".equals(goodsProductVo.getIsThird())){
                        ...
                        //查询所有库存看是否有货
                        Long wareStock = 0l;
                        List<ProductWare> productWares = this.productWareService.queryGoodsStock(browserecord.getGoodsId().longValue());
                        if(productWares !=null && productWares.size()>0){
                            for(ProductWare prow:productWares){
                                wareStock += prow.getWareStock();
                            }
                        ...
                        /*if(browserecord.getGoods()!=null && productWare !=null){

                            browserecord.getGoods().setGoodStock(productWare.getWareStock());
                        }else if(browserecord.getGoods()!=null){
                            browserecord.getGoods().setGoodStock(0l);
                        }*/
                        browserecord.getGoods().setGoodStock(wareStock);
                    }
                }
            }
        }
        ...
        try {
            if (null != custId) {
                ...
                if(browserecord!=null){
                    ...
                    browserecord.setModifiedTime(new Date());
