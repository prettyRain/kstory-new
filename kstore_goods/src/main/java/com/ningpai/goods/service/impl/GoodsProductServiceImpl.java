...
public class GoodsProductServiceImpl implements GoodsProductService {
    ...
    public int delProductByProductId(Long productId, String username) {
        ...
        try {
            ...
            //删除货品里的图片 xcy
            this.goodsProductMapper.deleteGoodsImageByInfoId(map);
            // 删除货品信息
     ...
     */
    @Transactional
    public int batchUploadProduct(String username, Long[] productIds, Integer status,Integer auditStatus) {
        // 定义一个HashMap集合
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            // 往map集合中放入参数
    ...
    public int saveProduct(GoodsProduct product, String username, String[] specIds, String[] specDetailIds, Map<String, Object> map) {
        ...
        try {
            ...
            if (this.goodsProductMapper.insertSelective(product) > 0) {
                if (null != specIds && specIds.length > 0) {
                    for (int i = 0; i < specIds.length; i++) {
                        ...
                        // 插入一条货品关联规格值记录
                        this.goodsProductReleSpecService.saveProductReleSpec(product.getGoodsInfoId(), Long.parseLong(specIds[i]), Long.parseLong(specDetailIds[i]), specR,
                                username);
                    }
                }
            }
            ...
            return product.getGoodsInfoId().intValue();
        } finally {
            newId = null;
            specRemark = null;
        }
    ...
    public PageBean queryProductForApp(PageBean pb, Long catIds, String productName){
        ...
        try {
            ...
            if (null != catIds) {
                map.put(ValueUtil.CATIDS, catIds!=null?catIds:null);
            }
            // 设置总行数
     ...
     * 根据商品id查询货品集合（商品预览用）
     * @param goodsId 商品id
     * @return List<GoodsProductVo>：货品集合
     */
    @Override
    public List<GoodsProductVo> queryProductsForPreview(Long goodsId) {
        // 根据商品id，查询货品详细列表
        return goodsProductMapper.queryProductsForPreview(goodsId);
    }
...
//    public GoodsDetailBean queryDetailBeanByProductId(Long productId) {
...
//        try {
//            detailBean.setProductVo(this.goodsProductMapper.selectByPrimaryKey(productId));
//            goodsMapper.queryGoodsByGoodsId(goodsId);
...
//    }
    ...
    /**
     * 修改货品时，只改变类目推荐和移动端推荐不需要审核
     * @param prePro 修改前货品
     * @param newPro 新货品
     * @param supportIds 支持id
     * @param specDetailId 规格详细id
     * @return true不需要审核false需要
     */
    public boolean compare(GoodsProduct pone, GoodsProduct ptwo, String[] supportIds, String[] specDetailId){
        ...
        //更新前货品的服务支持
        List<GoodsProductSupp> goodsProductSupps = goodsProductSuppService.queryAllSuppByProductId(pone.getGoodsInfoId());
        if(goodsProductSupps.size() == 0 && supportIds == null){
            flag = true;
        }else{
            Arrays.sort(supportIds);
            if(goodsProductSupps != null){
                String[] supPre = new String[goodsProductSupps.size()];
        ...
        List<GoodsProductReleSpecVo> goodsProductReleSpecVos = goodsProductReleSpecService.queryAllByProductId(pone.getGoodsInfoId());
        if(goodsProductReleSpecVos.size() == 0 && specDetailId == null){
            flag = true;
        }else{
            if(goodsProductReleSpecVos != null){
                String[] specPre = new String[goodsProductReleSpecVos.size()];
     ...
     */
    @Override
    public GoodsProduct selectByGoodsInfoItemNo(String code) {
        return goodsProductMapper.selectByGoodsInfoItemNo(code);
    }
}
