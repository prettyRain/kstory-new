...
public class DepositPayController {
    ...
    @RequestMapping("/queryRorderCode")
    @ResponseBody
    public String queryRorderCode(HttpServletRequest request){

       return request.getSession().getAttribute("rOrderCode").toString();
    }
    ...
    public void pay(HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        request.getSession().removeAttribute("rOrderCode");
        request.getSession().setAttribute("rOrderCode",orderCode);
        ...
        if ("1".equals(p.getPayType())) {// 支付宝支付
            ...
            tPay.withParamMap(paramMap);
            payContext = new PayContext(tPay);
        }else if ("5".equals(p.getPayType())) {//礼品卡
            remark = "在线充值-礼品卡";
            Map<String,Object> map = new HashMap<String,Object>();
            map.put("customerId",customerId);
            Deposit deposit = depositService.getDeposit(map);
            if(deposit == null){
                response.setCharacterEncoding("UTF-8");
                response.setContentType("text/html; charset=utf-8");
                response.getWriter().write("<script>alert('用户没有绑定礼品卡!')</script>");
                return;
            }
            map.put("preDeposit", deposit.getPreDeposit().add(new BigDecimal(totalFee)));
            int n = depositService.updateDeposit(map);
        }
        String dom = null;
     ...
     * @throws Exception
     */
    @RequestMapping("/deposit/giftcardRecharge")
    @ResponseBody
    public String giftcardRecharge(HttpServletRequest request, GiftcardVo giftcard) throws Exception{
        // 获取用户id
        Long customerId = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
        // 判断用户id是否为空
        if (customerId == null) {
        ...
        map.put("customerId",customerId);
        Deposit deposit = depositService.getDeposit(map);
        if(deposit == null || deposit.getGiftcardNo() == null || "".equals(deposit.getGiftcardNo())){
            return "4";
        }
        //判断礼品卡信息
        ...
        if(giftcardVo.getGiftcardPrice().compareTo(BigDecimal.ZERO)<=0){
            return "3";
        }
        map.put("preDeposit", deposit.getPreDeposit().add(giftcardVo.getGiftcardPrice()));
        //修改账户礼品卡
        depositService.updateDeposit(map);
        //修改礼品卡信息（激活状态：2，面值：0）
        giftcard.setGiftcardPrice(BigDecimal.valueOf(0.00));
        giftcard.setActiveFlag("2");
        giftcard.setId(giftcardVo.getId());
        int n = giftcardService.updateGiftcard(giftcard);
        if(n==1){
            //创建交易记录
            int randomNum = new Random().nextInt(9000) + 1000;
            String orderCode = "R" + UtilDate.mathString(new Date()) + randomNum;
            ...
            tTrade.setOrderPrice(giftcardVo.getGiftcardPrice());
            tTrade.setDelFlag("0");
            tTrade.setOrderCode(orderCode);
            tTrade.setOrderStatus("6");
            tTrade.setOrderType("0");
            tTrade.setTradeRemark("在线充值-礼品卡");
            //当前可用余额信息。
            tTrade.setCurrentPrice(deposit.getPreDeposit().add(deposit.getFreezePreDeposit()).add(giftcardVo.getGiftcardPrice()));
     ...
     * 异步查看用户订单状态
     * @param request
     * @param orderCode 充值订单id
     * @return
     */
    @ResponseBody
    @RequestMapping("/rechargepay_status_check")
    public String pay_status_check(HttpServletRequest request,String orderCode){
        // 获取用户id
        Long customerId = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
        // 判断用户id是否为空
        if (customerId == null || orderCode == null) {
            return null;
        }
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("orderCode",orderCode);
        map.put("customerId",customerId);
        Trade trade = tradeService.findByOrderCodeAndCustomerId(map);
        if(trade == null){
            LOGGER.debug("没有查到充值订单信息");
            return null;
        }
        return trade.getOrderStatus();
    }
