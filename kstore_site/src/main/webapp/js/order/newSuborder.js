...
$(function(){
    ...
    var dianziCardNu = $("#dianziCard").val();
    var totalCount = $("#totalCount").val();
    if(dianziCardNu != totalCount){
        loadAllAddress();
        //设置默认收货地址
    ...
    $(".fretype").click(function(){
        $(".fretype").removeClass('item-selected');
...
//			     			setAddress();
    ...
    $(".save_invoice").click(function(){
        ...
        //校验开关
        var _flag=false;
        $('.new_add').each(function(index,item){
            if($(item).hasClass('invoice-item-selected')){
                flag = false;
                var reg= /^([\u4e00-\u9fa5_A-Za-z0-9 \\`\\~\\!\\@\\#\\$\\^\\&\*\(\)\=\{\}\'\:\;\'\,[\]\.\/\?\~\！\@\#\￥\…\…\&\*\（\）\;\—\|\{\}\【\】\‘\；\：\”\“\'\。\，\、\？])+$/;
                if(!reg.test($(item).find(".invoice_title").val())){
                    $(item).find(".invoice_title").val("");
                    $(item).find(".invoice_title").attr("placeholder","不能输入特殊字符！");
                    _flag=true;
                }
                if(!$('#taxpayer_id').val()){
                ...
                //校验纳税人识别号
                if(!(/^[0-9-A-Z]{18}$|^[0-9-A-Z]{15}$|^[0-9-A-Z]{20}$/).test($('#taxpayer_id').val())){
                    $('#taxpayer_id').val('');
                    $('#taxpayer_id').attr('placeholder','纳税人识别号为15位、18位或20位且不能有小写字母');
                    _flag=true;
                    //结束each循环
                    return false;
                }
                $('input[name=taxpayerNumber]').val($('#taxpayer_id').val());
                $(".tr_invoiceTitleView").html($(item).find(".invoice_title").val());
                $(".ch_invoceTitleValue").val($(item).find(".invoice_title").val());
                $(".ch_invoiceType").html($(".invoiceType").next().html());
                $(".ch_invoceTypeValue").val($(".invoiceType").next().val());
            }
        })
        ...
        if(flag){
            //当选中个人时
            var reg= /^([\u4e00-\u9fa5_A-Za-z0-9 \\`\\~\\!\\@\\#\\$\\^\\&\*\(\)\=\{\}\'\:\;\'\,[\]\.\/\?\~\！\@\#\￥\…\…\&\*\（\）\;\—\|\{\}\【\】\‘\；\：\”\“\'\。\，\、\？])+$/;
            if($('#caption_personinput').val()==''){
                $('#caption_personinput').attr("placeholder","不能为空！");
                return;
            }
            if(!reg.test($('#caption_personinput').val())){
                $('#caption_personinput').val("");
                $('#caption_personinput').attr("placeholder","不能输入特殊字符！");
                return;
            }
            $(".tr_invoiceTitleView").html($('#caption_personinput').val());
            $(".ch_invoceTitleValue").val($('#caption_personinput').val());
            //当发票抬头为个人时 把识别号置空
    ...
    var bossSumPrice=$('#bosssumPrice').val()-$('#yfprice').val();	//boss商品总金额
...
function getExpressprice() {
    ...
    var presentScopeIds = $("input[name=presentScopeIds]").val();
    var isbuynow = $("#isbuynow").val();
    var dianziCardN = $("#dianziCard").val();
    //计算此次购买商品总运费价格
    $.ajax({
        type: "post",
        url: "../getnewexpressprice.htm",
        async: false,
        data: {addressId: addsId, shopIds: boxs, isbuynow:isbuynow, dianziCard:dianziCardN,presentScopeIds:presentScopeIds},
        dataType: "json",
        success: function (data) {
            if(data != null){
                var weight = parseFloat(data.goodsweight)/1000;
                $("#goodsweight").text("(重量："+weight.toFixed(2)+"kg)");
                ...
                $.each(data,function(key,value){
                    ...
                    if(data!=null&&data!=''){
                        ...
                        //
                        //             if (value!=null){
                        //                 $(".price_"+gooddata.productVo.goodsInfoId).html("￥" + gooddata.productVo.goodsInfoPreferPrice);
                        //
                        ...
                        if(key=="couponList"){
                            if(value!=null && value.length>0){
                                $(".cpl").html("("+value.length+")");
                                couponHtml += '<input type="hidden" value="'+value.length+'" id="cpLength">' +
                                    '<p><input type="radio" name="couponNo" value="" onclick="couponChange(this,0);" />' +
                                    '<label>不使用优惠劵</label></p>';
                                $.each(value,function(i,coupon){
                                    if(value!=null){
                                        if(coupon.couponRulesType=='1'){
                                            couponHtml += '<p><input type="radio" name="couponNo" value="'+coupon.codeNo +'" ' +
                                                'onclick = "couponChange(this, '+coupon.couponStraightDown.downPrice+');" />' +
                                                '<label>' +coupon.couponName+ '</label>' +
                                                '</p>';

                                        }else if(coupon.couponRulesType=='2'){
                                            couponHtml += '<p><input type="radio" name="couponNo" value="'+coupon.codeNo +'" ' +
                                                'onclick = "couponChange(this, '+coupon.couponFullReduction.reductionPrice+');" />' +
                                                '<label>' +coupon.couponName+ '</label>' +
                                                '</p>';
                                        }
                                    }
                                ...
                                $(".cpl").html("(0)");
                                couponHtml += '<p class="coupon_empty light_9">您账户中没有可使用的优惠券。</p>'

                            }
                            $("#codeNo").siblings().remove();
                            $("#codeNo").after(couponHtml);
                            $("#inputCoupon").html('');
                            $("#inputCoupon").show();
                            $(".coupon_total").html('');
                            $("#inputCoupon").append('<span>有优惠券券码？</span>'+
                                '<a class="open_link red" onclick="openLink(this);" href="javascript:;">[点此输入券码兑换优惠券]</a>'+
                                '<div class="mt10" style="display:none;">'+
                                '<input type="text" class="text fl" id="useCoupon" style="width: 200px" maxlength="50"/>'+
                                '<input type="button"  class="common_btn" value="兑换" onclick="checkCoupon()" style="width: 60px; height: 25px; display:inline-block; *display:inline; *zoom:1;"/>'+
                                '<input type="button"  class="common_btn" value="清空" onclick="clearCoupon()" style="width: 60px; height: 25px; display:inline-block; *display:inline; *zoom:1;"/>'+
                                '<span id="errorTips" style="color:red"></span>'+
                                '<div class="cb"></div>'+
                                '</div>');
                            $(".coupon_total").append('<p>共使用了<span class="red us">0</span>张优惠券&nbsp;&nbsp;可以优惠<span class="red up">0.00</span>元</p>');
                            $("#yh").html("-￥0.00");
                            $("#youhui").val("0.00");
                            $("#codeNo").val("");
                        }
               ...
               // $("#yhprice").html("-￥"+ accAdd( Subtr(totalprice,subprice),0));
                $("#lastpay").html("￥"+Subtr(totalprice, myhPrice));
                $("#lastpays").html("￥"+Subtr(totalprice, myhPrice));//应付金额
                payprice = Subtr(totalprice, myhPrice);
            }
        }
    });
    ...
    //金额小于1  设置总订单的价格为1分钱
    if (Subtr(accAdd(yfprice, payprice),$("#discount").val()) < 0.02) {
        $("#lastpay").html("￥" +0.01);
        $("#lastpays").html("￥" + 0.01);
    }else{
        $("#lastpay").html("￥" + accAdd(Subtr(accAdd(yfprice, payprice),accAdd($("#discount").val(),0)),0));
        $("#lastpays").html("￥" + accAdd(Subtr(accAdd(yfprice, payprice),accAdd($("#discount").val(),0)),0));
    }
...
function loadAllAddress(){
    ...
    $.ajax({type:"post",url:"../goods/queryCustAddressForSubOrder.html",async : false,success:function(data) {
        ...
        for (var i = 0; i < data.addresses.length; i++) {
            ...
            address_html += "<li>" +
                "<div style='width: 90px' class='consignee-item cust_address_" + address.addressId;
            ...
            if (address.isDefault == "1") {
                //没有零售商品时,默认地址都可以;有零售商品时,只能选择社区店.
                /*if(lingshouCount<1||(lingshouCount>0&&address.addressType=='1')) {*/
            ...
            address_html += " add-detail='" + address.addressDetail + "'" +
                ...
                " add-type='" + address.addressType + "'" +
                " add-comm-store-id='" + address.communityStoreId + "'" +
            ...
            if (address.addressName.length > 5) {
                strName = address.addressName.substring(0, 3) + "..." + address.addressName.substring(address.addressName.length - 1, address.addressName.length);
            } else {
                strName = address.addressName;
            }
            var addressType='';
            if(address.addressType=='1') {
                /* addressType = '<span style="border:1px solid #ffd220;border-radius:2px;padding:2px;margin-right:2px;">社区店</span>';*/
                addressType='社区店：';
            }
            address_html += " add-email='" + address.addressEmail + "'" +
                "><span limit='3' class='span_overflow' title='" + address.addressName + " " + address.province.provinceName + "'>" + strName + " " + address.province.provinceName + "</span><input type='hidden' class='district' value='"+address.district.districtId+"'/><b></b></div> " +
                "<div class='addr-detail'><span class='addr-name ml10' limit='6' title='" + address.addressName + "'>"+addressType + strName + "</span>" +
            ...
            /*if(lingshouCount<1||(lingshouCount>0&&address.addressType=='1')) {*/
            address_html += "<a class='setdefault-consignee' href='javascript:void(0);' id='addressflag' onclick='defaultAddress(" + address.addressId + ");'>设为默认地址</a>" +
        ...
        if(lingshouCount>0&&lingshouCount==totalCount){
            ...
            $(".consignee-item[add-type='0']").parent().remove();
            if ($(".cust_allAddress").find(".consignee-item").length < 1){
                $(".consignee-cont").append("<span style='color:red;'>注：您的订单中为零售商品，我们将为您配送至社区店，请新增社区店地址哦！</span>");
            }
        }else{
            ...
            $(".consignee-item[add-type='1']").parent().remove();
            if ($(".cust_allAddress").find(".consignee-item").length < 1){
                $(".consignee-cont").append("<span style='color:red;'>注：您订单中含有批发商品，我们将为您送货上门，请准确填写个人收货地址哦！</span>");
            }
        }
        //根据个数更改样式
        updateadd($(".cust_allAddress").find(".consignee-item").length);
        $(".cust_allAddress").show();
        var flagchoose = 0;
        ...
        $(".consignee-item").click(function(){
            var lingshouCount = $("#lingshouCount").val();
            var addressType = $(this).attr("add-type");
            /*if(lingshouCount>0 && addressType=='0') {
             return;
             }*/
            $(".consignee-item").removeClass('item-selected');
            $(this).addClass('item-selected');
            $(".ch_address").val($(this).attr("value"));
            $(".ch_distinctId").val($(this).find(".district").val());
            //底部收货人信息
            $(".consignee-foot").html("");
            $(".consignee-foot").html("<p>寄送至：" + $(this).next().find(".addr-info").html() + "<br>收货人：" + $(this).next().find(".addr-name").html() + " " + $(this).next().find(".addr-tel").html() + "</p>");
            //根据收货地址判断该地区仓库是否有库存
            checkProductByAddress();
            //计算运费
            getExpressprice();
            initziti(addressType);
        });
//
...
function defaultAddress(id){
    ...
    $(".ch_address").val(id);
    $(".cust_address_"+id).parents(".cust_allAddress").find(".consignee-item").each(function(){
        $(this).removeClass("item-selected");
    });
    var addressType = $(".cust_address_"+id).attr("add-type");
    $(".cust_address_"+id).addClass("item-selected");
    $.ajax({
        type: "POST",
...
function initziti(addressType){
    var id=$(".ch_address").val();
    $("#ziti").removeClass('item-selected')
    if(id==null||id==""){
        /*$("#ziti").addClass("disabled")*/;
        return;
    }

    $.get("../ajaxgetSince.htm?addressId="+id,function(data){
...
function toaddaddress() {
    $("#addressinfo").html('新增收货人信息');
    $.get("../iscusaddslegal.htm", function(data){
        if(data == true)
            dia(1);
        else
            dia(17);
...
function modiAddress(id){
    ...
    $("#addressType"+$(address).attr("add-type")).click();
    if($(address).attr("add-type")=='1') {

        $.ajax({
            url:'../selectCommunityStoreVo.htm?communityStoreId='+$(address).attr("add-comm-store-id"),
            success:function (res) {
                $("#infoProvinceCommunity").val(res.provinceId);
                $("#infoProvinceCommunity").trigger("change");
                $("#infoCityCommunity").val(res.cityId);
                $("#infoCityCommunity").trigger("change");
                $("#infoCountyCommunity").val(res.districtId);
                $("#infoCountyCommunity").trigger("change");
                $("#community_stores_select").val(res.communityStoreId);
                // $("infoProvinceCommunity").val(res.provinceId);
            }
        });
    }
...
function inputChange(){
    document.getElementById("useCoupon").oninput=document.getElementById("useCoupon").onpropertychange=function(){
        var couponCode = $("#useCoupon").val();
        if(couponCode == "" || couponCode == null){
            $(".add_coupon_warn").html("<em>* </em>请输入券号");
        }else if(!(/^[A-Za-z0-9]{16}$/).test(couponCode)){
            $(".add_coupon_warn").html("<em>* </em>券号格式不正确");
        }else{
...
function checkCoupon(){
    ...
    $.ajax({
        ...
        success:function(data){
            if(data == 2){
                $(".add_coupon_warn").html("<em>* </em>添加成功");
                setTimeout(function(){
                ...
                $(".add_coupon_warn").html("<em>* </em>您领取的该优惠券已达上限");
            }else{
                $(".add_coupon_warn").html("<em>* </em>输入的券码无效");
            }
        }
    });
}
/*function checkCoupon(){
    ...
    $.ajax({
        type: 'post',
        url:'../getUsedCouponBycodeNo.htm?codeNo='+$("#useCoupon").val()+"&box="+boxs+"&distinctId="+distinctId,
        async:false,
        dataType:"json",
        success: function(data) {
            if(data==0){
                ...
                $("#errorTips").html("当前优惠券领取张数已达到限领数量！");
            }else{
                $("#errorTips").html("");
            }
            if(data == -1){
...
//	$('#amount').val('');
    ...
    var bosscgprice = $('#bosscgprice').val();
    //boss订单总价格
    var bosssum= $('#bosssumPrice').val();
    var pointSet = $('#pointSet').val();          //积分兑换规则
    var amount = $('#amount').val();                //需要兑换的积分
    ...
    if($("#isOpen").val()=='1'){
        ...
        if(amount==""||amount==null){
            ...
            if(bosssum<1){
                flag=false;
                $("#tishi").html('注：该商品不支持积分支付!');
                $('#amount').val('');
            }else{
                //判断用户的积分情况
                if (parseInt(customerPoint) < parseInt(amount)) {
                    flag=false;
                    ...
                    if(amount %10 !=0){
                        ...
                        //积分兑换的金额不能大于订单总金额
                        if(zhekou>Subtr(bosssum,$(".up").html())){
                            flag=false;
                            var keyishiyong = accDiv(accMul(Subtr(bosssum,$(".up").html()),10),pointSet);
                            keyishiyong=parseInt(parseInt(accDiv(parseInt(keyishiyong),10))+"0");
                            ...
                            if (jifenString.indexOf('.')<0){
                                ...
                                if(jifenString.length-jifenString.indexOf('.')==1){
                                    ...
                                    if ((zhekoujiage+"").substr(0,jifenString.indexOf('.')+3) < 0.02) {
                                        $("#lastpay").html("￥" +0.01);
                                        $("#lastpays").html("￥" + 0.01);
                                    }else{
                                        $('#lastpay').html("￥"+accAdd(zhekoujiage,0));         //设置应付总金额
                                        $('#lastpays').html("￥"+accAdd(zhekoujiage,0));
                                    }
                                    //给总价格赋值  用于同优惠券一起使用
                                    $('#payPrice').val(accAdd(zhekoujiage,0));
                                }else{
                                    if (zhekoujiage < 0.02) {
                                        $("#lastpay").html("￥" +0.01);
                                        $("#lastpays").html("￥" + 0.01);
...
function defaultLoadAllAddress(){
    setDefaultForm();
    $(".save_update_add_id").val("");
    var addsId="";
    /*查询所有的收货地址信息*/
