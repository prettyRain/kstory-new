...
public interface GoodsBrandService {
     ...
     * @return 商品品牌
     */
    GoodsBrand selectGoodsBrandByBrandName(String brandName);
     ...
     */
    int isBrandLinked(Long[] brandIds);
     ...
     * @return 可用返回true  不可用返回false
     */
    boolean checkThirdApplayBrandName(Long thirdId, String brandName);

    //添加商品认证标识
    void saveAuth(GoodsAuth goodsAuth);
    //查询商品认证标识
    Object searchAuthByPageBean(PageBean pb);
    //删除商品认证标识
    void deleteGoodsAuth(Map map);
    //查询所有商品认证标识
    List<Auth> findAllAuth();
    //xcy 商品标识关联商品
    void saveGoodsSaveAuthRelation(Map authMap);
    //xcy 查询与商品关联的认证标识
    List<GoodsAuth> findAuthByGoodsId(Long goodsId);
    //xcy 删除商品与认证标识的关联
    void updateGoodsAuthRelation(Map deleteMap);
    //xcy 根据goodsinfoID 查询商品认证标识
    Object findAuthByGoodsInfoId(Long productId);
    //xcy 根据商品认证id 查询认证
    GoodsAuth queryAuthById(Long authId);
    //xcy 根据id修改商品认证标识
    void updateGoodsAuth(GoodsAuth auth);
    //xcy 查询所有品牌
    List<GoodsBrand> findBrands();
}
