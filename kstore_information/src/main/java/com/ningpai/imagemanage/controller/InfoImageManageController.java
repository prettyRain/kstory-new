...
public class InfoImageManageController {
    ...
    public ModelAndView saveImageManagerAction(MultipartHttpServletRequest request, HttpServletResponse response, @Valid InfoImageManage infoImageManage,
        ...
        List<MultipartFile> muFiles = request.getFiles("imgSrc");


        if (CollectionUtils.isNotEmpty(muFiles)) {
            for (MultipartFile muFile : muFiles) {
                ...
                if (muFile.getSize() > 0) {
                    ...
                    if (upConf != null && "1".equals(upConf.getUsedStatus())) {
                        /** 设置图片资源实体类的大图地址 */
                        infoImageManage.setBigImgUrl(map.get("2"));
                        /** 设置图片资源实体类的中图地址 */
                        infoImageManage.setMiddleImgUrl(map.get("1"));
                        /** 设置图片资源实体类的小图地址 */
                        infoImageManage.setSmallImgUrl(map.get("0"));
                    } else {
                        /** 设置图片资源实体类的大图地址 */
                        infoImageManage.setBigImgUrl(map.get("0"));
                        /** 设置图片资源实体类的中图地址 */
                        infoImageManage.setMiddleImgUrl(map.get("2"));
                        /** 设置图片资源实体类的小图地址 */
                        infoImageManage.setSmallImgUrl(map.get("1"));
                    }
    ...
    public ModelAndView deleteImageManageActionToFormerBath(Long[] pids, Long classifyId, HttpServletRequest request) {
        ...
        if (!ArrayUtils.isEmpty(pids)) {
            for (Long id : pids) {
                infoImageManageService.deleteInfoImageManage(id);
            }
        }
