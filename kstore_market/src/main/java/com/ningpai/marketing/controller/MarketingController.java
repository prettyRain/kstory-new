...
public class MarketingController {
    ...
    public ModelAndView marketGoodsList(PageBean pageBean, Marketing marketing, String marketingBeginTime, String marketingEndTime, HttpServletRequest request) {
        ...
        return new ModelAndView(MarketingPath.MARKETGOODSLIST, PAGEBEAN, marketingService.marketOrderList(pageBean, marketing, marketingBeginTime, marketingEndTime)).addObject(
                "sx", request.getSession().getAttribute(CSRFTokenManager.CSRF_TOKEN_FOR_SESSION_ATTR_NAME).toString()).addObject("codexs", codexService.queryCodexsForSelect());
    }
    ...
    public ModelAndView doAddPresentMarketing(HttpServletRequest request, @Valid Marketing marketing, String sTime, String eTime, String status, BindingResult result) {
        SimpleDateFormat formatDate = new SimpleDateFormat(DATETIME);
        ...
        // 非空验证 促销名称
        if (null != marketing.getMarketingName()) {
            LOGGER.info("新增满赠促销信息,促销的名称为：" + marketing.getMarketingName());
        }

        marketingService.doAddPresentMarketing(marketing, codexType, request, status);
     ...
     * KKK mobile 促销信息
     */
    @RequestMapping("/queryGoodsMarketAndGroup")
    @ResponseBody
    public Map<String, List> queryProductMarketAndGroupByForm(Long goodsInfoId, Long brandId, Long cateId) {

        Map<String, List> promotionMap = new HashMap<>(2);
        List<GoodsGroupVo> groupVoList = goodsGroupService.queryGroupVoListWithProductId(goodsInfoId);

        List<Marketing> marketingList = marketingService.selectMarketingByGoodsInfoId(goodsInfoId, brandId, cateId);
        //抢购倒计时设置：抢购结束一天内仍然有显示（抢购已结束）
        int rushStatus = marketingService.selectMarketingByGoodsInfoIdWithDay(goodsInfoId, brandId, cateId);
        List statusList = new ArrayList();
        statusList.add(rushStatus);
        promotionMap.put("statusList",statusList);
    ...
    /**
     * KKK mobile 查询货品对应的特惠组合
     * @param goodsInfoId
     * @return
     */
    @RequestMapping("/queryGoodsGroup")
    public ModelAndView queryGroupByGoodsInfoId(Long goodsInfoId) {

        List<GoodsGroupVo> groupVoList = goodsGroupService.queryGroupVoListWithProductId(goodsInfoId);

//        groupVoList.get(0).getGroupId()
        return new ModelAndView("goods/group_sales")
                .addObject("groupList", groupVoList);
    ...
    public ModelAndView newModifyBossMarketing(Long marketingId, String marketingFlag) {
        ...
        // 所有三级分类
                ...
                //抢购限时列表
                .addObject("rushTimes",marketingService.queryRushByFlag());
    ...
    public ModelAndView doModifyPresentMarketing(HttpServletRequest request, @Valid Marketing marketing, String sTime, String eTime, String status) {
        SimpleDateFormat formatDate = new SimpleDateFormat(DATETIME);
        try {
            marketing.setMarketingBegin(formatDate.parse(sTime));
            marketing.setMarketingEnd(formatDate.parse(eTime));
        ...
        marketingService.doUpdatePresentMarketing(marketing, codexType, status, request);
        return new ModelAndView(new RedirectView(MarketingPath.INITGOODSMARKETING));
