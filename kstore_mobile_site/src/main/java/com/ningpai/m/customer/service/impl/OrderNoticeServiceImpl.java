...
public class OrderNoticeServiceImpl implements OrderNoticeService {
    ...
    public List<Object> selectOrderNotices(Long customerId) {
        ...
        paramMap.put("customerId", customerId);
        //将全部未读修改为已读
        orderNoticeMapper.updateNoticeRead(customerId);
        return this.orderNoticeMapper.selectListNoPage(paramMap);
    }

    @Override
    public OrderNotice getUnReadAndNewNotice(Long customerId) {
        OrderNotice orderNotice = new OrderNotice();
        try {
            orderNotice.setUnReadNum(this.orderNoticeMapper.selectNotRead(customerId));
            Map<String,Object> paramMap = new HashMap<String,Object>();
            paramMap.put("customerId", customerId);
            paramMap.put(CustomerConstants.STARTNUM, 0);
            paramMap.put(CustomerConstants.ENDNUM, 1);
            List<Object> list = this.orderNoticeMapper.queryOrderNotice(paramMap);
            if(list.size() > 0){
                OrderNotice obj = (OrderNotice)list.get(0);
                orderNotice.setCreateTime(obj.getCreateTime());
                orderNotice.setNoticeTitle(obj.getNoticeTitle());
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void removeNoticeAll(Long customerId) {
        this.orderNoticeMapper.removeNoticeAll(customerId);
    }
