...
public class MarketingRushSiteController {
    ...
    public ModelAndView selectMarketingRushList(PageBean pb, HttpServletRequest request) {
        PageBean pb1 = pb;
        pb1.setPageSize(15);
        pb1 = goodsProductService.selectMarketingRushList(pb1, request,null,null);
        pb1.setUrl("oldmarketingrushlist");
        int tempId = topAndBottomService.getIndexDefalutTemp().getTempId();
        ModelAndView mav = new ModelAndView("marketingrush/oldmarketingrushlist").addObject("pb", pb1).addObject("avs",
                channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(tempId), null, null, ATID1, ADVERTTYPE, null, "0", null, "6",null));
        LOGGER.info("获取抢购秒杀列表");
        return topAndBottomService.getTopAndBottom(mav);
    }
    /**
     * 新的抢购秒杀列表  留着以后用2018.5.7
     * 
     * @param pb
     *            分页参数
     * @param request
     ...
     */
   // @RequestMapping("selectmrlist")
    public ModelAndView newselectMarketingRushList(PageBean pb, HttpServletRequest request) {

        //显示商品判断 1正常显示  2显示提前一天的   3显示结束后一天的
        Long show = 1l;
        //获取抢购时间段
        List<MarketingRushTime> marketingRushTimes = marketingService.queryRushByFlag();
        //判断现在的时间
        Date date = new Date();
        int hours = date.getHours();
        //抢购时间段id   时段
        Long tId = 0l, tDate = 0l;
        for(int i = 0;i < marketingRushTimes.size()-1;i++){
            if(marketingRushTimes.get(i).gettDate().intValue() <= hours && marketingRushTimes.get(i+1).gettDate().intValue() > hours){
                tId = marketingRushTimes.get(i).gettId();
                tDate = marketingRushTimes.get(i).gettDate();
            }
        }
        if(tId == 0l){
            //查询即将开始的
            if(hours < marketingRushTimes.get(0).gettDate().intValue()){
                tId = marketingRushTimes.get(0).gettId();
                tDate = marketingRushTimes.get(0).gettDate();
                show = 2l;

            }else{
                tId = marketingRushTimes.get(marketingRushTimes.size()-1).gettId();
                tDate = marketingRushTimes.get(marketingRushTimes.size()-1).gettDate();
                int endtime = 23;
                String temp2 = marketingRushTimes.get(marketingRushTimes.size()-1).getTemp2();
                if(StringUtils.isNotBlank(temp2)){
                    endtime = Integer.valueOf(temp2);
                }
                if(hours>=endtime){
                    //查询已结束的
                    show = 3l;
        ...
        Long channelId = 247l,navId = 311l;
        // 根据商品分类ID获得频道
        Channel channel = channelService.findChannelByID(channelId);
        PageBean pb1 = pb;
        pb1.setPageSize(50);
        pb1 = goodsProductService.selectMarketingRushList(pb1, request,tId,show);
        ...
        ModelAndView mav = new ModelAndView("marketingrush/marketingrushlist").addObject("pb", pb1).addObject("avs",
                channelAdverService.selectchannelAdverByParamForSite(null, Long.valueOf(tempId), null, null, ATID1, ADVERTTYPE, null, "0", null, "6",navId));
        // 获取页面轮播大广告
        ...
        mav.addObject("marketingRushTimes",marketingRushTimes);
        mav.addObject("setTimes",new Date().getTime());
        LOGGER.info("获取抢购秒杀列表");
     ...
     * @return
     */
    @RequestMapping("selectmrlistajax")
    @ResponseBody
    public Map selectMarketingRushListajax(PageBean pb, HttpServletRequest request, Long tId,Long show) {
       /* //显示商品判断 1正常显示  2显示提前一天的   3显示结束后一天的
        String show = "1";*/
        //获取抢购时间段
        List<MarketingRushTime> marketingRushTimes = marketingService.queryRushByFlag();
        if(tId == null || tId == 0l){
            tId = tId = marketingRushTimes.get(0).gettId();
        }
        PageBean pb1 = pb;
        pb1.setPageSize(14);
        pb1 = goodsProductService.selectMarketingRushList(pb1, request,tId,show);
        Map map = new HashMap();
        map.put("pb",pb1);
        map.put("marketingRushTimes",marketingRushTimes);
        map.put("show",show);
        return map;
    }
