...
public class OrderCargoController {
    ...
    public ModelAndView printview(Long orderId,String str,String personName,String cartName,String cartPersonName,String printType,String logisticsName,String logisticsMobileName) {
        //如果是在待装箱页面且在装箱之前的打印的话走下面的逻辑
        if("doPrint2".equals(printType)){
            ...
            if(StringUtils.isNotBlank(personName)){
                Element person=doc.select("input[name=person]").get(0);
                //给html中加入 业务员的值
                person.val(personName);
                //设置业务员给 订单配送类
                orderDistribution.setSalesman(personName);
            }
            if(StringUtils.isNotBlank(cartName)){
                Element cart=doc.select("input[name=cart]").get(0);
                //给html中加入  配送车辆的值
                cart.val(cartName);
                //设置配送车辆给 订单配送类
                orderDistribution.setCart(cartName);
            }
            if(StringUtils.isNotBlank(cartPersonName)){
                Element cartPerson=doc.select("input[name=cartPerson]").get(0);
                //给html中加入  司机的值
                cartPerson.val(cartPersonName);
                //设置配送司机给 订单配送类
                orderDistribution.setMotorman(cartPersonName);
            }
            if(StringUtils.isNotBlank(logisticsName)){
               Element logistics=doc.select("input[name=logistics]").get(0);
                //给html中加入  物流名称的值
               logistics.val(logisticsName);
                //设置物流名称给 订单配送类
               orderDistribution.setLogistics(logisticsName);
            }
            if(StringUtils.isNotBlank(logisticsMobileName)){
                Element logisticsMobile=doc.select("input[name=logisticsMobile]").get(0);
                //给html中加入  物流联系方式的值
                logisticsMobile.val(logisticsMobileName);
                //设置物流联系方式给 订单配送类
                orderDistribution.setLogisticsMobile(logisticsMobileName);
            }
            //把html 保存
            str=doc.toString();
            // 根据订单ID查询配送信息
            OrderDistribution distribution=orderService.queryOrderDistributionByOrderId(orderId);
            if(distribution!=null){
                //如果有配送信息 那么就修改
                orderService.updateOrderDistribution(orderDistribution);
            }else{
                //如果没有配送信息 那么就保存
                orderService.saveOrderDistribution(orderDistribution);
            }
        }
    ...
    public Map<String, Object> orderPickingajax(Long[] orderId, HttpServletRequest request, String status, String yStatus) {
        String pickingName = request.getSession().getAttribute("name").toString();
        return orderService.queryByPincking(orderId, (long) 0, pickingName, status);
    }
    ...
    public ModelAndView orderDeliveryList(Order order, PageBean pageBean, Long[] orderIds, HttpServletRequest request) {
        ...
        return new ModelAndView(OrderConstants.ORDERPICKINGLIST, PAGEBEAN, orderService.searchOrderList(order, pageBean, orderIds)).addObject(ORDER, order).addObject("sx",
                request.getSession().getAttribute(CSRFTokenManager.CSRF_TOKEN_FOR_SESSION_ATTR_NAME).toString());
    ...
    public Map<String, Object> orderContainerajax(Long orderId, HttpServletRequest request, String yStatus) {
        ...
        //配送信息
        OrderDistribution orderDistribution=this.orderService.queryOrderDistributionByOrderId(orderId);
        map.put("orderDistribution",orderDistribution);
        return map;
    ...
    public ModelAndView suspendOrder(Long orderId, String orderStatus, String reason, HttpServletRequest request, String orderCodex, String sign) {
        ...
        if (count > 0) {
            if("0".equals(sign)){
                ...
                return new ModelAndView(new RedirectView(OrderConstants.INITORDERPICKING));
            }else{
                return new ModelAndView(new RedirectView(OrderConstants.ORDERDELIVERYLIST));
            }
        }
        return null;
    }
     ...
     * @author chenp
     */
    @RequestMapping(value = "onekeydelivery")
    @ResponseBody
    public boolean isOneKeyDeliveryFinished(HttpServletRequest request, Long orderId, String expressNo, Long thirdId){
        //参数校验
        if (orderId == null || StringUtils.isEmpty(expressNo) || thirdId == null)
            return false;
        //一键发货业务处理
        return orderService.isOneKeyDeliveryFinished(orderId,expressNo,request, thirdId);
    }
