                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <div class="input-group">
                                ...
                                <input type="text" id="up_adverHerf" name="adverHref" class="form-control" readonly>
                            </div>

                        </div>
                        <sapn></sapn>
                    </div>
                    <p class="lianjie">注：请先选择点击广告后跳转类型。不选择跳转类型则只显示不做跳转；商品列表页填写商品分类ID；商品详情页填写货品ID；其它新页面填写链接地址以https或http开头。</p>
                    <div class="form-group">
...
<script>
   ...
   function  selectLinkType(n) {
       ...
       if(linkType == "all"){
           ...
           $("#up_adverHerf").removeClass("required");
       }else if(linkType == ""){
    ...
    function showAdver(channelAdverId){
        $.post("showtempadverajax.htm", {channelAdverId: channelAdverId},
                function (data) {
                    ...
                    // 图片链接地址
                    var urlType = data.adverHref;
                    if(urlType.indexOf("item")!=-1){
                        $("#up_adverHerf").removeAttrs("readonly");
                        $("#linkType [value='item-']").prop("selected", 'selected');
                        $('#up_adverHerf').val(urlType.split("-")[1]);
                    }else if(urlType.indexOf("list")!=-1){
                        $("#up_adverHerf").removeAttrs("readonly");
                        $("#linkType [value='list-']").prop("selected", 'selected');
                        $('#up_adverHerf').val(urlType.split("-")[1]);
                    }else if(urlType.indexOf("http")!=-1){
                        $("#up_adverHerf").removeAttrs("readonly");
                        $("#linkType [value='']").prop("selected", 'selected');
                        $('#up_adverHerf').val(data.adverHref);
                    }else{
                        $("#linkType [value='all']").prop("selected", 'selected');
                    }
