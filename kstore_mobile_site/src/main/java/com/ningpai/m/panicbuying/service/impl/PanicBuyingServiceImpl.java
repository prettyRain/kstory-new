...
public class PanicBuyingServiceImpl  implements PanicBuyingService{
     ...
     */
    @Override
    public MarketingRushUtil queryProductByPidOrCidOrBid(Long goodsId, Long cateId, Long brandId,HttpServletRequest request) {
        Map map = new HashMap();
        map.put("goodsId",goodsId);
        map.put("cateId",cateId);
        map.put("brandId",brandId);
        GoodsProductVo goodsProductVo = panicBuyingDao.queryProductByPidOrCidOrBid(map);
        if(goodsProductVo ==null){
            return null;
        }
        List<GoodsProductVo> list = new ArrayList<>();
        list.add(goodsProductVo);
        List<MarketingRushUtil> marketingRushUtils = setPricesByDistinctId(list,getDistinctId(request));
        if(marketingRushUtils!=null && marketingRushUtils.size()>0){
            return marketingRushUtils.get(0);
        }
        return null;
    }
