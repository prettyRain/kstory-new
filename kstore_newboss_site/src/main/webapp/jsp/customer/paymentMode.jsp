                ...
                <div class="box_method p20">
                    <c:forEach  var="pay" items="${pb.list }" varStatus="status">
                        <div class="method_item <c:if test="${pay.isOpen eq 0 }">disabled</c:if> ">
                            ...
                            <div class="bar">
                                <div class="links">
                                    <c:if test="${pay.payType==9}">
                                        <a href="#" onclick="showEditForm(1,${pay.payId})">编辑</a>
                                    </c:if>
                                    <c:if test="${pay.payType!=9}">
                                        <a href="#" onclick="showEditForm(${pay.payType},${pay.payId})">编辑</a>
                                    </c:if>

                                    <c:if test="${pay.payType==1 || pay.payType==9}">
            ...
            <div class="modal-body">
                <form role="form" class="form-horizontal" method="post" action="updatepay.htm?CSRFToken=${token}" enctype="multipart/form-data" id="editForm7">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-16">
                            <label class="radio-inline">
                                <input type="radio" name="isOpen" id="open73" value="1" onclick="showDefultRadio(7,1)"> 是
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="isOpen" id="open74" value="0" onclick="showDefultRadio(7,0)" checked> 否
                            </label>
                        </div>
                    </div>
                    <div class="form-group" id="defaultDiv7">
                        <label class="control-label col-sm-5">是否设为默认：</label>
                        <div class="col-sm-1"></div>
                        <div class="col-sm-16">
                            <label class="radio-inline">
                                <input type="radio" name="payDefault" id="open71" value="1"> 是
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="payDefault" id="open72" value="0" checked> 否
                            </label>
                        </div>
                    </div>
                </form>
