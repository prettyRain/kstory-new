...
public interface OrderService {
    ...
    int saveAddOrder(BigDecimal goodsAllPrice, BigDecimal freightPrice, String companyInfo, String customerRemark,
                     ...
                     String invoiceType, String invoiceContent, CustomerAddress customerAddress,String deliveryName);
     ...
     * @return
     */
    List<OrderRemark> selectOrderRemarkByOrderId(Map<String, Object> paramMap);
     ...
     * @return
     */
    String addorderremarksajax(OrderRemark orderRemark);

    Integer getTotalCount(Long orderId);

    Long queryWareByDistrict(Long district);

    List<Map<String,Long>> queryNumByOrderId(Long orderId,Long wareId);

    Long queryOrderWareByOrderId(Long orderId);

    List<Map<String,Long>> queryGoodinfoNumByOrderId(Long orderId);

    void saveAddressAfterUpdate(CustomerAddress customerAddress, String addressType, String distinctId, String orderId);
    //查询订单的旧收货地址
    OrderOldAddress queryOldOrder(Long orderId);
    /**
     * 修改订单地址的时候
     * @param orderId
     * @param updateState 1:修改中  2：修改结束
     */
    void updateOrderAddressState(Long orderId, String updateState);
     ...
     * @return
     */
    String isOrderGargo(long l);
    //查询修改状态
    String queryUpdateStateByOrderId(Long orderId);

    Long isOrderOldAddress(String orderId);
    // 根据订单ID查询配送信息
    OrderDistribution queryOrderDistributionByOrderId(Long orderId);
    //配送信息 修改
    void updateOrderDistribution(OrderDistribution orderDistribution);
    //配送信息 保存
    void saveOrderDistribution(OrderDistribution orderDistribution);

    String findOrderStatus(Map<String,Long> map);
    //添加通知消息
    void addNotive(OrderNotice notive);
    //定时器查询已发货和取消的订单
    void noticeByTime();
     ...
     * @param orderId
     */
    int splitOrder(List<Integer[]> newnumList, Long[] goodsInfoId, BigDecimal[] goodsprice, String[] isPresent, Long orderId);

    void updategiftcardPrice(Map<String,Object> map);

    /**
     * 根据订单状态查询符合条件的订单id
     * @return
     */
    List queryOrderIdByStatus(String status);
     ...
     * @return
     */
    int synOrderSuccess(Long orderId);
