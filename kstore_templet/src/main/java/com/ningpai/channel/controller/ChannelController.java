...
public class ChannelController {
    ...
    public ModelAndView queryChannelByPageBean(PageBean pb, String searchText) {
        Map<String, Object> map = new HashMap<String, Object>();
        /* 获取所有商品分类，遍历取出一级分类 */
        List<GoodsCate> goodsCateList = goodsCateService.queryAllFirstGradeGoosCate();
        map.put(GOODSCATELIST, goodsCateList);
        // 获取模板
        SysTemp sysTemp = tempService.getCurrTemp();
        PageBean pageBean=channelService.findChannelByPageBean(pb, searchText, (long) sysTemp.getTempId());
        // 返回查询的结果
        // 设置集合
        // 设置模板id
        return new ModelAndView("jsp/channel/channel_list", "pb", pageBean).addObject("tempId",  (long) sysTemp.getTempId()).addObject("map", map);
    }
    ...
    public ModelAndView createChannel(HttpServletRequest request, @Valid Channel channel, BindingResult bindingResult) {
        ...
        // 设置频道的url
        //channel.setChannelUrl("channel/" + channel.getChannelId() + ".html");

        SysTemp sysTemp = tempService.getCurrTemp();
        channel.setTempId((long) sysTemp.getTempId());
        // 添加频道信息（temp2：SEO标题，temp3：seo关键字，temp5：seo描述）
    ...
    public ModelAndView updateChannel(HttpServletRequest request, @Valid Channel channel, BindingResult bindingResult) {
        ...
        SysTemp sysTemp = tempService.getCurrTemp();
        channel.setTempId((long) sysTemp.getTempId());
        // 修改频道（temp2：SEO标题，temp3：seo关键字，temp5：seo描述）
